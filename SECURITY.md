# ZORIN Security Policy

This document describes ZORIN's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NPCAutomators/AGENT-ZORIN/security/advisories/new)
or **security@npcautomators.com**. Do not open public issues for
security vulnerabilities. **ZORIN does not operate a bug
bounty program.**
...
- Environment details (`zorin version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted ZORIN access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in ZORIN's documentation
...
ZORIN supports two OS-level isolation postures. They address
...
ZORIN supports this in two ways:

- **ZORIN's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, ZORIN's in-process heuristics
...
ZORIN filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in ZORIN
itself. Bugs in ZORIN's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   ZORIN does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, ZORIN's own documentation, or reasonable
  operator expectations would predict — including cases where
  ZORIN has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  zorin-home security, and similar. Reports against those
...
- ZORIN includes supply-chain guards for MCP server
