...
description: "Diagnose and fix common Zorin cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`zorin gateway` for foreground, or `zorin gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `zorin cron tick`.
...
```bash
date
zorin cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.zorin/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.zorin/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.zorin/.env` |
...
| `local` | Write access to `~/.zorin/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `zorin cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `zorin skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `zorin skills list`.
...
2. `~/.zorin/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `zorin cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Zorin config directory). Verify:
```bash
ls ~/.zorin/scripts/your-script.py   # Must exist
zorin cron edit <job_id> --script ~/.zorin/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `zorin skills install <skill-name>`.
...
Jobs are stored in `~/.zorin/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.zorin/cron/jobs.json
chmod 600 ~/.zorin/cron/jobs.json   # Your user should own it
```
...
```bash
zorin cron list                    # Show all jobs, states, next_run times
zorin cron run <job_id>            # Schedule for next tick (for testing)
zorin cron edit <job_id>           # Fix configuration issues
zorin logs                         # View recent Zorin logs
zorin skills list                  # Verify installed skills
```
...
1. Run the job with `zorin cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.zorin/logs/agent.log` for scheduler messages and `~/.zorin/logs/errors.log` for warnings
3. Open an issue at [github.com/NPCAutomators/AGENT-ZORIN](https://github.com/NPCAutomators/AGENT-ZORIN) with:
