---
sidebar_position: 8
title: "Use Voice Mode with Zorin"
description: "A practical guide to setting up and using Zorin voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Zorin
...
:::tip
[NPCAUTOMATORS Portal](/integrations/npcautomators-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Zorin sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Zorin.
...
## Step 1: make sure normal Zorin works first
...
Zorin supports both local and cloud speech stacks.
...
### If you use `zorin setup`

If you choose NeuTTS in the setup wizard, Zorin checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Zorin transcribes and responds
...
- asking Zorin to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Zorin loop.

## Tuning CLI behavior

### Silence threshold

If Zorin starts/stops too aggressively, tune:
...
Zorin stays a normal chat bot, but can speak replies.
...
- you want Zorin to function like a portable research or ops assistant
...
Zorin joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Zorin detects speech boundaries
- transcripts are posted in the associated text channel
- Zorin responds in text and audio
...
1. get text Zorin working
2. install `zorin-agent[voice]`
