...
```bash
zorin teams-pipeline token-health
zorin teams-pipeline token-health --force-refresh
```
...
```bash
zorin teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
zorin teams-pipeline maintain-subscriptions
zorin teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Zorin cron (recommended if you already run the Zorin gateway)

Zorin ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.zorin/scripts/`. First create the script:

```bash
mkdir -p ~/.zorin/scripts
cat > ~/.zorin/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec zorin teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.zorin/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
zorin cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/zorin-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Zorin Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=zorin
EnvironmentFile=/etc/zorin/env
ExecStart=/usr/local/bin/zorin teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/zorin-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Zorin Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now zorin-teams-pipeline-maintain.timer
systemctl list-timers zorin-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/zorin teams-pipeline maintain-subscriptions >> /var/log/zorin/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.zorin/.env` at the top of a wrapper script that crontab calls.
...
```bash
zorin teams-pipeline subscriptions   # should show expirationDateTime advanced
zorin teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
zorin teams-pipeline list
zorin teams-pipeline list --status failed
zorin teams-pipeline show <job-id>
```

### Replay a stored job

```bash
zorin teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
zorin teams-pipeline fetch --meeting-id <meeting-id>
zorin teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `zorin teams-pipeline maintain-subscriptions --dry-run`
- inspect `zorin teams-pipeline list --status failed`
...
- run `zorin teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `zorin teams-pipeline run`
...
- [ ] `zorin teams-pipeline validate` returns an OK snapshot
- [ ] `zorin teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Zorin cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
