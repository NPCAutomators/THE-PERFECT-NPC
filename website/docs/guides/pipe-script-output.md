...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `zorin send`."
---

# Pipe Script Output to Messaging Platforms

`zorin send` is a small, scriptable CLI that pushes a message to any
messaging platform Zorin is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | zorin send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `zorin
...
zorin send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
zorin send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
zorin send --list

# Filter by platform
zorin send --list telegram
```
...
Any platform Zorin ships adapters for works as a target:
...
`zorin send` resolves the message body in this order:

1. **Positional argument** — `zorin send --to telegram "hi"`
2. **`--file PATH`** — `zorin send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | zorin send --to telegram`

When stdin is a TTY (no pipe), Zorin does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  zorin send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `zorin send` reuses your Zorin config, the same script works on
any host where Zorin is installed — no need to export bot tokens into
...
`zorin send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  zorin send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | zorin send \
...
```bash
./train.py --epochs 200 && \
  zorin send --to telegram "training done" || \
  zorin send --to telegram "training failed (exit $?)"
```
...
zorin send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(zorin send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `zorin send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `zorin send` calls
the platform's REST endpoint directly using credentials from
`~/.zorin/.env` and `~/.zorin/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `zorin gateway start` and retry.
...
# Machine-readable
zorin send --list --json
```

The listing is built from `~/.zorin/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`zorin gateway
...
| Approach | Multi-platform | Reuses Zorin creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `zorin send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`zorin send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `zorin send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `zorin send` shares with cron delivery.
