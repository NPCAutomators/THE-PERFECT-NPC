...
description: "How the Zorin Desktop app signs in to a gated gateway using your system browser and PKCE — no embedded webview, no session cookies"
---

# Desktop Native Sign-In (RFC 8252)

When the Zorin Desktop app connects to a **gated gateway** (a hosted or
...
For Zorin specifically, native sign-in means:
...
```
Desktop app                Gateway (/auth/native/*)          NPCAUTOMATORS Portal (IDP)
...
desktop app* and an OAuth client *to the upstream identity provider* (NpcAutomators
...
brokerable OAuth provider registered (e.g. the bundled **NpcAutomators** provider). No
...
- [Run Zorin with NPCAUTOMATORS Portal](./run-zorin-with-npcautomators-portal.md)
