---
sidebar_position: 7
title: "Use SOUL.md with Zorin"
description: "How to use SOUL.md to shape ZORIN's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Zorin

`SOUL.md` is the **primary identity** for your Zorin instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Zorin to feel like the same assistant every time you talk to it — or if you want to replace the Zorin persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Zorin should be
- what Zorin should avoid stylistically
- how Zorin should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Zorin is and how Zorin speaks
...
Zorin now uses only the global SOUL file for the current instance:

```text
~/.zorin/SOUL.md
```

If you run Zorin with a custom home directory, it becomes:
...
Zorin automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Zorin does not overwrite it
- if the file exists but is empty, Zorin adds nothing from it to the prompt

## How Zorin uses it

When Zorin starts a session, it reads `SOUL.md` from `ZORIN_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Zorin feels.
...
Zorin already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Zorin should sound.

# Avoid
What Zorin should not do.

# Defaults
How Zorin should behave when ambiguity appears.
```
...
Then restart Zorin or start a new session.
...
### I edited SOUL.md but Zorin still sounds the same

Check:
- you edited `~/.zorin/SOUL.md` or `$ZORIN_HOME/SOUL.md`
...
### Zorin is ignoring parts of my SOUL.md
