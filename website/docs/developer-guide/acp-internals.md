...
The ACP adapter wraps Zorin' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
zorin acp / zorin-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.zorin/.env
  -> configure stderr logging
  -> construct ZorinACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'zorin-agent[acp]==<version>' zorin-acp`, pointed at the `zorin-agent` PyPI release.
...
- `allow_once` -> Zorin `once`
- `allow_always` -> Zorin `always`
- reject options -> Zorin `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Zorin tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["zorin-acp"])
...
Instead it reuses Zorin' runtime resolver:

- `acp_adapter/auth.py`
- `zorin_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Zorin provider/credentials. It also always advertises a terminal setup auth method (`zorin-setup`, args `--setup`) so first-run registry clients can open Zorin' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.zorin/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `zorin-acp` toolset definition
- `zorin_cli/main.py` — `zorin acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `zorin-acp` script
