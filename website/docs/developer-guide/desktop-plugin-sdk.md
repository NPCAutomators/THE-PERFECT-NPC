...
title: "Desktop Plugin SDK (@zorin/plugin-sdk)"
description: "Extend the native Zorin Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Zorin Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `ZorinPlugin`.
It imports one module — `@zorin/plugin-sdk` — and gets everything: the app's
...
`$ZORIN_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Zorin. This page is the **native
desktop app** (`zorin desktop`) SDK — the `@zorin/plugin-sdk` module and
`$ZORIN_HOME/desktop-plugins/`. The **web dashboard** (`zorin dashboard`) has
its own, unrelated plugin system on `window.__ZORIN_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Zorin Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$ZORIN_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

Both take the same `ZorinPlugin` contract, appear in **Settings → Plugins**, and
...
Create `$ZORIN_HOME/desktop-plugins/hello/plugin.js` (that's `~/.zorin/...`
by default, or `~/.zorin/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.zorin/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@zorin/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Zorin' }),
      jsx('div', {
...
The only importable specifiers are `@zorin/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `ZorinPlugin`:

```ts
interface ZorinPlugin {
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `zorin.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@zorin/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@zorin/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@zorin/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@zorin/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@zorin/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@zorin/plugin-sdk'
...
```javascript
import { queryClient } from '@zorin/plugin-sdk'
...
`dashboard/` subfolder of a regular Zorin plugin and declare it in a
`manifest.json`:
...
zorin-agent codebase directly (`zorin_state`, `zorin_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.zorin/`)
...
(`zorin.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `ZorinPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@zorin/plugin-sdk` alias.
2. It's still lint-fenced to `@zorin/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@zorin/plugin-sdk`, `react`,
...
| Plugin contract | `ZorinPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
...
### Agents: the `zorin-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`zorin-desktop-plugins`** skill — it carries the same contract as this page in
...
`$ZORIN_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `zorin logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@zorin/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.zorin/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.zorin/logs/errors.log` for
