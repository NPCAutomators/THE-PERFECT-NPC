...
| Variable | Meaning |
|----------|---------|
| `ZORIN_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `ZORIN_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `ZORIN_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Zorin itself — they're private to these helpers. The variables Zorin *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `zorin --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_zorin_root)" || { echo "htui: not in a Zorin checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$ZORIN_MAIN_CHECKOUT/.venv/bin/python" -m zorin_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `ZORIN_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_zorin_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `ZORIN_TUI_DIR` are mutually exclusive
`ZORIN_TUI_DIR` points Zorin at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `zorin --tui --dev` exits with an error. Run `unset ZORIN_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_zorin_root)" || { echo "hgui: not in a Zorin checkout" >&2; return 1; }
  deps="${ZORIN_GUI_DEPS_CHECKOUT:-$ZORIN_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _zorin_link_deps "$desktop" "$deps/apps/desktop"
    _zorin_link_deps "$root" "$deps"
  else
  ...
  trap '_zorin_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    ZORIN_DESKTOP_ZORIN_ROOT="$root" \
    ZORIN_DESKTOP_PYTHON="$ZORIN_MAIN_CHECKOUT/.venv/bin/python" \
    ZORIN_DESKTOP_IGNORE_EXISTING=1 \
    ZORIN_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `ZORIN_DESKTOP_ZORIN_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `zorin`. |
| `ZORIN_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `ZORIN_DESKTOP_IGNORE_EXISTING` | Ignores any `zorin` on `PATH` so it can't shadow the worktree. |
| `ZORIN_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Zorin checkout.
_zorin_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/zorin_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_zorin_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'zorin_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `zorin --tui --dev` and the `ZORIN_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/NPCAutomators/AGENT-ZORIN/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `ZORIN_*` variable Zorin reads
