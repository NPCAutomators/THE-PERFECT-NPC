...
description: "How Zorin stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `zorin_cli/cron.py` | CLI `zorin cron` subcommands |
...
Jobs are stored in `~/.zorin/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `zorin cron` commands are run or during active CLI sessions.
...
keep the process awake), it asks NpcAutomators infrastructure to arm exactly **one
managed one-shot per job at that job's real next-fire time**. At fire time NpcAutomators
...
The flow (the managed scheduler is provided by NpcAutomators; the agent holds no
scheduler credentials):

```
create/update a cron job
  → Chronos asks NpcAutomators to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing NpcAutomators token)
  → at fire time NpcAutomators calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped NpcAutomators-minted JWT)
...
Config (all non-secret; on hosted agents NpcAutomators sets these at provision time):

| key | meaning |
|---|---|
| `cron.provider` | `chronos` to activate (empty = built-in ticker) |
| `cron.chronos.portal_url` | NpcAutomators base URL (arming + the fire-token issuer) |
...
If Chronos is misconfigured or the agent isn't logged into NpcAutomators,
...
agent↔NpcAutomators wire contract lives in `docs/chronos-managed-cron-contract.md`.
...
```python
# ~/.zorin/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.zorin/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `zorin cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `zorin cron` CLI provides direct job management:

```bash
zorin cron list                    # Show all jobs
zorin cron create                  # Interactive job creation (alias: add)
zorin cron edit <job_id>           # Edit job configuration
zorin cron pause <job_id>          # Pause a running job
zorin cron resume <job_id>         # Resume a paused job
zorin cron run <job_id>            # Trigger immediate execution
zorin cron remove <job_id>         # Delete a job
```
