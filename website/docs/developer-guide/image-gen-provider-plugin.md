...
description: "How to build an image-generation backend plugin for ZORIN"
---
...
:::tip
Image-gen is one of several **backend plugins** Zorin supports. The others (with more specialized ABCs) are [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a Zorin Plugin](/developer-guide/plugins).
:::

## How discovery works

Zorin scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.zorin/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `zorin_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `zorin tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `zorin tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.zorin/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `zorin plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `zorin tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `zorin tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `zorin tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $ZORIN_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `zorin plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `zorin tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `zorin tools` model picker |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to `$ZORIN_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.zorin/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `zorin plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export ZORIN_HOME=/tmp/zorin-imggen-test
mkdir -p $ZORIN_HOME/plugins/image_gen/my-backend
# …copy __init__.py + plugin.yaml into that dir…

export MY_BACKEND_API_KEY=your-test-key
zorin plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $ZORIN_HOME/config.yaml
echo "  provider: my-backend" >> $ZORIN_HOME/config.yaml

# Exercise it
zorin -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `zorin tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
- [Build a Zorin Plugin](/developer-guide/plugins) — general tools/hooks/slash commands guide
