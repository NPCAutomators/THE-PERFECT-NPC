...
sidebar_position: 1
title: "Architecture"
description: "ZORIN internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of ZORIN internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ zorin_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ZorinCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── zorin_state.py           # SQLite session/state database with FTS5
├── zorin_constants.py       # ZORIN_HOME, profile-aware paths
...
│
├── zorin_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `zorin` subcommands (large file)
...
│   ├── skills_config.py      # zorin skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # zorin tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # zorin gateway start/stop
│
...
```text
User input → ZorinCLI.process_input()
...
Three discovery sources: `~/.zorin/plugins/` (user), `.zorin/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `zorin plugins` or `config.yaml`.
...
Exposes Zorin as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`zorin -p <name>`) gets its own ZORIN_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
