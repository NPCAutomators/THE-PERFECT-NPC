...
Connect Zorin to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Zorin supports two WeCom integration modes:
...
> Run `zorin gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Zorin decrypts the message, queues it for the agent
...
(Use `zorin gateway start` only after `zorin gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Zorin and the token the
...
in `~/.zorin/.env` values around `=` will also break signature checks. After
fixing, restart `zorin gateway run`.
...
Check `zorin gateway run` logs for the bound host/port. If the adapter bound to
