...
sidebar_position: 12
title: "Google Chat"
description: "Set up ZORIN as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect ZORIN to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Zorin
...
> Run `zorin gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Zorin can read (e.g.,
`~/.zorin/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a zorin restart)
...
- Principal: `zorin-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Zorin calls
...
- **App name**: whatever you want users to see ("Zorin" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/zorin-chat-events`.
...
`ADDED_TO_SPACE` event that Zorin uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Zorin

Add the Google Chat section to `~/.zorin/.env`:
...
Install the dependencies the Google Chat adapter needs (no Zorin extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Zorin is thinking…" marker, then
...
`~/.zorin/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Zorin is thinking…").
...
Thread support: when a user replies inside a thread, Zorin detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Zorin session.
...
3. Download the JSON. Move it onto the host that runs Zorin.
4. Register the client with Zorin (run under the profile you want it scoped to):
...
zorin -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Zorin home (e.g.
`~/.zorin/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.zorin/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.zorin/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Zorin isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `zorin gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Zorin will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.zorin/google_chat_user_tokens/<sanitized_email>.json` (filesystem
