...
Connect Zorin to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Zorin — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Zorin runs it in a
...
zorin photon setup --phone +15551234567
```
...
2. **Finds or creates** the `ZORIN` project on your account.
...
Runtime credentials are written to `~/.zorin/.env`
...
(device token, dashboard project id) lives in `~/.zorin/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Zorin
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Zorin replies with a pairing code. Approve it with:

```bash
zorin pairing approve photon <CODE>
```

Use `zorin pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.zorin/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.zorin/.env`):
...
By default Zorin responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Zorin` and
`@Zorin agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Zorin will reply.
...
assigned iMessage line Zorin uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run zorin photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `zorin photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `zorin photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `zorin photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Zorin sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Zorin wake words  | JSON list / comma / newline regex patterns for group mentions |
