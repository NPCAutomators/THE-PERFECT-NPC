...
description: "Chat with Zorin from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Zorin from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Zorin](/guides/use-voice-mode-with-zorin).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [NPCAUTOMATORS Portal](/integrations/npcautomators-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Zorin Gateway"]
...
For group chats, hooks, and automation flows, Zorin supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Zorin keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Zorin does not hide failures just because the text resembles a silence token.
...
```bash
zorin gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
zorin gateway              # Run in foreground
zorin gateway setup        # Configure messaging platforms interactively
zorin gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo zorin gateway install --system   # Linux only: install a boot-time system service
zorin gateway start        # Start the default service
zorin gateway stop         # Stop the default service
zorin gateway status       # Check default service status
zorin gateway status --system         # Linux only: inspect the system service explicitly
```
...
```bash
zorin gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Zorin sends heartbeats
...
| `/update` | Update ZORIN to the latest version |
...
with the `session_reset` section in `~/.zorin/config.yaml`:
...
Configure per-platform overrides in `~/.zorin/gateway.json`:
...
zorin pairing approve telegram XKGH5N7P

# Other pairing commands:
zorin pairing list          # View pending + approved users
zorin pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Zorin appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.zorin/config.yaml`:
...
Off by default. When enabled, Zorin prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.zorin/config.yaml`:
...
```bash
ZORIN_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
zorin gateway install               # Install as user service
zorin gateway start                 # Start the service
zorin gateway stop                  # Stop the service
zorin gateway status                # Check status
journalctl --user -u zorin-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo zorin gateway install --system
sudo zorin gateway start --system
sudo zorin gateway status --system
journalctl -u zorin-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Zorin installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit zorin-gateway` (or `sudo systemctl edit zorin-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `zorin update`. When `zorin update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart zorin-gateway` command (it never blocks on an interactive password prompt).
...
```bash
zorin gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `zorin update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo zorin update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/zorin-gateway`:

```
zorin ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed zorin-gateway*, /usr/bin/systemctl --no-ask-password start zorin-gateway*, /usr/bin/systemctl --no-ask-password restart zorin-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Zorin will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Zorin installations on the same machine (with different `ZORIN_HOME` directories), each gets its own systemd service name. The default `~/.zorin` uses `zorin-gateway`; other installations use `zorin-gateway-<hash>`. The `zorin gateway` commands automatically target the correct service for your current `ZORIN_HOME`.
:::

### macOS (launchd)

```bash
zorin gateway install               # Install as launchd agent
zorin gateway start                 # Start the service
zorin gateway stop                  # Stop the service
zorin gateway status                # Check status
tail -f ~/.zorin/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.zorin.gateway.plist`. It includes three environment variables:
...
- **ZORIN_HOME** — scopes the gateway to your Zorin installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `zorin gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ZORIN_HOME` directory gets its own launchd label. The default `~/.zorin` uses `ai.zorin.gateway`; other installations use `ai.zorin.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `zorin-cli` | Full access |
| Telegram | `zorin-telegram` | Full tools including terminal |
| Discord | `zorin-discord` | Full tools including terminal |
| WhatsApp | `zorin-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `zorin-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `zorin-slack` | Full tools including terminal |
| Google Chat | `zorin-google_chat` | Full tools including terminal |
| Signal | `zorin-signal` | Full tools including terminal |
| SMS | `zorin-sms` | Full tools including terminal |
| Email | `zorin-email` | Full tools including terminal |
| Home Assistant | `zorin-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `zorin-mattermost` | Full tools including terminal |
| Matrix | `zorin-matrix` | Full tools including terminal |
| DingTalk | `zorin-dingtalk` | Full tools including terminal |
| Feishu/Lark | `zorin-feishu` | Full tools including terminal |
| WeCom | `zorin-wecom` | Full tools including terminal |
| WeCom Callback | `zorin-wecom-callback` | Full tools including terminal |
| Weixin | `zorin-weixin` | Full tools including terminal |
| BlueBubbles | `zorin-bluebubbles` | Full tools including terminal |
| QQBot | `zorin-qqbot` | Full tools including terminal |
| Yuanbao | `zorin-yuanbao` | Full tools including terminal |
| Microsoft Teams | `zorin-teams` | Full tools including terminal |
| API Server | `zorin-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `zorin-webhook` | Full tools including terminal |
| Raft | `zorin-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.zorin/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
