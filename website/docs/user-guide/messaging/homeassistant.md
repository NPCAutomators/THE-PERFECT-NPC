...
description: Control your smart home with ZORIN via Home Assistant integration.
...
ZORIN integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "ZORIN"
5. Copy the token
...
ZORIN registers four tools for smart home control:
...
Configure which events the agent sees in `~/.zorin/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "ZORIN".
...
The adapter reads credentials from `~/.zorin/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Zorin profile
...
running Zorin — `curl -H "Authorization: Bearer <token>" <url>/api/` should
