...
`~/.zorin/config.yaml`, `~/.zorin/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/zorin`:

```text
/etc/zorin/
├── config.yaml     # managed config layer (wins over ~/.zorin/config.yaml)
└── .env            # managed env layer (wins over ~/.zorin/.env + shell)
```
...
The location can be relocated with the `ZORIN_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ZORIN_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Zorin.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ZORIN_MANAGED_DIR=/opt/org/zorin-policy
```

:::warning
A user who can set `ZORIN_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `zorin doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/zorin/config.yaml` (managed) | `/etc/zorin/.env` (managed) |
| 2 | `~/.zorin/config.yaml` (user) | `~/.zorin/.env` (user) |
...
```bash
zorin config        # shows a header naming the managed source + the pinned keys
zorin doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Zorin refuses and names the source:

```bash
$ zorin config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/zorin/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `zorin config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/zorin

# Pin some config values for every user on this machine
sudo tee /etc/zorin/config.yaml >/dev/null <<'YAML'
model:
...
sudo tee /etc/zorin/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/zorin
sudo chmod 0644 /etc/zorin/config.yaml /etc/zorin/.env
```

Changes take effect on the next Zorin start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`zorin doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Zorin as `root`), managed scope is advisory.
