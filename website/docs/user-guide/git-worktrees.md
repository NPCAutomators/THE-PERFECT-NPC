...
title: "Git Worktrees"
description: "Run multiple Zorin agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

ZORIN is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Zorin so each session has a clean, isolated working directory.

## Why Use Worktrees with Zorin?

Zorin treats the **current working directory** as the project root:

- CLI: the directory where you run `zorin` or `zorin chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.zorin/config.yaml`
...
git worktree add ../repo-feature feature/zorin-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/zorin-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Zorin there:

```bash
cd ../repo-feature

# Start Zorin in the worktree
zorin
```
...
git worktree add ../repo-experiment-a feature/zorin-a
git worktree add ../repo-experiment-b feature/zorin-b
```
...
- Works on its own branch (`feature/zorin-a` vs `feature/zorin-b`).
...
- Zorin checkpoint data under `~/.zorin/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Zorin experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/zorin-checkpoints-docs`, `feature/zorin-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Zorin from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `zorin -w` (Automatic Worktree Mode)

Zorin has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `zorin/zorin-<hash>`).
...
```bash
zorin -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `zorin -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Zorin session its own clean checkout.
