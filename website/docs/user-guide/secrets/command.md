...
secrets:
  command:
    ...
    # command: "pass show zorin/env"
    # command: "secret-tool lookup service zorin-env"
```
...
- The helper's **stderr is discarded** — vault CLI diagnostics can carry secret material, so they never reach Zorin' output. Failures log structured fields only (exit code / signal / errno), never the command string.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (Zorin discards its stderr on purpose) |
