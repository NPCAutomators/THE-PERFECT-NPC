...
| | |
|---|---|
| Source | Optional — install with `zorin skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Zorin MCP catalog. The
...
Two halves, in this order: the editor side must be up before Zorin connects.
...
### One-time, Zorin side
    ...
    zorin mcp install unreal-engine
...
`~/.zorin/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Zorin — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Zorin connects
...
2. Start the Zorin session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Zorin
they appear as:
...
then happens at `zorin mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Zorin runs batched calls
...
- **Start order matters.** Editor + server up first, then the Zorin
...
- **Timeouts on long operations.** Zorin' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.zorin/config.yaml` for
