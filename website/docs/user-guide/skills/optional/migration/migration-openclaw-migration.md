---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into ZORIN"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into ZORIN"
---
...
Migrate a user's OpenClaw customization footprint into ZORIN. Imports Zorin-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zorin skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | ZORIN (NPCAUTOMATORS) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Zorin`, `Memory`, `Persona`, `Import` |
| Related skills | [`zorin-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zorin-agent) |
...
# OpenClaw -> Zorin Migration

Use this skill when a user wants to move their OpenClaw setup into ZORIN with minimal manual cleanup.
...
```bash
zorin claw migrate              # Full interactive migration
...
zorin claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `zorin setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_zorin.py` to:

- import `SOUL.md` into the Zorin home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Zorin memory entries
- merge OpenClaw command approval patterns into Zorin `command_allowlist`
- migrate Zorin-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Zorin working-directory configuration
- copy OpenClaw skills into `~/.zorin/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Zorin workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.zorin/tts/`
- archive non-secret docs that do not have a direct Zorin destination
...
- `scripts/openclaw_to_zorin.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.zorin/skills/migration/openclaw-migration/scripts/openclaw_to_zorin.py`

Do not guess a shorter path like `~/.zorin/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.zorin/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Zorin-compatible secrets, currently:
...
Zorin CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Zorin. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Zorin workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Zorin skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.zorin/skills/migration/openclaw-migration/scripts/openclaw_to_zorin.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.zorin/skills/migration/openclaw-migration/scripts/openclaw_to_zorin.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.zorin/skills/migration/openclaw-migration/scripts/openclaw_to_zorin.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.zorin/skills/migration/openclaw-migration/scripts/openclaw_to_zorin.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.zorin/skills/migration/openclaw-migration/scripts/openclaw_to_zorin.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.zorin/skills/migration/openclaw-migration/scripts/openclaw_to_zorin.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Zorin unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Zorin targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Zorin destination. Unsupported auth blobs must still be reported as skipped.
...
- Zorin persona state imported
- Zorin memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.zorin/skills/openclaw-imports/`
