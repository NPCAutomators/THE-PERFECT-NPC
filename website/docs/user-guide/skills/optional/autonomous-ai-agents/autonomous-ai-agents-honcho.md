...
description: "Configure and use Honcho memory with Zorin -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session sum..."
---
...
Configure and use Honcho memory with Zorin -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zorin skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Zorin

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Zorin profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/zorin#running-honcho-locally-with-zorin

### Verify

```bash
zorin honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Zorin profile's AI peer
...
Honcho models conversations as interactions between **peers**. Zorin creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Zorin instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Zorin run |
| `global` | Single session across all directories |

Manual override: `zorin honcho map my-project-name`
...
Each Zorin profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block zorin.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `zorin.coder` host block in `honcho.json`
...
```bash
zorin honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Zorin when Honcho memory is active.
...
Config file: `$ZORIN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `zorin honcho setup`. Ensure `memory.provider: honcho` is in `~/.zorin/config.yaml`.

### Memory not persisting across sessions
Check `zorin honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `zorin profile create <name> --clone`. For existing profiles: `zorin honcho sync`.
...
| Command | Description |
|---------|-------------|
| `zorin honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `zorin honcho status` | Show resolved config, connection test, peer info for active profile |
| `zorin honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `zorin honcho disable` | Disable Honcho for the active profile |
| `zorin honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `zorin honcho peers` | Show peer identities across all profiles |
| `zorin honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `zorin honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `zorin honcho sessions` | List known directory-to-session-name mappings |
| `zorin honcho map <name>` | Map current working directory to a Honcho session name |
| `zorin honcho identity` | Seed AI peer identity or show both peer representations |
| `zorin honcho sync` | Create host blocks for all Zorin profiles that don't have one yet |
| `zorin honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Zorin + Honcho |
| `zorin memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `zorin memory status` | Show active memory provider and config |
| `zorin memory off` | Disable external memory provider |
