...
| | |
|---|---|
| Source | Optional — install with `zorin skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by ZORIN |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Zorin terminal calls (desktop app flow only)
...
### Service Account (recommended for Zorin)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ZORIN_HOME:-~/.zorin}/.env` (the skill will prompt for this on first load).
...
## Zorin Execution Pattern (desktop app flow)

Zorin terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/zorin-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/zorin-op.sock"
