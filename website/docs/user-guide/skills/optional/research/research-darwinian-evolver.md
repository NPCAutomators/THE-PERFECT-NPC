...
| | |
|---|---|
| Source | Optional — install with `zorin skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), ZORIN |
| License | MIT |
...
import upstream classes into Zorin itself.
...
```bash
mkdir -p ~/.zorin/cache/darwinian-evolver && cd ~/.zorin/cache/darwinian-evolver
...
```bash
cd ~/.zorin/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.zorin/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.zorin/skills/research/darwinian-evolver
DE_DIR=~/.zorin/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Zorin core.
   Custom driver scripts under `~/.zorin/skills/...` are user-side and fine.
