...
| | |
|---|---|
| Source | Optional — install with `zorin skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Zorin or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Zorin. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.zorin/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.zorin/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Zorin integration, either:

- configure the server in `~/.zorin/config.yaml` using the `native-mcp` skill, or
...
### Zorin cannot see the deployed server

The server-building part may be correct while the Zorin config is not. Load the `native-mcp` skill and configure the server in `~/.zorin/config.yaml`, then restart Zorin.
