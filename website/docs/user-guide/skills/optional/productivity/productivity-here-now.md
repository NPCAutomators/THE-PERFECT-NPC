...
| | |
|---|---|
| Source | Optional — install with `zorin skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ZORIN_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ZORIN_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ZORIN_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client zorin
```
...
```bash
PUBLISH="${ZORIN_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client zorin
```
...
```bash
DRIVE="${ZORIN_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `zorin`)    |
