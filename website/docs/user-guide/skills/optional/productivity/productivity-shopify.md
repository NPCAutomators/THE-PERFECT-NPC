...
| | |
|---|---|
| Source | Optional — install with `zorin skills install official/productivity/shopify` |
...
4. Save to `${ZORIN_HOME:-~/.zorin}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Zorin","productType":"Apparel","tags":["test"]}}'
```
