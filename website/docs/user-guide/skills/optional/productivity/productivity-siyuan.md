...
| | |
|---|---|
| Source | Optional — install with `zorin skills install official/productivity/siyuan` |
...
3. Store it in `${ZORIN_HOME:-~/.zorin}/.env`:
   ```
...
```yaml
# In ~/.zorin/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
