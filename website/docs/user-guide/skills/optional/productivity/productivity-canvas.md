...
| | |
|---|---|
| Source | Optional — install with `zorin skills install official/productivity/canvas` |
...
4. Name the token (e.g., "ZORIN"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ZORIN_HOME:-~/.zorin}/.env`:
...
```bash
CANVAS="python $ZORIN_HOME/skills/productivity/canvas/scripts/canvas_api.py"
