---
title: "Zorin Themes — Author a Zorin color theme that skins every surface"
sidebar_label: "Zorin Themes"
description: "Author a Zorin color theme that skins every surface"
---
...
Author a Zorin color theme that skins every surface.
...
Author a Zorin **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`zorin_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.zorin/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Zorin itself.
...
- Write access to the Zorin home dir — `~/.zorin` by default, or `$ZORIN_HOME`
  / the active profile's dir. Skins live in `<zorin-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `zorin config set`).
...
3. `write_file` it to `<zorin-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `zorin config set display.skin default` (or they can `/skin default`).
...
```
zorin skin set <key> <hex>      # e.g. zorin skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `zorin skin set background "#08201f"` changes
only the background; `zorin skin use <name>` / `zorin skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.zorin`** when a profile is active — resolve the real home
  from `$ZORIN_HOME` first, falling back to `~/.zorin`.
...
- **Name collisions**: a skin named like a desktop built-in (`mono`, `slate`,
  `cyberpunk`, `npcautomators`, `midnight`, `ember`) won't override that built-in on the
  GUI. Pick a fresh name.
- **Never hand-edit `config.yaml` to activate.** Use `zorin config set
...
- **You apply it, not the user.** `zorin config set display.skin <name>` is
...
- `read_file` the written `<zorin-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `zorin config get display.skin` and confirm it reports `<name>`.
