...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _zorin_env="${ZORIN_HOME:-$HOME/.zorin}/.env"; [ -f "$_zorin_env" ] && grep -q "^GITHUB_TOKEN=" "$_zorin_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_zorin_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${ZORIN_HOME:-$HOME/.zorin}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by ZORIN. Code looks clean — good test coverage, no security concerns."
