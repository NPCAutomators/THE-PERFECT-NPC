...
- Give it a name like "zorin-agent"
- Select scopes:
...
- Give it a title like "zorin-agent-&lt;machine-name>"
...
elif _zorin_env="${ZORIN_HOME:-$HOME/.zorin}/.env"; [ -f "$_zorin_env" ] && grep -q "^GITHUB_TOKEN=" "$_zorin_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_zorin_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
