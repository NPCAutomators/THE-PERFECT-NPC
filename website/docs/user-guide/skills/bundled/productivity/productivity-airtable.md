...
4. Store the token in `${ZORIN_HOME:-~/.zorin}/.env` (or via `zorin setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Zorin. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${ZORIN_HOME:-~/.zorin}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
