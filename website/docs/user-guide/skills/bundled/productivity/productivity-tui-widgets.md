---
title: "Tui Widgets — Author live widget apps for the Zorin TUI dock"
sidebar_label: "Tui Widgets"
description: "Author live widget apps for the Zorin TUI dock"
---
...
Author live widget apps for the Zorin TUI dock.
...
Author widget apps for the Zorin TUI (`zorin --tui`): glanceable ambient
...
- The TUI must be in use (`zorin --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.zorin/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.zorin/logs/tui_gateway_crash.log` if a widget never appears.
