---
title: "Zorin Desktop Plugins — Write desktop app plugins that add UI panes and commands"
sidebar_label: "Zorin Desktop Plugins"
...
| Path | `skills/zorin-desktop-plugins` |
| Version | `1.0.0` |
...
# Zorin Desktop Plugins Skill

Write plugins for the Zorin desktop app: statusbar items, layout panes,
...
system (`~/.zorin/plugins/`) is otherwise documented separately.
...
- The Zorin desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ZORIN_HOME/desktop-plugins/` (usually
  `~/.zorin/desktop-plugins/`).

## How to Run

1. Create `$ZORIN_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (relative to this skill directory) — that's
   `~/.zorin/...` by default, or `~/.zorin/profiles/<profile>/...` under a
...
The ONLY import surface is `@zorin/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.zorin/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@zorin/plugin-sdk`, `react`, and
