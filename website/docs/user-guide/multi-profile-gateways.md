...
If you only run one Zorin agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Zorin agents that should all
...
(`ai.zorin.gateway-<name>.plist`) or systemd user service
(`zorin-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
zorin profile create coder
zorin profile create personal-bot
zorin profile create research
...
```bash
zorin config set gateway.multiplex_profiles true
zorin gateway restart
```

Equivalently, in the default profile's `~/.zorin/config.yaml`:
...
You do **not** run `zorin gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `zorin gateway start` / `run` is a
...
home). `zorin status` reports the multiplexer and the profiles it serves;
`zorin status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/zorin-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: zorin-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    zorin gateway "$action"
  else
    zorin -p "$profile" gateway "$action"
  fi
}
...
```bash
zorin-gateways start      # start every configured profile
zorin-gateways stop       # stop every configured profile
zorin-gateways restart    # restart all
zorin-gateways status     # status across all
zorin-gateways list       # delegates to `zorin gateway list`
```

:::tip
The `default` profile is targeted with `zorin gateway <action>` (no `-p`),
not `zorin -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `zorin -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.zorin.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/zorin-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.zorin.gateway.plist` /
`zorin-gateway.service`.
...
```bash
# Default profile
tail -f ~/.zorin/logs/gateway.log
tail -f ~/.zorin/logs/gateway.error.log

# Named profile
tail -f ~/.zorin/profiles/<name>/logs/gateway.log
tail -f ~/.zorin/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.zorin/logs/gateway.log ~/.zorin/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
zorin logs -f                  # follow default profile
zorin -p coder logs -f         # follow one profile
zorin logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
zorin profile list             # profiles + model + gateway state
zorin-gateways status          # full status across every profile
launchctl list | grep zorin    # macOS — PIDs and labels
systemctl --user list-units 'zorin-gateway-*'   # Linux — units
```
...
The default profile uses `~/.zorin/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
zorin config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.zorin/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=zorin --why="gateways running" \
  sleep infinity &
...
`zorin-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.zorin/.env ~/.zorin/profiles/*/.env
```

## Updating the code

`zorin update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `zorin gateway start` after a previous `zorin gateway stop`. The
...
```bash
ps -ef | grep "zorin_cli.*-p <profile>"
cat ~/.zorin/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.zorin.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.zorin.gateway-<profile>.plist

# Linux
systemctl --user restart zorin-gateway-<profile>.service
```

### Health check

```bash
zorin doctor                  # default profile
zorin -p <profile> doctor     # one profile
```
