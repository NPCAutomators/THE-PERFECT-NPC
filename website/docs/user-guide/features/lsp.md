...
Zorin runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Zorin
...
1. Zorin captures a baseline of current diagnostics for the file.
...
…). Zorin auto-detects the binary on PATH or in
`<ZORIN_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Zorin at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ZORIN_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`zorin lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Zorin installs both packages together when you
run `zorin lsp install typescript` or auto-install fires on first
use.

## CLI

```
zorin lsp status          # service state + per-server install status
zorin lsp list            # registry, optionally --installed-only
zorin lsp install <id>    # eagerly install one server
zorin lsp install-all     # try every server with a known recipe
zorin lsp restart         # tear down running clients
zorin lsp which <id>      # print resolved binary path
```

`zorin lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ZORIN_HOME>/lsp/bin
...
When `install_strategy: auto`, Zorin installs binaries into
`<ZORIN_HOME>/lsp/bin/`. NPM packages land in
`<ZORIN_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Zorin-owned and is
...
Servers are kept alive for the life of the Zorin process. There's
...
**`zorin lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ZORIN_HOME>/lsp/bin/`. Run
`zorin lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `zorin lsp status`**
...
When `zorin lsp status` shows a `Backend warnings` section, install
...
`~/.zorin/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.zorin/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `zorin lsp restart` to clear the set;
