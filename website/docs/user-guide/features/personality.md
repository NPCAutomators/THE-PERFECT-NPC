...
description: "Customize ZORIN's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

ZORIN's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ZORIN_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Zorin is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Zorin now seeds a default `SOUL.md` automatically in:

```text
~/.zorin/SOUL.md
```

More precisely, it uses the current instance's `ZORIN_HOME`, so if you run Zorin with a custom home directory, it will use:
...
- Zorin creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Zorin loads `SOUL.md` only from `ZORIN_HOME`
- Zorin does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Zorin falls back to a built-in default identity
...
If Zorin loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ZORIN_HOME`, the personality belongs to the Zorin instance itself.

That also makes it easier to teach users:
- "Edit `~/.zorin/SOUL.md` to change Zorin' default personality."
...
- how Zorin should handle uncertainty, disagreement, or ambiguity
...
## What Zorin injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Zorin falls back to a built-in default identity ("You are ZORIN, an intelligent AI assistant created by NPCAUTOMATORS..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Zorin ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Zorin, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Zorin its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.zorin/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.zorin/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-zorin)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Zorin speaks
- `display.skin` and `/skin` affect how Zorin looks in the terminal
