...
description: "Plugins shipped with ZORIN that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Zorin ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.zorin/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Zorin Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.zorin/plugins/<name>/`
3. **Project** — `./.zorin/plugins/<name>/` (requires `ZORIN_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `zorin_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `zorin memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `zorin plugins list` and the interactive `zorin plugins` UI), but none load until you explicitly enable them:

```bash
zorin plugins enable disk-cleanup
```

Or via `~/.zorin/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Zorin. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
zorin plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `zorin plugins enable <name>`.
...
| `zorin-achievements` | dashboard tab | Steam-style collectible badges generated from your real Zorin session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `zorin memory` and `zorin plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZORIN_HOME` or `/tmp/zorin-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ZORIN_HOME | always | Never |
...
**State** — everything lives at `$ZORIN_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `ZORIN_HOME` or `/tmp/zorin-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `zorin plugins enable disk-cleanup` (or check the box in `zorin plugins`).

**Disabling again:** `zorin plugins disable disk-cleanup`.
...
**Enabling:** `zorin plugins enable security-guidance` (or check the box in `zorin plugins`).

**Disabling again:** `zorin plugins disable security-guidance`.
...
Traces Zorin turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Zorin' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `zorin logs`.
...
```bash
zorin tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Zorin and the next turn ships a trace.
...
Then put the credentials in `~/.zorin/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Zorin turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `ZORIN_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Zorin session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `zorin chat` session lives under one Langfuse session.

**Verify:**

```bash
zorin plugins list                 # observability/langfuse should show "enabled"
zorin chat -q "hello"              # check the Langfuse UI for a "Zorin turn" trace
```
...
| `ZORIN_LANGFUSE_RELEASE` | — | Release/version tag |
| `ZORIN_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `ZORIN_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `ZORIN_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Zorin-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Zorin-prefixed wins when both are set.
...
**Disabling:** `zorin plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.zorin/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
zorin plugins enable google_meet
...
**Disabling:** `zorin plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.zorin/cache/google_meet/` until you remove them.

### zorin-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Zorin session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Zorin feature changes.

**How it works:**

- Scans your entire `~/.zorin/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$ZORIN_HOME/plugins/zorin-achievements/state.json`
...
| Secret | Hidden until Zorin detects the first related signal in your history |

**API** — routes mount under `/api/plugins/zorin-achievements/`:
...
**State files** — live under `$ZORIN_HOME/plugins/zorin-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Zorin updates. |
...
**Enabling:** Nothing to enable — `zorin-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `zorin dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/zorin-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.zorin/plugins/zorin-achievements/` that ships no dashboard. The plugin's state files under `$ZORIN_HOME/plugins/zorin-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Zorin plugin — see [Build a Zorin Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.zorin/plugins/<name>/`
- Manifest source is reported as `bundled` in `zorin plugins list`
