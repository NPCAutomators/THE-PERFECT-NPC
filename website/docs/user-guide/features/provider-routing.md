...
description: Configure OpenRouter or NPCAUTOMATORS Portal provider preferences to optimize for cost, speed, or quality.
...
When using [OpenRouter](https://openrouter.ai) or [NPCAUTOMATORS Portal](/integrations/npcautomators-portal) as your LLM provider, ZORIN supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::tip
Traffic routed through NPCAUTOMATORS Portal respects the same provider preferences — and Portal subscribers get 10% off token-billed providers.
:::

## Configuration

Add a `provider_routing` section to your `~/.zorin/config.yaml`:
...
:::info
Provider routing only applies when using OpenRouter or NPCAUTOMATORS Portal. It has no effect with direct provider connections (e.g., connecting directly to the Anthropic API).
:::
...
Provider routing preferences are passed to OpenRouter or NPCAUTOMATORS Portal on agent chat requests and iteration-limit summaries via the `extra_body.provider` field. (`extra_body` is the OpenAI Python SDK argument; it becomes the top-level `provider` object in the JSON request.) Auxiliary tasks such as compression and title generation are configured independently under `auxiliary.<task>.extra_body`.

- **CLI mode** — configured in `~/.zorin/config.yaml`, loaded at startup
...
Provider routing controls which **sub-providers behind OpenRouter or NPCAUTOMATORS Portal** handle your requests. For automatic failover to an entirely different provider when your primary model fails, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
