...
description: "Use your NPCAUTOMATORS Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Zorin-managed provider subscription as their
...
| Use case | "Use Zorin as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the NPCAUTOMATORS Portal OAuth flow. Zorin stores
the refresh token in `~/.zorin/auth.json` — the same place all Zorin
...
Model:      Zorin-4-70B    # or Zorin-4.3-36B, Zorin-4-405B
```
...
Currently shipped: `npcautomators` (NPCAUTOMATORS Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `zorin_cli/proxy/adapters/`.
...
If you see `not logged in`, run `zorin portal`. If you see
...
The proxy only forwards paths the upstream actually serves. For NpcAutomators
Portal:
...
supports; check `portal.npcautomators.com/models`.
...
[portal.npcautomators.com](https://portal.npcautomators.com).
...
`zorin_cli/proxy/adapters/<provider>.py` and registering it in
