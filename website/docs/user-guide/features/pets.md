...
Zorin can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ZORIN_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Zorin renders the sprite at full fidelity when your
...
zorin pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `zorin pets list [query] [--limit N]` |
| List installed pets | `zorin pets list --installed` |
| Install a pet | `zorin pets install <slug> [--select] [--force]` |
| Set the active pet | `zorin pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `zorin pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `zorin pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `zorin pets off` |
| Remove an installed pet | `zorin pets remove <slug>` |
| Diagnose setup | `zorin pets doctor` |

`zorin pets show` flags:
...
Beyond installing pre-made pets from the gallery, Zorin can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `zorin pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Zorin state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **NPCAUTOMATORS Portal**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/NpcAutomators run a quality-first model chain by default.

- Resolution order prefers NPCAUTOMATORS Portal → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `zorin tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ZORIN_PET_IMAGE_PROVIDER` env var (e.g. `ZORIN_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Zorin is
...
Run `zorin pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Zorin uses its own
  profile-scoped `<ZORIN_HOME>/pets/` instead — install through `zorin pets`.
