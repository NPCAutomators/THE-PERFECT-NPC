...
Type `@` followed by a reference to inject content directly into your message. Zorin expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Zorin env: `$ZORIN_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ZORIN_HOME/skills/.hub/`
