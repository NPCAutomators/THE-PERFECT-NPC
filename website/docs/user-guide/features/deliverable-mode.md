...
When ZORIN runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.zorin/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.zorin/config.yaml` (switchable per session
via `/personality`).
...
If you use Zorin' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.zorin/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. ZORIN's deliverable
