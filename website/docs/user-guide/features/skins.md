...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Zorin CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Zorin CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.zorin/skins/mytheme.yaml
```

Or set the default skin in `~/.zorin/config.yaml`:
...
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `ZORIN` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `ZORIN` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `ZORIN` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to ZORIN! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Zorin ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default ZORIN_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.zorin/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.zorin/skins/mytheme.yaml
...
## Zorin Mod — Visual Skin Editor

[Zorin Mod](https://github.com/cocktailpeanut/zorin-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Zorin Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/zorin-mod/master/npcautomators.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Zorin skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.zorin/skins/`
- Activates a skin by updating `~/.zorin/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/zorin-mod.git
cd zorin-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.zorin/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Zorin Mod respects the `ZORIN_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `zorin_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.zorin/skins/` take precedence over built-in skins with the same name.
