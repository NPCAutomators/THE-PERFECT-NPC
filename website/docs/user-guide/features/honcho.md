...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Zorin's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Zorin instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
zorin memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `zorin` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Zorin syncs them back at session init.
...
The `zorin honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `zorin memory setup honcho` (or run `zorin memory setup` and pick it from the list); the `zorin honcho` subcommand then appears on the next invocation.

```bash
zorin memory setup honcho    # Configure Honcho directly (works before activation)
zorin honcho status          # Connection status, config, and key settings
zorin honcho setup           # Redirects to `zorin memory setup` (post-activation alias)
zorin honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
zorin honcho peer            # Show or update peer names + dialectic reasoning level
zorin honcho mode            # Show or set recall mode (hybrid/context/tools)
zorin honcho tokens          # Show or set token budget for context and dialectic
zorin honcho identity        # Seed or show the AI peer's Honcho identity
zorin honcho sync            # Sync Honcho config to all existing profiles
zorin honcho peers           # Show peer identities across all profiles
zorin honcho sessions        # List known Honcho session mappings
zorin honcho map             # Map current directory to a Honcho session name
zorin honcho enable          # Enable Honcho for the active profile
zorin honcho disable         # Disable Honcho for the active profile
zorin honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `zorin honcho`

If you previously used the standalone `zorin honcho setup`:
...
No re-login or re-setup needed. Run `zorin memory setup` and select "honcho" — the wizard detects your existing config.
