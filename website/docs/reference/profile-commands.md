...
This page covers all commands related to [Zorin profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `zorin profile` without a subcommand shows help.
...
```bash
zorin profile use <name>
```

Sets `<name>` as the active profile. All subsequent `zorin` commands (without `-p`) will use this profile.
...
zorin profile use default
```

## `zorin profile create`

```bash
zorin profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `zorin profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `zorin update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.zorin`), use `zorin skills opt-out` / `zorin skills opt-in`. |
...
zorin profile create work2 --clone-from work

# Clone everything from a specific profile
zorin profile create work2-backup --clone-from work --clone-all
```

## `zorin profile describe`

```bash
zorin profile describe [<name>] [options]
```
...
zorin profile describe researcher

# Set it explicitly
zorin profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
zorin profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
zorin profile describe --all --auto
```

## `zorin profile delete`

```bash
zorin profile delete <name> [options]
```
...
```bash
zorin profile delete mybot
zorin profile delete mybot --yes
```
...
```bash
zorin profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Zorin home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ zorin profile show work
Profile: work
Path:    ~/.zorin/profiles/work
...
```bash
zorin profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Zorin installation.
...
```bash
zorin profile alias work
# Creates/updates ~/.local/bin/work

zorin profile alias work --name mywork
# Creates ~/.local/bin/mywork

zorin profile alias work --remove
# Removes the wrapper script
```

## `zorin profile rename`

```bash
zorin profile rename <old-name> <new-name>
```
...
```bash
zorin profile rename mybot assistant
# ~/.zorin/profiles/mybot → ~/.zorin/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `zorin profile export`

```bash
zorin profile export <name> [options]
```
...
```bash
zorin profile export work
# Creates work.tar.gz in the current directory

zorin profile export work -o ./work-2026-03-29.tar.gz
```

## `zorin profile import`

```bash
zorin profile import <archive> [options]
```
...
```bash
zorin profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

zorin profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`zorin profile export` / `import` are still the right commands for
...
### `zorin profile install`

```bash
zorin profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `zorin -p telemetry`). |
...
zorin profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
zorin profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
zorin profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
zorin profile install ./telemetry/
```

### `zorin profile update`

```bash
zorin profile update <name> [--force-config] [--yes]
```
...
```bash
zorin profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Zorin version, author, env var requirements, the source URL/path, and
...
`zorin profile list` also shows the distribution name and version in a
`Distribution` column, and `zorin profile show <name>` / `delete <name>`
...
zorin profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
zorin profile install https://github.com/your-org/internal-assistant.git
```
...
`zorin_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Zorin version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Zorin can clone from.
3. Tell recipients to run `zorin profile install <your-repo-url>`.
...
## `zorin -p` / `zorin --profile`

```bash
zorin -p <name> <command> [options]
zorin --profile <name> <command> [options]
```

Global flag to run any Zorin command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
zorin -p work chat -q "Check the server status"
zorin --profile dev gateway start
zorin -p personal skills list
zorin -p work config edit
```

## `zorin completion`

```bash
zorin completion <shell>
```
...
- `zorin profile <TAB>` — subcommands (list, use, create, etc.)
- `zorin profile use <TAB>` — profile names
- `zorin -p <TAB>` — profile names
