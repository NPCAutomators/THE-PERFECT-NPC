...
description: "Run ZORIN directly on an Android phone with Termux"
---

# Zorin on Android with Termux
...
ZORIN can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Zorin from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Zorin now ships a Termux-aware installer path:
...
- links `zorin` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `zorin` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/zorin" "$PREFIX/bin/zorin"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `zorin` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.zorin/.env`.
...
### `zorin doctor` says ripgrep or Node is missing
