...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NPCAutomators/AGENT-ZORIN/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    zorin_meta = (fm.get("metadata") or {}).get("zorin") or {}
    tags = zorin_meta.get("tags") or []
    related = zorin_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `zorin skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with ZORIN"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Zorin ships with a large built-in skill library copied into `~/.zorin/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Zorin also syncs bundled skills on `zorin update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.zorin/skills/` tree, it is still shipped with Zorin; restore it with `zorin skills reset <name> --restore`.",
        "",
