#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Zorin docs site.
...
Both publish at:
  https://zorin-agent.npcautomators.com/docs/llms.txt
  https://zorin-agent.npcautomators.com/docs/llms-full.txt
...
SITE_BASE = "https://zorin-agent.npcautomators.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Zorin as a Python Library", None),
        ("guides/use-mcp-with-zorin", "Use MCP with Zorin", None),
        ("guides/use-voice-mode-with-zorin", "Use Voice Mode with Zorin", None),
        ("guides/use-soul-with-zorin", "Use SOUL.md with Zorin", None),
        ("guides/build-a-zorin-plugin", "Build a Zorin Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Zorin"),
...
def emit_llms_index() -> str:
    ...
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by NPCAUTOMATORS. A terminal-native "
        ...
        "backends. Works with NPCAUTOMATORS Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/NPCAutomators/"
        "AGENT-ZORIN/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/NPCAutomators/AGENT-ZORIN")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# ZORIN — Full Documentation\n",
        (
            "This file is the entire ZORIN documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Zorin, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://zorin-agent.npcautomators.com/docs\n",
        "Short index: https://zorin-agent.npcautomators.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
