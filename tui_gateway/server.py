...
load_zorin_dotenv(
    zorin_home=_zorin_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.zorin/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_zorin_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("ZORIN_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("ZORIN_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
_LONG_HANDLERS = frozenset(
    {
        ...
        # pool checks, pricing fetch, NpcAutomators tier check, optional custom-provider
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ZORIN_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
class _SlashWorker:
    """Persistent ZorinCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Zorin agent → needs provider credentials.
        # Tier-1 secrets (gateway/GitHub/infra) are still stripped (#29157).
        env = zorin_subprocess_env(inherit_credentials=True)
        if profile_home:
            ...
            # gateway's launch ZORIN_HOME (#40677).
            env["ZORIN_HOME"] = str(profile_home)
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from zorin_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from zorin_cli.plugins import invoke_hook
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `zorin --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("ZORIN_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `zorin --tui`, so it must NOT count as dead here (doing
...
def _max_live_sessions() -> int:
    try:
        from zorin_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its ZORIN_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_zorin_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s ZORIN_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_zorin_home``).
    ...
    def wrapper(rid, params):
        ...
        token = set_zorin_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``zorin dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("ZORIN_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `zorin` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ZORIN_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_zorin_home_override(profile_home)
                try:
                    from zorin_state import SessionDB
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from zorin_cli.runtime_provider import canonical_custom_identity
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from zorin_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    overlay; it does not deep-merge ``zorin_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _zorin_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_zorin_home_override()
        home = override if isinstance(override, str) and override else _zorin_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    zorin_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    from zorin_cli.config import atomic_config_write

    path = _zorin_home / "config.yaml"
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["ZORIN_GATEWAY_SESSION"] = "1"
    os.environ["ZORIN_EXEC_ASK"] = "1"
...
def resolve_skin() -> dict:
    try:
        from zorin_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``zorin config set display.skin``) OR edited the active skin's colors in
...
def _ensure_skin_watcher() -> None:
    ...
    Zorin activates (``zorin config set display.skin``) or recolors goes live on
    ...
    threading.Thread(target=_loop, name="zorin-skin-watcher", daemon=True).start()


def _resolve_model() -> str:
    env = (
        os.environ.get("ZORIN_MODEL", "")
        or os.environ.get("ZORIN_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from zorin_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``ZORIN_DESKTOP=1`` and ``ZORIN_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``ZORIN_DESKTOP_TERMINAL=1`` → "tui"
        (``zorin --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``zorin --tui``.)
    """
    if is_truthy_value(os.environ.get("ZORIN_DESKTOP")) and not is_truthy_value(
        os.environ.get("ZORIN_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads ZORIN_MODEL /
    ZORIN_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`zorin --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads ZORIN_MODEL /
    # ZORIN_INFERENCE_MODEL, which `zorin --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("ZORIN_TUI_PROVIDER", "").strip()
    ...
    try:
        from zorin_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("ZORIN_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from zorin_cli.runtime_provider import (
...
def _load_approval_mode() -> str:
    from zorin_cli.config import DEFAULT_CONFIG, _deep_merge
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`zorin_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ZORIN_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("ZORIN_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Zorin): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `zorin --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ZORIN_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from zorin_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ZORIN_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ZORIN_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ZORIN_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _ZORIN_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off zorin-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ZORIN_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    cfg = None
    try:
        from zorin_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (ZORIN_MODEL /
    # ZORIN_INFERENCE_MODEL / ZORIN_TUI_PROVIDER / ZORIN_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `zorin --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # ZORIN_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("ZORIN_DEV_CREDITS")):
        try:
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from zorin_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from zorin_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from zorin_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from zorin_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from zorin_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("ZORIN_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("ZORIN_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``ZorinCLI.__init__`` and ``gateway/run.py``:
...
def _resolve_runtime_with_fallback(
    ...
    """
    from zorin_cli.auth import AuthError
    from zorin_cli.runtime_provider import resolve_runtime_provider
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from zorin_cli.fallback_config import resolve_entry_api_key
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses zorin_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ZORIN_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ZORIN_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("ZORIN_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ZORIN_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind ZORIN_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `ZORIN_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_zorin_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_zorin_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_zorin_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds ZORIN_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``zorin --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``zorin gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    # NpcAutomators credits block — agent-independent (a portal fetch), so it shows even
    # with zero API calls or on a resumed session. The TUI /usage panel renders
    # these lines regardless of `calls`. Fail-open: [] when not logged into NpcAutomators
    # or on any portal hiccup.
    try:
        from agent.account_usage import npcautomators_credits_lines

        credits = npcautomators_credits_lines()
        if credits:
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Zorin has to support both the legacy 8-row petdex atlas and the current
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from zorin_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from zorin_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from zorin_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from zorin_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from zorin_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_zorin_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("ZORIN_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
def _(rid, params: dict) -> dict:
    ...
    True only when a reference-capable image backend (NPCAUTOMATORS Portal / OpenRouter /
...
# (agent/billing_view.py + zorin_cli/npcautomators_billing.py) — same as /topup.


def _serialize_billing_error(exc) -> dict:
    """Map a BillingError into the result.error envelope the TUI branches on."""
    from zorin_cli.npcautomators_billing import (
...
def _(rid, params: dict) -> dict:
    ...
    from zorin_cli.npcautomators_billing import BillingError, post_subscription_preview
...
def _(rid, params: dict) -> dict:
    ...
    billing:manage.
    """
    from zorin_cli.npcautomators_billing import BillingError, put_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    """
    from zorin_cli.npcautomators_billing import BillingError, delete_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    from zorin_cli.npcautomators_billing import BillingError, post_subscription_upgrade
...
def _(rid, params: dict) -> dict:
    ...
    """
    from zorin_cli.npcautomators_billing import BillingError, post_charge
    from agent.billing_view import new_idempotency_key
...
def _(rid, params: dict) -> dict:
    ...
    """
    from zorin_cli.npcautomators_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from zorin_cli.npcautomators_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from zorin_cli.auth import step_up_npcautomators_billing_scope
        from zorin_cli.npcautomators_billing import BillingError
        ...
        granted = step_up_npcautomators_billing_scope(
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Zorin profile home
    # (~/.zorin/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $ZORIN_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_zorin_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _(rid, params: dict) -> dict:
    from zorin_cli.input_sanitize import sanitize_user_prompt_text
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ZORIN_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_zorin_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from zorin_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from zorin_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: zorin_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from zorin_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _zorin_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".zorin" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.zorin/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.zorin/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Zorin Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Zorin/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Zorin Desktop. If it serves Zorin/Desktop UI or another unrelated app, stop that process and report failure.",
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from zorin_cli.model_switch import parse_model_flags_detailed
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from zorin_cli.models import resolve_fast_mode_overrides
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("ZORIN_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["ZORIN_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("ZORIN_YOLO_MODE", None)
                    nv = "0"
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under ZORIN_HOME (~/.zorin by default) — config/sessions/skills,
    ...
    zorin_home = os.path.realpath(str(get_zorin_home()))

    return real == home or real == zorin_home or real.startswith(zorin_home + os.sep)
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    ZORIN_HOME may be an intentional prose/data workspace. The pre-Projects
    ...
    zorin_home = os.path.normcase(os.path.realpath(str(get_zorin_home())))
    return real == home or real == zorin_home
...
def _discover_repos_payload(
    ...
    repos even with zero zorin sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (zorin home subtree + bare
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from zorin_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_zorin_home), "display": display_zorin_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _zorin_home / "config.yaml"
        try:
...
def _(rid, params: dict) -> dict:
    try:
        from zorin_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from zorin_cli.runtime_provider import resolve_runtime_provider
        from zorin_cli.auth import has_usable_secret
        from zorin_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Zorin provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.zorin/.env`` into the gateway process via
    ``zorin_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from zorin_cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `zorin` is interactive — use `/zorin chat -q …` or run `zorin` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`zorin setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`zorin gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`zorin sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`zorin config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m zorin_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "zorin_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=min(int(params.get("timeout", 240)), 600),
            cwd=os.getcwd(),
            # cli.exec runs `python -m zorin_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from zorin_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from zorin_cli.commands import SlashCommandCompleter
...
def _model_picker_context(agent):
    ...
    from zorin_cli.inventory import load_picker_context
...
def _(rid, params: dict) -> dict:
    try:
        from zorin_cli.inventory import build_models_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from zorin_cli.config import is_managed
        from zorin_cli.inventory import build_models_payload
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `zorin model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.zorin/.env via the unified credential lifecycle
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from zorin_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Zorin" if role == "assistant" else role.title()
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("ZORIN_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("ZORIN_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["ZORIN_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from zorin_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["ZORIN_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from zorin_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from zorin_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from zorin_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("ZORIN_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("ZORIN_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_zorin_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``zorin journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from zorin_cli.skills_hub import do_install
        ...
        if action == "browse":
            from zorin_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from zorin_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``zorin plugins`` / the dashboard, so the three surfaces
