...
def _default_registry_path() -> Path:
    return get_zorin_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_zorin_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_zorin_home = expected_zorin_home if expected_zorin_home is not None else str(get_zorin_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["ZORIN_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("zorin_home") or "")
        if got_home and got_home != self.expected_zorin_home:
            raise RuntimeError(f"compute host ZORIN_HOME mismatch: {got_home} != {self.expected_zorin_home}")
