...
Usage (via cron with --no-agent):
    ...
    zorin cron create zorin-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ZORIN_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name zorin-issues --repo NPCAutomators/AGENT-ZORIN --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Zorin .env file
(``${ZORIN_HOME:-~/.zorin}/.env``) to avoid the 60 req/hr
