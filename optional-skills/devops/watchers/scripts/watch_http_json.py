...
Usage (via cron with --no-agent):
    ...
    zorin cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ZORIN_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Zorin-Watcher/1.0"})
