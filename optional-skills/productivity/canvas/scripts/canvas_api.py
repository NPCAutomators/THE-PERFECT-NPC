...
def _check_config():
    ...
    if missing:
        zorin_env = os.path.join(
            os.environ.get("ZORIN_HOME", os.path.expanduser("~/.zorin")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {zorin_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for ZORIN"
    )
