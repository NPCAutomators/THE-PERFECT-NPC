...
Usage:
  ...
  <server_name> matches the files in $ZORIN_HOME/mcp-tokens/<server>.json etc.
...
def _zorin_home():
    # Prefer Zorin' own resolver (profile-safe); fall back to env then ~/.zorin.
    try:
        from zorin_constants import get_zorin_home
        return str(get_zorin_home())
    except Exception:
        return os.environ.get("ZORIN_HOME") or os.path.expanduser("~/.zorin")


def _tokens_dir():
    return os.path.join(_zorin_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "zorin-diag", "version": "1.0"}},
        },
        headers={
