...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ZORIN_HOME", os.path.expanduser("~/.zorin")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ZORIN_HOME", Path.home() / ".zorin")) / "skills" / "red-teaming" / "godmode"
...
ZORIN_HOME = Path(os.getenv("ZORIN_HOME", Path.home() / ".zorin"))
CONFIG_PATH = ZORIN_HOME / "config.yaml"
PREFILL_PATH = ZORIN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # NpcAutomators/Zorin models — already uncensored, just needs clean prompt
    "zorin": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "zorin" in model_lower or "npcautomators" in model_lower:
        return "zorin"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Zorin config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.zorin/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Zorin for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Zorin for changes to take effect.")
