...
All per-subject data lives under PDD_DATA_DIR (default: $ZORIN_HOME/unbroker),
which is the same trust boundary Zorin uses for .env and OAuth tokens.
"""
...
def zorin_home() -> Path:
    return Path(os.environ.get("ZORIN_HOME") or (Path.home() / ".zorin"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else zorin_home() / "unbroker"
