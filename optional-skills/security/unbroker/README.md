...
The design is **Zorin-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
zorin skills install official/security/unbroker
```

Then start a new Zorin session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Zorin home directory (`0600`, optionally
