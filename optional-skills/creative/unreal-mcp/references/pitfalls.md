...
### 1. Start order: editor first, Zorin second

Zorin probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Zorin session.
...
restart the editor, restart the Zorin session.
...
server fails to bind (Output Log) or Zorin' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.zorin/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Zorin

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Zorin' connection
lives in `~/.zorin/config.yaml` via `zorin mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Zorin — just don't
mistake it for the Zorin setup step.
...
explicitly warns against overlapping calls. Zorin executes same-turn tool
...
### 9. Timeouts: Zorin gives up before Unreal does

Zorin' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Zorin runs elsewhere (SSH backend,
