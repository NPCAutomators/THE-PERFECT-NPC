#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `zorin kanban list` and
...
def zorin_available() -> bool:
    return shutil.which("zorin") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed zorin CLI."""
    try:
        out = subprocess.run(
            ["zorin", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `zorin kanban list`
    out = subprocess.run(
        ["zorin", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["zorin", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not zorin_available():
        print("ERROR: 'zorin' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
