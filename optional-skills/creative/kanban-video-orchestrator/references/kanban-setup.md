...
the actual `setup.sh` that creates the project workspace, configures Zorin
...
> [NPCAutomators/kanban-video-pipeline](https://github.com/NPCAutomators/kanban-video-pipeline).
...
2. **Create profiles** — `zorin profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.zorin/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `zorin kanban create` assigned to the director
...
```bash
zorin profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.zorin/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.zorin/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Zorin
...
Each profile gets a `SOUL.md` at `~/.zorin/profiles/<name>/SOUL.md` that
...
```bash
zorin kanban create "Direct production of <video title>" \
    --assignee director \
...
the Zorin `.env` (`${ZORIN_HOME:-$HOME/.zorin}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _zorin_env="${ZORIN_HOME:-$HOME/.zorin}/.env"
    if grep -q "^${var}=" "$_zorin_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_zorin_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_zorin_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY zorin ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY zorin OPENROUTER_API_KEY || exit 1
# ...
```
