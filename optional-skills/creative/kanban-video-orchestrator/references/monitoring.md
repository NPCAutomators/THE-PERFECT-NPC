...
zorin kanban watch --tenant <project-slug>

# Snapshot of the board
zorin kanban list --tenant <project-slug>
zorin kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
zorin kanban stats --tenant <project-slug>

# Visual dashboard (browser)
zorin dashboard

# Inspect a specific task (includes comments + events)
zorin kanban show <task-id>

# Follow a single task's event stream
zorin kanban tail <task-id>
```

Verify available subcommands with `zorin kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `zorin kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `zorin kanban block <id>` then `zorin kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `zorin kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `zorin kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `zorin profile list` (profile exists?), `zorin status` (gateway/dispatcher up?), and verify tenant. |
...
zorin kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
zorin kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(zorin kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
zorin kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
zorin kanban block <task-id>
zorin kanban archive <task-id>

# Diagnose what happened
zorin kanban show <task-id>      # task body, comments, recent events
zorin kanban tail <task-id>      # follow the live event stream
zorin kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `zorin kanban create` for the director
...
while true; do
    clear
    zorin kanban list --tenant <slug>
    echo "---"
    zorin kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `zorin kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `zorin profile list` and start
