...
Maps each role archetype to the Zorin skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public zorin-agent
...
## Zorin skills relevant to video production

### Visual / rendering skills (`zorin-agent/skills/creative/`)
...
### Design / pre-production skills (`zorin-agent/skills/creative/`)
...
### Audio / media skills (`zorin-agent/skills/creative/` + `skills/media/`)
...
These are **not** Zorin skills but external CLIs / APIs that profiles invoke.
...
## Built-in Zorin tools for media review

These are native Zorin tools — not invoked via terminal but through their own
...
key is present in `${ZORIN_HOME:-~/.zorin}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Zorin profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ZORIN_HOME:-~/.zorin}/.env` → environment variable.
...
When a new Zorin-public video skill ships:
