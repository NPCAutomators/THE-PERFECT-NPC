"""Centralized logging setup for ZORIN.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.zorin/logs/`` (profile-aware via ``get_zorin_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``zorin_cli.web_server``,
    ``zorin_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Zorin (TUI, gateway, ``hy_memory`` server, MCP
...
from zorin_constants import get_config_path, get_zorin_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_zorin_session_injector", False):
    ...
    _session_record_factory._zorin_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``zorin logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``zorin logs --component gateway``.
    "gateway": ("gateway", "zorin_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("zorin_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "zorin_cli.web_server",
        "zorin_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Zorin logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    zorin_home
        Override for the Zorin home directory.  Falls back to
        ``get_zorin_home()`` (profile-aware).
    log_level
    ...
    home = zorin_home or get_zorin_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_zorin_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._zorin_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the zorin group. However, both ``_open()`` (initial creation)
...
# the module header): when several Zorin processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._zorin_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="zorin-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_zorin_queue", False):
