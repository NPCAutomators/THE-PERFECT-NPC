...
The Zorin docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
NPCAUTOMATORS Portal). This module fetches, validates, and caches that manifest,
...
DEFAULT_CATALOG_URL = (
    "https://portal.npcautomators.com/docs/api/model-catalog.json"
)
...
def _cache_path() -> Path:
    ...
    return get_zorin_home() / "cache" / "model_catalog.json"
...
def get_curated_npcautomators_models() -> list[str] | None:
    """Return NPCAUTOMATORS Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("npcautomators")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Zorin silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`zorin update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``zorin update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.zorin/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``zorin model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
