"""Helpers for loading Zorin .env files consistently across entrypoints."""
...
# load_zorin_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Paths we've already emitted a UTF-32 refuse-to-mangle warning for.
# load_zorin_dotenv can call _sanitize_env_file_if_needed multiple times
# for the same file (user env + project env + hot-reload); once per path
# is enough.
_WARNED_UTF32_PATHS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_zorin_dotenv().
# Used by setup / `zorin model` flows to label detected credentials so
...
# ZORIN_HOME paths we've already pulled external secrets for during this
# process.  ``load_zorin_dotenv()`` is called at module-import time from
# several hot modules (cli.py, zorin_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_zorin_dotenv()`` call.  Returns
...
def get_secret_source_values(
    zorin_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``zorin_home``."""
    home_key = str(Path(zorin_home).resolve())
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(home_key, {}))


def reset_secret_source_cache() -> None:
    """Forget which ZORIN_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `zorin setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``zorin_cli.config._sanitize_env_lines`` which
    already knows all valid Zorin env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from zorin_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_zorin_dotenv(
    *,
    zorin_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Zorin environment files with user config taking precedence.

    Behavior:
    - `~/.zorin/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(zorin_home or os.getenv("ZORIN_HOME", Path.home() / ".zorin"))
    ...
    #   EnvironmentFile=-/path/to/.zorin/.op.env
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of ZORIN_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Zorin reads
    ...
    owns the once-per-ZORIN_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_zorin_dotenv()`` runs at import
    time from several hot modules (cli.py, zorin_cli/main.py,
    ...
    if not cfg:
        ...
        # on its next load_zorin_dotenv() call instead of never.
        return
    ...
    # so the 3-5 import-time load_zorin_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `zorin model`
