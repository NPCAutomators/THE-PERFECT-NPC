"""
zorin fallback — manage the fallback provider chain.
...
Subcommands:
  zorin fallback [list]   Show the current fallback chain (default when no subcommand)
  zorin fallback add      Pick provider + model via the same picker as `zorin model`,
                           then append the selection to the chain
  zorin fallback remove   Pick an entry to delete from the chain
  zorin fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.zorin/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from zorin_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `zorin model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  zorin fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `zorin model`, then append the selection to the chain."""
    from zorin_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `zorin model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `zorin fallback list` to view, or `zorin fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``zorin fallback [subcommand]``."""
