"""Slash command definitions and autocomplete for the Zorin CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Zorin config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Zorin works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Zorin is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.zorin/skills/ for newly installed or removed skills",
    ...
    CommandDef("subscription", "View your NpcAutomators plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your NpcAutomators balance and manage billing on the portal", "Info"),
    ...
    CommandDef("update", "Update ZORIN to the latest version", "Info"),
    CommandDef("version", "Show ZORIN version", "Info", aliases=("v",)),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[npcautomators|local]"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`zorin_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/zorin`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Zorin ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Zorin
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``zorin skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``zorin skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``zorin skills list`` and
...
# unrelated command landed. These claim their slots right after /zorin,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /zorin <command>).
...
# native slot, the alias spelling stays reachable via /zorin reset).
...
# ``/zorin <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/zorin" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /zorin topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /zorin moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /zorin debug on Slack.
_SLACK_VIA_ZORIN_ONLY = frozenset({"topup", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/zorin <verb>`` subcommand.
    ...
    can still reach them via ``/zorin <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/zorin`` is always reserved as the first entry so the
    legacy ``/zorin <subcommand>`` form keeps working for anything that
    ...
    # Reserve /zorin as the catch-all top-level command.
    entries.append(("zorin", "Talk to Zorin or run a subcommand", "[subcommand] [args]"))
    seen.add("zorin")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ZORIN_ONLY:
            # Intentionally Slack-via-/zorin only (see _SLACK_VIA_ZORIN_ONLY).
            return
    ...
    # everything except /zorin, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://zorin-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /zorin handler.

    Maps both canonical names and aliases so /zorin bg do stuff works
    the same as /zorin background do stuff.

    Plugin-registered slash commands are included so ``/zorin <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from zorin_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
