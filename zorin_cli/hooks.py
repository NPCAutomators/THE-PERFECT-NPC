"""zorin hooks — inspect and manage shell-script hooks.

Usage::

    zorin hooks list
    zorin hooks test <event> [--for-tool X] [--payload-file F]
    zorin hooks revoke <command>
    zorin hooks doctor

Consent records live under ``~/.zorin/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.zorin/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``zorin hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: zorin hooks {list|test|revoke|doctor}")
        print("Run 'zorin hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.zorin/config.yaml.")
        print("See `zorin hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `zorin hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `zorin hooks test` and `zorin hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Zorin wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `zorin hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `zorin hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `zorin hooks doctor`.")
