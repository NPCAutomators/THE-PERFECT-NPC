"""``zorin logs`` — view and filter Zorin log files.
...
Usage examples::
    ...
    zorin logs                    # last 50 lines of agent.log
    zorin logs -f                 # follow agent.log in real time
    zorin logs errors             # last 50 lines of errors.log
    zorin logs gateway -n 100    # last 100 lines of gateway.log
    zorin logs gui -f            # follow gui.log (dashboard/pty/ws)
    zorin logs desktop -f        # follow desktop.log (Electron app boot/backend)
    zorin logs --level WARNING    # only WARNING+ lines
    zorin logs --session abc123   # filter by session ID substring
    zorin logs --component tools  # only tool-related lines
    zorin logs --since 1h         # lines from the last hour
    zorin logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_zorin_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Zorin runs — try 'zorin chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_zorin_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_zorin_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_zorin_home()}/logs/")
        return

    print(f"Log files in {display_zorin_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'zorin chat' to generate logs)")
