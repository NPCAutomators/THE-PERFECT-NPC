"""``zorin dashboard register`` — register a self-hosted dashboard OAuth client.

Automates what a user otherwise does by hand: open the NPCAUTOMATORS Portal
``/local-dashboards`` page in a browser, click "register", copy the
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.zorin/.env``
as ``ZORIN_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh NPCAUTOMATORS Portal access token from the existing login
     (``~/.zorin/auth.json``), refreshing it if needed. Fails fast with a
     "run `zorin setup`" hint when the user isn't logged in.
  ...
  3. Writes ``ZORIN_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``ZORIN_DASHBOARD_PORTAL_URL`` into ``~/.zorin/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``ZORIN_DASHBOARD_PORTAL_URL`` env (used for testing against a
      ...
      2. The ``portal_base_url`` stored on the NpcAutomators login — this is the
    ...
    try:
        from zorin_cli.auth import DEFAULT_NPCAUTOMATORS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``zorin dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "NPCAUTOMATORS Portal rejected the access token (401). "
                "Try `zorin auth add npcautomators` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
        ...
        raise RuntimeError(
            f"Could not reach NPCAUTOMATORS Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    from zorin_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    ZORIN_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    ZORIN_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    ZORIN_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
        "  Heads up — NpcAutomators login only *engages* on a non-loopback bind. A plain\n"
        "  `zorin dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require NpcAutomators login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require NpcAutomators login (e.g. exposing on your LAN or a public host):")
        print("    zorin dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from zorin_cli.auth import AuthError, resolve_npcautomators_access_token
    from zorin_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets ZORIN_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `zorin dashboard register` is not available in a managed/hosted "
            "install.\n"
            "  The dashboard OAuth client is provisioned by the hosting platform."
        )
        sys.exit(1)

    # 1. Resolve a fresh NpcAutomators access token (refreshes if near expiry). Fail fast
    #    with a setup hint when the user isn't logged in.
    try:
        access_token = resolve_npcautomators_access_token()
    except AuthError as exc:
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into NPCAUTOMATORS Portal.")
            print("  Run `zorin setup` (or `zorin auth add npcautomators`) first, then retry.")
        else:
            print(f"✗ Could not resolve a NPCAUTOMATORS Portal access token: {exc}")
        sys.exit(1)
    ...
    # ZORIN_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (ZORIN_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("ZORIN_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("ZORIN_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write ZORIN_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  ZORIN_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      ZORIN_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    default_portal = "https://portal.npcautomators.com"
    existing_portal = None
    try:
        existing_portal = get_env_value("ZORIN_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("ZORIN_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # the portal, e.g. https://zorin.example.com/auth/callback. At serve time
    # the dashboard auth layer (dashboard_auth/routes._redirect_uri) reconstructs
    # that same callback by taking ZORIN_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as ZORIN_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("ZORIN_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("ZORIN_DASHBOARD_PUBLIC_URL", public_url)
