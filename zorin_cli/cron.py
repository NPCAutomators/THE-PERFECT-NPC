...
# commands at execution time when ``_ZORIN_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: zorin gateway install", Colors.DIM))
    print(color("                    sudo zorin gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  zorin cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'zorin cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: zorin gateway restart")
        ...
        print("    zorin gateway install    # Install as a user service")
        print("    sudo zorin gateway install --system  # Linux servers: boot-time system service")
        print("    zorin gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: zorin cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
