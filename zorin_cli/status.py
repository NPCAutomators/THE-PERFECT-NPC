"""
Status command for zorin CLI.

Shows the status of all ZORIN components.
"""
...
from zorin_cli.auth import AuthError, resolve_provider
from zorin_cli.colors import Colors, color
from zorin_cli.config import get_env_path, get_env_value, get_zorin_home, load_config
from zorin_cli.models import provider_label
...
from zorin_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``zorin config``'s
...
def show_status(args):
    """Show status of all ZORIN components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ ZORIN Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        npcautomators_status = {}
        codex_status = {}
        qwen_status = {}
        minimax_status = {}

    npcautomators_account_info = None
    if (
        npcautomators_status.get("logged_in")
        or npcautomators_status.get("access_token")
        or npcautomators_status.get("portal_base_url")
        or npcautomators_status.get("inference_credential_present")
        or npcautomators_status.get("error_code")
    ):
        try:
            npcautomators_account_info = get_npcautomators_portal_account_info()
        except Exception:
            npcautomators_account_info = None

    npcautomators_logged_in = bool(
        npcautomators_status.get("logged_in")
        or (npcautomators_account_info and npcautomators_account_info.logged_in)
    )
    npcautomators_inference_present = bool(
        npcautomators_status.get("inference_credential_present")
        or (npcautomators_account_info and npcautomators_account_info.inference_credential_present)
    )
    npcautomators_error = npcautomators_status.get("error")
    if npcautomators_logged_in:
        npcautomators_label = "logged in"
    elif npcautomators_inference_present:
        npcautomators_label = "not logged in (NpcAutomators inference key configured)"
    else:
        npcautomators_label = "not logged in (run: zorin portal)"
    print(
        f"  {'NPCAUTOMATORS Portal':<12}  {check_mark(npcautomators_logged_in)} "
        f"{npcautomators_label}"
    )
    portal_url = npcautomators_status.get("portal_base_url") or "(unknown)"
    inference_url = (
        npcautomators_status.get("inference_base_url")
        or (npcautomators_account_info.inference_base_url if npcautomators_account_info else None)
    )
    access_exp = _format_iso_timestamp(npcautomators_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(npcautomators_status.get("agent_key_expires_at"))
    refresh_label = "yes" if npcautomators_status.get("has_refresh_token") else "no"
    if npcautomators_logged_in or portal_url != "(unknown)" or npcautomators_error:
        print(f"    Portal URL: {portal_url}")
    if npcautomators_inference_present and inference_url:
        print(f"    Inference:  {inference_url}")
    if npcautomators_logged_in or npcautomators_status.get("access_expires_at"):
        print(f"    Access exp: {access_exp}")
    if npcautomators_logged_in or npcautomators_inference_present or npcautomators_status.get("agent_key_expires_at"):
        print(f"    Key exp:    {key_exp}")
    if npcautomators_logged_in or npcautomators_status.get("has_refresh_token"):
        print(f"    Refresh:    {refresh_label}")
    if npcautomators_error:
        print(f"    Error:      {npcautomators_error}")

    codex_logged_in = bool(codex_status.get("logged_in"))
    print(
        f"  {'OpenAI Codex':<12}  {check_mark(codex_logged_in)} "
        f"{'logged in' if codex_logged_in else 'not logged in (run: zorin model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: zorin auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed NpcAutomators/Codex/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: zorin auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    # NpcAutomators Subscription Features
    # =========================================================================
    if managed_npcautomators_tools_enabled():
        features = get_npcautomators_subscription_features(config)
        print()
        print(color("◆ NpcAutomators Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.npcautomators_auth_present:
            print("  NPCAUTOMATORS Portal   ✗ not logged in")
        else:
            print("  NPCAUTOMATORS Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_npcautomators:
                state = "active via NPCAUTOMATORS subscription"
            elif feature.active:
                current = feature.current_provider or "configured provider"
                state = f"active via {current}"
            elif feature.included_by_default and features.npcautomators_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.npcautomators_auth_present:
                state = "available via subscription (optional)"
            else:
                state = "not configured"
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_npcautomators)} {state}")
    elif npcautomators_logged_in or npcautomators_inference_present:
        # NpcAutomators OAuth without entitlement, or an opaque inference key without
        # Portal account information, cannot enable the Tool Gateway.
        print()
        print(color("◆ NpcAutomators Tool Gateway", Colors.CYAN, Colors.BOLD))
        message = format_npcautomators_portal_entitlement_message(
            npcautomators_account_info,
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: zorin model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from zorin_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from zorin_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   zorin gateway")
    ...
    jobs_file = get_zorin_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_zorin_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'zorin doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'zorin setup' to configure", Colors.DIM))
    print()
