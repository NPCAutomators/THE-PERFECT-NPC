...
the tool registry or launching Zorin (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``zorin-cli`` to the non-existent ``zorin``. ``resolve_toolset('zorin')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``zorin-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``zorin`` instead of ``zorin-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `zorin tools` to reconfigure."
        )
    return warnings
