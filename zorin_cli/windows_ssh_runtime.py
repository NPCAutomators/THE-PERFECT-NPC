...
def _root() -> Path:
    return get_zorin_home() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, zorin_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(zorin_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the zorin exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs zorin_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "zorin_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "zorin_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, zorin_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, zorin_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves zorin_cli's own location and prepends its parent, because the
    ...
    # the zorin_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the zorin_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["zorinPath"])
    if not os.path.isabs(configured_path):
        raise ValueError("Zorin path must be absolute")
    zorin_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(zorin_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("Zorin Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('zorin_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_zorin(zorin_path: str) -> dict[str, Any]:
    path = os.path.abspath(zorin_path)
    if not os.path.isabs(zorin_path) or not os.path.isfile(path):
        raise ValueError("Zorin path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "zorinHome": str(get_zorin_home()), "python": sys.executable}
