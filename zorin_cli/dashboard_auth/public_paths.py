...
* ``zorin_cli.web_server.auth_middleware`` — loopback / ``--insecure``
  mode, gates on the ephemeral ``_SESSION_TOKEN``.
* ``zorin_cli.dashboard_auth.middleware.gated_auth_middleware`` —
...
liveness probe (``npcautomators-account-service`` ``fly-provider.ts``
