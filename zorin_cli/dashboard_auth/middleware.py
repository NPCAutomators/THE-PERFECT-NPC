...
from zorin_cli.dashboard_auth import list_session_providers
from zorin_cli.dashboard_auth.audit import AuditEvent, audit_log
from zorin_cli.dashboard_auth.base import (
...
from zorin_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /zorin``, the
    ``login_url`` is prefixed (``/zorin/login?next=...``) so the
    ...
    """
    from zorin_cli.dashboard_auth.prefix import prefix_from_request
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from zorin_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["zorin.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``zorin_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``zorin_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a self-hosted-OIDC session hits the `npcautomators` provider first, which
        # tries to reach NPCAUTOMATORS Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from zorin_cli.dashboard_auth.cookies import clear_session_cookies
        from zorin_cli.dashboard_auth.prefix import prefix_from_request
