...
from ``zorin_constants`` or other zorin_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ZORIN_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``zorin_constants.get_zorin_home`` semantics: env var wins,
    else ``~/.zorin``. A local copy avoids an import cycle with the
    middleware which lives below ``zorin_cli``.
    """
    home = os.environ.get("ZORIN_HOME") or str(Path.home() / ".zorin")
