...
Visual styling mirrors the NPCAUTOMATORS design system (the
``@npcautomators/ui`` package the React dashboard uses): the same
...
``tests/zorin_cli/test_dashboard_auth_401_reauth.py``.
"""
...
<style>
  /* Brand fonts shipped by @npcautomators/ui — same files the SPA loads. */
  @font-face {{
...
<body>
<main>
  <div class="brand">NpcAutomators<span class="dot"></span>Research</div>
  <div class="card">
    <h1>Sign in</h1>
    <p class="subtitle">Choose a sign-in method to continue to the ZORIN dashboard.</p>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Sign-in unavailable — ZORIN</title>
<style>
  @font-face {
    font-family: 'Collapse';
    font-style: normal;
...
<p>Install <code>plugins/dashboard-auth-npcautomators</code> (default) or another
