...
Pure logic: walks a Zorin config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `zorin doctor` and a future `zorin migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Zorin config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Zorin config and return retirement issues.
...
def apply_migration(
    ...
    from zorin_cli.config import require_readable_config_before_write
