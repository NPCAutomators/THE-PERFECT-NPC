...
# xAI's public API is OpenAI-compatible for the endpoints Zorin commonly
# uses. The Responses endpoint is included because Zorin' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Zorin-managed OAuth credentials."""

    auth_hint = "zorin auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`zorin auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`zorin auth reset xai-oauth` or re-authenticate with "
                    "`zorin auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `zorin auth add xai-oauth --type oauth`."
            )
