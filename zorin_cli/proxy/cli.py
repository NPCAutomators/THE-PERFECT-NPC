"""CLI handlers for the ``zorin proxy`` subcommand."""
...
from zorin_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "zorin proxy requires aiohttp. Install one of:\n"
        "  pip install 'zorin-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "npcautomators"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"zorin auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Zorin proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Zorin proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: zorin proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``zorin proxy <subcommand>``."""
    ...
    print(
        "zorin proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  zorin proxy start [--provider npcautomators|xai] [--host 127.0.0.1] [--port 8645]\n"
