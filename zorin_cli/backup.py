"""
Backup and import commands for zorin CLI.

`zorin backup` creates a zip archive of the entire ~/.zorin/ directory
(excluding the zorin-agent repo and transient files).

`zorin import` restores from a backup zip, overlaying onto the current
ZORIN_HOME root.
"""
...
from zorin_constants import get_default_zorin_root, get_zorin_home, display_zorin_home
...
# ``zorin-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/zorin-agent/``
...
# ZORIN_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "zorin-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ZORIN_HOME) —
...
# File names that ``zorin import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the NpcAutomators portal
...
# Reserved archive subtree for provider state that lives OUTSIDE ZORIN_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ZORIN_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to zorin root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``zorin-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/zorin-agent/`` — must be preserved.
        if part == "zorin-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ZORIN_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Zorin home directory."""
    zorin_root = get_default_zorin_root()

    if not zorin_root.is_dir():
        print(f"Error: Zorin home directory not found at {zorin_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"zorin-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"zorin-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_zorin_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(zorin_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(zorin_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``zorin-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "zorin-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(zorin_root)
    ...
    # outside ZORIN_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_zorin_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: zorin import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Zorin backup.
    ...
    # Look for telltale files that a zorin home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Zorin backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.zorin/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a zorin dir name
        if prefix in {".zorin", "zorin"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_zorin_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (zorin_root / "config.yaml").exists()
        has_env = (zorin_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Zorin configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ZORIN_HOME.
            ...
            # NpcAutomators portal. Matched by basename so both the root profile and
            # named profiles (profiles/<name>/gateway_state.json) are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = zorin_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(zorin_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_zorin_home()}."
            )
        ...
        profiles_dir = zorin_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # zorin_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: zorin profile list  (after installing zorin)")

        # Guidance
        print()
        if not (zorin_root / "zorin-agent").is_dir():
            print("Note: The zorin-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: zorin update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  zorin -p {pname} gateway install")

        print("Done. Your Zorin configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and zorin backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ZORIN_HOME).
...
# — `zorin update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ZORIN_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ZORIN_HOME and the entry is silently
...
def _quick_snapshot_root(zorin_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(zorin_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ZORIN_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``zorin update``.
    ...
    Args:
        ...
        zorin_home: Override for the Zorin home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(zorin_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for zorin backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_zorin_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, zorin_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``zorin_root`` to ``out_path``.
...
def _pre_update_backup_dir(zorin_home: Optional[Path] = None) -> Path:
    home = zorin_home or get_zorin_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ZORIN_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ZORIN_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``zorin update``) should continue even if the backup fails.
    """
    zorin_root = zorin_home or get_default_zorin_root()
    if not zorin_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(zorin_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, zorin_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `zorin claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ZORIN_HOME under ``backups/`` before a
    ``zorin claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``zorin import <archive>``.  Writes to
    ``<ZORIN_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `zorin import` and the
