...
- ``zorin_cli.providers``        -- canonical provider identity + overlays
- ``zorin_cli.model_normalize``  -- per-provider name formatting
...
from zorin_cli.providers import (
    ProviderDef,
...
_ZORIN_MODEL_WARNING = (
    "NPCAUTOMATORS Zorin 3 & 4 models are NOT agentic and are not designed "
    "for use with ZORIN. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real NPCAUTOMATORS Zorin 3 / Zorin 4 chat families.
# The previous substring check (`"zorin" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``zorin-brain:qwen3-14b-ctx16k`` that just
# happen to carry "zorin" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NPCAutomators/Zorin-3-Llama-3.1-70B, zorin-4-405b, openrouter/zorin3:70b
# Negative examples it must NOT match:
#   zorin-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NPCAUTOMATORS_ZORIN_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])zorin[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_npcautomators_zorin_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real NpcAutomators Zorin 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NPCAUTOMATORS_ZORIN_NON_AGENTIC_RE.search(model_name))


def _check_zorin_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a NpcAutomators Zorin 3/4 chat model."""
    if is_npcautomators_zorin_non_agentic(model_name):
        return _ZORIN_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``zorin config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from zorin_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "npcautomators")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "npcautomators")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, NpcAutomators, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'zorin model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'zorin doctor' — config issues detected:"
        ...
        from zorin_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from zorin_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from zorin_cli.config import load_config
            from zorin_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from zorin_cli.providers import resolve_user_provider as _ruser
    ...
    if not validation.get("accepted"):
        override = False
        if user_providers:
            from zorin_cli.config import is_provider_enabled
    ...
    # zorin_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from zorin_cli.models import normalize_opencode_base_url
    ...
    zorin_warn = _check_zorin_model_warning(new_model)
    if zorin_warn:
        warnings.append(zorin_warn)
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_npcautomators_tier: bool = False,
    ...
    Uses the curated model lists from zorin_cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_npcautomators_tier`` bypasses the short NpcAutomators tier cache for explicit
    ...
    from zorin_cli.models import (
        ...
        clear_provider_models_cache, get_curated_npcautomators_model_ids,
    )
    ...
    # Compared against zorin_id / mdev_id (section 1), pid / zorin_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from zorin_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by zorin provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "npcautomators" pulls from the remote model-catalog manifest published at
    # https://raw.githubusercontent.com/NPCAutomators/AGENT-ZORIN/main/website/static/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Zorin release. Falls back to the in-repo
    # _PROVIDER_MODELS["npcautomators"] snapshot when the manifest is unreachable.
    curated["npcautomators"] = get_curated_npcautomators_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from zorin_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from zorin_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Zorin-mapped providers ---
    from zorin_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from zorin_cli.models import _PROVIDER_ALIASES as _CANON_ALIASES
    from zorin_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for zorin_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(zorin_id)
        if (
            _alias_target
            and _alias_target != zorin_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        # Resolve the canonical provider profile name.  Skip zorin_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose zorin_id matches the canonical profile name so
        ...
        # same zorin_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if zorin_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(zorin_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ZORIN_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Zorin may not route yet.
        ...
        from zorin_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(zorin_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(zorin_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        zorin_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `zorin model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(zorin_id)
        if not model_ids:
            model_ids = curated.get(zorin_id, [])
            if zorin_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(zorin_id, model_ids)
    ...
    # --- 2. Check Zorin-only providers (npcautomators, openai-codex, copilot, opencode-go) ---
    from zorin_cli.providers import ZORIN_OVERLAYS
    from zorin_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Zorin provider ID.
    # ZORIN_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Zorin IDs ("copilot").
    _mdev_to_zorin = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ZORIN_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Zorin slug — e.g. "github-copilot" → "copilot"
        zorin_slug = _mdev_to_zorin.get(pid, pid)
        if zorin_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or zorin_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(zorin_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, zorin_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or zorin_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(zorin_slug):
                    has_creds = True
                elif for_picker:
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(zorin_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", zorin_slug, exc)
        ...
        # configured.
        if not has_creds and zorin_slug == "anthropic":
            try:
                ...
                if (zorin_creds and zorin_creds.get("accessToken")) or \
        ...
        if zorin_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(zorin_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(zorin_slug)
                model_ids = _ids if _ids else (curated.get(zorin_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(zorin_slug, []) or curated.get(pid, [])
        elif zorin_slug == "npcautomators":
            # NpcAutomators serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `zorin model` picker deliberately shows ONLY the curated agentic
            ...
            # order. Mirror that exactly (see _model_flow_npcautomators in main.py) so
            ...
            model_ids = curated.get("npcautomators", [])
            try:
                from zorin_cli.models import (
                    get_pricing_for_provider as _npcautomators_pricing,
                    check_npcautomators_free_tier as _npcautomators_free,
                    union_with_portal_free_recommendations as _union_free,
                    union_with_portal_paid_recommendations as _union_paid,
                )
                from zorin_cli.auth import get_provider_auth_state as _npcautomators_state

                _pricing = _npcautomators_pricing("npcautomators") or {}
                _portal = ""
                try:
                    _st = _npcautomators_state("npcautomators") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _npcautomators_free(force_fresh=force_fresh_npcautomators_tier):
            ...
            if not model_ids:
                model_ids = curated.get(zorin_slug, []) or curated.get(pid, [])
                if zorin_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(zorin_slug, model_ids)
        total = len(model_ids)
        if zorin_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": zorin_slug,
            "name": get_label(zorin_slug),
            "is_current": zorin_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "zorin",
        ...
        seen_slugs.add(zorin_slug.lower())
        _record_builtin_endpoint(zorin_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ZORIN_OVERLAYS (keeps /model in sync
    # with `zorin model`).
    try:
        from zorin_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Zorin writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Zorin's own writer uses (mirrors section-4 grouping).
        ...
        for grp in ep_groups.values():
            ...
            if should_probe:
                try:
                    from zorin_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Zorin's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, zorin_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from zorin_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`zorin_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from zorin_cli.models import fetch_openrouter_models
