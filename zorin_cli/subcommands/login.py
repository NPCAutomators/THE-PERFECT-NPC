...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``zorin login`` was removed in favor of ``zorin auth`` / ``zorin model``
    (the runtime handler in ``zorin_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``zorin login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``zorin --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``zorin login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``zorin model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `zorin auth` to manage credentials, "
            "`zorin model` to select a provider, or `zorin setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `zorin model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: zorin-cli)"
    )
