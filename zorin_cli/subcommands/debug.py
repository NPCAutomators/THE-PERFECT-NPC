...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for ZORIN. Use 'zorin debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zorin debug share              Upload debug report (asks for confirmation)
    zorin debug share --yes        Skip confirmation (for scripts/CI)
    zorin debug share --lines 500  Include more log lines
    zorin debug share --expire 30  Keep paste for 30 days
    zorin debug share --local      Print report locally (no upload)
    zorin debug share --no-redact  Disable upload-time secret redaction
    zorin debug share --npcautomators       Upload to NpcAutomators-internal storage (private)
    zorin debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to NpcAutomators-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by NpcAutomators staff (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'zorin debug share'",
    )
