"""``zorin gateway`` and ``zorin proxy`` subcommand parsers.

Extracted verbatim from ``zorin_cli/main.py:main()`` (god-file Phase 2).
...
from zorin_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if ZORIN_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy zorin.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Zorin gateway unit files "
            "(e.g. zorin.service) left over from older installs. Profile "
            "units (zorin-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your NPCAUTOMATORS Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.zorin/.env. "
            "Requires being logged in (zorin setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.zorin/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. NPCAUTOMATORS Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="npcautomators",
        help="Upstream provider: npcautomators or xai (default: npcautomators). See `zorin proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
