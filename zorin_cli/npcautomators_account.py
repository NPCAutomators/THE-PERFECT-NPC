"""Normalized NPCAUTOMATORS Portal account entitlement helpers."""
...
NpcAutomatorsAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

# Free tool-pool coverage categories. Kept byte-for-byte aligned with the
# Portal's TOOL_COVERAGE_CATEGORIES (npcautomators-account-service
...
_account_info_cache: tuple[str, float, "NpcAutomatorsPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class NpcAutomatorsPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class NpcAutomatorsPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class NpcAutomatorsToolAccessInfo:
...
@dataclass(frozen=True)
class NpcAutomatorsPortalAccountInfo:
    logged_in: bool
    source: NpcAutomatorsAccountInfoSource
    fresh: bool
    ...
    npcautomators_client: Optional[str] = None
    ...
    subscription: Optional[NpcAutomatorsPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[NpcAutomatorsPaidServiceAccessInfo] = None
    tool_access: Optional[NpcAutomatorsToolAccessInfo] = None
...
def npcautomators_portal_billing_url(account_info: Optional[NpcAutomatorsPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized NPCAUTOMATORS account snapshot."""
    try:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_NPCAUTOMATORS_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def npcautomators_portal_topup_url(account_info: Optional[NpcAutomatorsPortalAccountInfo] = None) -> str:
    ...
    """
    base_billing = npcautomators_portal_billing_url(account_info)  # {base}/billing
...
def format_npcautomators_portal_entitlement_message(
    account_info: Optional[NpcAutomatorsPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing NpcAutomators tool-gateway entitlement.
    ...
    """
    billing_url = npcautomators_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current NPCAUTOMATORS Portal "
    ...
    if account_info is None:
        return (
            f"Zorin could not verify your NPCAUTOMATORS Portal entitlement, so {capability} "
            f"is unavailable. Run `zorin model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"NpcAutomators inference credentials are configured, but Zorin cannot verify "
                f"your NPCAUTOMATORS Portal paid access for {capability}. Log in with "
                f"`zorin model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to NPCAUTOMATORS Portal to use {capability}: run `zorin model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Zorin could not verify your NPCAUTOMATORS Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `zorin model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Zorin could not find a NPCAUTOMATORS Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `zorin model` to "
            f"authenticate again; if the problem persists, contact NpcAutomators support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `zorin model` to refresh Zorin."
        return message

    return (
        f"Your NPCAUTOMATORS Portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your NPCAUTOMATORS Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current NPCAUTOMATORS Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your NPCAUTOMATORS Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your NPCAUTOMATORS Portal account has no usable paid credits{credit_detail}, so "
...
def reset_npcautomators_portal_account_info_cache() -> None:
    """Clear the short-lived account-info cache used by tests."""
    global _account_info_cache
    _account_info_cache = None


def get_npcautomators_portal_account_info(
    *,
    force_fresh: bool = False,
    min_jwt_ttl_seconds: int = 60,
) -> NpcAutomatorsPortalAccountInfo:
    """Return normalized NPCAUTOMATORS Portal account entitlement information.
...
def _fresh_account_info(
    *,
    ...
    try:
        from zorin_cli.auth import get_provider_auth_state, resolve_npcautomators_access_token

        access_token = resolve_npcautomators_access_token()
        ...
        payload = _fetch_npcautomators_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[NpcAutomatorsPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque NpcAutomators keys."""
    try:
        entry = _select_npcautomators_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_npcautomators_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_npcautomators_pool_entry() -> Optional[Any]:
...
def _fetch_npcautomators_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from zorin_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = NpcAutomatorsPaidServiceAccessInfo(
        allowed=paid_access,
    ...
    return NpcAutomatorsPortalAccountInfo(
        ...
        npcautomators_client=_coerce_str(claims.get("npcautomators_client")),
        portal_base_url=portal_base_url,
...
def _tool_access_from_value(value: Any) -> Optional[NpcAutomatorsToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`NpcAutomatorsToolAccessInfo`. Fails closed: a non-object value
    ...
    return NpcAutomatorsToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[NpcAutomatorsPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return NpcAutomatorsPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[NpcAutomatorsPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return NpcAutomatorsPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
