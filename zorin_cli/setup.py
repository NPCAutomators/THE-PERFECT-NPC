"""
Interactive setup wizard for ZORIN.
...
from tools.tool_backend_helpers import managed_npcautomators_tools_enabled
from utils import base_url_hostname
from zorin_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs"
...
# display_zorin_home imported lazily at call sites (stale-module safety during zorin update)
...
from zorin_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from zorin_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Zorin Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Zorin using environment variables or config commands:")
    print_info("  zorin config set model.provider custom")
    print_info("  zorin config set model.base_url http://localhost:8080/v1")
    print_info("  zorin config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'zorin setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``ZORIN_NONINTERACTIVE=1`` (see ``zorin_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("ZORIN_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``ZORIN_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from zorin_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'zorin setup')")


def _print_setup_summary(config: dict, zorin_home):
    ...
    tool_status = []
    subscription_features = get_npcautomators_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'zorin setup' to configure"))


    # Web tools (Exa, Parallel, Firecrawl, or Tavily)
    if subscription_features.web.managed_by_npcautomators:
        tool_status.append(("Web Search & Extract (NPCAUTOMATORS subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_npcautomators:
        tool_status.append(("Browser Automation (NpcAutomators Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via NpcAutomators), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_npcautomators:
        tool_status.append(("Image Generation (NPCAUTOMATORS subscription)", True, None))
        ...
        try:
            ...
            from zorin_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `zorin tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_npcautomators:
        tool_status.append(("Video Generation (FAL via NPCAUTOMATORS subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from zorin_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_npcautomators:
        tool_status.append(("Text-to-Speech (OpenAI via NPCAUTOMATORS subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'zorin setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'zorin setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_npcautomators:
        tool_status.append(("Modal Execution (NPCAUTOMATORS subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'zorin setup terminal'"))
    elif managed_npcautomators_tools_enabled() and subscription_features.npcautomators_auth_present:
        tool_status.append(("Modal Execution (optional via NPCAUTOMATORS subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via zorin auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'zorin setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {zorin_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('zorin setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('zorin setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('zorin setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('zorin setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('zorin setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('zorin config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('zorin config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('zorin config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('zorin', Colors.GREEN)}              Start chatting")
    print(f"   {color('zorin gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('zorin doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `zorin tools` and `zorin setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``zorin model``)
    ...
    provider added to ``zorin model`` is automatically available here.
    ...
    # Delegate to the shared zorin model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from zorin_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: zorin model")
    ...
    # on demand via `zorin auth add`, `zorin setup` vision, and
    # `zorin setup tts`. This keeps both quick and full setup thin.

    # Tool Gateway prompt is already shown by _model_flow_npcautomators() above.
    save_config(config)
...
def _install_neutts_deps() -> bool:
    ...
    from zorin_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``zorin model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from zorin_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_npcautomators_tools_enabled() and subscription_features.npcautomators_auth_present:
        choices.append("NpcAutomators Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("npcautomators-openai")
    choices.extend(
        [
    ...
    selected_via_npcautomators = selected == "npcautomators-openai"
    if selected == "npcautomators-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed NpcAutomators gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.zorin/.env."
            )
    ...
    if selected == "neutts":
    ...
    elif selected == "openai" and not selected_via_npcautomators:
        ...
        # via the Zorin auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via zorin setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'zorin setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Zorin runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `zorin setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `zorin setup terminal`.
        ...
        managed_modal_available = bool(
            managed_npcautomators_tools_enabled()
            and
            get_npcautomators_subscription_features(config).npcautomators_auth_present
        ...
        if managed_modal_available:
            modal_choices = [
                "Use my NPCAUTOMATORS subscription",
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed NpcAutomators gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ZORIN_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ZORIN_MAX_ITERATIONS")
    ...
    print_info("  Run `zorin setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ZORIN_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.zorin/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from zorin_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_zorin_home(Path(get_zorin_home()))
    except Exception:
        pass
...
def _profile_name_from_zorin_home(zorin_home) -> str | None:
    """Return the active profile name when ZORIN_HOME is a profile dir."""
    if zorin_home.parent.name == "profiles":
        return zorin_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Zorin delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Zorin to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from zorin_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  zorin config edit")
    print_info("   Open config in your editor:  zorin config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from zorin_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Zorin from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'zorin setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     zorin config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_zorin_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: zorin gateway install --system")
                print_info("  Or run in foreground:  zorin gateway")
        else:
            from zorin_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   zorin gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   zorin gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `zorin setup tools` and `zorin tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from zorin_cli.config
    ...
    if section_key == "model":
        ...
        from zorin_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_zorin.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_zorin migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_zorin", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Zorin.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Zorin to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Zorin at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Zorin at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Zorin at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Zorin at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Zorin equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Zorin config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Zorin.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Zorin's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(zorin_home: Path) -> bool:
    ...
    print_info("Zorin can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: zorin claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=zorin_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: zorin claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Zorin configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Zorin config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Zorin (use zorin claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot NPCAUTOMATORS Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``zorin setup --portal`` and ``zorin portal``. This is the
    NpcAutomators-Portal slice of the first-time quick setup, collapsed into a single
    shareable command so a brand-new user goes from zero to a fully working
    Zorin session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``zorin setup`` and hunt for the quick-setup option.

    The login + model selection + provider switch + Tool Gateway opt-in are all
    delegated to ``_model_flow_npcautomators`` — the exact same flow quick setup uses
    (``_run_first_time_quick_setup``) and the same one ``zorin model`` runs
    when you pick NpcAutomators. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``zorin portal`` always offers a model picker,
    and there is a single source of truth for the NpcAutomators onboarding steps.
    """
    ...
    print(color("│     ⚕ Zorin Setup — NPCAUTOMATORS Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your NPCAUTOMATORS Portal sub.")
    print()
    print_info("  Sign up: https://portal.npcautomators.com/manage-subscription")
    print()

    # _model_flow_npcautomators handles BOTH the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # NpcAutomators model picker), then offers the Tool Gateway opt-in and sets
    # provider=npcautomators via the login/model save. This is the same routine quick
    # setup calls, so `zorin portal` == quick setup's NpcAutomators step.
    try:
        from zorin_cli.main import _model_flow_npcautomators

        _model_flow_npcautomators(config)
    except (KeyboardInterrupt, EOFError, SystemExit):
        # _login_npcautomators raises SystemExit(130)/(1) on cancel/failure; the
        # logged-out path inside _model_flow_npcautomators catches it, but the
        ...
        print_info("  You can retry later with `zorin portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_npcautomators error during `zorin portal`: %s", exc)
        print()
        print_error(f"  NPCAUTOMATORS Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `zorin portal`.")
        return

    # Re-sync the in-memory config from disk — _model_flow_npcautomators (and the
    ...
    print_info("  Run `zorin portal info` to inspect routing.")
    print_info("  Run `zorin` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      zorin setup           — full or quick (auto-detected)
      zorin setup model     — just model/provider
      zorin setup tts       — just text-to-speech
      zorin setup terminal  — just terminal backend
      zorin setup gateway   — just messaging platforms
      zorin setup tools     — just tool configuration
      zorin setup agent     — just agent settings
    """
    from zorin_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot NPCAUTOMATORS Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Zorin Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ ZORIN Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your ZORIN installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, zorin_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Zorin configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'zorin setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(zorin_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Zorin?",
            [
                "Quick Setup (NPCAUTOMATORS Portal) — free OAuth login, no API keys, model + tools (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, zorin_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, zorin_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {zorin_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'zorin config edit'")
    ...
    # Tune later with `zorin setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, zorin_home)


def _run_first_time_quick_setup(config: dict, zorin_home, is_existing: bool):
    """Streamlined first-time setup via NPCAUTOMATORS Portal: OAuth, model, terminal & messaging.

    Routes straight to the NPCAUTOMATORS Portal provider — runs the device-code OAuth
    login, picks a NpcAutomators model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``zorin setup <section>``
    or switch providers with ``zorin model``.
    """
    from zorin_cli.config import load_config

    # Step 1: NPCAUTOMATORS Portal — OAuth login + model selection.
    # _model_flow_npcautomators() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # NpcAutomators model picker). Provider is set to "npcautomators" by the login/model save.
    print()
    print_header("NPCAUTOMATORS Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.npcautomators.com/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("NPCAUTOMATORS Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_npcautomators error during quick setup: %s", exc)
        print_warning(f"NPCAUTOMATORS Portal setup encountered an error: {exc}")
        print_info("You can try again later with: zorin model")

    # Re-sync the wizard's config dict from disk — _model_flow_npcautomators (and the
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'zorin setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    zorin setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  zorin setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``zorin tools`` (which
    ...
    try:
        ...
        from zorin_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``zorin setup agent`` /
    ``zorin config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, zorin_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `zorin update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        zorin tools")
        print_info("  Seed skills:         zorin skills opt-in --sync")
        print_info("  Add MCP servers:     zorin mcp add")
        print_info("  Enable plugins:      zorin plugins")
        print_info("  Tune agent settings: zorin setup agent")
        print()
        _print_setup_summary(config, zorin_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, zorin_home)


def _blank_slate_walkthrough(config: dict, zorin_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`zorin update` runs from re-injecting them. Opt back in any")
            print_info("time with `zorin skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `zorin tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `zorin plugins list` / `zorin plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `zorin plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `zorin mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `zorin mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   zorin tools")
    print_info("  Seed skills:         zorin skills opt-in --sync")
    print_info("  Add MCP servers:     zorin mcp add")
    print_info("  Tune agent settings: zorin setup agent")
    print()
...
def _run_quick_setup(config: dict, zorin_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'zorin setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Zorin to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'zorin setup gateway'.")
