"""Shell completion script generation for zorin CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_zorin_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# ZORIN bash completion
# Add to ~/.bashrc:
#   eval "$(zorin completion bash)"

_zorin_profiles() {{
    local profiles_dir="$HOME/.zorin/profiles"
...
_zorin_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_zorin_profiles)" -- "$cur"))
        return
    fi
...
complete -F _zorin_completion zorin
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef zorin
# ZORIN zsh completion
# Add to ~/.zshrc:
...
_zorin_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.zorin/profiles" ]]; then
        profiles+=($HOME/.zorin/profiles/*(N/:t))
    fi
...
_zorin() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_zorin_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'zorin command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# ZORIN fish completion",
        "# Add to your config:",
        "#   zorin completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __zorin_profiles",
        "    echo default",
        "    if test -d $HOME/.zorin/profiles",
        "        for d in $HOME/.zorin/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c zorin -f -s p -l profile"
        " -d 'Profile name' -xa '(__zorin_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__zorin_profiles)' -d 'Profile name'"
                )
