"""
Dump command for zorin CLI.

Outputs a compact, plain-text summary of the user's Zorin setup
...
from zorin_cli.config import get_zorin_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.zorin/.env.
    ...
    exports. ``zorin debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.zorin_build_sha`` by the Dockerfile's
    ``ZORIN_GIT_SHA`` build-arg (see ``zorin_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``zorin dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from zorin_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(zorin_home: Path) -> int:
...
def _cron_summary(zorin_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = zorin_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"zorin_home:      {display_zorin_home()}")
    ...
    api_keys = [
        ...
        ("NPCAUTOMATORS_API_KEY", "npcautomators"),
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.zorin/.env: a managed
        ...
        # A credential added via `zorin auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `zorin auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["zorin-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(zorin_home)}")
    lines.append(f"  skills:             {_count_skills(zorin_home)}")
