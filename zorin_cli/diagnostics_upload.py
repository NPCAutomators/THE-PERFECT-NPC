"""Client for uploading ``zorin debug share`` bundles to NpcAutomators-internal S3.

This is the opt-in (``--npcautomators``) destination for ``zorin debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a NpcAutomators-owned
S3 bucket via a short-lived signed URL minted by the NPCAUTOMATORS account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by NpcAutomators staff (and allowlisted Discord mods) through a
...
# Base URL of the NPCAUTOMATORS account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "ZORIN_DIAGNOSTICS_BASE_URL", "https://portal.npcautomators.com"
)
...
_USER_AGENT = "zorin-agent/debug-share"
...
def share_to_npcautomators(report_bundle: bytes) -> dict:
    """Orchestrate the full NpcAutomators-S3 upload of a gzipped *report_bundle*.
