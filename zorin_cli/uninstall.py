...
- Keep data: Remove code but keep ~/.zorin/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Zorin PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing zorin-agent or zorin PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# ZORIN" comment and following line
                if '# ZORIN' in line or '# zorin-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('zorin' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing zorin
                if 'zorin' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the zorin wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "zorin",
        Path("/usr/local/bin/zorin"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains zorin_cli reference)
                content = wrapper.read_text()
                if 'zorin_cli' in content or 'zorin-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(zorin_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``zorin`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Zorin home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (zorin_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``zorin gateway run`` processes
    ...
    try:
        from zorin_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from zorin_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ZORIN_HOME`` and ``ZORIN_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\zorin\git\cmd``, ``%LOCALAPPDATA%\zorin\git\bin``,
#      ``%LOCALAPPDATA%\zorin\git\usr\bin``, ``%LOCALAPPDATA%\zorin\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\zorin\git\`` and Node to
#      ``%LOCALAPPDATA%\zorin\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``zorin dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\zorin\gateway-service\`` and sometimes
...
def _zorin_path_markers(zorin_home: Path) -> list[str]:
    """Path-entry substrings that identify Zorin-owned User-PATH entries."""
    root = str(zorin_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare zorin-agent install dir.
    markers = [root + "\\zorin-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ZORIN_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "zorin".  We don't want to catch
    # unrelated entries like "czorin-foo" or "ephermeral", so we look for
    # backslash-zorin as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(zorin_home: Path) -> list[str]:
    """Strip Zorin-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _zorin_path_markers(zorin_home)
            kept: list[str] = []
...
def remove_zorin_env_vars_windows() -> list[str]:
    """Delete ZORIN_HOME and ZORIN_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ZORIN_HOME", "ZORIN_GIT_BASH_PATH"):
                try:
...
def _is_default_zorin_home(zorin_home: Path) -> bool:
    """Return True when ``zorin_home`` points at the default (non-profile) root."""
    try:
        from zorin_constants import get_default_zorin_root
        return zorin_home.resolve() == get_default_zorin_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ZORIN_HOME directory.

    We shell out to ``zorin -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ZORIN_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m zorin_cli.main` so we don't depend on a `zorin`
    #    wrapper that may be half-removed mid-uninstall.
    zorin_invocation = [_sys.executable, "-m", "zorin_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                zorin_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ZORIN_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``zorin uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ZORIN_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from zorin_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(zorin_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Zorin Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Zorin Chat GUI installation was found.")
        print(f"  Checked: {zorin_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Zorin agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(zorin_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Zorin agent at {zorin_home / 'zorin-agent'}")
        print(f"  • Your config, sessions, and secrets under {zorin_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Zorin agent is still installed. Run 'zorin' to use the CLI,")
    print("or 'zorin uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.zorin/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.zorin/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie ZORIN_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_zorin_home(zorin_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.zorin data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ ZORIN Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {zorin_home / 'config.yaml'}")
    print(f"  Secrets: {zorin_home / '.env'}")
    print(f"  Data:    {zorin_home / 'cron/'}, {zorin_home / 'sessions/'}, {zorin_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ZORIN_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Zorin data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Zorin code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, zorin_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Zorin PATH entries from shell configs / Windows User PATH")
    print("  • Zorin wrapper scripts and Zorin-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Zorin config/data: {zorin_home}")
        if _is_default_zorin_home(zorin_home):
        ...
        print(f"  • Keep Zorin config/data: {zorin_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``zorin`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ZORIN_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in zorin_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\zorin\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(zorin_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Zorin-owned PATH entries in User environment")

        log_info("Removing ZORIN_HOME / ZORIN_GIT_BASH_PATH User env vars...")
        removed_env = remove_zorin_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Zorin-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing zorin command...")
    ...
    #     (only when they still point into this Zorin home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Zorin-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(zorin_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Zorin-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(zorin_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ZORIN_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from zorin_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(zorin_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.zorin/, just remove the zorin-agent subdir
            if zorin_home in project_root.parents or project_root.parent == zorin_home:
    ...
    #     under ZORIN_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(zorin_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(zorin_home)
    ...
    # 5. Optionally remove ~/.zorin/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ZORIN_HOME dirs live under
        ...
        try:
            if zorin_home.exists():
                shutil.rmtree(zorin_home)
                log_success(f"Removed {zorin_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {zorin_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {zorin_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NPCAutomators/AGENT-ZORIN/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NPCAutomators/AGENT-ZORIN/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using ZORIN! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m zorin_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import zorin_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``zorin_constants`` + ``zorin_cli.colors``
    (and lazily ``zorin_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m zorin_cli.uninstall")
    parser.add_argument(
        "--mode",
