"""On-demand supply-chain audit for ZORIN installs.

Scans three surfaces a Zorin user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Zorin venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.zorin/plugins``
...
def _discover_plugins(zorin_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.zorin/plugins``.
...
def run_audit(
    *,
    ...
    home = zorin_home or Path(get_zorin_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, zorin_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(zorin_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `zorin security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, zorin_home=home
    )
    if total == 0:
