#!/usr/bin/env python3
"""
Zorin CLI - Main entry point.

Usage:
    zorin                     # Interactive chat (default)
    zorin chat                # Interactive chat
    zorin gateway             # Run gateway in foreground
    zorin gateway start       # Start gateway as service
    zorin gateway stop        # Stop gateway service
    zorin gateway status      # Show gateway status
    zorin gateway install     # Install gateway service
    zorin gateway uninstall   # Uninstall gateway service
    zorin setup               # Interactive setup wizard
    zorin logout              # Clear stored authentication
    zorin status              # Show status of all components
    zorin cron                # Manage cron jobs
    zorin cron list           # List cron jobs
    zorin cron status         # Check if cron scheduler is running
    zorin doctor              # Check configuration and dependencies
    zorin honcho setup                    # Configure Honcho AI memory integration
    zorin honcho status                   # Show Honcho config and connection status
    zorin honcho sessions                 # List directory → session name mappings
    zorin honcho map <name>               # Map current directory to a session name
    zorin honcho peer                     # Show peer names and dialectic settings
    zorin honcho peer --user NAME         # Set user peer name
    zorin honcho peer --ai NAME           # Set AI peer name
    zorin honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    zorin honcho mode                     # Show current memory mode
    zorin honcho mode [hybrid|honcho|local]  # Set memory mode
    zorin honcho tokens                   # Show token budget settings
    zorin honcho tokens --context N       # Set session.context() token cap
    zorin honcho tokens --dialectic N     # Set dialectic result char cap
    zorin honcho identity                 # Show AI peer identity representation
    zorin honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    zorin honcho migrate                  # Step-by-step migration guide: OpenClaw native → Zorin + Honcho
    zorin version             Show version
    zorin update              Update to latest version
    zorin uninstall           Uninstall ZORIN
    zorin acp                 Run as an ACP server for editor integration
    zorin sessions browse     Interactive session picker with search

    zorin claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: zorin_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``zorin_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``zorin update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``zorin_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, zorin crashes on import and the user can't run
# ``zorin update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import zorin_bootstrap  # noqa: F401
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from zorin_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'zorin' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``zorin tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``zorin.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("zorin")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"zorin", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"zorin")
        # Windows: the .exe name is already ``zorin.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* zorin_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ZORIN_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".zorin", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``ZORIN_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``zorin … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("ZORIN_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``ZORIN_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ZORIN_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`zorin --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from zorin_cli import __release_date__, __version__

    print(f"ZORIN v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``zorin --version`` before config/logging imports on Termux."""
    if os.environ.get("ZORIN_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from zorin_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from zorin_cli.subcommands.cron import build_cron_parser
from zorin_cli.subcommands.gateway import build_gateway_parser
from zorin_cli.subcommands.profile import build_profile_parser
from zorin_cli.subcommands.model import build_model_parser
from zorin_cli.subcommands.setup import build_setup_parser
from zorin_cli.subcommands.postinstall import build_postinstall_parser
from zorin_cli.subcommands.whatsapp import build_whatsapp_parser
from zorin_cli.subcommands.slack import build_slack_parser
from zorin_cli.subcommands.login import build_login_parser
from zorin_cli.subcommands.logout import build_logout_parser
from zorin_cli.subcommands.auth import build_auth_parser
from zorin_cli.subcommands.status import build_status_parser
from zorin_cli.subcommands.webhook import build_webhook_parser
from zorin_cli.subcommands.hooks import build_hooks_parser
from zorin_cli.subcommands.doctor import build_doctor_parser
from zorin_cli.subcommands.security import build_security_parser
from zorin_cli.subcommands.dump import build_dump_parser
from zorin_cli.subcommands.debug import build_debug_parser
from zorin_cli.subcommands.backup import build_backup_parser
from zorin_cli.subcommands.import_cmd import build_import_cmd_parser
from zorin_cli.subcommands.config import build_config_parser
from zorin_cli.subcommands.skin import build_skin_parser
from zorin_cli.subcommands.console import build_console_parser
from zorin_cli.subcommands.version import build_version_parser
from zorin_cli.subcommands.update import build_update_parser
from zorin_cli.subcommands.uninstall import build_uninstall_parser
from zorin_cli.subcommands.dashboard import build_dashboard_parser
from zorin_cli.subcommands.gui import build_gui_parser
from zorin_cli.subcommands.logs import build_logs_parser
from zorin_cli.subcommands.prompt_size import build_prompt_size_parser
from zorin_cli.subcommands.memory import build_memory_parser
from zorin_cli.subcommands.acp import build_acp_parser
from zorin_cli.subcommands.tools import build_tools_parser
from zorin_cli.subcommands.insights import build_insights_parser
from zorin_cli.subcommands.skills import build_skills_parser
from zorin_cli.subcommands.pairing import build_pairing_parser
from zorin_cli.subcommands.plugins import build_plugins_parser
from zorin_cli.subcommands.mcp import build_mcp_parser
from zorin_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (zorin tools, zorin setup, zorin model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'zorin {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any zorin module import.
#
# Many modules cache ZORIN_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ZORIN_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.zorin/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ZORIN_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `zorin mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Zorin' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo zorin -p <name>` against the invoking user's home.
        ...
        candidate = home / ".zorin" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`zorin chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors zorin_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ZORIN_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.zorin/profiles/coder or
    # /opt/data/profiles/coder).  If ZORIN_HOME points to the zorin root
    # instead (e.g. systemd hardcodes ZORIN_HOME=/root/.zorin), we must
    # still read active_profile — the user may have switched profiles via
    # `zorin profile use` and the gateway should honour that choice.
    # See issue #22502.
    zorin_home_env = os.environ.get("ZORIN_HOME", "")
    if profile_name is None and zorin_home_env:
        if Path(zorin_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the zorin root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as ZORIN_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``zorin gateway run`` to mean
    # "the root ZORIN_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("ZORIN_S6_SUPERVISED_CHILD"):
        try:
            from zorin_constants import get_default_zorin_root

            active_path = get_default_zorin_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ZORIN_HOME
    if profile_name is not None:
        try:
            from zorin_cli.profiles import resolve_profile_env

            zorin_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            zorin_home = _resolve_sudo_user_profile_env(profile_name)
            if not zorin_home:
            ...
            # A bug in profiles.py must NEVER prevent zorin from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ZORIN_HOME"] = zorin_home
...
# Load .env from ~/.zorin/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from zorin_cli.config import get_zorin_home
from zorin_cli.env_loader import load_zorin_dotenv

load_zorin_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ZORIN_REDACT_SECRETS env
# var BEFORE zorin_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_zorin_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from zorin_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "ZORIN_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ZORIN_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `zorin` subcommands
...
try:
    from zorin_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from zorin_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if zorin_constants not importable yet
...
from zorin_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to zorin_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (zorin_cli.main._model_flow_*) keep resolving unchanged.
from zorin_cli.model_setup_flows import (
    _prompt_auth_credentials_choice,
    _model_flow_openrouter,
    _model_flow_npcautomators,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_zorin_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ZORIN_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ZORIN_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from zorin_cli.config import get_env_path, get_zorin_home, load_config
    from zorin_cli.auth import get_auth_status

    # Determine whether Zorin itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from zorin_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_zorin_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from zorin_cli.auth import PROVIDER_REGISTRY
    ...
    # Check for NPCAUTOMATORS Portal OAuth credentials
    auth_file = get_zorin_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Zorin has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Zorin to use their tokens.
    if _has_zorin_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from zorin_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, zorin_bin
        cli_args: the original CLI arguments (everything after 'zorin')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    zorin_bin = container_info["zorin_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo zorin {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo zorin {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  zorin --tui --resume {target}")
    if title:
        print(f'  zorin --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @zorin/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@zorin" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/zorin-ink/package.json",
    "packages/zorin-ink/index.js",
    "packages/zorin-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ZORIN_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ZORIN_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ZORIN_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ZORIN_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    zorin_home = os.environ.get("ZORIN_HOME") or str(Path.home() / ".zorin")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ZORIN_HOME": zorin_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(zorin_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(zorin_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if zorin_cli_dir is None:
        zorin_cli_dir = Path(__file__).parent
    bundled = zorin_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``zorin update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ZORIN_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Zorin checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `zorin update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Zorin checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `zorin --tui`\n"
        "If the checkout is still inconsistent, run `zorin update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ZORIN_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ZORIN_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from zorin_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ZORIN_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ZORIN_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ZORIN_TUI_DIR (e.g. `unset ZORIN_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # ships zorin_cli/tui_dist/entry.js in the wheel but never ships ui-tui/
    ...
    if tui_dev:
        # Keep the local @zorin/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @zorin/ink resolves through
        # packages/zorin-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "zorin-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from zorin_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("ZORIN_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["ZORIN_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("ZORIN_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["ZORIN_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("ZORIN_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["ZORIN_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["ZORIN_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from zorin_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="zorin-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ZORIN_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ZORIN_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["ZORIN_MODEL"] = model
        env["ZORIN_INFERENCE_MODEL"] = model
    if provider:
        env["ZORIN_TUI_PROVIDER"] = provider
        env["ZORIN_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ZORIN_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ZORIN_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ZORIN_TUI_SKILLS"] = value
    if query:
        env["ZORIN_TUI_QUERY"] = query
    if image:
        env["ZORIN_TUI_IMAGE"] = image
    if checkpoints:
        env["ZORIN_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ZORIN_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ZORIN_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ZORIN_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ZORIN_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ZORIN_ACCEPT_HOOKS"] = "1"
    ...
    # ZORIN_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `zorin --tui` try to
    ...
    # still set ZORIN_TUI_RESUME themselves.
    env.pop("ZORIN_TUI_RESUME", None)
    if resume_session_id:
        env["ZORIN_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `zorin update` so
    ...
    if code == 42:
        from zorin_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ZORIN_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``zorin kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``zorin kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ZORIN_KANBAN_BOARD"):
        return
    try:
        from zorin_cli.kanban_db import get_current_board

        os.environ["ZORIN_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.zorin/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Zorin — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Zorin still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``zorin … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("ZORIN_TUI") == "1":
        return True
    try:
        from zorin_cli.config import load_config
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'zorin sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from zorin_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from zorin_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'zorin doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Zorin isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  zorin setup")
        print()

        from zorin_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'zorin setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from zorin_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ZORIN_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.zorin/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ZORIN_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ZORIN_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ZORIN_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from zorin_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from zorin_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from zorin_cli.config import get_env_value, save_env_value
    from zorin_constants import find_node_executable, with_zorin_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Zorin?")
        print()
    ...
    # has no creds.json.  Every subsequent `zorin gateway` then paid a 30s
    ...
    session_dir = get_zorin_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: zorin gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `zorin gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  zorin gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ ZORIN'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  zorin gateway")
        ...
        print()
        print("  Or install as a service: zorin gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'zorin whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``zorin whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``zorin_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from zorin_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from zorin_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from zorin_cli.config import stamp_install_method
    from zorin_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Zorin post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from zorin_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``zorin model``) and the setup wizard
    ...
    from zorin_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ZORIN_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from zorin_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'zorin model' for "
                "available providers, or run 'zorin doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in zorin_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``zorin model``
    ...
    # matching against zorin_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    elif selected_provider == "npcautomators":
        _model_flow_npcautomators(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.zorin/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.zorin/.env if the active provider is not 'custom'.
    ...
    """
    from zorin_cli.config import get_env_value, save_env_value, load_config
...
#
# Zorin uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `zorin model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `zorin model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`zorin_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from zorin_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from zorin_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Zorin only falls back to a lightweight backend (OpenRouter,")
        print("  NPCAUTOMATORS Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``zorin model`` flow, then route aux tasks to them here.
    """
    from zorin_cli.config import load_config
    from zorin_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from zorin_cli.auth import _prompt_model_selection
    from zorin_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from zorin_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from zorin_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from zorin_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Zorin URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from zorin_cli.curses_ui import curses_radiolist
...
# downstream call sites read `zorin_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `zorin` invocation — including
# fast paths like `zorin --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from zorin_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``zorin setup`` / ``zorin model``.
    ...
    recover from a malformed paste without editing ``~/.zorin/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from zorin_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from zorin_cli.config import save_env_value
    from zorin_cli.secret_prompt import masked_secret_prompt
    ...
    from zorin_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `zorin setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from zorin_constants import display_zorin_home as _dhh_fn

            print(
                f"    Zorin will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Zorin CLI with a provider."""
    from zorin_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from zorin_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from zorin_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from zorin_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from zorin_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from zorin_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``zorin slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: zorin slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `zorin slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from zorin_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from zorin_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from zorin_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from zorin_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from zorin_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `zorin security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from zorin_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from zorin_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from zorin_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from zorin_cli.config import config_command
...
def cmd_skin(args):
    ...
    from zorin_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Zorin home directory to a zip file."""
    if getattr(args, "quick", False):
        from zorin_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from zorin_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Zorin backup from a zip file."""
    from zorin_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from zorin_cli.config import detect_install_method
    from zorin_cli.banner import format_banner_version_label
    ...
    try:
        from zorin_cli.banner import check_for_updates
        from zorin_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall ZORIN (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from zorin_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from zorin_cli.uninstall import run_gui_uninstall
    ...
    from zorin_cli.uninstall import run_uninstall
...
# Critical files that Zorin must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "zorin_cli/main.py",
    "zorin_cli/config.py",
    "zorin_cli/__init__.py",
    "zorin_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``zorin`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="zorin-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``zorin update --gateway`` so interactive prompts (stash restore,
    config migration) are forwarded to the messenger instead of being silently
    skipped.
    """
    import json as _json
    import uuid as _uuid
    from zorin_constants import get_zorin_home
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``zorin update``
    ...
    ``zorin_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "zorin_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $ZORIN_HOME."""
    from zorin_constants import get_zorin_home
    return get_zorin_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``zorin`` launcher present but ``zorin_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the zorin venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — zorin venv python3, no nix-shell overhead
    ...
    # a zorin venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``zorin update`` to stash the
    lockfile — repeatedly.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "zorin_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``zorin web``).
    ...
    # build path usable on a stock `py -m zorin_cli.main web` invocation.
    ...
    from zorin_constants import with_zorin_node_path
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_zorin_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (zorin web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``zorin update`` can unconditionally call ``zorin desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``zorin desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ZORIN_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ZORIN_HOME."""
    from zorin_constants import get_zorin_home
    return get_zorin_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``zorin update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Zorin.app/Contents/MacOS/Zorin"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Zorin.exe",
            release_dir / "win-ia32-unpacked" / "Zorin.exe",
            release_dir / "win-arm64-unpacked" / "Zorin.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "zorin",
            release_dir / "linux-unpacked" / "Zorin",
            release_dir / "linux-arm64-unpacked" / "zorin",
            release_dir / "linux-arm64-unpacked" / "Zorin",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Zorin'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_zorin_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Zorin.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Zorin.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Zorin.exe: Access is
    ...
    an unrelated "Zorin" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Zorin is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Zorin.app/Contents/MacOS/Zorin  ->  app bundle = .../Zorin.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``zorin desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Zorin Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Zorin Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the ZORIN_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from zorin_logging import setup_logging as _setup_logging_gui
    ...
    # with_zorin_node_path() copies os.environ when called with no arg.
    env = with_zorin_node_path()
    if getattr(args, "fake_boot", False):
        env["ZORIN_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ZORIN_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "zorin_root", None):
        env["ZORIN_DESKTOP_ZORIN_ROOT"] = str(Path(args.zorin_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ZORIN_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["ZORIN_DESKTOP_CWD"] = os.getcwd()
    ...
    # `ZORIN_DESKTOP_DISABLE_GPU=... zorin desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "ZORIN_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ZORIN_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  zorin gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Zorin-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → zorin-setup →
            # zorin update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `zorin update` path).
            nixos_env = with_zorin_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Zorin.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Zorin.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Zorin.exe, close any")
                    print("  running Zorin desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> zorin desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Zorin is
    ...
    if source_mode:
        print("→ Launching Zorin Desktop from source build...")
    ...
    print(f"→ Launching packaged Zorin Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``zorin dashboard`` processes other than ourselves.

    ``zorin dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``zorin update`` replaces files on disk, the running
    ...
    ``zorin-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Zorin Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``zorin serve`` as
    ...
    environment variable ``ZORIN_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "zorin dashboard",
        "zorin_cli.main dashboard",
        "zorin_cli/main.py dashboard",
        # The headless backend (`zorin serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "zorin serve",
        "zorin_cli.main serve",
        "zorin_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            from zorin_cli._subprocess_compat import windows_hide_flags
            ...
            # (rather than `pgrep -f "zorin.*dashboard"`) keeps us consistent
            # with `zorin_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `zorin update`.
    ...
    print("  Preview now:  zorin curator run --dry-run")
    print("  Pause it:     zorin curator pause")
    print(
        "  Docs:         https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/features/curator"
    )


def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `zorin update`.
    ...
    try:
        ...
        return
    db_path = get_zorin_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `zorin sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    zorin sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  zorin sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``zorin update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``zorin update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: zorin curator status)"
    )
...
_DASHBOARD_SYSTEMD_UNIT = "zorin-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Zorin installs Linux services in the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``zorin dashboard`` processes.

    Called at the end of ``zorin update`` (default ``reason``) and also
    from ``zorin dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``zorin update`` path), a detected
    ``zorin-dashboard.service`` is restarted through systemd instead of
    raw-killing its main PID.
    """
    if restart_managed and _restart_managed_dashboard_service(reason):
        return

    # When the Zorin Desktop Electron app spawns this dashboard as a
    # backend child, it sets ZORIN_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ZORIN_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    zorin dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.zorin-update-staging"
    backup = f"{dst}.zorin-update-old"
...
def _update_via_zip(args):
    """Update ZORIN by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `zorin update "
            f"--branch {branch}`, or update against main with `zorin update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NPCAutomators/AGENT-ZORIN/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="zorin-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"zorin-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for zorin-agent itself
        ...
        # GitHub ZIPs extract to zorin-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"zorin-agent-{branch}")
    ...
    from zorin_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: zorin skills list-modified  "
    ...
    try:
        from zorin_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "zorin-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Zorin behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes zorin completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Zorin couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Zorin couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Zorin behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Zorin couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Zorin couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `zorin update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NPCAutomators/AGENT-ZORIN.git",
    "git@github.com:NPCAutomators/AGENT-ZORIN.git",
    "https://github.com/NPCAutomators/AGENT-ZORIN",
    "git@github.com:NPCAutomators/AGENT-ZORIN",
}
OFFICIAL_REPO_URL = "https://github.com/NPCAutomators/AGENT-ZORIN.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_zorin_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_zorin_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Zorin repository.")
        print("  This means you may miss updates from NPCAutomators/AGENT-ZORIN.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NPCAutomators/AGENT-ZORIN.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NPCAutomators/AGENT-ZORIN.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``zorin update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from zorin_constants import get_default_zorin_root

    default_home = get_default_zorin_root()
...
# Install-scoped breadcrumb dropped right before ``zorin update`` mutates the
...
# survives and the next ``zorin`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $ZORIN_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``zorin update`` left half-done.
    ...
    protocol stream (``zorin acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if zorin.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _zorin_exe_shims(scripts_dir)
            if shims:
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Zorin is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Zorin from a different terminal, "
    ...
    try:
        ...
        print(
            "⚠ A previous `zorin update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from zorin_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``zorin update`` is still progressing even if pip/uv itself is silent.
    """
...
def _zorin_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_zorin_instances(
    ...
    The Zorin Desktop Electron app spawns ``zorin.EXE`` as a backend child,
    so during ``zorin update`` the user-invoked process and the desktop's
    ...
    shims (``zorin.exe`` / ``zorin-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``zorin update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``zorin.exe``) is a separate process from the Python
    ...
    for shim in _zorin_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated zorin.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``zorin update``
    ...
    #      zorin.exe sitting *under* a non-Zorin parent (e.g. a Zorin
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "zorin.exe"
    lines = ["✗ Another zorin.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Zorin Desktop, exit any open `zorin` REPLs, and")
    lines.append("  stop the gateway (`zorin gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `zorin update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_zorin_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``zorin.exe``.
    ...
    when ``zorin update`` runs, ``zorin.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``zorin.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next zorin invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Zorin Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Zorin Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _zorin_exe_shims(scripts_dir):
        ...
        print(
            "    Close Zorin Desktop, exit other `zorin` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `zorin update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_zorin_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``zorin.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``zorin.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``zorin`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_zorin_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``zorin.exe.old.*`` left by prior updates.

    Called early on every zorin invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``zorin update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `zorin update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``zorin.exe`` / ``zorin-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_zorin_exe`` for the rationale.
    """
    ...
    # hours later inside ``zorin update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``zorin.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``zorin.exe`` shim is locked during ``zorin update``, or when uv/distlib
    skips a launcher write. The symptom is ``zorin-agent.exe`` and
    ``zorin-acp.exe`` present but ``zorin.exe`` missing, so ``zorin`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `zorin update --force` after "
            "closing other zorin processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m zorin_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``zorin update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``zorin.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``zorin`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `zorin update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `zorin update --force` after closing other zorin processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from zorin_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ZORIN_HOME/bin/uv``, but on Termux the official
    ...
    """
    from zorin_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `zorin update` is exactly the step
...
def _npm_lockfile_changed(zorin_root: Path) -> bool:
    ...
    try:
        ...
        cache_file = zorin_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(zorin_root: Path) -> None:
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    """
    from zorin_constants import find_node_executable
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from zorin_constants import is_wsl
        ...
        if is_wsl() and path_npm and _is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `zorin update`.")
            failed = ["repo root"]
            if any(
    ...
    # Zorin profile using this checkout. Keep one per-checkout cache under the
    # shared Zorin root rather than rerunning npm once per named profile.
    shared_zorin_root = get_default_zorin_root()
    if not _npm_lockfile_changed(shared_zorin_root):
    ...
    # need desktop during `zorin update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `zorin update`.")
        return list(labels)
    ...
    # hung. The chatty npm-deprecation noise during `zorin update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_zorin_root)
...
class _UpdateOutputStream:
    """Stream wrapper used during ``zorin update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.zorin/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``zorin update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``zorin update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.zorin/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``zorin update --gateway``) the update is already
    ...
    try:
        ...
        # zorin_cli.config.get_zorin_home to simulate setup failure.
        from zorin_cli.config import get_zorin_home as _get_zorin_home

        logs_dir = _get_zorin_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.zorin/logs/update.log`` only, never the terminal.

    During ``zorin update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``zorin update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``zorin update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from zorin_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from zorin_cli.config import recommended_update_command
        from zorin_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``zorin update`` without requiring a reinstall.  The
    ...
    ``zorin`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/zorin.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v zorin'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/zorin, code at /usr/local/lib/zorin-agent).
    fhs_link = Path("/usr/local/bin/zorin")
    ...
    path_comment = (
        "# ZORIN — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of ZORIN_HOME under
      ``backups/`` (restorable via ``zorin import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from zorin_cli.backup import create_quick_snapshot
    ...
    try:
        from zorin_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_zorin_home so the user sees ~/.zorin/...
    try:
        from zorin_constants import get_zorin_home, display_zorin_home

        home = get_zorin_home()
        try:
            display_path = f"{display_zorin_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  zorin import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``zorin update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``zorin update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run zorin from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.zorin-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".zorin-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The zorin.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m zorin_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``zorin
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Zorin processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Zorin Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Zorin desktop app / other Zorin terminals, then re-run:"
    )
    lines.append("    zorin update")
    lines.append("  (or use `zorin update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    zorin.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from zorin_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Zorin...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m zorin_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: zorin gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``zorin gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from zorin_cli import gateway_windows
        from zorin_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``zorin-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("zorin-gateway"):
            continue
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``zorin update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update ZORIN to the latest version.
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `zorin update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Zorin via pip (for PyPI installs)."""
    from zorin_cli import __version__
    from zorin_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "zorin-agent"]
        ...
        cmd = [pipx, "upgrade", "zorin-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "zorin-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "zorin-agent"]
    ...
    print("✓ Update complete! Restart zorin to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating ZORIN...")
    print()

    # On Windows, abort early if another zorin.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_zorin_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `zorin serve` backend) will keep .pyd
    ...
    # the zorin.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Zorin windows/gateways and re-run: zorin update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # orphan merge-conflict markers in zorin_cli/config.py bricked
        # every user who ran ``zorin update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``zorin
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``zorin update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_zorin_home added to zorin_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``zorin`` launch finishes the
        ...
        # build.  ``zorin desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``zorin desktop`` shouldn't be forced into a full
        # Electron build by ``zorin update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and _resolve_node_runtime_npm() and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "zorin_cli.main", "desktop", "--build-only"]
            ...
            # debuggable.
            #
            # Start the build subprocess with the Zorin-managed Node on PATH:
            # when `zorin update` runs inside the desktop updater chain
            # (Desktop → zorin-setup → zorin update), the shell PATH
            # customizations are lost, so a bare-PATH child would fail with
            # `node: not found` before cmd_gui can self-heal.
            from zorin_constants import with_zorin_node_path

            _build_env = with_zorin_node_path()
            build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT, env=_build_env)
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT, env=_build_env)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `zorin desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from zorin_constants import display_zorin_home as _dhh
        ...
        # current — so copy it straight over ~/.zorin/cache/model_catalog.json
        ...
        # modules in this process.  Reload zorin_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_zorin_home() added since the last release.
        try:
            import importlib
            import zorin_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ZORIN_HOME so
        # it is not affected by sync_skills()'s module-level ZORIN_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ZORIN_HOME env var points at the default or a named profile.
        try:
            from zorin_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from zorin_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'zorin config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: zorin config migrate")
            else:
                print()
                print("Skipped. Run 'zorin config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from zorin_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `zorin curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``zorin update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from zorin_cli.tools_config import install_cua_driver
        ...
        # When running as ``zorin update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_zorin_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... zorin-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... zorin-gateway*)
            ...
            # Discover all zorin-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: zorin gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `zorin update` restart it automatically, allow\n"
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: zorin gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: zorin -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Zorin gateway unit files are still installed.
        # When both zorin.service (from a pre-rename install) and the
        # current zorin-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `zorin update` surfaces the issue until the user migrates.
        try:
            from zorin_cli.gateway import (
                has_legacy_zorin_units,
                _find_legacy_zorin_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_zorin_units():
                print()
                print("⚠ Legacy Zorin gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_zorin_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (zorin.service) fight the current")
                print("  zorin-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    zorin gateway migrate-legacy")
                print()
        ...
        print("  zorin model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``zorin -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from zorin_constants import display_zorin_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `zorin profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_zorin_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → zorin -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.zorin)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  zorin profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     zorin -p {name} chat")
                else:
        ...
        from zorin_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from zorin_constants import get_zorin_home as _hh
        ...
        from zorin_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `zorin profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → zorin -p {name}  ({wrapper})")
        print()
        ...
        from zorin_cli.profiles import profile_exists, validate_alias_name
        ...
        from zorin_cli.profiles import rename_profile
        ...
        from zorin_cli.profiles import export_profile
        ...
        from zorin_cli.profiles import import_profile
        ...
        import tempfile
        from zorin_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="zorin_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  zorin -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      zorin -p {plan.manifest.name} chat")
        ...
        from zorin_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `zorin profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"zorin -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from zorin_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("zorin_requires"):
            print(f"Requires:     Zorin {data['zorin_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from zorin_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.zorin_requires:
        print(f"  Requires: Zorin {mf.zorin_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``zorin dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``zorin dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No zorin dashboard processes running.")
        return 0

    print(f"{len(pids)} zorin dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``zorin dashboard register`` for OAuth.
    ...
    try:
        from zorin_cli.web_server import should_require_auth
    ...
    try:
        from zorin_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via NPCAUTOMATORS Portal (run `zorin dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    zorin dashboard register\n"
            "  It provisions a NPCAUTOMATORS Portal OAuth client and writes "
            "ZORIN_DASHBOARD_OAUTH_CLIENT_ID into ~/.zorin/.env for you.\n"
            "  Docs: https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/"
    ...
    try:
        from zorin_cli.config import load_config, save_config
        from zorin_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
    ...
    try:
        from zorin_cli.plugins import discover_plugins
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from zorin_cli.windows_ssh_runtime import read_token
        return read_token(path)

    import stat as _stat
    from pathlib import Path as _Path
    from zorin_constants import get_zorin_home as _get_zorin_home

    if not os.path.isabs(path):
        raise SystemExit("--ssh-session-token-file must be absolute")

    token_path = _Path(path)
    token_root = _get_zorin_home() / "desktop-ssh"
    try:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No zorin dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `zorin update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with zorin serve")
    ...
    # launches the dashboard (`worker dashboard` → ZORIN_HOME points into
    ...
    try:
        from zorin_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("ZORIN_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "zorin_cli.main",
            "-p", "default",
        ...
        # ZORIN_HOME.  We must resolve the root explicitly instead of just
        # dropping ZORIN_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ZORIN_HOME=/opt/data`), so an unset
        # ZORIN_HOME falls back to $HOME/.zorin = /opt/data/.zorin — an
        ...
        # update-button guard also misfires).  get_default_zorin_root()
        # returns the root for both layouts: ~/.zorin for a standard install
        ...
        try:
            from zorin_constants import get_default_zorin_root
            env["ZORIN_HOME"] = str(get_default_zorin_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ZORIN_HOME) rather than block the reroute.
            env.pop("ZORIN_HOME", None)
    ...
    # the same logs directory as every other Zorin surface.
    try:
        from zorin_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["ZORIN_SERVE_HEADLESS"] = "1"
    elif "ZORIN_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ZORIN_WEB_DIST"])
            if "ZORIN_WEB_DIST" in os.environ
            else PROJECT_ROOT / "zorin_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom ZORIN_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "ZORIN_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # ZORIN_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["ZORIN_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ ZORIN_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset ZORIN_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads ZORIN_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["ZORIN_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from ZORIN_WEB_DIST: {_dist_root}")

    # Discover and load plugins so any DashboardAuthProvider plugin
    # (e.g. plugins/dashboard_auth/npcautomators) registers BEFORE start_server's
    ...
    try:
        from zorin_cli.mcp_startup import start_background_mcp_discovery
    ...
    from zorin_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with NPCAUTOMATORS Portal."""
    from zorin_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from zorin_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from zorin_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from zorin_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Zorin log files."""
    from zorin_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Zorin command console."""
    from zorin_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from zorin_cli.models import CANONICAL_PROVIDERS as _cp
        return ["auto"] + [p.slug for p in _cp]
    except Exception:
        # Fallback: static list guarantees the CLI always works
        return [
            "auto", "openrouter", "npcautomators", "openai-codex", "xai-oauth", "copilot-acp", "copilot",
...
# so that in ``zorin -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``zorin_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``zorin -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``zorin`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ZORIN_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that ZORIN_YOLO_MODE is set before ANY
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from zorin_cli._parser import build_top_level_parser
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ZORIN_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ZORIN_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ZORIN_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `zorin --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_zorin_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_zorin_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_zorin_home()}/memories/\n")
    else:
        from zorin_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch ZORIN as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from zorin_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from zorin_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from zorin_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from zorin_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from zorin_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from zorin_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from zorin_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from zorin_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from zorin_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for zorin CLI."""
    # Cosmetic: make the process show up as 'zorin' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from zorin_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``zorin.exe.old.*`` quarantine files left by previous
    # ``zorin update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_zorin_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``zorin update``
    ...
    # ``zorin skills install update``) merely defers recovery one launch;
    # under-matching (missing ``zorin -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in zorin_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from zorin_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from zorin_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `zorin model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.zorin/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/secrets/"
        ),
    )
    ...
    from zorin_cli import secrets_cli as _secrets_cli
    from zorin_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from zorin_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in zorin_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in zorin_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in zorin_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in zorin_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `zorin whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in zorin_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from zorin_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in zorin_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in zorin_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in zorin_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in zorin_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in zorin_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in zorin_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # portal command — NPCAUTOMATORS Portal status + Tool Gateway routing
    # =========================================================================
    from zorin_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from zorin_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from zorin_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in zorin_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in zorin_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in zorin_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in zorin_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in zorin_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in zorin_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.zorin/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo zorin uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from zorin_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in zorin_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in zorin_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in zorin_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in zorin_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in zorin_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in zorin_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from zorin_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in zorin_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``zorin --help``, ``zorin version``, ``zorin logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from zorin_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from zorin_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Zorin shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from zorin_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from zorin_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in zorin_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in zorin_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `zorin computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `zorin tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `zorin computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Zorin app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from zorin_cli.tools_config import _cua_driver_cmd
            # Honor ZORIN_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from zorin_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: zorin computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: zorin computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: zorin computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: zorin computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: zorin computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: zorin computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in zorin_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def _add_session_filter_args(p, default_older_help):
        ...
        p.add_argument(
            ...
            "(e.g. 'sonnet', 'gpt-5', 'zorin')",
        )
        p.add_argument(
            "--provider",
            help="Only match sessions billed through this provider "
            "(e.g. openrouter, anthropic, npcautomators)",
        )
        p.add_argument(
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <zorin home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            from zorin_state import workspace_key as _ws_key
            ...
            from zorin_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from zorin_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from zorin_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_zorin_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from zorin_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_zorin_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_zorin_home() / "sessions"
            ...
            # `zorin sessions prune`: no time window and no filters at all
            ...
            # Launch zorin --resume <id> by replacing the current process
            ...
            if result.get("vacuumed") is False:
                print("  (VACUUM was skipped or failed — run "
                      "`zorin sessions optimize` later to reclaim freed space.)")
    ...
    # =========================================================================
    # insights command  (parser built in zorin_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in zorin_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in zorin_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in zorin_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in zorin_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in zorin_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in zorin_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in zorin_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Zorin-Setup.exe success screen tells users to
    # run `zorin desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in zorin_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in zorin_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in zorin_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``zorin -c Pokemon Agent Dev`` → ``zorin -c 'Pokemon Agent Dev'``
    ...
    from zorin_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'zorin -c model' where 'model' is consumed as the
    ...
    # --yolo: set ZORIN_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (zorin hooks list, cron
