"""Profile distributions — shareable, packaged Zorin profiles via git.

A distribution is a Zorin profile published as a git repository (or
...
* ``zorin profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``zorin skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``zorin profile``, not a parallel tree):

    zorin profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "zorin-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            zorin_requires=str(data.get("zorin_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.zorin_requires:
            out["zorin_requires"] = self.zorin_requires
        if self.author:
...
def check_zorin_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Zorin {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``zorin profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Zorin profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Zorin distribution."
        )

    # Version check up-front so we fail fast
    check_zorin_requires(manifest.zorin_requires, zorin_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.zorin).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="zorin_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `zorin profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`zorin profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="zorin_dist_update_") as tmp:
        plan = plan_install(
