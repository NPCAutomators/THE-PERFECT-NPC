"""
Multi-provider authentication system for ZORIN.

Supports OAuth device code flows (NPCAUTOMATORS Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.zorin/auth.json with cross-process file locking.
...
NpcAutomators authentication paths:
...
from zorin_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# NPCAUTOMATORS Portal defaults
DEFAULT_NPCAUTOMATORS_PORTAL_URL = "https://portal.npcautomators.com"
DEFAULT_NPCAUTOMATORS_INFERENCE_URL = "https://inference-api.npcautomators.com/v1"
DEFAULT_NPCAUTOMATORS_CLIENT_ID = "zorin-cli"
NPCAUTOMATORS_INFERENCE_INVOKE_SCOPE = "inference:invoke"
NPCAUTOMATORS_BILLING_MANAGE_SCOPE = "billing:manage"
DEFAULT_NPCAUTOMATORS_SCOPE = NPCAUTOMATORS_INFERENCE_INVOKE_SCOPE
NPCAUTOMATORS_DEVICE_CODE_SOURCE = "device_code"
NPCAUTOMATORS_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
NPCAUTOMATORS_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from zorin_cli import __version__ as _ZORIN_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _ZORIN_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"zorin-cli/{_ZORIN_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/guides/oauth-over-ssh"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "npcautomators": ProviderConfig(
        id="npcautomators",
        name="NPCAUTOMATORS Portal",
        auth_type="oauth_device_code",
        portal_base_url=DEFAULT_NPCAUTOMATORS_PORTAL_URL,
        inference_base_url=DEFAULT_NPCAUTOMATORS_INFERENCE_URL,
        client_id=DEFAULT_NPCAUTOMATORS_CLIENT_ID,
        scope=DEFAULT_NPCAUTOMATORS_SCOPE,
    ),
...
def get_anthropic_key() -> str:
    ...
    ``~/.zorin/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from zorin_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from zorin_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.zorin/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``zorin auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `zorin model` to re-authenticate."

    if error.code == "subscription_required":
        if error.provider == "npcautomators":
            return _format_npcautomators_entitlement_auth_error(error)
...
def _format_npcautomators_entitlement_auth_error(error: AuthError) -> str:
    try:
        from zorin_cli.npcautomators_account import (
            format_npcautomators_portal_entitlement_message,
            get_npcautomators_portal_account_info,
        )

        account_info = get_npcautomators_portal_account_info(force_fresh=True)
        message = format_npcautomators_portal_entitlement_message(
            account_info,
            capability="NpcAutomators model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in NPCAUTOMATORS Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("ZORIN_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.zorin/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_zorin_home() / "auth.json"
    # Seat belt: if pytest is running and ZORIN_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch ZORIN_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".zorin" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set ZORIN_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom ZORIN_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_zorin_root()
    except Exception:
        return None
    profile_home = get_zorin_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.zorin/auth.json`` even when ZORIN_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_zorin_root()`` for a profile-shaped ZORIN_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".zorin" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared NpcAutomators store) don't share reentrancy
...
def _auth_store_lock(
    ...
    *,
    ...
    ``_npcautomators_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared NpcAutomators lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and (
        ...
        if isinstance(raw.get("providers"), dict):
            _migrate_stale_npcautomators_portal_url(raw["providers"])
        return raw
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "npcautomators_portal" in systems:
            providers["npcautomators"] = systems["npcautomators_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "npcautomators" if providers else None}
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated NpcAutomators refresh token only to
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``npcautomators``) that were only authenticated at
    global scope. Once the user runs ``zorin auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``zorin auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``zorin auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or NpcAutomators device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Zorin.
    ...
    if pconfig is None:
        from zorin_cli.providers import get_provider
    ...
    # the user initiated inside Zorin (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "zorin_pkce", "manual"}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `zorin logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from zorin_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'zorin doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'zorin model' for available providers, or run 'zorin doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `zorin auth add openrouter`
    ...
    # `zorin auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.zorin/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'zorin model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.zorin/.env.",
...
_NPCAUTOMATORS_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({
    "api.npcautomators.com",
})

# Allowlist of valid NPCAUTOMATORS Portal hosts. A portal_base_url outside this
...
_NPCAUTOMATORS_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
    "portal.npcautomators.com",
    "localhost",
    "127.0.0.1",
})


def _migrate_stale_npcautomators_portal_url(providers: Dict[str, Any]) -> None:
    npcautomators = providers.get("npcautomators")
    if not isinstance(npcautomators, dict):
        return
    stored = (npcautomators.get("portal_base_url") or "").strip()
    if stored:
        parsed = urlparse(stored)
        if parsed.hostname in _NPCAUTOMATORS_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale npcautomators portal_base_url %s -> %s",
                stored, DEFAULT_NPCAUTOMATORS_PORTAL_URL,
            )
            npcautomators["portal_base_url"] = DEFAULT_NPCAUTOMATORS_PORTAL_URL


# Allowlist of hosts the NPCAUTOMATORS Portal proxy is willing to forward inference
...
# (NPCAUTOMATORS_INFERENCE_BASE_URL) bypass validation — that's the documented
# dev/staging escape hatch and the env source is already trusted (the
# user set it themselves).
_ALLOWED_NPCAUTOMATORS_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.npcautomators.com",
})


def _validate_npcautomators_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``NPCAUTOMATORS_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        logger.warning(
            "npcautomators: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_NPCAUTOMATORS_INFERENCE_HOSTS:
        logger.warning(
            "npcautomators: refusing inference URL host %r from Portal response "
...
def _npcautomators_inference_env_override() -> Optional[str]:
    """Return the user-set ``NPCAUTOMATORS_INFERENCE_BASE_URL`` override, if any.
    ...
    """
    return _optional_base_url(os.getenv("NPCAUTOMATORS_INFERENCE_BASE_URL"))


def _npcautomators_portal_env_override() -> Optional[str]:
    """Return the user/deployment-set Portal base URL override, if any.

    Mirrors ``_npcautomators_inference_env_override()``: ``ZORIN_PORTAL_BASE_URL`` /
    ``NPCAUTOMATORS_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Zorin at a non-production NPCAUTOMATORS Portal (e.g. a hosted agent
    provisioned on npcautomators-account-service's `staging` environment, which stamps
    ``ZORIN_PORTAL_BASE_URL=https://portal.staging-npcautomators.com`` into
    ...
    gated by ``_NPCAUTOMATORS_PORTAL_ALLOWED_HOSTS``: that allowlist exists to reject
    ...
    return _optional_base_url(
        os.getenv("ZORIN_PORTAL_BASE_URL") or os.getenv("NPCAUTOMATORS_PORTAL_BASE_URL")
    )
...
def _npcautomators_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = NPCAUTOMATORS_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if NPCAUTOMATORS_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _npcautomators_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    ...
    return (
        _npcautomators_invoke_jwt_status(
            token,
            scope=scope,
...
def _assert_npcautomators_inference_jwt_usable(
    state: Dict[str, Any],
    *,
    access_token: Any = None,
) -> None:
    token = state.get("access_token") if access_token is None else access_token
    reason = _npcautomators_invoke_jwt_status(
        token,
    ...
    if reason is None:
        return
    raise AuthError(
        "NPCAUTOMATORS Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: zorin auth add npcautomators",
        provider="npcautomators",
        code=reason,
        relogin_required=True,
    )


def _log_npcautomators_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.info("NpcAutomators inference auth: using NAS invoke JWT")
    _oauth_trace(
        "npcautomators_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _npcautomators_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_npcautomators_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _npcautomators_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_npcautomators_invoke_jwt(
    state: Dict[str, Any],
    *,
    access_token: Any = None,
    sequence_id: Optional[str] = None,
    ...
    _set_npcautomators_agent_key_from_invoke_jwt(state)
    _log_npcautomators_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_NPCAUTOMATORS_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _npcautomators_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _NPCAUTOMATORS_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("ZORIN_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `zorin model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.zorin/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    from zorin_cli.config import get_env_value

    candidates = (
        explicit,
        get_env_value("ZORIN_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set ZORIN_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("ZORIN_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ZORIN_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ZORIN_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `zorin auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `zorin auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `zorin auth spotify` first.",
                provider="spotify",
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the NpcAutomators / xAI-OAuth / Codex-OAuth / MiniMax pattern.
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `zorin auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.zorin/.env, and return it.
    ...
    print("       App name:     anything (e.g. zorin-agent)")
    ...
    # Persist so subsequent `zorin auth spotify` runs skip the wizard.
    save_env_value("ZORIN_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("ZORIN_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved ZORIN_SPOTIFY_CLIENT_ID to ~/.zorin/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "ZORIN_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Zorin:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Zorin is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.zorin/auth.json (not ~/.codex/)
#
# Zorin maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Zorin auth store (~/.zorin/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `zorin auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `zorin auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `zorin auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `zorin auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``zorin setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``zorin auth add openai-codex``
      ...
      ``zorin auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Zorin auth store (~/.zorin/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``zorin auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Zorin auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Zorin auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run zorin auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `zorin auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Zorin auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Zorin keeps its OWN
        ...
        # the Codex CLI (or another Zorin process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Zorin's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("ZORIN_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("ZORIN_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Zorin processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.zorin/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `zorin model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `zorin model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `zorin model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `zorin model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.zorin/auth.json even when ZORIN_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``zorin auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.zorin/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `zorin model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``ZORIN_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `zorin model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Zorin (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `zorin model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``zorin model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("ZORIN_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("ZORIN_XAI_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# NPCAUTOMATORS Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared NpcAutomators token store — lets OAuth credentials persist across profiles
# so a new `zorin --profile <name> auth add npcautomators --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${ZORIN_SHARED_AUTH_DIR}/npcautomators_auth.json, defaulting to
# ``<zorin-root>/shared/npcautomators_auth.json`` where ``<zorin-root>`` is what
# ``get_default_zorin_root()`` returns — ``~/.zorin`` on Linux/macOS,
# ``%LOCALAPPDATA%\zorin`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's ZORIN_HOME so named profiles (which
# typically live under ``<zorin-root>/profiles/<name>/``) all see the
# same file.
#
...
NPCAUTOMATORS_SHARED_STORE_FILENAME = "npcautomators_auth.json"
_npcautomators_shared_lock_holder = threading.local()


def _npcautomators_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared NpcAutomators token store.

    Honors ``ZORIN_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<zorin-root>/shared/``, where ``<zorin-root>`` is what
    :func:`zorin_constants.get_default_zorin_root` returns — so
    Linux/macOS classic installs land at ``~/.zorin/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\zorin\\shared\\``, and
    Docker / custom ``ZORIN_HOME`` deployments at
    ``<ZORIN_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("ZORIN_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from zorin_constants import get_default_zorin_root
    return get_default_zorin_root() / "shared"


def _npcautomators_shared_store_path() -> Path:
    path = _npcautomators_shared_auth_dir() / NPCAUTOMATORS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Zorin root, refuse rather than silently corrupt cross-profile
    # state. Tests must set ZORIN_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from zorin_constants import get_default_zorin_root
        real_home_shared = (
            get_default_zorin_root() / "shared" / NPCAUTOMATORS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared NpcAutomators auth store during test run: "
                f"{path}. Set ZORIN_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _npcautomators_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared NpcAutomators OAuth store.
    ...
    ``_try_import_shared_npcautomators_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _npcautomators_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No ZORIN_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _npcautomators_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared NpcAutomators auth lock",
    ):
        yield


def _merge_shared_npcautomators_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local NpcAutomators state."""
    shared = _read_shared_npcautomators_state()
    if not shared:
        return False
...
def _write_shared_npcautomators_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the NpcAutomators OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_NPCAUTOMATORS_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_NPCAUTOMATORS_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_NPCAUTOMATORS_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_NPCAUTOMATORS_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _npcautomators_shared_store_lock():
            path = _npcautomators_shared_store_path()
            path.parent.mkdir(parents=True, exist_ok=True)
            # secure_parent_dir refuses to chmod / or top-level dirs (#25821).
            secure_parent_dir(path)
            tmp = path.with_name(f"{path.name}.tmp.{os.getpid()}.{uuid.uuid4().hex}")
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed NpcAutomators
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "npcautomators_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared NpcAutomators auth store: %s", exc)


def _read_shared_npcautomators_state() -> Optional[Dict[str, Any]]:
    """Return the shared NpcAutomators OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared NpcAutomators auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_npcautomators_state(reason: str) -> None:
    """Remove the shared NpcAutomators OAuth store after a terminal token failure."""
    try:
        with _npcautomators_shared_store_lock():
            path = _npcautomators_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("npcautomators_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared NpcAutomators auth store: %s", exc)


def _is_terminal_npcautomators_refresh_error(exc: Exception) -> bool:
    """True when retrying the same NpcAutomators refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "npcautomators"
...
def _quarantine_npcautomators_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Zorin
    # has a known bug class where credential-shaped literals get corrupted in logs.
    forensic: Dict[str, Any] = {
        "reason": reason,
        "error_code": error.code,
        # No session_id field exists on NpcAutomators state; provenance is client_id +
    ...
    logger.warning(
        "NpcAutomators OAuth state quarantined (terminal auth death): %s",
    ...
    _clear_shared_npcautomators_state(reason)
    invalidate_npcautomators_auth_status_cache()


def _quarantine_npcautomators_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded NpcAutomators pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("npcautomators")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {NPCAUTOMATORS_DEVICE_CODE_SOURCE, f"manual:{NPCAUTOMATORS_DEVICE_CODE_SOURCE}"}
    for entry in entries:
    ...
    if removed:
        pool["npcautomators"] = retained
        _oauth_trace(
            "npcautomators_pool_device_code_quarantined",
            reason=reason,
            error_code=error.code,
        )
    return removed
...
def _try_import_shared_npcautomators_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate NpcAutomators OAuth state from the shared store.
    ...
    for ``persist_npcautomators_credentials()``.
    ...
    """
    try:
        with _npcautomators_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_npcautomators_state()
            if not shared:
                return None

            # Build a full state dict so refresh_npcautomators_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_NPCAUTOMATORS_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_NPCAUTOMATORS_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_NPCAUTOMATORS_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_NPCAUTOMATORS_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_npcautomators_state(updated_state)

            refreshed = refresh_npcautomators_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_npcautomators_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "npcautomators_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_npcautomators_refresh_error(exc):
            _clear_shared_npcautomators_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared NpcAutomators import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "npcautomators_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-npcautomators-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="npcautomators", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="npcautomators", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the NpcAutomators portal
    ...
    # called POST /api/oauth/token with Zorin's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Zorin's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "NPCAUTOMATORS Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Zorin install sharing "
            "~/.zorin/auth.json) called POST /api/oauth/token with Zorin's "
            "refresh token without persisting the rotated token back.\n"
            "NpcAutomators refresh tokens are single-use — only Zorin may call the "
            "refresh endpoint. For health checks, use `zorin auth status` "
            "instead.\n"
            "Re-authenticate with: zorin auth add npcautomators"
        )
        relogin = True

    raise AuthError(description, provider="npcautomators", code=code, relogin_required=relogin)


def fetch_npcautomators_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the NpcAutomators inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="npcautomators", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Zorin models — they're not reliable for agentic tool-calling
            if "zorin" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _npcautomators_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
def resolve_npcautomators_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """Resolve a refresh-aware NPCAUTOMATORS Portal access token for managed tool gateways."""
    with _provider_state_transaction("npcautomators") as (
        auth_store,
        state,
        state_source_path,
    ):
        ...
        if not state:
            raise AuthError(
                "Zorin is not logged into NPCAUTOMATORS Portal.",
                provider="npcautomators",
                relogin_required=True,
            )

        # ZORIN_PORTAL_BASE_URL / NPCAUTOMATORS_PORTAL_BASE_URL is the trusted
        # operator/deployment override (mirrors NPCAUTOMATORS_INFERENCE_BASE_URL) and
        ...
        # no override is set.
        env_portal_override = _npcautomators_portal_env_override()
        if env_portal_override:
            portal_base_url = env_portal_override.rstrip("/")
        else:
            portal_base_url = (
                _optional_base_url(state.get("portal_base_url"))
                or DEFAULT_NPCAUTOMATORS_PORTAL_URL
            ).rstrip("/")

            parsed_portal_url = urlparse(portal_base_url)
            if parsed_portal_url.hostname and parsed_portal_url.hostname not in _NPCAUTOMATORS_PORTAL_ALLOWED_HOSTS:
                logger.warning(
        ...
        client_id = str(state.get("client_id") or DEFAULT_NPCAUTOMATORS_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)

        with _npcautomators_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_npcautomators_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for NPCAUTOMATORS Portal login.",
                    provider="npcautomators",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state_to_source(auth_store, "npcautomators", state, state_source_path)
                return access_token
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_npcautomators_refresh_error(exc):
                        _quarantine_npcautomators_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_npcautomators_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
            ...
            _save_provider_state_to_source(auth_store, "npcautomators", state, state_source_path)
            _write_shared_npcautomators_state(state)
            return state["access_token"]


def refresh_npcautomators_oauth_pure(
    access_token: str,
    refresh_token: str,
    client_id: str,
    portal_base_url: str,
    ...
    *,
    ...
    scope: str = DEFAULT_NPCAUTOMATORS_SCOPE,
    obtained_at: Optional[str] = None,
    ...
    """Refresh NpcAutomators OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_NPCAUTOMATORS_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_NPCAUTOMATORS_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_NPCAUTOMATORS_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_NPCAUTOMATORS_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _npcautomators_invoke_jwt_status(
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for NPCAUTOMATORS Portal.",
                    provider="npcautomators",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            refreshed_url = _validate_npcautomators_inference_url_from_network(refreshed.get("inference_base_url"))
            state["inference_base_url"] = refreshed_url or DEFAULT_NPCAUTOMATORS_INFERENCE_URL
            state["obtained_at"] = now.isoformat()
            state["expires_in"] = access_ttl
            state["expires_at"] = datetime.fromtimestamp(
        ...
        _assert_npcautomators_inference_jwt_usable(state)
        _select_npcautomators_invoke_jwt(state)

    return state


def refresh_npcautomators_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh NpcAutomators OAuth from a state dict. Thin wrapper around refresh_npcautomators_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_npcautomators_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "zorin-cli"),
        state.get("portal_base_url", DEFAULT_NPCAUTOMATORS_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_NPCAUTOMATORS_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_NPCAUTOMATORS_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_npcautomators_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist NpcAutomators OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    NpcAutomators credentials are read at runtime from two independent locations:

    - ``providers.npcautomators``: singleton state read by
      ``resolve_npcautomators_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.npcautomators``: used by the runtime ``pool.select()`` path.

    Historically ``zorin auth add npcautomators`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.npcautomators``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.npcautomators`` then calls ``load_pool("npcautomators")`` so
    ...
    ``zorin auth add npcautomators --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("npcautomators")`` instead of the
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "npcautomators", state)
        _save_auth_store(auth_store)

    # Mirror to the shared store so a new profile can one-tap import
    # these credentials via `zorin auth add npcautomators --type oauth`. Best-
    # effort: any I/O failure is logged and swallowed (the per-profile
    # auth.json is still the source of truth).
    _write_shared_npcautomators_state(state)

    pool = load_pool("npcautomators")
    return next(
        (e for e in pool.entries() if e.source == NPCAUTOMATORS_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_npcautomators_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.npcautomators changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("npcautomators")
    except Exception as exc:
        logger.debug("Failed to sync NpcAutomators credential pool from auth store: %s", exc)


def resolve_npcautomators_runtime_credentials(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """
    Resolve NpcAutomators inference credentials for runtime use.
    ...
    with _provider_state_transaction("npcautomators") as (
        ...
        if not state:
            raise AuthError("Zorin is not logged into NPCAUTOMATORS Portal.",
                            provider="npcautomators", relogin_required=True)
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("ZORIN_PORTAL_BASE_URL")
                or os.getenv("NPCAUTOMATORS_PORTAL_BASE_URL")
                or DEFAULT_NPCAUTOMATORS_PORTAL_URL
            ).rstrip("/")
            ...
            if env_portal_override:
                ...
                if (
                    not portal_host
                    or portal_host not in _NPCAUTOMATORS_PORTAL_ALLOWED_HOSTS
                    ...
                    portal_url = DEFAULT_NPCAUTOMATORS_PORTAL_URL
            ...
            stored_inference_url = (
                _validate_npcautomators_inference_url_from_network(
                    _optional_base_url(state.get("inference_base_url"))
                )
                or DEFAULT_NPCAUTOMATORS_INFERENCE_URL
            )
            effective_inference_url = (
                _npcautomators_inference_env_override() or stored_inference_url
            )
            effective_client_id = str(
                state.get("client_id") or DEFAULT_NPCAUTOMATORS_CLIENT_ID
            )
            return (
                portal_url,
                stored_inference_url,
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed NpcAutomators auth-status cache warm during read paths.
            if (
                _npcautomators_effective_provider_state(state)
                == _npcautomators_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "npcautomators_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
            try:
                ...
                _oauth_trace(
                    "npcautomators_state_persist_failed",
                    sequence_id=sequence_id,
                    reason=reason,
                    error_type=type(exc).__name__,
                )
            ...
            # _write_shared_npcautomators_state.
            _write_shared_npcautomators_state(state)

        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "npcautomators_runtime_credentials_start",
            sequence_id=sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        )
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                with _npcautomators_shared_store_lock(
                    ...
                    if _merge_shared_npcautomators_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        (
                            portal_base_url,
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for NPCAUTOMATORS Portal login.",
                                provider="npcautomators", relogin_required=True)

            invoke_jwt_status = _npcautomators_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            if force_refresh or invoke_jwt_status is not None:
                with _npcautomators_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        ...
                        state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
                        state["scope"] = refreshed.get("scope") or state.get("scope")
                        # Heal a poisoned stored value (see refresh_npcautomators_oauth_pure):
                        ...
                        # persisted to auth.json below. The NPCAUTOMATORS_INFERENCE_BASE_URL
                        # env override is layered on for the client/return value
                        # only (see below) — it is never persisted.
                        refreshed_url = _validate_npcautomators_inference_url_from_network(refreshed.get("inference_base_url"))
                        stored_inference_base_url = refreshed_url or DEFAULT_NPCAUTOMATORS_INFERENCE_URL
                        inference_base_url = (
                            _npcautomators_inference_env_override() or stored_inference_base_url
                        )
            ...
            _assert_npcautomators_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_npcautomators_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_npcautomators_runtime_credentials_final")

    if state_persisted:
        _sync_npcautomators_pool_from_auth_store()

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a NpcAutomators inference API key",
                        provider="npcautomators", code="server_error")
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": NPCAUTOMATORS_AUTH_PATH_INVOKE_JWT,
        ...
        "auth_path": NPCAUTOMATORS_AUTH_PATH_INVOKE_JWT,
...
def _empty_npcautomators_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_npcautomators_pool_status() -> Dict[str, Any]:
    """Best-effort status from the credential pool.

    This is a fallback only. The auth-store provider state is the runtime source
    of truth because it is what ``resolve_npcautomators_runtime_credentials()`` refreshes.
    """
    try:
        from agent.credential_pool import load_pool
        ...
        if not pool or not pool.has_credentials():
            return _empty_npcautomators_auth_status()
...
# ── Process-level memo for get_npcautomators_auth_status() ──
# get_npcautomators_auth_status() validates state by calling resolve_npcautomators_runtime_credentials(),
# which does a synchronous OAuth refresh POST to portal.npcautomators.com. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`zorin tools`, status panels,
# subscription-feature checks) call it many times per render — `zorin tools` → "All Platforms"
...
# `zorin auth login/logout/add/remove` invalidate naturally on the next call.
_NPCAUTOMATORS_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_npcautomators_auth_status_cache: Optional[Tuple[float, str, Optional[float], Dict[str, Any]]] = None
...
def invalidate_npcautomators_auth_status_cache() -> None:
    """Clear the get_npcautomators_auth_status() process-level memo.

    Call this from any code path that mutates NpcAutomators auth state without going
    through resolve_npcautomators_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _npcautomators_auth_status_cache
    _npcautomators_auth_status_cache = None


def get_npcautomators_auth_status() -> Dict[str, Any]:
    """Status snapshot for NpcAutomators auth.
    ...
    ``invalidate_npcautomators_auth_status_cache()`` explicitly.
    """
    global _npcautomators_auth_status_cache
    now = time.monotonic()
    auth_file_key, mtime = _auth_file_cache_key()
    cached = _npcautomators_auth_status_cache
    if cached is not None:
        ...
        if (
            ...
            and (now - cached_at) < _NPCAUTOMATORS_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_npcautomators_auth_status()
    _npcautomators_auth_status_cache = (now, auth_file_key, mtime, dict(status))
    return status


def _compute_npcautomators_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_npcautomators_auth_status(). See that function."""
    state = get_provider_auth_state("npcautomators")
    if state:
        base_status = {
        ...
        try:
            creds = resolve_npcautomators_runtime_credentials()
            refreshed_state = get_provider_auth_state("npcautomators") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_npcautomators_pool_status()


# Enum values reported on the dashboard /api/status as ``npcautomators_session_valid``.
...
NPCAUTOMATORS_SESSION_VALID = "valid"
NPCAUTOMATORS_SESSION_TERMINAL = "terminal"
NPCAUTOMATORS_SESSION_UNKNOWN = "unknown"


def get_npcautomators_session_validity() -> str:
    """Classify the NpcAutomators bootstrap session for the dashboard /api/status probe.

    Returns one of:
      - ``"valid"``    — a usable NpcAutomators credential is present (login healthy).
      - ``"terminal"`` — the NpcAutomators session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no NPCAUTOMATORS provider state, or a transient/
    ...
    # into the NPCAUTOMATORS provider state when it clears dead tokens (the exact path
    ...
    if state:
        ...
        if isinstance(last_err, dict) and last_err.get("relogin_required"):
            ...
            if not (state.get("access_token") or state.get("refresh_token")):
                return NPCAUTOMATORS_SESSION_TERMINAL

    try:
        status = get_npcautomators_auth_status()
    except Exception:
        # Status computation itself failed — indeterminate, not terminal.
        return NPCAUTOMATORS_SESSION_UNKNOWN

    if status.get("logged_in"):
        return NPCAUTOMATORS_SESSION_VALID
    ...
    # No NPCAUTOMATORS provider state at all, or a non-terminal not-logged-in condition
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `zorin auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `zorin auth` and
    # `zorin model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("ZORIN_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("ZORIN_COPILOT_ACP_ARGS", "").strip()
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target == "npcautomators":
        return get_npcautomators_auth_status()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``zorin doctor`` runs the live
    ...
    try:
        from zorin_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Zorin' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `zorin doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from zorin_cli.copilot_auth import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set ZORIN_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from zorin_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `zorin logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"npcautomators", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _confirm_expensive_model_selection(
    ...
    *,
    ...
    try:
        from zorin_cli.model_cost_guard import expensive_model_warning
...
def _prompt_model_selection(
    ...
    """
    from zorin_cli.models import _format_price_per_mtok
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_NPCAUTOMATORS_PORTAL_URL).rstrip("/")
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from zorin_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'zorin model' or 'zorin setup' instead."""
    print("The 'zorin login' command has been removed.")
    print("Use 'zorin auth' to manage credentials,")
    print("'zorin model' to select a provider, or 'zorin setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.zorin/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Zorin-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Zorin auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Zorin will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("ZORIN_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Zorin will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Zorin gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Zorin creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Zorin auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Zorin auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Zorin creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``zorin auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``zorin auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Zorin auth store (~/.zorin/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Zorin login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the NpcAutomators / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `zorin model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``zorin status`` that just want to know
...
def _npcautomators_device_code_login(
    *,
    ...
    """Run the NpcAutomators device-code flow and return full OAuth state without persisting."""
    pconfig = PROVIDER_REGISTRY["npcautomators"]
    portal_base_url = (
        portal_base_url
        or os.getenv("ZORIN_PORTAL_BASE_URL")
        or os.getenv("NPCAUTOMATORS_PORTAL_BASE_URL")
    ...
    requested_inference_url = (
        inference_base_url
        or os.getenv("NPCAUTOMATORS_INFERENCE_BASE_URL")
    ...
    print(f"Starting Zorin login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        return refresh_npcautomators_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `zorin model` again to finish setup.")
            raise SystemExit(1)
        raise


def npcautomators_token_has_billing_scope() -> bool:
    """Return True if the currently-held NpcAutomators token carries ``billing:manage``.
    ...
    """
    try:
        state = get_provider_auth_state("npcautomators") or {}
    except Exception:
        return False
    scope = state.get("scope")
    if not isinstance(scope, str):
        return False
    return NPCAUTOMATORS_BILLING_MANAGE_SCOPE in scope.split()


def step_up_npcautomators_billing_scope(
    *,
    open_browser: bool = True,
    timeout_seconds: float = 15.0,
    on_verification: Optional[Callable[[str, str], None]] = None,
    ...
    targets the same deployment (incl. a preview via ``ZORIN_PORTAL_BASE_URL`` set
    at the original login). Persists to the auth store + shared store + pool, exactly
    like ``_login_npcautomators`` — but WITHOUT the model picker (this is a scope upgrade, not
    a fresh login).

    Returns True iff the new token carries ``billing:manage``.
    """
    prior = get_provider_auth_state("npcautomators") or {}
    pconfig = PROVIDER_REGISTRY["npcautomators"]
    ...
    requested: list[str] = []
    for tok in (prior_scope.split() or [NPCAUTOMATORS_INFERENCE_INVOKE_SCOPE, "tool:invoke"]):
        if tok and tok not in requested:
            requested.append(tok)
    if NPCAUTOMATORS_BILLING_MANAGE_SCOPE not in requested:
        requested.append(NPCAUTOMATORS_BILLING_MANAGE_SCOPE)
    scope = " ".join(requested)

    auth_state = _npcautomators_device_code_login(
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "npcautomators", auth_state)
        _save_auth_store(auth_store)

    # Mirror to shared store + reseed the pool (best-effort), same as _login_npcautomators.
    try:
        _write_shared_npcautomators_state(auth_state)
    except Exception:
        pass
    try:
        _sync_npcautomators_pool_from_auth_store()
    ...
    return isinstance(granted, str) and NPCAUTOMATORS_BILLING_MANAGE_SCOPE in granted.split()


def _login_npcautomators(args, pconfig: ProviderConfig) -> None:
    """NPCAUTOMATORS Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing NpcAutomators credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_npcautomators_state()
        if shared:
            try:
                shared_path = _npcautomators_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing NpcAutomators OAuth credentials at {shared_path}")
            else:
                print("Found existing shared NpcAutomators OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating NpcAutomators session from shared credentials...")
                auth_state = _try_import_shared_npcautomators_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        # overwrites it to "npcautomators".  If the user picks "Skip (keep current)"
        ...
        # leave the config in a half-updated state (provider=npcautomators but model
        ...
        try:
            ...
            from zorin_cli.models import (
                get_curated_npcautomators_model_ids, get_pricing_for_provider,
                check_npcautomators_free_tier, partition_npcautomators_models_by_tier,
                union_with_portal_free_recommendations,
                union_with_portal_paid_recommendations,
            )
            model_ids = get_curated_npcautomators_model_ids()

            print()
            unavailable_models: list = []
            unavailable_message = ""
            if model_ids:
                pricing = get_pricing_for_provider("npcautomators")
                # Force fresh account data for model selection so recent credit
                # purchases are reflected immediately.
                free_tier = check_npcautomators_free_tier(force_fresh=True)
                ...
                if free_tier:
                    try:
                        ...
                        _account_info = get_npcautomators_portal_account_info(force_fresh=True)
                        unavailable_message = (
                            format_npcautomators_portal_entitlement_message(
                                _account_info,
                                capability="paid NpcAutomators models",
                            )
                            or ""
                        )
                    except Exception:
                    ...
                    # as free so users on older Zorin builds still see
                    # newly-launched free models without a CLI release.
                    model_ids, pricing = union_with_portal_free_recommendations(
                        model_ids, pricing, _portal_for_recs,
                    )
                    model_ids, unavailable_models = partition_npcautomators_models_by_tier(
            ...
            if model_ids:
                ...
                selected_model = _prompt_model_selection(
                    ...
                    confirm_provider="npcautomators",
                    confirm_base_url=inference_base_url,
                    confirm_api_key=runtime_key,
                )
            elif unavailable_models:
                _url = (_portal or DEFAULT_NPCAUTOMATORS_PORTAL_URL).rstrip("/")
                print("No free models currently available.")
                print(unavailable_message or f"Upgrade at {_url} to access paid models.")
            else:
                print("No curated models available for NPCAUTOMATORS Portal.")
        except Exception as exc:
        ...
        # them to NpcAutomators with a mismatched model.  The NpcAutomators OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "npcautomators".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. NpcAutomators credentials saved for future use.")
            print("  Run `zorin model` again to switch to NPCAUTOMATORS Portal.")
            return

        config_path = _update_config_for_provider(
            "npcautomators", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=npcautomators)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Zorin will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `zorin model` or configure an API key to use Zorin.")
        else:
