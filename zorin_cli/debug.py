"""``zorin debug`` debug tools for ZORIN.

Currently supports:
    zorin debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.zorin/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--npcautomators`` to upload instead to NpcAutomators-internal
                          storage (AWS S3) via a signed URL minted by the
                          NPCAUTOMATORS account service: the bundle is private
                          (viewable only by NpcAutomators staff / allowlisted mods via
...
_REDACTION_BANNER = (
    "[zorin debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.zorin/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``zorin debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``zorin debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `zorin debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "zorin-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``zorin debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.zorin/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``zorin debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "zorin-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----ZorinDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from zorin_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_zorin_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``zorin dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``zorin dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and NpcAutomators-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the NpcAutomators-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_NPCAUTOMATORS_BUNDLE_FORMAT = "zorin-debug-share/1"
...
def collect_share_bundle(
    ...
    NpcAutomators-S3 path (``--npcautomators``).  Centralising it guarantees the NpcAutomators bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the NpcAutomators path must never see raw
    logs.
...
def build_npcautomators_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the NpcAutomators envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "zorin-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
    ...
    envelope = {
        "format": _NPCAUTOMATORS_BUNDLE_FORMAT,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``zorin debug share`` (CLI) and the
    ...
    # shared collector so the paste.rs and NpcAutomators-S3 paths build identical,
    ...
    if redact:
        logger.info(
            "zorin debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    npcautomators = getattr(args, "npcautomators", False)
    ...
    if npcautomators:
        _run_debug_share_npcautomators(args, log_lines=log_lines, redact=redact)
        return
    ...
    try:
        ...
        print("\nRun `zorin debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  zorin debug delete <url>")

    print("\nShare these links with the Zorin team for support.")


_NPCAUTOMATORS_PRIVACY_NOTICE = """\
⚠️  --npcautomators: This uploads your debug bundle to NpcAutomators-INTERNAL storage (AWS S3),
    NOT a public paste service. The following is included:
  • System info (OS, Python/Zorin version, provider, which API keys are
  ...
  • The bundle is viewable only by NpcAutomators staff (and allowlisted Discord mods)
...
def _run_debug_share_npcautomators(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``zorin debug share --npcautomators``: upload the bundle to NpcAutomators-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the NpcAutomators envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from zorin_cli.diagnostics_upload import share_to_npcautomators

    print(_NPCAUTOMATORS_PRIVACY_NOTICE)
    if not _confirm_upload(args):
        return
    if not redact:
        print(
    ...
    if redact:
        logger.info(
            "zorin debug share --npcautomators: applied force-mode redaction before upload"
        )
    blob = build_npcautomators_bundle(bundle, redact=redact)

    print("Uploading to NpcAutomators diagnostics storage...")
    try:
        res = share_to_npcautomators(blob)
    except Exception as exc:
        print(
            f"\nNpcAutomators upload failed: {exc}\n"
            "\nThe NpcAutomators diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `zorin debug share --local` to print the report instead, "
            "or `zorin debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to NpcAutomators (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with the NpcAutomators team — only NpcAutomators staff "
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: zorin debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'zorin debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``zorin debug`` call.
    ...
    # best-effort — any failure is swallowed so ``zorin debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: zorin debug <command>")
        print()
        print("Commands:")
        ...
        print("  --npcautomators       Upload to NpcAutomators-internal storage (private, staff-only,")
