"""Per-provider model-selection wizard flows for ``zorin setup`` / ``zorin model``.

Extracted from ``zorin_cli/main.py`` as part of the god-file decomposition
campaign (``~/.zorin/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from zorin_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``zorin_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from zorin_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``zorin_cli.main`` at import time -> no import cycle.
"""
...
def _prune_replaced_custom_model_config_credentials(
    ...
    *,
    ...
    try:
        ...
        from zorin_cli.auth import read_credential_pool, write_credential_pool
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from zorin_cli.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.zorin/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `zorin setup` from scratch. OpenRouter
    ...
    from zorin_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from zorin_cli.auth import _save_model_choice, deactivate_provider
    from zorin_cli.config import load_config, save_config
    ...
    if not presets:
        print("No MoA presets configured. Run `zorin moa configure <name>` first.")
        return
...
def _model_flow_npcautomators(config, current_model="", args=None):
    """NPCAUTOMATORS Portal provider: ensure logged in, then pick model."""
    from zorin_cli.auth import (
    ...
    from zorin_cli.npcautomators_subscription import prompt_enable_tool_gateway

    state = get_provider_auth_state("npcautomators")
    if not state or not state.get("access_token"):
        print("Not logged into NPCAUTOMATORS Portal. Starting login...")
        print()
        try:
            ...
            _login_npcautomators(mock_args, PROVIDER_REGISTRY["npcautomators"])
            ...
            return
        # login_npcautomators already handles model selection + config update
        return
    ...
    try:
        ...
        if relogin:
            ...
            print("Re-authenticating with NPCAUTOMATORS Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    if not free_tier:
        try:
            refreshed_creds = resolve_npcautomators_runtime_credentials(
                force_refresh=True,
            )
            if refreshed_creds:
    ...
    _npcautomators_portal_url = ""
    try:
        _npcautomators_state = get_provider_auth_state("npcautomators")
        if _npcautomators_state:
            _npcautomators_portal_url = _npcautomators_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if free_tier:
        ...
        model_ids, pricing = union_with_portal_free_recommendations(
            model_ids, pricing, _npcautomators_portal_url,
        )
    ...
    if not model_ids and not unavailable_models:
        print("No models available for NPCAUTOMATORS Portal after filtering.")
        return

    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from zorin_cli.auth import DEFAULT_NPCAUTOMATORS_PORTAL_URL

            _url = (_npcautomators_portal_url or DEFAULT_NPCAUTOMATORS_PORTAL_URL).rstrip("/")
    ...
    selected = _prompt_model_selection(
        ...
        portal_url=_npcautomators_portal_url,
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate NpcAutomators as the provider and update config
        inference_url = creds.get("base_url", "")
        _update_config_for_provider("npcautomators", inference_url)
        ...
        if isinstance(current_model_cfg, dict):
            ...
            model_cfg = {}
        model_cfg["provider"] = "npcautomators"
        ...
        save_config(config)
        print(f"Default model set to: {selected} (via NPCAUTOMATORS Portal)")
        # Offer Tool Gateway enablement for paid subscribers
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `zorin auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``zorin auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from zorin_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from zorin_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from zorin_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from zorin_cli.auth import _save_model_choice, deactivate_provider
    from zorin_cli.config import get_env_value, load_config, save_config
    ...
    from zorin_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `zorin model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from zorin_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from zorin_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from zorin_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Zorin will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Zorin will install it now (the preflight below "
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from zorin_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from zorin_cli.auth import _save_model_choice, deactivate_provider
    from zorin_cli.config import load_config, save_config
    from zorin_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from zorin_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from zorin_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from zorin_cli.config import save_env_value, load_config, save_config
    from zorin_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from zorin_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from zorin_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Zorin turns to `copilot --acp`.")
    print("  Zorin currently starts its own ACP subprocess for each request.")
    print("  Zorin uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ZORIN_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from zorin_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from zorin_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from zorin_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so zorin knows where to find it
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from zorin_cli.config import load_config, save_config, get_env_value
    from zorin_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.zorin/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``zorin_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from zorin_cli.main import _prompt_provider_choice
    from zorin_cli.auth import ZAI_ENDPOINTS
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Zorin typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Zorin, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, NpcAutomators."
                )
                print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from zorin_cli.main import _run_anthropic_oauth_flow
    from zorin_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'zorin model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
