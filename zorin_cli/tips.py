...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or zorin -c.",
    ...
    "Ctrl+Z suspends Zorin to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "zorin -c resumes your most recent CLI session. zorin -c \"project name\" resumes by title.",
    "zorin -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "zorin -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "zorin chat -t web,terminal enables only specific toolsets for a focused session.",
    "zorin chat -s github-pr-workflow preloads a skill at launch.",
    "zorin chat -q \"query\" runs a single non-interactive query and exits.",
    "zorin chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "zorin chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "zorin --yolo bypasses all dangerous command approval prompts for the entire session.",
    "zorin chat --source telegram tags the session for filtering in zorin sessions list.",
    "zorin -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "zorin doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "zorin dump outputs a compact setup summary — great for bug reports.",
    "zorin config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "zorin config edit opens config.yaml in your default editor.",
    "zorin config check scans for missing or stale configuration options.",
    "zorin sessions browse opens an interactive session picker with search.",
    "zorin sessions stats shows session counts by platform and database size.",
    "zorin sessions prune --older-than 30 cleans up old sessions.",
    "zorin skills search react --source skills-sh searches the skills.sh public directory.",
    "zorin skills check scans installed hub skills for upstream updates.",
    "zorin skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "zorin skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "zorin mcp add github --command npx adds MCP servers from the command line.",
    "zorin mcp serve runs Zorin itself as an MCP server for other agents.",
    "zorin auth add lets you add multiple API keys for credential pool rotation.",
    "zorin completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "zorin logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "zorin backup creates a zip backup of your entire Zorin home directory.",
    "zorin profile create coder creates an isolated profile that becomes its own command.",
    "zorin profile create work --clone copies your current config and keys to a new profile.",
    "zorin update syncs new bundled skills to ALL profiles automatically.",
    "zorin gateway install sets up Zorin as a system service (systemd/launchd).",
    "zorin memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "zorin webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: zorin tools disables unused tools, zorin skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "zorin models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as zorin -w).",
    ...
    "ZORIN_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Zorin tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'zorin profile create coder' creates the 'coder' command.",
    "zorin profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Zorin prints a resume command with session ID and stats.",
    "zorin sessions export backup.jsonl exports all sessions for backup or analysis.",
    "zorin -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "zorin skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .zorin/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: zorin cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Zorin runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "zorin gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Zorin via a local macOS server.",
    ...
    "Context files (.zorin.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Zorin loads project context from .zorin.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "zorin mcp opens an interactive picker of NpcAutomators-approved MCPs you can install in one keystroke.",
    "zorin mcp catalog lists NpcAutomators-approved MCP servers shipped with the repo.",
    "zorin mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — zorin tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.zorin/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "zorin chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "zorin plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "zorin acp runs Zorin as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ZORIN_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "zorin auth add npcautomators or zorin auth add openai-codex sets up OAuth-based providers.",
    ...
    "zorin status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.zorin/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Zorin your own.",
    ...
    "Before a gateway session reset, Zorin auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.zorin/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Zorin state via ZORIN_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ZORIN_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ZORIN_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.zorin/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'zorin profile create ls' would be rejected.",
    "zorin profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "zorin claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ZORIN_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via zorin tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.zorin/.env.",
    "Skills from trusted repos (NPCAutomators) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.zorin/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Zorin auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Zorin config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Zorin is working.',
    ...
    '/reload reloads ~/.zorin/.env into the running session — pick up new API keys without restarting.',
    ...
    "ZORIN_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.zorin/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.zorin/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'zorin curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "zorin curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'zorin curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'zorin auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your NPCAUTOMATORS subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'ZORIN_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ZORIN_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'zorin dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'zorin dashboard embeds the full Zorin TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.zorin/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.zorin/dashboard-plugins/ — no npm build required.',
    ...
    'ZORIN_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ZORIN_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'ZORIN_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ZORIN_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.zorin/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.zorin/skills/ so drop-in skills appear without restarting the session.',
    ...
    'zorin -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'zorin chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'zorin chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'zorin chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "zorin chat --source tool tags programmatic chats so they don't clutter zorin sessions list.",
    'zorin dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'zorin sessions rename <ID> "new title" renames any past session; zorin sessions delete <ID> removes one.',
    'zorin import restores a full Zorin backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'zorin fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'zorin pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'zorin setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'zorin status --deep runs the full health sweep across every component; plain zorin status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ZORIN_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ZORIN_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.zorin/plugins/ — trust-gated by design.',
    "ZORIN_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ZORIN_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ZORIN_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ZORIN_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ZORIN_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ZORIN_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ZORIN_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ZORIN_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ZORIN_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ZORIN_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ZORIN_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Zorin block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from zorin sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.zorin/dashboard-plugins/.',
]
