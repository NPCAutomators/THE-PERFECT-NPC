"""``zorin project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$ZORIN_HOME/projects.db`` store
(see :mod:`zorin_cli.projects_db`).
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'ZORIN'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``zorin project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: zorin project <action> [options]\n"
                "Run 'zorin project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `zorin project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `zorin project add-folder`.",
            file=sys.stderr,
        )
        return 1
