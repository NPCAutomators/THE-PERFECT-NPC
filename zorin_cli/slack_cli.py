"""``zorin slack ...`` CLI subcommands.

Today only ``zorin slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ zorin slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Zorin. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Zorin deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Zorin on Slack's older Assistant messaging
    ...
    """
    from zorin_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Zorin in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Zorin in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Zorin agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``zorin_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$ZORIN_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Zorin")
    ...
    """
    name = getattr(args, "name", None) or "Zorin"
    description = getattr(args, "description", None) or "Your Zorin agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("ZORIN_HOME") or str(Path.home() / ".zorin")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Zorin app\n"
