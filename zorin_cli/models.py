...
Add, remove, or reorder entries here — both `zorin setup` and
`zorin` provider-selection will pick up the change automatically.
"""
...
from zorin_cli import __version__ as _ZORIN_VERSION
from zorin_cli.urllib_security import open_credentialed_url

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_ZORIN_USER_AGENT = f"zorin-cli/{_ZORIN_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `zorin model`
    flow without maintaining a separate static list.
    """
    from zorin_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ZORIN_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Zorin-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ZORIN_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Zorin refreshes the cache from
...
# ---------------------------------------------------------------------------
# NPCAUTOMATORS Portal free-model helper
# ---------------------------------------------------------------------------
# The NPCAUTOMATORS Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# NPCAUTOMATORS Portal account tier detection
# ---------------------------------------------------------------------------
def is_npcautomators_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_npcautomators_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split NpcAutomators models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/npcautomators/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["npcautomators"]`` list happens to contain or whether the
    ...
    older Zorin that doesn't ship that model in its hardcoded curated
    ...
    * ``pricing`` gets a synthetic ``{"prompt": "0", "completion": "0"}``
      ...
      map, so :func:`partition_npcautomators_models_by_tier` keeps it.

    Failures (network, parse, missing field) are silent and degrade to
    returning the inputs unchanged.
    """
    try:
        payload = fetch_npcautomators_recommended_models(
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/npcautomators/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["npcautomators"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Zorin that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_npcautomators_models_by_tier` keeps free models selectable;
...
def check_npcautomators_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current NPCAUTOMATORS Portal user is on a free (unpaid) tier.
    ...
    try:
        ...
        account_info = get_npcautomators_portal_account_info(force_fresh=force_fresh)
...
# ---------------------------------------------------------------------------
# NPCAUTOMATORS Portal recommended models
#
...
NPCAUTOMATORS_RECOMMENDED_MODELS_PATH = "/api/npcautomators/recommended-models"
_NPCAUTOMATORS_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_npcautomators_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def _npcautomators_recommended_disk_path() -> "Path":
    """Disk path for the persisted recommended-models cache."""
    from zorin_constants import get_zorin_home
    return get_zorin_home() / "cache" / "npcautomators_recommended_cache.json"


def _read_npcautomators_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    """
    try:
        with open(_npcautomators_recommended_disk_path(), encoding="utf-8") as fh:
            blob = json.load(fh)
...
def _write_npcautomators_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    if not data:
        return
    path = _npcautomators_recommended_disk_path()
    try:
        try:
        ...
        logging.getLogger(__name__).debug(
            "npcautomators recommended-models disk cache write failed: %s", exc
        )


def fetch_npcautomators_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the NPCAUTOMATORS Portal's curated recommended-models payload.

    Hits ``<portal>/api/npcautomators/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_NPCAUTOMATORS_RECOMMENDED_CACHE_TTL`` seconds in process; pass
    ``force_refresh=True`` to bypass the in-process cache.

    A successful live fetch is also persisted to a per-base disk cache
    (``$ZORIN_HOME/cache/npcautomators_recommended_cache.json``) as last-known-good.
    ...
    """
    base = (portal_base_url or "https://portal.npcautomators.com").rstrip("/")
    now = time.monotonic()
    cached = _npcautomators_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _NPCAUTOMATORS_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{NPCAUTOMATORS_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    if data:
        ...
        _npcautomators_recommended_cache[base] = (data, now)
        _write_npcautomators_recommended_disk(base, data)
        return data
    ...
    disk = _read_npcautomators_recommended_disk(base)
    if disk:
        _npcautomators_recommended_cache[base] = (disk, now)
        return disk
...
def _resolve_npcautomators_portal_url() -> str:
    ...
    try:
        ...
        return str(DEFAULT_NPCAUTOMATORS_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://portal.npcautomators.com"
...
def get_npcautomators_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_npcautomators_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_npcautomators_portal_url()
    payload = fetch_npcautomators_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_npcautomators_free_tier()
        except Exception:
...
# this list:  zorin model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `zorin model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `zorin model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("npcautomators",           "NPCAUTOMATORS Portal",              "NPCAUTOMATORS Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Zorin provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `zorin model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``zorin model``, the
...
# In-repo fallback for the model Zorin silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from zorin_cli.model_catalog import get_default_model_from_cache
...
# curated-list entry [0]. Metered aggregators (NPCAUTOMATORS Portal, OpenRouter) order
...
# ``zorin model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in zorin_cli/web_server.py and
# ``partition_npcautomators_models_by_tier`` — which can hit the Portal.
_SILENT_DEFAULT_PROVIDERS: frozenset[str] = frozenset({"npcautomators", "openrouter"})
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``zorin auth add openai-codex`` without
    ``zorin model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``zorin model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    zorin-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (NPCAUTOMATORS Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from zorin_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — zorin-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Zorin lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_npcautomators_model_ids() -> list[str]:
    """Return the curated NPCAUTOMATORS Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["npcautomators"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from zorin_cli.model_catalog import get_curated_npcautomators_models
        remote = get_curated_npcautomators_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("npcautomators", []))
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, NPCAUTOMATORS Portal).
    """
...
_DEFAULT_NPCAUTOMATORS_INFERENCE_BASE = "https://inference-api.npcautomators.com"


def _resolve_npcautomators_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for NPCAUTOMATORS Portal pricing.

    The NpcAutomators inference ``/v1/models`` endpoint exposes pricing without
    ...
    try:
        ...
        pass
    return ("", _DEFAULT_NPCAUTOMATORS_INFERENCE_BASE)


def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, npcautomators, novita)."""
    ...
    if normalized == "npcautomators":
        api_key, base_url = _resolve_npcautomators_pricing_credentials()
        if base_url:
            # NpcAutomators base_url typically looks like https://inference-api.npcautomators.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``zorin model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from zorin_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        npcautomators:zorin-3                           →  ("npcautomators", "zorin-3")
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, NpcAutomators, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"npcautomators", "openrouter", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from zorin_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model npcautomators` or `/model anthropic`, treat it as a
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            ...
            # curated list is ordered most-capable-first (e.g. NPCAUTOMATORS Portal),
            # entry [0] is the priciest flagship, and typing ``/model npcautomators``
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Zorin' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``zorin_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Zorin should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``zorin auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.zorin/.env``.
    ...
    """
    try:
        from zorin_cli.auth import resolve_api_key_provider_credentials
...
#   - "npcautomators": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, NpcAutomators),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Zorin release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Zorin release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "npcautomators":
        # Try live NPCAUTOMATORS Portal /models endpoint
        try:
            from zorin_cli.auth import fetch_npcautomators_models, resolve_npcautomators_runtime_credentials
            creds = resolve_npcautomators_runtime_credentials()
            if creds:
                live = fetch_npcautomators_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["npcautomators"] snapshot — so newly
        # added Portal models still surface without a Zorin release.
        manifest_ids = get_curated_npcautomators_model_ids()
        if manifest_ids:
            return manifest_ids
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``zorin model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $ZORIN_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from zorin_constants import get_zorin_home
    return get_zorin_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, npcautomators portal), the
    relevant tokens live in ``$ZORIN_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ZORIN_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Zorin' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ZORIN_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"zorin-agent/{_ZORIN_VERSION}"
    ...
    if isinstance(request_headers, dict):
        ...
        from zorin_cli.config import normalize_extra_headers
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=zorin"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from zorin_constants import get_zorin_home
    return get_zorin_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `zorin moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Zorin will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Zorin cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Zorin actually ships.  (#46850)
