"""
Profile management for multiple isolated Zorin instances.

Each profile is a fully independent ZORIN_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.zorin/profiles/<name>/`` by default.

The "default" profile is ``~/.zorin`` itself — backward compatible,
zero migration needed.

Usage::

    zorin profile create coder          # fresh profile + bundled skills
    zorin profile create coder --clone  # also copy config, .env, SOUL.md, skills
    zorin profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    zorin -p coder chat                 # or via flag
    zorin profile use coder             # set as sticky default
    zorin profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See zorin_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `zorin profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `zorin
...
# `zorin skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Zorin infrastructure directories that only the default profile
         (``~/.zorin``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_zorin_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.zorin) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "zorin-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when ZORIN_HOME equals the
...
# into the export. The set below identifies the *known Zorin profile
# artifacts* at the root of ZORIN_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``zorin_constants``.
...
_RESERVED_NAMES = frozenset({
    "zorin", "default", "test", "tmp", "root", "sudo",
})

# Zorin subcommands that cannot be used as profile names/aliases
_ZORIN_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the zorin root, NOT to the current ZORIN_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ZORIN_HOME points outside
    ``~/.zorin``, profiles live under ``ZORIN_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_zorin_home() / "profiles"


def _get_default_zorin_home() -> Path:
    """Return the default (pre-profile) ZORIN_HOME path.

    In standard deployments this is ``~/.zorin``.
    In Docker/custom deployments where ZORIN_HOME is outside ``~/.zorin``
    (e.g. ``/opt/data``), returns ZORIN_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_zorin_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Zorin installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ZORIN_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_zorin_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, zorin subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _ZORIN_SUBCOMMANDS:
        return f"'{canon}' conflicts with a zorin subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            zorin_exe = shutil.which("zorin") or "zorin"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(zorin_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Zorin. If we
    ...
    try:
        ...
        # not be migrated. The next `zorin doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``zorin -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``zorin profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``zorin -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``zorin profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ZORIN_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Zorin config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_zorin_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, zorin_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ZORIN_HOME.
    ...
    The returned ``zorin_home`` is the path to pass to
    ``set_zorin_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_zorin_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``zorin update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.zorin)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.zorin/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `zorin -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Zorin profile.\n"
    ...
    if not soul_path.exists():
        try:
            from zorin_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `zorin update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`zorin profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Zorin (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `zorin profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `zorin -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ZORIN_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``zorin profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ZORIN_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_zorin_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Zorin *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Zorin backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``ZORIN_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `zorin -p <canon> profile
    ...
    zorin_markers = ("zorin_cli.main", "zorin-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Zorin process: either an entrypoint marker in argv, or
            # a resolved executable named `zorin`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_zorin = (
                any(marker in joined for marker in zorin_markers)
                or exe_name == "zorin"
                or exe_name.startswith("zorin")
            )
            if not is_zorin:
                continue
            ...
            # ...or by ZORIN_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("ZORIN_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.zorin).\n"
            "To remove everything, use: zorin uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_zorin_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ZORIN_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from zorin_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ZORIN_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ZORIN_HOME")
    try:
        os.environ["ZORIN_HOME"] = str(profile_dir)
        from zorin_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ZORIN_HOME"] = old_home
        elif "ZORIN_HOME" in os.environ:
            del os.environ["ZORIN_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.zorin/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: zorin profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ZORIN_HOME.

    Returns ``"default"`` if ZORIN_HOME is not set or points to ``~/.zorin``.
    Returns the profile name if ZORIN_HOME points into ``~/.zorin/profiles/<name>``.
    Returns ``"custom"`` if ZORIN_HOME is set to an unrecognized path.
    """
    from zorin_constants import get_zorin_home
    zorin_home = get_zorin_home()
    resolved = zorin_home.resolve()

    default_resolved = _get_default_zorin_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      ZORIN_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Zorin file the
      user may have lying alongside ZORIN_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.zorin itself — its parent is ~/ and its
        # directory name is ".zorin", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: zorin profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.zorin itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.zorin). "
            "Specify a different name: zorin profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="zorin_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"zorin_{old_name}"
    legacy_old_host = f"zorin.{old_name}"
    new_host = f"zorin_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_zorin_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("zorin_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ZORIN_HOME path string.

    Called early in the CLI entry point, before any zorin modules
    are imported, to set the ZORIN_HOME environment variable.
    """
