...
on Japanese locales, etc.).  Zorin's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``zorin_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Zorin is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ZORIN_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ZORIN_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Zorin-managed Git install directories so
    ...
    # already-running shells don't see the change — which means zorin
    ...
    # install without Zorin-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Zorin's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Zorin's
    ``zorin config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Zorin (User env var in Windows Settings, or
    export in a PowerShell profile) and Zorin picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Zorin-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\zorin\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``zorin`` from right after install) retain their old PATH.

    Any subprocess Zorin spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Zorin-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "zorin", "git", "cmd"),
        os.path.join(local_appdata, "zorin", "git", "bin"),
        os.path.join(local_appdata, "zorin", "git", "usr", "bin"),
        # Zorin venv Scripts directory — host of the zorin.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes zorin, but harmless to include.
        os.path.join(local_appdata, "zorin", "zorin-agent", "venv", "Scripts"),
