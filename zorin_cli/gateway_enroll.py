"""``zorin gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh NPCAUTOMATORS Portal access token from the existing login
     (``~/.zorin/auth.json``) — the same path ``zorin dashboard register``
     uses (``resolve_npcautomators_access_token``). This proves *which NpcAutomators org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.zorin/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or NPCAUTOMATORS Portal).
    ...
    self-hosted-IdP path; otherwise NPCAUTOMATORS Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your NPCAUTOMATORS Portal "
                "token could not be verified — try `zorin auth add npcautomators` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from zorin_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `zorin gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO NPCAUTOMATORS Portal); otherwise the NPCAUTOMATORS Portal token.
    try:
    ...
    #    save_env_value writes them to ~/.zorin/.env (0600 dir) and never logs.
    to_write = {
