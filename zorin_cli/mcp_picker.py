"""MCP picker — interactive `zorin mcp picker` (also the default `zorin mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``zorin mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `zorin plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from zorin_cli.cli_output import prompt_yes_no
from zorin_cli.curses_ui import curses_single_select
from zorin_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Zorin session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from zorin_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `zorin mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: zorin mcp install <name>    Picker: zorin mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Zorin is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Zorin — run `zorin update` "
...
def show_catalog() -> None:
    """`zorin mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`zorin mcp picker` (and default `zorin mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`zorin mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `zorin mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
