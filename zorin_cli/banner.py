...
Pure display functions with no ZorinCLI state dependency.
"""
import json
import logging
import os
...
# (e.g. nix-built zorin — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NPCAutomators/AGENT-ZORIN.git"
_OFFICIAL_REPO_CANONICAL = "github.com/npcautomators/agent-zorin"
...
def _fetch_pypi_latest(package: str = "zorin-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Zorin update is available.

    Two paths: if ``ZORIN_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = zorin_home / ".update_check"
    embedded_rev = os.environ.get("ZORIN_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # ZORIN_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `zorin update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/zorin/update/check`
    ...
    try:
        from zorin_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $ZORIN_HOME/zorin-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = zorin_home / "zorin-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Zorin git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ZORIN_HOME/zorin-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``zorin_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from zorin_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NPCAutomators/AGENT-ZORIN/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Zorin checkout. Cached per-process. Release URL always points at the
    canonical NPCAutomators/AGENT-ZORIN repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"ZORIN v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ZORIN_CADUCEUS
    except Exception:
        _bskin = None
        _hero = ZORIN_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]NPCAUTOMATORS[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]NPCAUTOMATORS[/]")

    if os.getenv("ZORIN_YOLO_MODE"):
    ...
    try:
        from zorin_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from zorin_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built zorin; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ZORIN_AGENT_LOGO
        console.print(_logo)
        console.print()
