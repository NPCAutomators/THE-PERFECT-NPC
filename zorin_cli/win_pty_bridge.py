"""Windows ConPTY bridge for the `zorin dashboard` chat tab.

Drop-in counterpart to ``zorin_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``zorin_cli.web_server`` consumes: ``spawn``, ``read``,
