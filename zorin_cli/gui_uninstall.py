"""
Zorin Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``zorin desktop`` / ``zorin gui``)
     Built inside the agent checkout under ``$ZORIN_HOME/zorin-agent/``:
       ...
       - ``$ZORIN_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Zorin.app`` or ``~/Applications/Zorin.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Zorin`` (NSIS per-user)
...
the app name ("Zorin"), separate from ``$ZORIN_HOME``:
  - macOS:   ``~/Library/Application Support/Zorin``
  - Windows: ``%APPDATA%\\Zorin``
  - Linux:   ``$XDG_CONFIG_HOME/Zorin`` (default ``~/.config/Zorin``)
...
``zorin uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(zorin_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return zorin_home / "zorin-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Zorin"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Zorin"
...
def source_built_gui_artifacts(zorin_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``zorin desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``zorin-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(zorin_home)
    ...
    return [
        ...
        zorin_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Zorin" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Zorin.app"),
            home / "Applications" / "Zorin.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Zorin).
            local_base / "Programs" / "Zorin",
            # Older / alternate layout some builds used.
            local_base / "zorin-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Zorin")
    else:
        ...
        paths += [
            data_base / "applications" / "zorin.desktop",
            data_base / "applications" / "Zorin.desktop",
        ]
    return paths


def agent_is_installed(zorin_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ZORIN_HOME.
    ...
    if (agent_root / "zorin_cli").is_dir():
        return True
...
def gui_is_installed(zorin_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(zorin_home):
        if p.exists():
            return True
...
def gui_install_summary(zorin_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = zorin_home if zorin_home is not None else get_zorin_home()
...
def uninstall_gui(zorin_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``zorin-agent/zorin_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ZORIN_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove zorin' or "
            "'sudo dnf remove zorin'). AppImage builds are a single file you "
