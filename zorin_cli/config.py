"""
Configuration management for ZORIN.

Config files are stored in ~/.zorin/ for easy access:
- ~/.zorin/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.zorin/.env         - API keys and secrets

This module provides:
- zorin config          - Show current configuration
- zorin config edit     - Open config in editor
- zorin config get      - Print a resolved configuration value
- zorin config set      - Set a specific value
- zorin config unset    - Remove a user configuration value
- zorin config wizard   - Re-run setup wizard
"""
...
from zorin_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``zorin config set`` (which rewrites
    ...
    ``config.yaml`` in place: zorin never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.zorin/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``zorin logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  zorin config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Zorin makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Zorin itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``zorin update``, the TUI build.
...
#   on every plugin install / ``zorin update``.
...
# * ``ZORIN_HOME`` / ``ZORIN_PROFILE`` / ``ZORIN_CONFIG`` /
#   ``ZORIN_ENV`` — Zorin runtime location flags. Writing these into
...
#
# IMPORTANT: ``ZORIN_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ZORIN_LANGFUSE_PUBLIC_KEY,
# ZORIN_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Zorin runtime location — never via dashboard env writer.
    # NOT a ZORIN_* blanket: integration credentials (ZORIN_GEMINI_*,
    # ZORIN_LANGFUSE_*, ZORIN_SPOTIFY_*, ...) ARE allowed.
    "ZORIN_HOME", "ZORIN_PROFILE", "ZORIN_CONFIG", "ZORIN_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Zorin runtime location "
            "(ZORIN_HOME, ZORIN_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ZORIN_TOOL_PROGRESS", "ZORIN_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `zorin plugins enable
    # observability/langfuse` or `zorin tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ZORIN_LANGFUSE_ENV",
    "ZORIN_LANGFUSE_RELEASE",
    "ZORIN_LANGFUSE_SAMPLE_RATE",
    "ZORIN_LANGFUSE_MAX_CHARS",
    "ZORIN_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from zorin_cli.colors import Colors, color
from zorin_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ZORIN_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_zorin_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Zorin is running in package-manager-managed mode.

    Two signals: the ZORIN_MANAGED env var (set by the systemd service),
    or a .managed marker file in ZORIN_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade zorin-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``zorin_cli/`` — i.e. the git checkout for source
    installs, ``/opt/zorin`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$ZORIN_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Zorin was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$ZORIN_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ZORIN_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.zorin``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ZORIN_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.zorin:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ZORIN_HOME``, a
    ...
    ``docker`` and ``zorin update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``npcautomators/agent-zorin`` image bakes a ``docker``
        stamp into ``/opt/zorin`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ZORIN_HOME.
    try:
    ...
    #    host install whose shared $ZORIN_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``zorin update``.
    try:
        method = (
            (get_zorin_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``zorin_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ZORIN_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/zorin`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Zorin lives in a ``uv tool`` layout.

    ``uv tool install zorin-agent`` places the install at
    ``.../uv/tools/zorin-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``zorin-agent`` happens to be uv-tool-installed on the machine while
    the *active* Zorin is a regular pip/venv install, causing
    ``zorin update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/zorin-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade zorin-agent"
    if method == "docker":
        return "docker pull npcautomators/agent-zorin:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade zorin-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade zorin-agent"
        return "pip install --upgrade zorin-agent"
    return "zorin update"
...
# banner, the TUI/desktop session info panel, and ``zorin update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``zorin update`` / ``zorin update --check`` so the wording — and the
...
# Long-form text for ``zorin update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# zorin_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Zorin, it doesn't update
...
✗ ``zorin update`` doesn't apply inside the Docker container.

ZORIN runs as a published image (npcautomators/agent-zorin), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull npcautomators/agent-zorin:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate zorin-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm npcautomators/agent-zorin:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/npcautomators/agent-zorin/tags
  • Your config and session history live under ``$ZORIN_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``zorin update`` inside Docker.
...
def format_managed_message(action: str = "modify this Zorin installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ZORIN_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Zorin installation is managed by NixOS "
            f"(ZORIN_MANAGED={env_hint}).\n"
            "Edit services.zorin-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Zorin installation is managed by Homebrew "
            f"(ZORIN_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade zorin-agent"
        )

    return (
        f"Cannot {action}: this Zorin installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Zorin."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ZORIN_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, zorin_bin
    or None if container mode is not active, we're already inside the
    container, or ZORIN_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ZORIN_DEV") == "1":
        return None
    ...
    container_mode_file = get_zorin_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "zorin-agent")
    exec_user = info.get("exec_user", "zorin")
    zorin_bin = info.get("zorin_bin", "/data/current-package/bin/zorin")
...
# Re-export from zorin_constants — canonical definition lives there.
from zorin_constants import get_zorin_home, get_process_zorin_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_zorin_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_zorin_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_zorin_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ZORIN_UID / ZORIN_GID env vars set by Docker deployments.

    Docker containers running Zorin commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ZORIN_HOME once, but
    subdirectories created at runtime by ``ensure_zorin_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ZORIN_UID", "").strip()
    gid_str = os.environ.get("ZORIN_GID", "").strip()
    try:
...
def _chown_to_zorin_uid(path) -> None:
    """Chown ``path`` to ``ZORIN_UID:ZORIN_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_zorin_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_zorin_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the zorin group can
    share state with the gateway service.

    The mode can be overridden via the ZORIN_HOME_MODE environment variable
    (e.g. ZORIN_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ZORIN_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ZORIN_UID``/``ZORIN_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ZORIN_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_zorin_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Zorin runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ZORIN_CONTAINER") or os.environ.get("ZORIN_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ZORIN_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ZORIN_HOME, upgrading legacy empty templates.
...
# ensure_zorin_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_ZORIN_HOME_ENSURED: set = set()


def ensure_zorin_home():
    """Ensure ~/.zorin directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``ZORIN_HOME`` we only re-run
    ...
    ``get_zorin_home()`` and therefore re-run for the new path.
    """
    home = get_zorin_home()
    key = str(home)

    if key in _ZORIN_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``zorin profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old ZORIN_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_zorin_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _ZORIN_HOME_ENSURED.add(key)


def _ensure_zorin_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ZORIN_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Zorin-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Zorin
        ...
        # identity slot (SOUL.md). Empty by default. The ZORIN_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Zorin adds a coding operating brief
        ...
        # ``ZORIN_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             ZORIN_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ZORIN_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Zorin auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Zorin sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Zorin
        ...
        # bundled Zorin image, which drops to the `zorin` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Zorin sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Zorin should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``zorin chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.zorin/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: zorin sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .zorin.md, or .cursorrules before Zorin applies
    ...
    # single read_file page exceeds these limits, Zorin truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Zorin' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); zorin =
                                      # Zorin' compression threshold triggers
    ...
    # See tools/kanban_tools.py and zorin_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``zorin dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        "transient_retries": 2,
        "vision": {
            "provider": "auto",    # auto | openrouter | npcautomators | codex | custom
        ...
        # Invoked by ``zorin kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``zorin kanban decompose`` and the kanban
        ...
        # ``zorin profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `zorin model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `zorin` (and `zorin chat`) launches by default:
        ...
        # REPL and `--tui` (or ZORIN_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `zorin --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `zorin -c` muscle memory.  Default off so existing
        # users aren't surprised.  ZORIN_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `zorin --tui` drops a one-time hint
        ...
        # NpcAutomators credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # Keep disabled by default: periodic redraws in non-fullscreen mode can
        # stamp stale status bars and prompts into terminal scrollback. Input,
        # agent, and resize events still invalidate the UI when state changes.
        ...
        # can set `path`/`paths` to ZORIN_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/zorin`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `zorin pets`. Disabled until
        ...
        "pet": {
            ...
            # get_zorin_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled NPCAUTOMATORS Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``ZORIN_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ZORIN_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.npcautomators.com).
        "oauth": {
        ...
        # (``ZORIN_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # npcautomators-account-service at deploy time via the
        # ``ZORIN_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ZORIN_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/zorin``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Zorin appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.zorin/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.zorin/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, NpcAutomators, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.zorin/cache/delegation/
    ...
    # response. If not, Zorin feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Zorin auto-pauses the goal and
    ...
    "moa": {
        ...
        # <zorin_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.zorin/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ZORIN_SKILL_DIR} and ${ZORIN_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `zorin curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `zorin curator run --consolidate` overrides
        ...
        # normally restored on every `zorin update`, so pruning them only
        ...
        # skipped), snapshot ~/.zorin/skills/ into
        # ~/.zorin/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `zorin curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for zorin-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *ZORIN*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ZORIN_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Zorin' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.zorin/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ZORIN_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `zorin doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``zorin_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Zorin to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ZORIN_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing NPCAUTOMATORS Portal token — there is no token key here.
        "chronos": {
            ...
            "portal_url": "https://portal.npcautomators.com",
        ...
        # Also overridable via ZORIN_CRON_MAX_PARALLEL env var.
        ...
        # ZORIN_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `zorin -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `zorin` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `zorin kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     zorin-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Zorin tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.zorin/logs/.
    ...
    # curated model lists for OpenRouter and NPCAUTOMATORS Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a zorin-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://raw.githubusercontent.com/NPCAutomators/AGENT-ZORIN/main/website/static/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `zorin model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal ZORIN_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.zorin/sessions/sessions.json entirely.
        ...
        # the ZORIN_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.zorin.gateway` that defenses 1-2 don't
        ...
        # ``ZORIN_GATEWAY_MAX_STARTS`` / ``ZORIN_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.zorin/.env,
        ...
        # behavior: files must live under the Zorin cache, under
        ...
        # user. Bridged to ZORIN_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Zorin
        # cache (~/.zorin/cache/{documents,images,audio,video,screenshots})
        ...
        # to ZORIN_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ZORIN_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ZORIN_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.zorin/state.db.
    ...
    "sessions": {
        ...
        # default of ``zorin sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.zorin/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `zorin sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `zorin update` prints a one-line notice with
        ...
        # ~/.zorin/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to ZORIN_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # ZORIN_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <ZORIN_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``zorin backup``-style zip
        #     of ZORIN_HOME into <ZORIN_HOME>/backups/, restorable with
        #     ``zorin import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `zorin update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ZORIN_HOME>/lsp/bin/`` on first use.
    ...
    # `zorin tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.zorin/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.zorin/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.zorin/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Zorin may use this encrypted
            ...
            # ~/.zorin/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.zorin/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Zorin disables it for our users unless they
        # explicitly opt in here. When false (default), Zorin sets
    ...
    # Zorin Desktop (Electron app) launch options. These only affect
    # `zorin desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ZORIN_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
# selection step (NPCAUTOMATORS Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NPCAUTOMATORS_BASE_URL": {
        "description": "NPCAUTOMATORS Portal base URL override",
        "prompt": "NPCAUTOMATORS Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Zorin mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'zorin model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for NpcAutomators Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for NpcAutomators Subscribers only, used to derive vendor hosts, e.g. npcautomators.com -> firecrawl-gateway.npcautomators.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for NpcAutomators Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit NpcAutomators Subscriber access token for tool-gateway requests (optional; otherwise read from the Zorin auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Zorin, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @zorin:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ZORIN_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #zorin)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: zorin-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'zorin-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Zorin API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Zorin API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Zorin API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ZORIN_TOOL_PROGRESS and ZORIN_TOOL_PROGRESS_MODE are deprecated —
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `zorin config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.zorin.config`` entries, then checks
    ...
    try:
        ...
        # should never break `zorin update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Zorin' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Zorin versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``zorin_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (zorin_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (zorin_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (zorin_cli/setup.py)
    "known_plugin_toolsets", # written/read by zorin_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'zorin setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Zorin won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, zorin send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'zorin doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ZORIN_HOME", "~/.zorin")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "zorin update / zorin -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ZORIN_TOOL_PROGRESS")
            old_mode = get_env_value("ZORIN_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ZORIN_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ZORIN_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ZORIN_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ZORIN_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ZORIN_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_zorin_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `zorin plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `zorin curator status` has no
    ...
    #   3. Creates `~/.zorin/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_zorin_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_zorin_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `zorin config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `zorin config set`"
                    )
    ...
    # behavior set it to true explicitly via `zorin config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from zorin_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from zorin_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: zorin config set <key> <value>")
    ...
    # `zorin update`, but only the version bump is persisted.
    ...
    # metadata.zorin.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.zorin/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_zorin_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``zorin config set``
    ...
    backends) — while display paths (``zorin status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _zorin_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.zorin/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.zorin/config.yaml.
    ...
    (which change ``ZORIN_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/zorin/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_zorin_dotenv()
...
#   openai-codex (OAuth — zorin auth) — OpenAI Codex
#   npcautomators         (OAuth — zorin auth) — NPCAUTOMATORS Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.zorin/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Zorin writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.zorin/.env.
    ...
    interactive menu render (`zorin tools`, `zorin setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `zorin tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Zorin env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.zorin/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.zorin/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.zorin/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.zorin/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Zorin — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Zorin vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.zorin/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.zorin/.env`` over ``os.environ``.

    Used for Zorin-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Zorin Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from zorin_cli.auth import get_anthropic_key
    ...
    # Warn on stale ZORIN_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ZORIN_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ZORIN_MAX_ITERATIONS={_env_ghost} "
                f"(run 'zorin doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  zorin config edit     # Edit config file", Colors.DIM))
    print(color("  zorin config set <key> <value>", Colors.DIM))
    print(color("  zorin setup           # Run setup wizard", Colors.DIM))
    print()
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``zorin config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``zorin config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            exposes this via ``zorin config set --force``.
    """
    if is_managed():
    ...
    # so a fresh `zorin config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `zorin config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `zorin config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: zorin config get <key> [--json]")
            print()
            print("Examples:")
            print("  zorin config get model")
            print("  zorin config get terminal.backend")
            print("  zorin config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: zorin config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  zorin config set model anthropic/claude-sonnet-4")
            print("  zorin config set terminal.backend docker")
            print("  zorin config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: zorin config unset <key>")
            print()
            print("Examples:")
            print("  zorin config unset model")
            print("  zorin config unset terminal.backend")
            print("  zorin config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'zorin config migrate' to add them")
        ...
        print("  zorin config           Show current configuration")
        print("  zorin config edit      Open config in editor")
        print("  zorin config get <key>          Print a resolved config value")
        print("  zorin config set <key> <value>   Set a config value")
        print("  zorin config unset <key>        Remove a config value")
        print("  zorin config check     Check for missing/outdated config")
        print("  zorin config migrate   Update config with new options")
        print("  zorin config path      Show config file path")
        print("  zorin config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``zorin config`` UI so users
