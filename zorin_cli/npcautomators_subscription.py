"""Helpers for NPCAUTOMATORS subscription managed-tool capabilities."""
...
from zorin_cli.config import get_env_value, load_config
...
# Maps a tools_config provider's ``managed_npcautomators_feature`` to the tool-pool
# coverage category (zorin_cli.npcautomators_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `zorin tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_npcautomators_subscription_features.
...
class NpcAutomatorsFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_npcautomators: bool
...
@dataclass(frozen=True)
class NpcAutomatorsSubscriptionFeatures:
    subscribed: bool
    npcautomators_auth_present: bool
    provider_is_npcautomators: bool
    features: Dict[str, NpcAutomatorsFeatureState]
    account_info: Optional[NpcAutomatorsPortalAccountInfo] = None

    @property
    def web(self) -> NpcAutomatorsFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> NpcAutomatorsFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> NpcAutomatorsFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> NpcAutomatorsFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> NpcAutomatorsFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> NpcAutomatorsFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> NpcAutomatorsFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[NpcAutomatorsFeatureState]:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_npcautomators_managed_defaults`` from flipping a working local setup
    to the managed gateway.
    """
    if get_env_value("ZORIN_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_npcautomators_subscription_features(
    config: Optional[Dict[str, object]] = None,
    *,
    force_fresh: bool = False,
) -> NpcAutomatorsSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    config = dict(config)
    model_cfg = _model_config_dict(config)
    provider_is_npcautomators = str(model_cfg.get("provider") or "").strip().lower() == "npcautomators"

    try:
        if force_fresh:
            account_info = get_npcautomators_portal_account_info(force_fresh=True)
        else:
            account_info = get_npcautomators_portal_account_info()
    except Exception:
        account_info = None
    ...
    npcautomators_auth_present = bool(account_info and account_info.logged_in)

    def _entitled_for(category: str) -> bool:
        return bool(account_info and account_info.tool_gateway_entitled_for(category))
    subscribed = provider_is_npcautomators or npcautomators_auth_present
    ...
    # requires `pip install faster-whisper`. For NpcAutomators subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_npcautomators_managed_defaults below.
    ...
    # Tool Gateway via `zorin model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("ZORIN_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("ZORIN_LOCAL_STT_COMMAND"))
    ...
    managed_web_available = (
        managed_tools_flag
        and npcautomators_auth_present
    ...
    features = {
        "web": NpcAutomatorsFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_npcautomators=web_managed,
        ...
        "image_gen": NpcAutomatorsFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_npcautomators=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if direct_fal else ("NpcAutomators Subscription" if image_managed else ""),
            explicit_configured=direct_fal,
        ),
        "video_gen": NpcAutomatorsFeatureState(
            key="video_gen",
            label="Video generation",
            included_by_default=False,
            available=video_available,
            active=video_active,
            managed_by_npcautomators=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if direct_fal_video else ("NpcAutomators Subscription" if video_managed else ""),
            explicit_configured=direct_fal_video,
        ),
        "tts": NpcAutomatorsFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_npcautomators=tts_managed,
        ...
        "stt": NpcAutomatorsFeatureState(
            key="stt",
            label="Speech-to-text",
            included_by_default=True,
            available=stt_available,
            active=stt_active,
            managed_by_npcautomators=stt_managed,
        ...
        "browser": NpcAutomatorsFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_npcautomators=browser_managed,
        ...
        "modal": NpcAutomatorsFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_npcautomators=modal_managed,
    ...
    return NpcAutomatorsSubscriptionFeatures(
        subscribed=subscribed,
        npcautomators_auth_present=npcautomators_auth_present,
        provider_is_npcautomators=provider_is_npcautomators,
        features=features,
...
def apply_npcautomators_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_npcautomators:
        return set()
    ...
    # (requires `pip install faster-whisper`); for NpcAutomators subscribers we
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid NpcAutomators subscriber or
    is not using NpcAutomators as their provider.
    """
    ...
    # Quick provider check without the heavy get_npcautomators_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "npcautomators":
        return [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_npcautomators which fires implicitly when
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from zorin_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    try:
        account_info = get_npcautomators_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "NPCAUTOMATORS subscription"
    ...
    if pool_only:
        title = "Your free NpcAutomators tool pool — pick the tools to enable:"
    else:
        title = (
            "Your NPCAUTOMATORS subscription includes the Tool Gateway — "
...
# ---------------------------------------------------------------------------
# Inline NPCAUTOMATORS Portal login for the Tool Gateway picker (`zorin tools`)
# ---------------------------------------------------------------------------


def ensure_npcautomators_portal_access(
    *,
    capability: str = "the NpcAutomators Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the NpcAutomators Tool Gateway, logging in if
    needed.

    Used by ``zorin tools`` when a user selects a NpcAutomators-managed Tool Gateway
    backend (e.g. "Firecrawl (NPCAUTOMATORS Portal)").  Unlike ``zorin model``'s NpcAutomators
    login, this:
    ...
    It only performs the NPCAUTOMATORS Portal device-code OAuth (when the user isn't
    ...
    try:
        info = get_npcautomators_portal_account_info(force_fresh=True)
    except Exception:
        info = None
    if _entitled(info):
        return True
    ...
    if info is None or not info.logged_in:
        if not _run_npcautomators_portal_login_only(capability=capability):
            return False
        try:
            info = get_npcautomators_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_npcautomators_portal_login_only(*, capability: str) -> bool:
    """Run the NPCAUTOMATORS Portal device-code OAuth and persist credentials only.
    ...
    try:
        from zorin_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _npcautomators_device_code_login,
            _read_shared_npcautomators_state,
            _save_auth_store,
            _save_provider_state,
            _sync_npcautomators_pool_from_auth_store,
            _try_import_shared_npcautomators_state,
            _write_shared_npcautomators_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start NPCAUTOMATORS Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a NPCAUTOMATORS Portal login.")
    try:
        proceed = input("  Log in to NPCAUTOMATORS Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped NPCAUTOMATORS Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to NpcAutomators.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing NpcAutomators OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_npcautomators_state(timeout_seconds=15.0)
        ...
        _sync_npcautomators_pool_from_auth_store()
        print("  NPCAUTOMATORS Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _npcautomators_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  NPCAUTOMATORS Portal login failed: {exc}")
        return False
