"""`zorin checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.zorin/checkpoints/``.  Actions:

    zorin checkpoints               # same as `status`
    zorin checkpoints status        # total size, project count, breakdown
    zorin checkpoints list          # per-project checkpoint counts + workdir
    zorin checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    zorin checkpoints clear [-f]    # nuke the entire base (asks first)
    zorin checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    zorin checkpoints
    zorin checkpoints prune --retention-days 3 --max-size-mb 200
    zorin checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: zorin checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``zorin checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `zorin checkpoints` → status
