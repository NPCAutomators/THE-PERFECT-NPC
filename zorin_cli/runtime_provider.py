...
from zorin_cli.auth import (
    ...
    _npcautomators_inference_env_override,
    format_auth_error,
    resolve_provider,
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from zorin_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from zorin_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Zorin' tool dispatch. Gated behind config key
...
def _npcautomators_inference_base_url_override() -> str:
    """Return the trusted NpcAutomators runtime base URL override, if configured.

    Delegates to ``auth._npcautomators_inference_env_override`` so every
    ``NPCAUTOMATORS_INFERENCE_BASE_URL`` read shares one normalization path
    ...
    """
    return _npcautomators_inference_env_override() or ""
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "openai-codex":
        ...
        api_mode = "chat_completions"
        base_url = _npcautomators_inference_base_url_override() or base_url
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from zorin_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from zorin_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("ZORIN_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``npcautomators``, ``openrouter``, …) so
            # accidentally shadowing a canonical provider still resolves to
            # the built-in. See tests/zorin_cli/test_runtime_provider_resolution.py
...
def canonical_custom_identity(
    *,
    ...
    2. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``ZORIN_INFERENCE_PROVIDER`` equivalent). When the agent
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("ZORIN_INFERENCE_PROVIDER", "").strip()
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'zorin model' or "
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "npcautomators":
        state = auth_mod.get_provider_auth_state("npcautomators") or {}
        base_url = (
            explicit_base_url
            or _npcautomators_inference_base_url_override()
            or str(state.get("inference_base_url") or auth_mod.DEFAULT_NPCAUTOMATORS_INFERENCE_URL).strip().rstrip("/")
        )
        ...
        # by resolve_npcautomators_runtime_credentials().
        api_key = explicit_api_key or (
            str(state.get("agent_key") or "").strip()
            if _agent_key_is_usable(
                state,
                max(60, env_int("ZORIN_NPCAUTOMATORS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_npcautomators_runtime_credentials(
                timeout_seconds=float(_getenv("ZORIN_NPCAUTOMATORS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
...
def resolve_runtime_provider(
    *,
    ...
    from zorin_cli.config import is_provider_enabled, load_config
    ...
    if pool and pool.has_credentials():
        ...
        # For NpcAutomators, the pool entry's runtime_api_key is the agent_key
        # compatibility field. It must be an invoke JWT. The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `zorin auth list`). If the key is
        # expired/missing, refresh the selected pool entry before falling back
        # to singleton auth resolution.
        if provider == "npcautomators" and entry is not None:
            min_ttl = max(60, env_int("ZORIN_NPCAUTOMATORS_MIN_KEY_TTL_SECONDS", 1800))
            npcautomators_state = {
                "agent_key": getattr(entry, "agent_key", None),
                "agent_key_expires_at": getattr(entry, "agent_key_expires_at", None),
                "scope": getattr(entry, "scope", None),
            }
            if not _agent_key_is_usable(npcautomators_state, min_ttl):
                logger.debug("NpcAutomators pool entry agent_key expired/missing, refreshing selected pool entry")
                try:
                    refreshed = pool.try_refresh_current()
                except Exception as exc:
                    logger.debug("NpcAutomators pool entry refresh failed: %s", exc)
                    refreshed = None
                ...
                if not pool_api_key or not _agent_key_is_usable(npcautomators_state, min_ttl):
                    logger.debug("NpcAutomators pool entry agent_key still unavailable, falling through to runtime resolution")
                    pool_api_key = ""
        if (
            entry is not None
            and pool_api_key
    ...
    if provider == "npcautomators":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected NpcAutomators but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected NPCAUTOMATORS provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "zorin-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from zorin_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Zorin canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Zorin-compatible importers).
