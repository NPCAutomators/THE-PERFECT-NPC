"""Windows UTF-8 bootstrap for Zorin entry points.
...
should be imported at the very top of every Zorin entry point
(``zorin``, ``zorin-agent``, ``zorin-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Zorin modules.

    Zorin ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Zorin source root
    ...
    """
    root = src_root or os.environ.get("ZORIN_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ZORIN_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ZORIN_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import zorin_bootstrap``
# (or ``from zorin_bootstrap import apply_windows_utf8_bootstrap``) at
