"""
Zorin MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Zorin-specific extra)

Usage:
    zorin mcp serve
    zorin mcp serve --verbose
...
logger = logging.getLogger("zorin.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ZORIN_HOME."""
    try:
        from zorin_constants import get_zorin_home
        return get_zorin_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ZORIN_HOME", Path.home() / ".zorin")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from zorin_constants import get_zorin_home
        directory_file = get_zorin_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ZORIN_HOME", Path.home() / ".zorin")
...
class EventBridge:
    ...
    This is the Zorin equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from zorin_constants import get_zorin_home
            db_file = get_zorin_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ZORIN_HOME", Path.home() / ".zorin")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Zorin MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "zorin",
        instructions=(
            "ZORIN messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Zorin MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
