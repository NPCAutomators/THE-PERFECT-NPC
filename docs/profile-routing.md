...
`zorin config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.zorin/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `ZORIN_HOME`, secret scope, and
