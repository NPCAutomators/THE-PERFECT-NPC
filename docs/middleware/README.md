...
Zorin middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Zorin'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Zorin calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Zorin retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Zorin guardrails,
...
- `telemetry_schema_version`: currently `zorin.observer.v1`
- `middleware_schema_version`: currently `zorin.middleware.v1`
...
Zorin stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Zorin runs
them as a nested chain in registration order. Middleware failures are fail-open:
Zorin logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Zorin applies middleware in this order:
...
For each tool call, Zorin applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Zorin pre-execution path against the effective arguments:
...
```bash
zorin plugins enable <plugin-name>
```

For isolated local testing, use one `ZORIN_HOME` for plugin enablement and the
agent run:

```bash
export ZORIN_HOME=/tmp/zorin-middleware-test
mkdir -p "$ZORIN_HOME"
zorin plugins enable <plugin-name>
zorin chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run zorin plugins enable <plugin-name>
uv run zorin chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["zorin_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/zorin-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Zorin expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Zorin treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Zorin preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Zorin does not convert downstream
