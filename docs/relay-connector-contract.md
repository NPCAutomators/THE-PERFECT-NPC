...
This document is the formal interface between the **Zorin gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`NPCAutomators/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `zorin gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`zorin gateway enroll` → connector `/relay/enroll`, or
...
(`NPCAutomators/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `zorin` CLI.
