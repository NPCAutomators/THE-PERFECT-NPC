...
The canonical transcript store is SQLite via `SessionDB` (from `zorin_state`). The
...
Counts consecutive restarts via a JSON file (`{ZORIN_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `zorin update`, `zorin gateway restart`,
or `/restart`.
