...
When running Zorin inside Docker, the default `network_mode: host` gives the
...
The Zorin [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ZORIN_UID=$(id -u) ZORIN_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=zorin,zorin-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://zorin-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Zorin trust model and vulnerability reporting
