# Zorin Observer Hooks

Zorin observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Zorin' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Zorin catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Zorin only builds
