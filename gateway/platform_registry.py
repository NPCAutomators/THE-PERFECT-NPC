...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``zorin setup`` display).
    ...
    # built-ins).  Used by ``zorin gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `zorin`
        # invocation -- including plain `zorin chat`, which never touches any
        ...
        # `zorin setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``zorin setup``/``gateway status``, channel
