"""Relay/connector support package for the Zorin gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``zorin gateway enroll`` writes them to
    ``~/.zorin/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``ZORIN_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``zorin gateway enroll``).
...
def _resolve_relay_identity_token() -> str:
    ...
    ``zorin gateway enroll`` CLI. Two modes, in precedence order:

      1. **Generic OIDC client-credentials** (air-gapped / self-hosted-IdP, NO
         NPCAUTOMATORS Portal): when ``gateway.idp.token_url`` (or
      ...
      2. **NPCAUTOMATORS Portal** (default): ``resolve_npcautomators_access_token()`` — existing
    ...
    if not token_url:
        # Mode 2 — NPCAUTOMATORS Portal (default, unchanged behaviour).
        from zorin_cli.auth import resolve_npcautomators_access_token

        return resolve_npcautomators_access_token()
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own NpcAutomators access token. In
    that case the runtime resolves the agent's own NpcAutomators access token (the same
    ``resolve_npcautomators_access_token()`` the enroll CLI / dashboard register use),
    ...
    up. The creds live ONLY in process memory — never written to ``~/.zorin/.env``.
    ...
    sets neither ``ZORIN_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``zorin gateway enroll``: has a PINNED
        ``GATEWAY_RELAY_SECRET`` -> skipped (the secret-present guard below).
      - A self-hosted box with a relay URL but no NAS identity:
        ``resolve_npcautomators_access_token()`` fails -> graceful no-op.
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'zorin'}"
