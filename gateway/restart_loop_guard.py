...
Defenses 1 and 2 (the ``_ZORIN_GATEWAY`` guard on ``zorin gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.zorin.gateway")``,
...
State lives in ``<ZORIN_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_zorin_home() / "gateway" / "restart_loop.json"
