...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *ZORIN*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ZORIN_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/zorin), this function
    mirrors the bridge source to a writable ZORIN_HOME location and returns that
    ...
    # Try ZORIN_HOME location first
    zorin_home = get_zorin_home()
    zorin_home_bridge = zorin_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ZORIN_HOME if needed
    if zorin_home_bridge.exists():
        return zorin_home_bridge

    # Mirror the bridge source to ZORIN_HOME
    try:
        zorin_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            zorin_home_bridge,
            dirs_exist_ok=False,
