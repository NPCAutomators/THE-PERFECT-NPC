...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.zorin to the active profile home."""
    ...
    if raw == "~/.zorin":
        return zorin_home
    if raw.startswith("~/.zorin/"):
        return zorin_home / raw.removeprefix("~/.zorin/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return zorin_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under ZORIN_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from zorin_constants import get_zorin_home

    scripts_root = (get_zorin_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.zorin" or raw_text.startswith("~/.zorin/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__zorin_ignore__") is True
        ):
            return False, None
