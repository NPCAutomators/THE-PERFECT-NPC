...
There are two ways to add a platform to the Zorin gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.zorin/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Zorin code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `zorin gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `zorin_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Zorin core.
...
And add it to the `zorin-gateway` composite:

```python
"zorin-gateway": {
    "includes": [..., "zorin-your-platform"]
}
```
...
## 12. Status Display (`zorin_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zorin_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zorin_cli/ toolsets.py \
  --include="*.py" -l | sort -u
