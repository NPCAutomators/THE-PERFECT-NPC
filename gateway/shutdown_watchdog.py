...
2. An event-loop heartbeat file at ``<ZORIN_HOME>/state/gateway.heartbeat`` so
...
def _process_zorin_home() -> Path:
    """ZORIN_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<ZORIN_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_zorin_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from zorin_logging import drain_log_queue
