...
from zorin_constants import get_zorin_dir, get_zorin_home
...
PAIRING_DIR = get_zorin_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from zorin_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{ZORIN_HOME}/pairing`` while newer code/docs may
    write ``{ZORIN_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    ``<ZORIN_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<ZORIN_HOME>/pairing/``
    directory (backward-compat for the ``zorin pairing`` CLI).
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp ZORIN_HOME
        # and PairingStore may be constructed before the env is set.
        if profile:
            from zorin_constants import get_zorin_home
            self._dir = get_zorin_home() / "profiles" / profile / "pairing"
        else:
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `zorin` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> zorin pairing approve ...` as root, "
                    "re-run with `docker exec -u zorin <container> ...` and "
                    "chown the existing file to the zorin user, or restart the "
