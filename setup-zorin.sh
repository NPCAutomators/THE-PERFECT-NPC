...
# 5. Symlinks the 'zorin' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/zorin-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink zorin into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up zorin command..."

ZORIN_BIN="$SCRIPT_DIR/venv/bin/zorin"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ZORIN_BIN" "$COMMAND_LINK_DIR/zorin"
echo -e "${GREEN}✓${NC} Symlinked zorin → $COMMAND_LINK_DISPLAY_DIR/zorin"
...
# ============================================================================
# Seed bundled skills into ~/.zorin/skills/
# ============================================================================

ZORIN_SKILLS_DIR="${ZORIN_HOME:-$HOME/.zorin}/skills"
mkdir -p "$ZORIN_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.zorin/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ZORIN_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  zorin status        # Check configuration"
if is_termux; then
    echo "  zorin gateway       # Run gateway in foreground"
else
    echo "  zorin gateway install # Install gateway service (messaging + cron)"
fi
echo "  zorin cron list     # View scheduled jobs"
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m zorin_cli.main setup
fi
