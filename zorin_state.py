...
DEFAULT_DB_PATH = get_zorin_home() / "state.db"
...
# optimized via ``zorin sessions optimize-storage``. A legacy DB sits at
...
# zorin_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``zorin_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see zorin_cli/kanban_db.py) would
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (zorin_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_zorin_state.py:600-632)
        ...
        probe_session_id = f"_zorin_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.zorin/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``zorin sessions optimize-storage`` on a capable host).
#
...
# triggers repairable UNTIL the user opts into `zorin db optimize`. This is
...
class SessionDB:
    ...
    # With multiple zorin processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`zorin db optimize`),
            ...
            # ``zorin_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `zorin update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._zorin_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._zorin_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `zorin sessions optimize-storage` on a host "
    ...
    #
    # `optimize_fts_storage()` (the `zorin sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`zorin sessions optimize-storage`) ──
    #
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `zorin sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `zorin db optimize`) must keep its EXISTING
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, NpcAutomators) receive
        ...
        if repair_alternation and messages:
            # Lazy import: zorin_state already depends on agent.* (see
    ...
    def get_conversation_root(self, session_id: str) -> str:
        ...
        identifier — used for NPCAUTOMATORS Portal ``conversation=`` usage tagging.
    ...
    def search_messages(
        ...
        Threshold: ZORIN_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("ZORIN_SEARCH_SLOW_MS", "1000"))
            except (TypeError, ValueError):
                threshold = 1000.0
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Zorin surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``zorin sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Zorin, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Zorin session.

        A Zorin session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Zorin session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``ZORIN_DISABLE_FTS_TRIGRAM`` or not yet created), so
