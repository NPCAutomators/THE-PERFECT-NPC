...
# Zorin WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ZORIN_TINI_SHIM_TARGET overrides the `/init` path (and
# ZORIN_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ZORIN_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ZORIN_TINI_SHIM_WRAPPER:-/opt/zorin/docker/main-wrapper.sh}"
