...
#
# Wired into the image as /etc/cont-init.d/01-zorin-setup by the
...
ZORIN_HOME="${ZORIN_HOME:-/opt/data}"
INSTALL_DIR="/opt/zorin"

# Drop to zorin via s6-setuidgid, but skip it when already non-root.
as_zorin() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid zorin "$@"; }
...
# starts non-root. The baked install tree under /opt/zorin is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ZORIN_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ZORIN_UID/ZORIN_GID — or the PUID/PGID aliases — which the
...
# `id -u zorin` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# zorin UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u zorin)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-zorin UID).
...
and the baked /opt/zorin install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ZORIN_UID=\$(id -u) -e ZORIN_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the zorin user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ZORIN_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid zorin mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ZORIN_HOME paths whose parents only
# root can create (e.g. `ZORIN_HOME=/home/zorin/.zorin` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ZORIN_UID/ZORIN_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ZORIN_UID/
# ZORIN_GID still win when both are set.  See #15290, salvages #25872.
ZORIN_UID="${ZORIN_UID:-${PUID:-}}"
ZORIN_GID="${ZORIN_GID:-${PGID:-}}"

if [ -n "${ZORIN_UID:-}" ] && validate_uid_gid "$ZORIN_UID" && [ "$ZORIN_UID" != "$(id -u zorin)" ]; then
    echo "[stage2] Changing zorin UID to $ZORIN_UID"
    usermod -u "$ZORIN_UID" zorin
fi
if [ -n "${ZORIN_GID:-}" ] && validate_uid_gid "$ZORIN_GID" && [ "$ZORIN_GID" != "$(id -g zorin)" ]; then
    echo "[stage2] Changing zorin GID to $ZORIN_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ZORIN_GID" zorin 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised zorin user (UID 10000)
...
# zorin process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes zorin. Idempotent across container
...
#   - zorin is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G zorin 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] zorin already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" zorin 2>/dev/null; then
        echo "[stage2] Added zorin to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group zorin failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ZORIN_UID is remapped or the top-level $ZORIN_HOME isn't owned by
# the runtime zorin UID, restore ownership to zorin — but ONLY for the
# directories zorin actually writes to. The full $ZORIN_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of zorin-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_zorin_uid=$(id -u zorin)
...
chown_zorin_tree() {
    ...
    chown -R zorin:zorin "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$ZORIN_HOME" 2>/dev/null)" != "$actual_zorin_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ZORIN_HOME (targeted) to zorin ($actual_zorin_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ZORIN_HOME: chown the directory itself (not its contents)
    # so zorin can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ZORIN_HOME"; then
        :
    else
        chown zorin:zorin "$ZORIN_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ZORIN_HOME failed (rootless container?) — continuing"
    fi
    # Zorin-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by zorin (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ZORIN_HOME/$sub" ]; then
            chown_zorin_tree "$ZORIN_HOME/$sub"
        fi
    done
fi
...
# zorin user. Hosted/container instances keep mutable state under
# $ZORIN_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ZORIN_DISABLE_LAZY_INSTALLS=1. Keeping /opt/zorin root-owned and
...
# $ZORIN_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ZORIN_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to zorin
...
# Always reset ownership of $ZORIN_HOME/profiles to zorin on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> zorin …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as zorin and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$ZORIN_HOME/profiles" ]; then
    chown_zorin_tree "$ZORIN_HOME/profiles"
fi

# Always reset ownership of $ZORIN_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged zorin runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$ZORIN_HOME/cron" ]; then
    chown_zorin_tree "$ZORIN_HOME/cron"
fi
...
# zorin pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged zorin gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ZORIN_HOME is
...
# is negligible.
if [ -d "$ZORIN_HOME/platforms/pairing" ]; then
    chown_zorin_tree "$ZORIN_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ZORIN_HOME/pairing" ]; then
    chown_zorin_tree "$ZORIN_HOME/pairing"
fi

# Reset ownership of zorin-owned top-level state files on every boot.
# The targeted data-volume chown above only covers zorin-owned
# *subdirectories*; loose state files living directly under $ZORIN_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> zorin …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged zorin runtime then hits
...
# sweep so host-owned files in a bind-mounted $ZORIN_HOME are never
...
# entries of zorin_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ZORIN_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ZORIN_HOME/$f"; then
            :
        else
            chown zorin:zorin "$ZORIN_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the zorin runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ZORIN_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ZORIN_HOME/config.yaml"; then
        :
    else
        chown zorin:zorin "$ZORIN_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ZORIN_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as zorin user ---
# Run as zorin via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ZORIN_HOME values
...
as_zorin mkdir -p \
    ...
    "$ZORIN_HOME/sessions" \
    "$ZORIN_HOME/logs" \
    "$ZORIN_HOME/logs/gateways" \
    "$ZORIN_HOME/hooks" \
    "$ZORIN_HOME/memories" \
    "$ZORIN_HOME/skills" \
    "$ZORIN_HOME/skins" \
    "$ZORIN_HOME/plans" \
    "$ZORIN_HOME/workspace" \
    "$ZORIN_HOME/home" \
    "$ZORIN_HOME/pairing" \
    "$ZORIN_HOME/platforms/pairing" \
    "$ZORIN_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/zorin/.install_method (see Dockerfile), NOT written here into
# $ZORIN_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ZORIN_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.zorin:/opt/data) and sometimes shared with a
...
# 'zorin update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ZORIN_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/zorin stamp, so deleting the data-dir copy is safe).
if [ -f "$ZORIN_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ZORIN_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ZORIN_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ZORIN_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ZORIN_HOME/$dest"; then
            :
        else
            as_zorin cp "$INSTALL_DIR/$src" "$ZORIN_HOME/$dest"
        fi
    fi
}
...
if [ -f "$ZORIN_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ZORIN_HOME/.env"; then
        :
    else
        chown zorin:zorin "$ZORIN_HOME/.env" 2>/dev/null || true
        chmod 600 "$ZORIN_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $ZORIN_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `zorin update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ZORIN_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ZORIN_HOME/config.yaml" ]; then
    s6-setuidgid zorin "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ZORIN_HOME/auth.json" ] && [ -n "${ZORIN_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ZORIN_HOME/auth.json"; then
        :
    else
        printf '%s' "$ZORIN_AUTH_JSON_BOOTSTRAP" > "$ZORIN_HOME/auth.json"
        chown zorin:zorin "$ZORIN_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ZORIN_HOME/auth.json"
    fi
fi

# auth.json: re-seed a TERMINALLY-DEAD NpcAutomators bootstrap session (self-heal).
#
# The [ ! -f ] guard above deliberately refuses to clobber an existing
# auth.json, so a container whose NpcAutomators bootstrap session took a terminal
# invalid_grant (tokens cleared, providers.npcautomators.last_auth_error.relogin_required
...
# supply a freshly-issued session via ZORIN_AUTH_JSON_REBOOTSTRAP (distinct
# from the create-only *_BOOTSTRAP var); this helper swaps ONLY the
# providers.npcautomators entry when the on-disk entry is provably terminal OR the
...
if [ -f "$ZORIN_HOME/auth.json" ] && [ -n "${ZORIN_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ZORIN_HOME/auth.json"; then
        :
    else
        s6-setuidgid zorin "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_npcautomators_session.py" \
            "$ZORIN_HOME/auth.json" \
            || echo "[stage2] Warning: docker_rebootstrap_npcautomators_session.py failed; continuing"
    fi
fi
...
# can set ZORIN_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ZORIN_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ZORIN_HOME/gateway_state.json" ] && \
        [ "${ZORIN_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ZORIN_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ZORIN_HOME/gateway_state.json"
        chown zorin:zorin "$ZORIN_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ZORIN_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_zorin "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/zorin/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Zorin spawns for the
...
# main-wrapper.sh propagates it into the supervised ``zorin`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-zorin, dashboard, etc.).
