...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``zorin://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``zorin://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"zorin://blueprint/{quote(blueprint.key)}{qs}"
