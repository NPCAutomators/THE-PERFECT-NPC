...
``zorin gateway restart`` (or ``launchctl kickstart ai.zorin.gateway``
or ``systemctl restart zorin-gateway``).  When the cron fires, the
...
``zorin cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``zorin gateway``, ``launchctl ... zorin-gateway``,
``systemctl ... zorin-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_ZORIN_GATEWAY=1``, and
``zorin gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `zorin gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:zorin\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a zorin-gateway label. macOS launchd
    # labels look like `ai.zorin.gateway` / `zorin-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated zorin services (e.g.
    # `launchctl unload ai.zorin.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bzorin[.\-]?gateway)"
    # Branch C: systemctl ops on a zorin-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bzorin[.\-]?gateway)"
    # Branch D: pkill / kill targeting the zorin gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<ZORIN_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.zorin/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_zorin_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `zorin gateway restart` from a shell outside "
