# nix/desktop.nix — Zorin Desktop (Electron) app build + wrapper
#
# `zorinAgent` is the fully-built `.#default` package — it ships the
# `zorin` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ZORIN_DESKTOP_ZORIN` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Zorin CLI").
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "zorin-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "zorin-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "zorin-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/zorin-desktop $out/bin
    cp -r ${renderer}/* $out/share/zorin-desktop/
    ...
    substituteInPlace $out/share/zorin-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/zorin-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ZORIN_DESKTOP_ZORIN to the absolute path of the nix-built `zorin`
    # binary so the desktop's resolver step 4 ("existing Zorin CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/zorin-desktop \
      --add-flags "$out/share/zorin-desktop" \
      --set ZORIN_DESKTOP_ZORIN "${lib.getExe zorinAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for ZORIN";
    homepage = "https://github.com/NPCAutomators/AGENT-ZORIN";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "zorin-desktop";
  };
}
