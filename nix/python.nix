...
let
  ...
  # $ZORIN_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$ZORIN_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        zorin-agent = prev.zorin-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "zorin-agent-env" {
    zorin-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "zorin-agent-editable-env" {
    zorin-agent = dependency-groups;
  };
}
