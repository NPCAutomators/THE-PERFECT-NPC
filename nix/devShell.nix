...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      zorinNpmLib = self'.packages.default.passthru.zorinNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. zorin-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "zorin" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../zorin} $out/bin/zorin
          '')
        ...
        shellHook = ''
          ${combinedNonNpm}
          ${zorinNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export ZORIN_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          echo "ZORIN dev shell in $ZORIN_PYTHON_SRC_ROOT"
          echo "Ready. Run 'zorin' or 'sandbox zorin' to start."
        '';
      };
    };
}
