...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      zorin-agent = self'.packages.default;
      zorinVenv = zorin-agent.zorinVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "zorin-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${zorinVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "zorin-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "zorin-build-package" { } ''
          echo "PASS: package built at ${zorin-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "zorin-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "zorin-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${zorin-agent}/bin/zorin || (echo "FAIL: zorin binary missing"; exit 1)
          test -x ${zorin-agent}/bin/zorin-agent || (echo "FAIL: zorin-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${zorin-agent}/bin/zorin version 2>&1 | grep -qi "zorin" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "zorin-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in zorin zorin-agent zorin-acp; do
            test -x ${zorin-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "zorin-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking zorin --help ==="
          ${zorin-agent}/bin/zorin --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${zorin-agent}/bin/zorin --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "zorin-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${zorin-agent}/share/zorin-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${zorin-agent}/share/zorin-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ZORIN_BUNDLED_SKILLS" ${zorin-agent}/bin/zorin || \
            (echo "FAIL: ZORIN_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ZORIN_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${zorin-agent}/share/zorin-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${zorin-agent}/share/zorin-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ZORIN_OPTIONAL_SKILLS" ${zorin-agent}/bin/zorin || \
            (echo "FAIL: ZORIN_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ZORIN_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "zorin-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${zorin-agent}/share/zorin-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${zorin-agent}/share/zorin-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ZORIN_BUNDLED_PLUGINS" ${zorin-agent}/bin/zorin || \
            (echo "FAIL: ZORIN_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ZORIN_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "zorin-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${zorin-agent}/share/zorin-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${zorin-agent}/share/zorin-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${zorin-agent}/share/zorin-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ZORIN_BUNDLED_LOCALES" ${zorin-agent}/bin/zorin || \
            (echo "FAIL: ZORIN_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ZORIN_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (ZORIN_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ZORIN_BUNDLED_LOCALES=${zorin-agent}/share/zorin-agent/locales \
            ${zorinVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ZORIN_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${zorinVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${zorinVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ZORIN_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "zorin-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${zorin-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${zorin-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ZORIN_TUI_DIR" ${zorin-agent}/bin/zorin || \
            (echo "FAIL: ZORIN_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ZORIN_TUI_DIR set in wrapper"
        ...
        # Verify ZORIN_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        zorin-node = pkgs.runCommand "zorin-node-version" { } ''
          set -e
          echo "=== Checking ZORIN_NODE in wrapper ==="
          grep -q "ZORIN_NODE" ${zorin-agent}/bin/zorin || \
            (echo "FAIL: ZORIN_NODE not set in wrapper"; exit 1)
          echo "PASS: ZORIN_NODE present in wrapper"

          ZORIN_NODE=$(sed -n "s/^export ZORIN_NODE='\(.*\)'/\1/p" ${zorin-agent}/bin/zorin)
          test -x "$ZORIN_NODE" || (echo "FAIL: ZORIN_NODE=$ZORIN_NODE not executable"; exit 1)
          echo "PASS: ZORIN_NODE executable at $ZORIN_NODE"

          NODE_MAJOR=$("$ZORIN_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ZORIN_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ZORIN_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "zorin-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ZORIN_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ZORIN_MANAGED guards ==="
          check_blocked "config set" ${zorin-agent}/bin/zorin config set model foo
          check_blocked "config edit" ${zorin-agent}/bin/zorin config edit
        ...
        extra-python-packages = let
          ...
          zorinWithExtra = zorin-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "zorin-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${zorinWithExtra}/bin/zorin || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${zorinWithExtra}/bin/zorin || \
          ...
          if grep -q "PYTHONPATH" ${zorin-agent}/bin/zorin; then
        ...
        extra-dependency-groups = let
          zorinWithGroups = zorin-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "zorin-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${zorinWithGroups}"
          echo "venv: ${zorinWithGroups.zorinVenv}"
        ...
        messaging-variant = pkgs.runCommand "zorin-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.zorinVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "zorin-config-roundtrip" {
          ...
          merge_and_load() {
            local zorin_home="$1"
            export ZORIN_HOME="$zorin_home"
            ${configMergeScript} ${nixSettings} "$zorin_home/config.yaml"
            ${zorinVenv}/bin/python3 -c '
import json, sys
