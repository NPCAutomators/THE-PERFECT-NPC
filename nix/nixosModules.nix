# nix/nixosModules.nix — NixOS module for zorin-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: zorin runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ZORIN_HOME/.env
# and read by zorin at startup — no container recreation needed for env changes.
#
# Tool resolution: the zorin wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.zorin-agent;
    effectivePackage =
    ...
    zorin-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-zorin-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "zorin-config-attrs";
      description = "Zorin YAML config (attrset), merged deeply via lib.recursiveUpdate.";
      check = builtins.isAttrs;
      merge = _loc: defs: lib.foldl' lib.recursiveUpdate { } (map (d: d.value) defs);
    };

    # Generate config.yaml from Nix attrset (YAML is a superset of JSON).
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — zorin
    ...
    generatedConfigFile = pkgs.writeText "zorin-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # ZORIN_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "zorin-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ZORIN_DOC_EOF'\n${value}\nZORIN_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "zorin-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/zorin";
    ...
    # zorin user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "zorin-container-entrypoint" ''
      set -eu

      ZORIN_UID="''${ZORIN_UID:?ZORIN_UID must be set}"
      ZORIN_GID="''${ZORIN_GID:?ZORIN_GID must be set}"

      # ── Group: ensure a group with GID=$ZORIN_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ZORIN_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="zorin"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ZORIN_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ZORIN_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ZORIN_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ZORIN_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="zorin"
        TARGET_HOME="/home/zorin"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ZORIN_UID" -g "$ZORIN_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ZORIN_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ZORIN_UID:$ZORIN_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ZORIN_HOME is owned by the target user.
      ...
      if [ -n "''${ZORIN_HOME:-}" ] && [ -d "$ZORIN_HOME" ]; then
        find "$ZORIN_HOME" \! -user "$ZORIN_UID" -exec chown "$ZORIN_UID:$ZORIN_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/zorin-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/zorin-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/zorin ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/zorin
        chmod 0440 /etc/sudoers.d/zorin
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ZORIN_UID" --regid="$ZORIN_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ZORIN_HOME/.env.
    ...
    # Default: /var/lib/zorin/workspace → /data/workspace.
  ...
  in {
    options.services.zorin-agent = with lib; {
      enable = mkEnableOption "ZORIN gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = zorin-agent;
        description = "The zorin-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/zorin";
        description = "State directory. Contains .zorin/ subdir (ZORIN_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Zorin config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ZORIN_HOME/.env at activation time.
          Zorin reads this file on every startup via load_zorin_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ZORIN_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ZORIN_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `zorin gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the zorin user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the zorin plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Zorin discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          zorin_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as zorin (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in zorin-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the zorin CLI to environment.systemPackages and export
          ZORIN_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.zorin symlink to the service
            stateDir. These users are automatically added to the zorin group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.zorin-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the zorin CLI to system PATH and export ZORIN_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.zorin/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ZORIN_HOME = "${cfg.stateDir}/.zorin";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.zorin-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the zorin user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.zorin-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed zorin
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure zorin is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.zorin        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zorin/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zorin/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zorin/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zorin/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zorin/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."zorin-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.zorin
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.zorin ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.zorin ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.zorin -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.zorin/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.zorin/$_subdir"
            chmod 2770 "${cfg.stateDir}/.zorin/$_subdir"
            find "${cfg.stateDir}/.zorin/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # zorin-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.zorin/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.zorin/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zorin/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.zorin/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.zorin/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zorin/.managed
          chmod 0644 ${cfg.stateDir}/.zorin/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.zorin/.container-mode <<'ZORIN_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    zorin_bin=${containerDataDir}/current-package/bin/zorin
    ZORIN_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zorin/.container-mode
            chmod 0644 ${cfg.stateDir}/.zorin/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.zorin/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.zorin";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.zorin" ]; then
                  rm -f "${symlinkPath}"
                  echo "zorin-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.zorin -> stateDir/.zorin for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.zorin";
                target = "${cfg.stateDir}/.zorin";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "zorin-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.zorin/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.zorin/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.zorin/auth.json
              fi
            ''}
          ''}
          ...
          # Zorin reads $ZORIN_HOME/.env at startup via load_zorin_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.zorin/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ZORIN_NIX_ENV_EOF'
    ${envFileContent}
    ZORIN_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.zorin/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.zorin/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zorin/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.zorin-agent = {
          ...
          serviceConfig = {
            ...
            # $ZORIN_HOME/.env by the activation script. load_zorin_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/zorin"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the zorin group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.zorin-agent = {
          description = "ZORIN Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ZORIN_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ZORIN_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ZORIN_UID="$ZORIN_UID" \
                --env ZORIN_GID="$ZORIN_GID" \
                --env ZORIN_HOME=${containerDataDir}/.zorin \
                --env ZORIN_MANAGED=true \
                --env HOME=${containerHomeDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/zorin gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
