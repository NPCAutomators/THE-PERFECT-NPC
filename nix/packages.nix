# nix/packages.nix — ZORIN package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      minimal = pkgs.callPackage ./zorin-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.zorinTui;
        web = full.zorinWeb;
        desktop = full.zorinDesktop;

        update-npm-lockfile = full.zorinNpmLib.updateNpmLockfile;
      };
    };
}
