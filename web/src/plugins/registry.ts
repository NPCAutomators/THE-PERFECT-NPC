 ...
 *
 * Plugins call window.__ZORIN_PLUGINS__.register(name, Component)
 ...
 * Exposed at runtime as ``window.__ZORIN_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__ZORIN_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__ZORIN_PLUGIN_SDK__ = {
    ...
    // window.__ZORIN_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — NpcAutomators DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
