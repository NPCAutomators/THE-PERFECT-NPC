/**
 * Zorin Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__ZORIN_PLUGIN_SDK__`` and
 * ``window.__ZORIN_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__ZORIN_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@npcautomators/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``ZorinPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@zorin/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @npcautomators/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__ZORIN_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__ZORIN_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__ZORIN_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface ZorinPluginSDK {
  ...
  /**
   * Shared UI primitives (NpcAutomators DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __ZORIN_PLUGIN_SDK__?: ZorinPluginSDK;
    __ZORIN_PLUGINS__?: PluginRegistry;
  }
}
