...
interface ZorinConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function ZorinConsoleModal({ open, onClose }: ZorinConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("zorin> ");
  const inputPromptRef = useRef("zorin> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "zorin> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Zorin Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="zorin-console-title"
    >
      <div
        className={cn(
          themedBody,
