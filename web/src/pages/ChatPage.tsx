/**
 * ChatPage — embeds `zorin --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (zorin_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "zorin.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ZORIN_SESSION_TOKEN__ &&
    !window.__ZORIN_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `zorin dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__ZORIN_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Zorin TUI has enabled xterm mouse-events
    ...
    //      payloads upload to ZORIN_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to ZORIN_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[zorin-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__ZORIN_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets ZORIN_HOME pointed at the
    ...
    // forwarding them into Zorin. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="zorin-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
