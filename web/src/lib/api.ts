import { buildZorinWebSocketUrl } from "@zorin/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/zorin/). The Python backend
// injects ``window.__ZORIN_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ZORIN_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ZORIN_BASE_PATH = readBasePath();
const BASE = ZORIN_BASE_PATH;
...
declare global {
  interface Window {
    __ZORIN_SESSION_TOKEN__?: string;
    __ZORIN_BASE_PATH__?: string;
    ...
    __ZORIN_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Zorin-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__ZORIN_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``zorin update``, ``zorin gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ZORIN_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ZORIN_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("zorin.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("zorin.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__ZORIN_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("zorin.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
 ...
 *
 * The dashboard's gated-mode WS auth (``zorin_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ZORIN_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Zorin-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``zorin_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ZORIN_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateZorin: () =>
    fetchJSON<ActionResponse>("/api/zorin/update", { method: "POST" }),
  checkZorinUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/zorin/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * NPCAUTOMATORS Portal contract V1 (the access token has no email/name claims —
...
export interface SkillHubSource {
  ...
  /** zorin-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
  ...
  /** Phase 7: registered ``DashboardAuthProvider`` names (e.g. ``["npcautomators"]``).
   ...
   * updates are handled by the outer launcher instead of ``zorin update``. */
  can_update_zorin?: boolean;
  config_path: string;
...
export interface ToolsetProvider {
  ...
  requires_npcautomators_auth: boolean;
  is_active: boolean;
}
