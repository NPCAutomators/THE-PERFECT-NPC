...
const SIDEBAR_COLLAPSED_KEY = "zorin-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="zorin-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="zorin-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateZorin = status?.can_update_zorin === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'zorin update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "zorin update";
    return (
      t.status.updateZorinConfirmMessage ??
      `This will run 'zorin update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateZorinConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateZorin) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateZorin,
      runningLabel: t.status.updatingZorin,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Zorin gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateZorinConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateZorinConfirmTitle ?? `${t.status.updateZorin}?`}
    />
    </>
  );
}
