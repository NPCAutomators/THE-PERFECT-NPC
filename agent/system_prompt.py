...
upstream prefix cache warm.  See ``zorin-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, npcautomators subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_npcautomators_subscription_prompt``,
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ZORIN_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ZORIN_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Zorin never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ZORIN_HOME persona while keeping
    ...
    # Pointer to the zorin-agent skill + docs for user questions about Zorin itself.
    stable_parts.append(ZORIN_AGENT_HELP_GUIDANCE)
    ...
    # ZORIN_KANBAN_TASK env var). Normal chat sessions never see
    ...
    npcautomators_subscription_prompt = _r.build_npcautomators_subscription_prompt(agent.valid_tool_names)
    if npcautomators_subscription_prompt:
        stable_parts.append(npcautomators_subscription_prompt)
    ...
    # Coding posture (base Zorin, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Zorin profile the agent is running
    # under so it doesn't conflate ~/.zorin/skills/ (default profile) with
    # ~/.zorin/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Zorin profile: default. Other profiles (if any) live "
            "under " + str(get_zorin_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Zorin profile: {active_profile}. This session reads "
            f"and writes {get_zorin_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_zorin_home()}/skills/, {get_zorin_home()}/plugins/, "
            f"{get_zorin_home()}/cron/, {get_zorin_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Zorin). Every other surface (desktop chat panel,
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Zorin never rebuilds or reinjects parts of it
