...
# un-throttled log fires on *every* selection. On Windows several Zorin
...
# readiness handshake ("Timed out connecting to Zorin backend after
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "npcautomators":
            # NpcAutomators stores the runtime inference credential in agent_key for
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._npcautomators_invoke_jwt_is_usable(
                        token,
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): npcautomators, openai-codex, and xai-oauth rotate the
    ...
    ``zorin_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``zorin model`` / ``zorin auth`` which
        ...
        Mirrors the NpcAutomators/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Zorin process
    ...
    def _sync_npcautomators_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a NpcAutomators pool entry from auth.json if tokens differ.

        NpcAutomators OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_npcautomators_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the NPCAUTOMATORS Portal.
        """
        if self.provider != "npcautomators" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
                state = _load_provider_state(auth_store, "npcautomators")
            if not state:
                return entry
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing NpcAutomators state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync NpcAutomators entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently NpcAutomators, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every NpcAutomators/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``zorin`` invocation that defaults to the active provider
        (e.g. setup wizard, ``zorin auth status``) would land on
        ...
        # device-code sources (npcautomators, openai-codex, xAI) use ``device_code``.
        ...
        try:
            with _auth_store_lock():
                ...
                # zorin_cli.auth._save_xai_oauth_tokens (#43589); the pool
                ...
                if self.provider == "npcautomators":
                    ...
                    _store_provider_state(auth_store, "npcautomators", state, set_active=False)
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Zorin
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``ZORIN_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "ZORIN_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "ZORIN_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("zorin_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Zorin
            ...
            elif self.provider == "npcautomators":
                synced = self._sync_npcautomators_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_npcautomators_runtime_credentials(
                    force_refresh=force,
                )
                updated = self._sync_npcautomators_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("zorin_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as npcautomators — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the NpcAutomators quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/npcautomators — another Zorin process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and NpcAutomators quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    self._persist(removed_ids=removed_ids)
                    return None
            # For npcautomators: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "npcautomators":
                synced = self._sync_npcautomators_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("NpcAutomators refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                    ...
                    return updated
                if auth_mod._is_terminal_npcautomators_refresh_error(exc):
                    logger.debug("NpcAutomators refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "npcautomators") or {
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_npcautomators_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_npcautomators_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                        ...
                        logger.debug("Failed to clear terminal NpcAutomators OAuth state: %s", clear_exc)

                    singleton_sources = {
                        auth_mod.NPCAUTOMATORS_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.NPCAUTOMATORS_DEVICE_CODE_SOURCE}",
                    }
                    removed_ids = [
                        item.id for item in self._entries
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "npcautomators":
            # NpcAutomators refresh can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Zorin profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For npcautomators entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_npcautomators_runtime_credentials(), making this entry's
            ...
            # re-authed via `zorin model` / `zorin auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `zorin model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``zorin auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `zorin auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `zorin auth remove <provider> <N>` is stable across all source types.
    try:
        from zorin_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Zorin PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `zorin setup` ("Claude
        ...
        # Claude Code CLI, zorin_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.zorin/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `zorin setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"zorin_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_zorin_oauth_credentials

        for source_name, creds in (
            ("zorin_pkce", read_zorin_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_npcautomators_credentials(label=...) when the user ran
            # `zorin auth add npcautomators --label <name>`).  Fall back to the
        ...
        # Zorin auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from zorin_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.zorin/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `zorin auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `zorin auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Zorin auth store.  Without this gate the removal is instantly
        ...
        # Zorin owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``zorin model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``zorin auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with npcautomators/codex).
            source = "device_code"
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.zorin/.env over os.environ — the user's config file is the
    # authoritative source for Zorin credentials. Stale env vars from parent
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_zorin_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `zorin auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.zorin/.env.
    ...
    try:
        from zorin_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from zorin_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.zorin/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `zorin auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Zorin
