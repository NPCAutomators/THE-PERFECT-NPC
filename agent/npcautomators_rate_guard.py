"""Cross-session rate limit guard for NPCAUTOMATORS Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether NPCAUTOMATORS Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from NpcAutomators triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Zorin retries), and every one of those calls counts
...
_STATE_FILENAME = "npcautomators.json"


def _state_path() -> str:
    """Return the path to the NpcAutomators rate limit state file."""
    try:
        from zorin_constants import get_zorin_home
        base = get_zorin_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".zorin")
...
def record_npcautomators_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that NPCAUTOMATORS Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "NpcAutomators rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write NpcAutomators rate limit state: %s", exc)


def npcautomators_rate_limit_remaining() -> Optional[float]:
    """Check if NPCAUTOMATORS Portal is currently rate-limited.
...
def clear_npcautomators_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful NpcAutomators request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear NpcAutomators rate limit state: %s", exc)
...
def is_genuine_npcautomators_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from NPCAUTOMATORS Portal is a real account rate limit.

    NPCAUTOMATORS Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Zorin, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on NpcAutomators is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on NpcAutomators.

    Tripping the cross-session breaker on (b) blocks ALL NpcAutomators requests
    (and all models, since NpcAutomators is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  NpcAutomators emits
