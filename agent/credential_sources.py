"""Unified removal contract for every credential source Zorin reads from.

Zorin seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.zorin/.env
    claude_code   — ~/.claude/.credentials.json
    zorin_pkce   — ~/.zorin/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (npcautomators, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `zorin auth add`
...
unify here is **removal**:
    ...
    ``zorin auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, npcautomators device_code (partial), zorin_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"npcautomators"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.zorin/.env  → clear it
    ...
    """
    from zorin_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Zorin.",
            f"  The pool entry is now suppressed — Zorin will ignore "
            f"{env_var} until you run `zorin auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Zorin stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `zorin auth add anthropic` to re-enable if needed.",
    ])


def _remove_zorin_pkce(provider: str, removed) -> RemovalResult:
    """~/.zorin/.anthropic_oauth.json is ours — delete it outright."""
    from zorin_constants import get_zorin_home

    result = RemovalResult()
    oauth_file = get_zorin_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Zorin Anthropic OAuth credentials")
...
def _remove_npcautomators_device_code(provider: str, removed) -> RemovalResult:
    """NpcAutomators OAuth lives in auth.json providers.npcautomators — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `zorin auth add npcautomators` (or any other path that writes providers.npcautomators)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `zorin auth add npcautomators` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as NpcAutomators: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``zorin auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `zorin model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Zorin auth store is not enough — _seed_from_singletons() would
    ...
    via ``zorin auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `zorin auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `zorin auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Zorin stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `zorin auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="zorin_pkce",
        remove_fn=_remove_zorin_pkce,
        description="~/.zorin/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="npcautomators", source_id="device_code",
        remove_fn=_remove_npcautomators_device_code,
        description="auth.json providers.npcautomators",
    ))
    register(RemovalStep(
