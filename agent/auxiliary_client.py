...
Resolution order for text tasks (auto mode):
  ...
  3. NPCAUTOMATORS Portal (~/.zorin/auth.json active provider)
...
from zorin_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``ZORIN_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `zorin update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Zorin owns auxiliary retry + provider/model fallback policy (the
    ...
    # Zorin sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Zorin control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Zorin triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/NPCAutomators/AGENT-ZORIN",
    "X-Title": "ZORIN",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from zorin_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ZORIN_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ZORIN_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("ZORIN_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("ZORIN_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "ZorinAgent",
}
...
# NPCAUTOMATORS Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by NPCAUTOMATORS Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with zorin_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from agent.portal_tags import npcautomators_portal_tags as _npcautomators_portal_tags


def _npcautomators_extra_body() -> dict:
    """Return a fresh NPCAUTOMATORS Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``zorin_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _npcautomators_portal_tags()}


# Backwards-compatible module attribute. Some callers (tests, third-party
# plugins) read ``NPCAUTOMATORS_EXTRA_BODY`` directly; keep it as a snapshot of the
# current tags. Callers that need the freshest value should call
# ``_npcautomators_extra_body()`` or import ``npcautomators_portal_tags`` directly.
NPCAUTOMATORS_EXTRA_BODY = _npcautomators_extra_body()

# Set at resolve time — True if the auxiliary client points to NPCAUTOMATORS Portal
auxiliary_is_npcautomators: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_NPCAUTOMATORS_MODEL = "google/gemini-3-flash-preview"
_NPCAUTOMATORS_DEFAULT_BASE_URL = "https://inference-api.npcautomators.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_zorin_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (ZORIN)",
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for npcautomators).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "npcautomators":
        # Funnel through the canonical auth-layer reader so the env override
        # shares one normalization path with the rest of the NPCAUTOMATORS resolution.
        from zorin_cli.auth import _npcautomators_inference_env_override

        env_url = _npcautomators_inference_env_override()
        if env_url:
            return env_url
    # runtime_base_url handles provider-specific logic (e.g. npcautomators prefers inference_base_url).
...
def _npcautomators_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("ZORIN_NPCAUTOMATORS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Zorin plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``zorin_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_npcautomators_auth() -> Optional[dict]:
    """Read and validate ~/.zorin/auth.json for an active NPCAUTOMATORS provider.

    Returns the provider state dict if NpcAutomators is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("npcautomators")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _NPCAUTOMATORS_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "npcautomators":
            return None
        provider = data.get("providers", {}).get("npcautomators", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read NpcAutomators auth: %s", exc)
        return None


def _npcautomators_api_key(provider: dict) -> str:
    """Extract a usable NpcAutomators inference JWT from stored auth state."""
    from zorin_cli.auth import _npcautomators_invoke_jwt_is_usable
    ...
    for token_key, expiry_key in (
        ...
        if not isinstance(token, str) or not token.strip():
            continue
        if _npcautomators_invoke_jwt_is_usable(
            token,
...
def _npcautomators_base_url() -> str:
    """Resolve the NpcAutomators inference base URL from env or default."""
    return os.getenv("NPCAUTOMATORS_INFERENCE_BASE_URL", _NPCAUTOMATORS_DEFAULT_BASE_URL)


def _resolve_npcautomators_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve NpcAutomators auxiliary credentials from the selected pool entry."""
    try:
        from zorin_cli.auth import _agent_key_is_usable

        pool = load_pool("npcautomators")
    except Exception as exc:
        logger.debug("Auxiliary NpcAutomators pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary NpcAutomators pool selection failed: %s", exc)
        return None
    ...
    if force_refresh or not _agent_key_is_usable(state, _npcautomators_min_key_ttl_seconds()):
        try:
            refreshed = pool.try_refresh_current()
        except Exception as exc:
            logger.debug("Auxiliary NpcAutomators pool refresh failed: %s", exc)
            refreshed = None
    ...
    api_key = _npcautomators_api_key(provider)
    base_url = _pool_runtime_base_url(entry, _NPCAUTOMATORS_DEFAULT_BASE_URL)
    if not api_key or not base_url:
        return None
    return api_key, base_url


def _resolve_npcautomators_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh NpcAutomators runtime credentials when available.
    ...
    """
    pooled = _resolve_npcautomators_pool_runtime_api(force_refresh=force_refresh)
    if pooled is not None:
        return pooled

    try:
        from zorin_cli.auth import resolve_npcautomators_runtime_credentials

        creds = resolve_npcautomators_runtime_credentials(
            timeout_seconds=env_float("ZORIN_NPCAUTOMATORS_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary NpcAutomators runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``zorin auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``zorin_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from zorin_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Zorin auth store.
    ...
    try:
        from zorin_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from zorin_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from zorin_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from zorin_cli.models import copilot_default_headers
...
def _try_npcautomators(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting NpcAutomators —
    # if another session already recorded a 429, skip NpcAutomators entirely
    # to avoid piling more requests onto the tapped RPH bucket.
    try:
        from agent.npcautomators_rate_guard import npcautomators_rate_limit_remaining
        _remaining = npcautomators_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping NPCAUTOMATORS Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            _mark_provider_unhealthy("npcautomators", ttl=_remaining)
            return None, None
    except Exception:
        pass

    npcautomators = _read_npcautomators_auth()
    runtime = _resolve_npcautomators_runtime_api(force_refresh=False)
    if runtime is None and not npcautomators:
        logger.warning(
            "Auxiliary NpcAutomators client unavailable: no NpcAutomators authentication found "
            "(run: zorin auth)."
        )
        _mark_provider_unhealthy("npcautomators", ttl=60)
        return None, None
    if runtime is None and npcautomators:
        logger.debug(
            "Auxiliary NpcAutomators: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_npcautomators
    auxiliary_is_npcautomators = True
    logger.debug("Auxiliary client: NPCAUTOMATORS Portal")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/npcautomators/recommended-models endpoint is the authoritative source:
    # it distinguishes paid vs free tier recommendations, and get_npcautomators_recommended_aux_model
    # auto-detects the caller's tier via check_npcautomators_free_tier().  Fall back to
    # _NPCAUTOMATORS_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _NPCAUTOMATORS_MODEL
    try:
        from zorin_cli.models import get_npcautomators_recommended_aux_model
        recommended = get_npcautomators_recommended_aux_model(vision=vision)
        if recommended:
            model = recommended
            logger.debug(
    ...
    if runtime is not None:
        ...
        api_key = _npcautomators_api_key(npcautomators or {})
        if not api_key:
            logger.warning(
                "Auxiliary NpcAutomators client unavailable: no usable inference JWT found "
                "(run: zorin auth add npcautomators)."
            )
            _mark_provider_unhealthy("npcautomators", ttl=60)
            return None, None
        base_url = str((npcautomators or {}).get("inference_base_url") or _npcautomators_base_url()).rstrip("/")
    return (
...
def _refresh_npcautomators_recommended_model(
    *, vision: bool, stale_model: Optional[str]
) -> Optional[str]:
    """Re-fetch the NPCAUTOMATORS Portal's recommended model after a stale-model 404.
    ...
    the NpcAutomators → OpenRouter catalog, every auxiliary call 404s with
    ...
    model name to retry with:
      ...
      * ``_NPCAUTOMATORS_MODEL`` (google/gemini-3-flash-preview), the known-good default,
    ...
    try:
        ...
        fresh = get_npcautomators_recommended_aux_model(vision=vision, force_refresh=True)
    except Exception as exc:
        logger.debug(
            "NpcAutomators recommended-model refresh failed (%s); using default %s",
            exc, _NPCAUTOMATORS_MODEL,
        )
    ...
    if _NPCAUTOMATORS_MODEL.strip().lower() != stale:
        return _NPCAUTOMATORS_MODEL
    return None
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `zorin setup` or `zorin model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    Mirrors the ``_try_anthropic`` / ``_try_npcautomators`` shape but delegates to
    :func:`zorin_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from zorin_cli.runtime_provider import _resolve_azure_foundry_runtime
...
_AUTO_PROVIDER_LABELS = {
    ...
    "_try_npcautomators": "npcautomators",
...
def _get_provider_chain() -> List[tuple]:
    ...
    return [
        ...
        ("npcautomators", _try_npcautomators),
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second zorin
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``zorin config aux reset``)."""
...
def _npcautomators_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh NPCAUTOMATORS account API says paid access is allowed."""
    try:
        from zorin_cli.npcautomators_account import get_npcautomators_portal_account_info

        account_info = get_npcautomators_portal_account_info(force_fresh=True)
        return account_info.paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary NpcAutomators paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the NpcAutomators → OpenRouter catalog. The NpcAutomators
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.npcautomators.com"):
        return "npcautomators"
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "openai-codex":
            from zorin_cli.auth import resolve_codex_runtime_credentials
...
def _auth_refresh_provider_for_route(
    ...
    Copilot/Codex/Anthropic/NpcAutomators routes too. (#20832)
    """
    normalized = _normalize_aux_provider(resolved_provider)
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference-api.npcautomators.com"):
        return "npcautomators"
    return normalized
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "npcautomators": "npcautomators",
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Zorin' built-in discovery
...
def _resolve_auto(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/NpcAutomators get their chosen chat model; users
      ...
      2. OpenRouter → NpcAutomators → custom → Codex → API-key providers (fallback
         chain, only used when the main provider has no working client).
    """
    global auxiliary_is_npcautomators, _stale_base_url_warned
    auxiliary_is_npcautomators = False  # Reset — _try_npcautomators() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
    ...
    #    scenario where a user switches providers via `zorin model` but the
    #    old OPENAI_BASE_URL lingers in ~/.zorin/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: zorin model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.zorin/.env",
    ...
    # on aggregators (OpenRouter, NpcAutomators) who previously got routed to a
    ...
    if main_provider == "moa":
        try:
            from zorin_cli.config import load_config
            from zorin_cli.moa_config import resolve_moa_preset
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from zorin_cli.runtime_provider import _get_named_custom_provider
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from zorin_cli.copilot_auth import copilot_request_headers
    ...
    # See _create_openai_client: disable SDK-internal retries so Zorin owns
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "npcautomators", "openai-codex" (or "codex"),
    ...
    # ── NPCAUTOMATORS Portal (OAuth) ──────────────────────────────────────────
    if provider == "npcautomators":
        ...
        client, default = _try_npcautomators(vision=_is_vision)
        if client is None:
            logger.warning("resolve_provider_client: npcautomators requested "
                           "but NPCAUTOMATORS Portal not configured (run: zorin auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: zorin model)")
                return None, None
    ...
    # OpenRouter / NpcAutomators bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: zorin model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        ...
        # entries that coincidentally match a canonical provider (e.g. ``npcautomators``)
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: zorin doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("zorin_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from zorin_cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "npcautomators":
            return resolve_provider_client("npcautomators", model, async_mode)
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(
    ...
    if provider == "npcautomators":
        return _try_npcautomators(vision=True)
    if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → NpcAutomators → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. NpcAutomators — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      :func:`_resolve_provider_vision_default`). NpcAutomators is the
        ...
        #   3. NPCAUTOMATORS Portal (vision-capable aggregator fallback)
        ...
        if main_provider and main_provider not in {"auto", ""}:
            ...
            if main_provider == "npcautomators":
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes NPCAUTOMATORS Portal product tags when the auxiliary client is backed
    by NPCAUTOMATORS Portal. Returns empty dict otherwise.
    """
    return _npcautomators_extra_body() if auxiliary_is_npcautomators else {}
...
def auxiliary_max_tokens_param(value: int, *, model: Optional[str] = None) -> dict:
    ...
    if (not or_key
            and _read_npcautomators_auth() is None
            and (
...
def _refresh_npcautomators_auxiliary_client(
    *,
    cache_provider: str,
    model: Optional[str],
    async_mode: bool,
    ...
    """Refresh NpcAutomators runtime creds, rebuild the client, and replace the cache entry."""
    runtime = _resolve_npcautomators_runtime_api(force_refresh=True)
    if runtime is None:
        return None, model
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`zorin_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    # body fields (Gemini ``thinking_config``), and OpenRouter/NpcAutomators use
    ...
    if provider == "npcautomators" and "tags" not in merged_extra:
        merged_extra["tags"] = _npcautomators_portal_tags()
    if merged_extra:
        kwargs["extra_body"] = merged_extra

    # Native Anthropic Messages adapters do not consume ``extra_body``. Carry
    # the normalized Zorin reasoning config through a private kwarg so the
...
def call_llm(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Zorin reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `zorin model`."
                    )
    ...
    try:
        ...
        # ── Stale-model self-heal (NPCAUTOMATORS Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the NpcAutomators → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to NpcAutomators-routed calls.
        _heal_is_npcautomators = (
            resolved_provider == "npcautomators"
            or base_url_host_matches(_base_info, "inference-api.npcautomators.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_npcautomators:
            healed_model = _refresh_npcautomators_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in NpcAutomators catalog; "
        ...
        # ── NpcAutomators auth refresh parity with main agent ──────────────────
        client_is_npcautomators = (
            resolved_provider == "npcautomators"
            or base_url_host_matches(_base_info, "inference-api.npcautomators.com")
        )
        if (
            _is_payment_error(first_err)
            and client_is_npcautomators
            and _npcautomators_portal_account_has_fresh_paid_access()
        ):
            refreshed_client, refreshed_model = _refresh_npcautomators_auxiliary_client(
                cache_provider=resolved_provider or "npcautomators",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed NpcAutomators runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_npcautomators:
            refreshed_client, refreshed_model = _refresh_npcautomators_auxiliary_client(
                cache_provider=resolved_provider or "npcautomators",
                model=final_model,
                async_mode=False,
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed NpcAutomators runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        if (_is_auth_error(first_err)
                ...
                and not client_is_npcautomators):
        ...
        # When the resolved provider returns 401 and neither the NpcAutomators
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        # the NpcAutomators → OpenRouter catalog, 404'ing every auxiliary call. Force a
        # fresh Portal fetch and retry once with the current recommendation.
        _heal_is_npcautomators = (
            resolved_provider == "npcautomators"
            or base_url_host_matches(_client_base, "inference-api.npcautomators.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_npcautomators:
            healed_model = _refresh_npcautomators_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in NpcAutomators catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed NpcAutomators runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_npcautomators:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed NpcAutomators runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
