"""Centralized NPCAUTOMATORS Portal request tags.

Every Zorin request that hits the NPCAUTOMATORS Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
NpcAutomators can attribute usage to ZORIN and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=zorin-agent",
        "client=zorin-client-v<__version__>",
    ]

The version is sourced live from ``zorin_cli.__version__`` so it auto-aligns
...
``zorin_cli.__version__`` is the canonical source of truth.
"""
...
# active conversation id here and ``npcautomators_portal_tags()`` picks it up as a
...
_conversation_id: ContextVar[Optional[str]] = ContextVar(
    "npcautomators_portal_conversation_id", default=None
)
...
def _zorin_version() -> str:
    """Return the current Zorin release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``zorin_cli`` cannot be imported (should
...
def zorin_client_tag() -> str:
    """Return the ``client=...`` tag for NPCAUTOMATORS Portal requests.

    Format: ``client=zorin-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=zorin-client-v{_zorin_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Zorin session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Zorin conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.zorin/sessions/`` storage, session logs, and lineage.
...
def npcautomators_portal_tags(session_id: str | None = None) -> List[str]:
    """Return the canonical list of NPCAUTOMATORS Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(npcautomators_portal_tags())``).

    When ``session_id`` is provided, a ``conversation=<session_id>`` tag is
    appended so Portal usage can be attributed to a specific Zorin
    ...
    """
    tags = ["product=zorin-agent", zorin_client_tag()]
