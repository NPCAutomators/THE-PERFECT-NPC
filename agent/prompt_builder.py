...
from zorin_constants import get_zorin_home, get_skills_dir, is_wsl
...
_ZORIN_MD_NAMES = (".zorin.md", "ZORIN.md")


def _find_zorin_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.zorin.md`` or ``ZORIN.md``.
    ...
    # could pick up a .zorin.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _ZORIN_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are ZORIN, an intelligent AI assistant created by NPCAUTOMATORS. "
...
ZORIN_AGENT_HELP_GUIDANCE = (
    "You run on ZORIN (by NPCAUTOMATORS). When the user needs help with "
    "Zorin itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `zorin-agent` skill with skill_view(name='zorin-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.zorin/kanban.db`. Your task id is in "
    "`$ZORIN_KANBAN_TASK`; your workspace is `$ZORIN_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ZORIN_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ZORIN_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ZORIN_KANBAN_BRANCH:-wt/$ZORIN_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ZORIN_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`zorin profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `zorin kanban <verb>` for board operations. Use "
...
#
# The zorin-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to zorin-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `zorin computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Zorin "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Zorin terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Zorin desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Zorin WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Zorin itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Zorin runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Zorin itself is running. The host OS, home, and cwd "
                f"of the Zorin process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Zorin "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Zorin
    ...
    extra = (os.getenv("ZORIN_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_zorin_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("ZORIN_PLATFORM") or os.environ.get("ZORIN_SESSION_PLATFORM")
    if platform:
        return platform
    ...
    if get_session_env is None:
        return ""
    try:
        return get_session_env("ZORIN_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.zorin/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot ZORIN itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `zorin-agent` skill "
            "first. It has the actual commands (e.g. `zorin config set …`, `zorin tools`, "
            "`zorin setup`) so you don't have to guess or invent workarounds.\n"
...
def build_npcautomators_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact NPCAUTOMATORS subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import NPCAUTOMATORS subscription helper: %s", exc)
        return ""

    if not managed_npcautomators_tools_enabled():
        return ""
    ...
    features = get_npcautomators_subscription_features()

    def _status_line(feature) -> str:
        if feature.managed_by_npcautomators:
            return f"- {feature.label}: active via NPCAUTOMATORS subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.npcautomators_auth_present:
            return f"- {feature.label}: included with NPCAUTOMATORS subscription, not currently selected"
        if feature.key == "modal" and features.npcautomators_auth_present:
            return f"- {feature.label}: optional via NPCAUTOMATORS subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# NpcAutomators Subscription",
        "NPCAUTOMATORS subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, OpenAI Whisper STT, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a NpcAutomators-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, OpenAI Whisper, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that NPCAUTOMATORS subscription would unlock or simplify, suggest NPCAUTOMATORS subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: zorin setup, zorin setup tools, zorin setup terminal, zorin status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ZORIN_HOME and return its content, or None.
    ...
    """
    try:
        from zorin_cli.config import ensure_zorin_home
        ensure_zorin_home()
    except Exception as e:
        logger.debug("Could not ensure ZORIN_HOME before loading SOUL.md: %s", e)

    soul_path = get_zorin_home() / "SOUL.md"
...
def _load_zorin_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.zorin.md / ZORIN.md — walk to git root."""
    zorin_md_path = _find_zorin_md(cwd_path)
    if not zorin_md_path:
        return ""
    try:
        content = zorin_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = zorin_md_path.name
        try:
            rel = str(zorin_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".zorin.md", context_length=context_length,
            read_path=str(zorin_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", zorin_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .zorin.md / ZORIN.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ZORIN_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Zorin install/source
    ...
    # explicitly configured cwd is honored verbatim — the Zorin tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Zorin in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Zorin install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_zorin_md(cwd_path, context_length)
    ...
    # SOUL.md from ZORIN_HOME only — skip when already loaded as identity
    if not skip_soul:
