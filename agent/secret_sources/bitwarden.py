...
Zorin pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.zorin/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<zorin_home>/bin/bws`` on
  first use.  Zorin pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.zorin/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``zorin`` invocations don't
  hammer the API.
* Failures NEVER block Zorin startup.  Missing binary, no network,
...
# In-process cache so repeated load_zorin_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `zorin chat -q ...`
...
# <zorin_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.zorin/.env (which
...
_ENCRYPTED_CACHE_INFO = b"zorin-bws-encrypted-cache-v1"
...
def _zorin_bin_dir() -> Path:
    """Where Zorin stores its managed binaries.  Profile-aware."""
    from zorin_constants import get_zorin_home

    return get_zorin_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<zorin_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _zorin_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``zorin secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _zorin_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="zorin-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "zorin-agent"})
    try:
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a Zorin
...
# ---------------------------------------------------------------------------
# Public entry point — called from zorin_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_zorin_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `zorin secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `zorin secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `zorin secrets bitwarden setup` to install."
        )
        return result
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            return (
                "Run `zorin secrets bitwarden token` to paste a fresh access "
                "token (create one in the Bitwarden web app: Secrets Manager → "
                "Machine accounts → Access tokens).  Wrong region?  Re-run "
                "`zorin secrets bitwarden setup` and pick EU/self-hosted."
            )
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`zorin secrets bitwarden token`) so the
