...
cache problem must never block Zorin startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Zorin home used for cache paths.

    ``home_path`` is whatever ``load_zorin_dotenv()`` already resolved;
    falling back to ``$ZORIN_HOME`` / ``~/.zorin`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("ZORIN_HOME", Path.home() / ".zorin"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<zorin_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ZORIN_HOME is ever made traversable.
            try:
