...
Owns one Codex thread per Zorin session. Drives `turn/start`, consumes
...
# Zorin' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ZORIN_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Zorin/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Zorin session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ZORIN_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ZORIN_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="zorin",
            client_title="ZORIN",
            client_version=_get_zorin_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Zorin' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Zorin' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own zorin-tools callback we
            # auto-accept — the user already approved Zorin' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Zorin approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Zorin returns 'once', 'session', 'always', or 'deny'.
...
def _get_zorin_version() -> str:
    """Best-effort Zorin version string for codex's userAgent line."""
    try:
