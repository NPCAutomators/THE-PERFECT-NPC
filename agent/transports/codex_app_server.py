...
projection into Zorin' display, approval bridging, transcript projection into
...
"codex_app_server"`. Zorin' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Zorin secret — gateway
        ...
        spawn_env = zorin_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ZORIN_KANBAN_TASK"):
            kanban_db = spawn_env.get("ZORIN_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ZORIN_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ZORIN_HOME", os.path.expanduser("~/.zorin")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "zorin",
        client_title: str = "ZORIN",
