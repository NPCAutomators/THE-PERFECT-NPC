...
from zorin_constants import PARTIAL_STREAM_STUB_ID
from zorin_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Zorin tool use: "
    ...
    return (
        ...
        f"context, but Zorin needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Zorin config, set `model.ollama_num_ctx: 65536` "
...
def _npcautomators_entitlement_message(capability: str) -> str:
    try:
...
def _print_npcautomators_entitlement_guidance(agent, capability: str) -> bool:
    message = _npcautomators_entitlement_message(capability)
    if not message:
        return False
...
def _is_npcautomators_inference_route(provider: str, base_url: str) -> bool:
    provider = (provider or "").strip().lower()
    if provider == "npcautomators":
        return True
    base = str(base_url or "")
    return (
        base_url_host_matches(base, "inference-api.npcautomators.com")
    )
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_npcautomators_inference_route(provider, base_url):
        return _npcautomators_entitlement_message(capability)
...
def _try_refresh_npcautomators_paid_entitlement_credentials(agent) -> bool:
    """Refresh NpcAutomators runtime credentials after a fresh paid-entitlement check."""
    try:
        ...
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_npcautomators_client_credentials(
            force=True,
        )
    except Exception:
        return False
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `zorin fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from zorin_cli.moa_config import decode_moa_turn
    ...
    # all run inside Codex). Default Zorin path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Zorin internals.
        #
        # Zorin invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Zorin tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── NPCAUTOMATORS Portal rate limit guard ──────────────────────
            # If another session already recorded that NpcAutomators is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "npcautomators":
                try:
                    from agent.npcautomators_rate_guard import (
                        npcautomators_rate_limit_remaining,
                        format_remaining as _fmt_npcautomators_remaining,
                    )
                    _npcautomators_remaining = npcautomators_rate_limit_remaining()
                    if _npcautomators_remaining is not None and _npcautomators_remaining > 0:
                        _npcautomators_msg = (
                            f"NPCAUTOMATORS Portal rate limit active — "
                            f"resets in {_fmt_npcautomators_remaining(_npcautomators_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"⏳ {_npcautomators_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_npcautomators_msg}")
            ...
            try:
                ...
                try:
                    from zorin_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("ZORIN_DUMP_REQUESTS"):
                ...
                from zorin_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Zorin/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                # Clear NpcAutomators rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting NpcAutomators.
                if agent.provider == "npcautomators":
                    try:
                        from agent.npcautomators_rate_guard import clear_npcautomators_rate_limit
                        clear_npcautomators_rate_limit()
                    except Exception:
                        pass
                ...
                if (
                    ...
                    and _is_npcautomators_inference_route(
                        getattr(agent, "provider", "") or "",
                        getattr(agent, "base_url", "") or "",
                    )
                    and not _retry.npcautomators_paid_entitlement_refresh_attempted
                ):
                    _retry.npcautomators_paid_entitlement_refresh_attempted = True
                    if _try_refresh_npcautomators_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 NpcAutomators paid access verified — "
                ...
                if (
                    agent.api_mode == "chat_completions"
                    and agent.provider == "npcautomators"
                    and status_code == 401
                    and not _retry.npcautomators_auth_retry_attempted
                ):
                    _retry.npcautomators_auth_retry_attempted = True
                    if agent._try_refresh_npcautomators_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 NpcAutomators agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 NpcAutomators 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_npcautomators_entitlement_guidance(agent, "NpcAutomators model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: zorin auth add npcautomators")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.npcautomators.com")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `zorin doctor` for credential-chain diagnostics, or")
                    ...
                    from zorin_constants import display_zorin_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Zorin-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: zorin config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: zorin config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── NPCAUTOMATORS Portal: record rate limit & skip retries ─────
                # When NpcAutomators returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: NPCAUTOMATORS Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Zorin).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every NpcAutomators model for minutes.  We use
                # ``is_genuine_npcautomators_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_npcautomators_rate_limit = False
                    try:
                        from agent.npcautomators_rate_guard import (
                            is_genuine_npcautomators_rate_limit,
                            record_npcautomators_rate_limit,
                        )
                        ...
                        _genuine_npcautomators_rate_limit = is_genuine_npcautomators_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_npcautomators_rate_limit:
                            record_npcautomators_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                            ...
                            logger.info(
                                "NpcAutomators 429 looks like upstream capacity "
                    ...
                    if _genuine_npcautomators_rate_limit:
                        ...
                        # top-of-loop NpcAutomators guard handles fallback or
                ...
                # which Zorin' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Zorin' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`zorin",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Zorin may add API-only content not present in persisted
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "npcautomators" and _print_npcautomators_entitlement_guidance(
                            agent,
                            "NpcAutomators model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "npcautomators"} and status_code == 401:
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `zorin auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `zorin model`.", force=True)
                            else:  # npcautomators
                                agent._vprint(f"{agent.log_prefix}   💡 NPCAUTOMATORS Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: zorin portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.npcautomators.com", force=True)
                                # ``:free`` is OpenRouter slug syntax; NPCAUTOMATORS Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         NPCAUTOMATORS Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         NpcAutomators catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: zorin setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        zorin fallback add   (interactive picker — same as `zorin model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.zorin/config.yaml` to extend the per-call "
                            f"timeout. (Zorin's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Zorin mid-turn, resume logic still sees the
                ...
                try:
                    ...
                    from zorin_cli.plugins import get_pre_verify_continue_message, has_hook
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("ZORIN_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
