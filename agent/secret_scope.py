...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ZORIN_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Zorin runtime / deployment
    "ZORIN_HOME", "ZORIN_PROFILE", "ZORIN_GATEWAY_LOCK_DIR",
    "ZORIN_MAX_ITERATIONS", "ZORIN_MAX_TOKENS", "ZORIN_API_TIMEOUT",
    "ZORIN_REDACT_SECRETS", "ZORIN_NPCAUTOMATORS_TIMEOUT_SECONDS",
    "_ZORIN_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ZORIN_KANBAN_DB", "ZORIN_KANBAN_WORKSPACES_ROOT", "ZORIN_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "ZORIN_KANBAN_",
    "ZORIN_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(zorin_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(zorin_home)
    secrets = load_env_file(home / ".env")

    try:
        from zorin_cli.env_loader import get_secret_source_values
