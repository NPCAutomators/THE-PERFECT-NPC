...
``auth.json``.  They intentionally have no dependency on ``zorin_cli.auth`` so
...
# Sources Zorin owns and can intentionally persist in auth.json.  Everything
...
_PERSISTABLE_PROVIDER_SOURCES = frozenset({
    ("anthropic", "zorin_pkce"),
    ("minimax-oauth", "oauth"),
    ("npcautomators", "device_code"),
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Zorin-owned OAuth/device-code state)
