...
as ``kind: backend``) or ``~/.zorin/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``zorin tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ZORIN_HOME/cache/videos/``, creating parents as needed."""
    from zorin_constants import get_zorin_home

    path = get_zorin_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ZORIN_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$ZORIN_HOME/cache/videos/``.
