...
Storage
-------
Bundles live in ``~/.zorin/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ZORIN_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``zorin bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ZORIN_HOME.

    Honors ``ZORIN_BUNDLES_DIR`` for tests; falls back to
    ``<ZORIN_HOME>/skill-bundles``.
    """
    override = os.environ.get("ZORIN_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_zorin_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `zorin bundles` CLI subcommand.
# ---------------------------------------------------------------------------
