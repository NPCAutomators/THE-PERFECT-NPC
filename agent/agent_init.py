...
from zorin_cli.timeouts import get_provider_request_timeout
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from zorin_cli.providers import ZORIN_OVERLAYS, get_provider

        overlay = ZORIN_OVERLAYS.get(provider)
    ...
    try:
        ...
        from zorin_cli.models import normalize_provider as normalize_model_provider
        from zorin_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: zorin config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$ZORIN_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_zorin_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$ZORIN_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .zorin.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / ZORIN_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.zorin/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from zorin_cli.config import load_config as _load_pc_cfg
    ...
    # x-npcautomators-credits-* response headers after each API call.  Session-start
    # remaining is latched the first time a header is ever seen so we can
    # report cumulative micros spent.  Surfaced behind ZORIN_DEV_CREDITS.
    ...
    # both live under ~/.zorin/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from zorin_logging import setup_logging, setup_verbose_logging
    setup_logging(zorin_home=_ra()._zorin_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that zorin_logging.setup_logging() does not
        ...
        # handler level inside zorin_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from zorin_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from zorin_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `zorin model` to "
                        "select a provider, or run `zorin setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff ZORIN_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["ZORIN_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.zorin/sessions/ alongside gateway sessions
    zorin_home = get_zorin_home()
    agent.logs_dir = zorin_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.zorin/sessions/session_{sid}.json)
    ...
    try:
        from zorin_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from zorin_cli.config import load_config as _load_agent_config
    ...
    # management API (the historical Zorin behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "zorin"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Zorin' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "zorin", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, zorin, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from zorin_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from zorin_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from zorin_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by ZORIN.  Choose a model with at least "
    ...
    # NpcAutomators Zorin 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from zorin_cli.model_switch import _check_zorin_model_warning

            _zorin_warn = _check_zorin_model_warning(agent.model or "")
            if _zorin_warn:
                _user_msg = (
                    "⚠ NPCAUTOMATORS Zorin 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_zorin_warn)
        except Exception:
            pass
