...
MoA-enabled; the normal Zorin agent loop still owns tool calling and turn
...
def _slot_runtime(slot: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        # bedrock SigV4 signing, npcautomators Portal tags — still fire. Those branches
...
def _run_references_parallel(
    ...
    *,
    ...
    # callbacks + the NPCAUTOMATORS Portal conversation tag) into each worker so
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Zorin boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Zorin agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Zorin agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Zorin agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
