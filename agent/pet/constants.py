...
row taxonomy is inferred from the atlas shape so Zorin can render both legacy
...
# rows (1536x1872), while older Zorin/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Zorin' activity state names. They are not always identical to the
...
# Legacy Zorin/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Zorin activity names -> accepted row-name aliases in descending
