...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ZORIN_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Zorin expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``zorin_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_zorin_home()`` themselves.
        """
        if "zorin_home" not in kwargs:
            from zorin_constants import get_zorin_home
            kwargs["zorin_home"] = str(get_zorin_home())
        for provider in self._providers:
            try:
