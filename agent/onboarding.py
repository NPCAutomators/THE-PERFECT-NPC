...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Zorin starts and finds ``~/.openclaw/``.

    Points users at ``zorin claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``zorin claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Zorin, run "
        "`zorin claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `zorin claw cleanup` (renames it to ~/.openclaw.pre-migration — "
