...
Plugins built on ZORIN often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Zorin
...
:class:`~zorin_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Zorin' main config
...
handles every provider, fallback chain, and per-task override Zorin
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`zorin_cli.plugins.PluginContext`
