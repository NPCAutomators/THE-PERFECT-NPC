"""Upload a Zorin session transcript to Hugging Face as an agent trace.

Zorin stores sessions in its own SQLite store (``zorin_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``zorin trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Zorin' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "zorin-traces"
_ZORIN_VERSION = "zorin-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Zorin OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Zorin' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Zorin conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.zorin/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `zorin trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/zorin-traces`` dataset. Returns a
