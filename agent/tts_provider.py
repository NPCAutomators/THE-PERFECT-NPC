...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Zorin with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.zorin/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Zorin
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``zorin tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``zorin setup`` for
