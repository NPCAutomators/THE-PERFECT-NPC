...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `zorin backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - zorin_home (str): The active ZORIN_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.zorin``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "zorin").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'zorin memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], zorin_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'zorin memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``zorin_home`` is the active ZORIN_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ZORIN_HOME.

        ``zorin backup`` only walks ZORIN_HOME, so any provider state kept
        ...
        of the archive, and ``zorin import`` restores them to their original
