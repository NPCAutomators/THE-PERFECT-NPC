# OpenAI-Compatible API Server for ZORIN
...
SSE streaming. By exposing this endpoint, zorin-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  zorin-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.zorin/.env`
2. Run `zorin gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with zorin-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns zorin-agent as a model) |
...
Authorization: Bearer zorin-api-key-here
...
But zorin-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "zorin-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured zorin-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of zorin-agent's core prompt
...
Once implemented, zorin-agent works as a drop-in backend for:
