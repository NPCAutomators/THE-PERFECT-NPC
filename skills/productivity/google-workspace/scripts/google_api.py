#!/usr/bin/env python3
"""Google Workspace API CLI for ZORIN.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Zorin-facing JSON contract and falls back to the Python client
...
from _zorin_home import get_zorin_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ZORIN_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for ZORIN")
