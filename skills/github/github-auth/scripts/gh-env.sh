#!/usr/bin/env bash
# GitHub environment detection helper for ZORIN skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _zorin_env="${ZORIN_HOME:-$HOME/.zorin}/.env"; [ -f "$_zorin_env" ] && grep -q "^GITHUB_TOKEN=" "$_zorin_env" 2>/dev/null; then
