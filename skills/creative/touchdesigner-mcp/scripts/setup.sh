...
ZORIN_HOME_DIR="${ZORIN_HOME:-$HOME/.zorin}"
ZORIN_CFG="${ZORIN_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Zorin config has twozero_td MCP entry ──
if [[ ! -f "$ZORIN_CFG" ]]; then
    echo -e " ${FAIL} Zorin config not found at ${ZORIN_CFG}"
    manual_steps+=("Create ${ZORIN_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ZORIN_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Zorin config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Zorin config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ZORIN_CFG} manually"); }
    manual_steps+=("Restart Zorin session to pick up config change")
fi
