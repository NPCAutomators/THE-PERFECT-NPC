...
## 5. Zorin Configuration

### Config location

`$ZORIN_HOME/config.yaml` (defaults to `~/.zorin/config.yaml` when `ZORIN_HOME` is unset)
...
Restart the Zorin session for changes to take effect. The MCP connection is
