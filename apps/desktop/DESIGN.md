...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-npcautomators` + a `--stroke-npcautomators` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-npcautomators`,
...
```
shadow-npcautomators           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-npcautomators) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-npcautomators` | the overlay hairline (pairs with `shadow-npcautomators`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph — the
  NPCAUTOMATORS mark on a white tile, softly rounded, identical in light/dark.
...
- [ ] Tokens (`--ui-*`, `shadow-npcautomators`, `--stroke-npcautomators`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-npcautomators` + `border-(--stroke-npcautomators)`, no hard border?
