// Backend subcommand routing for the desktop-managed Zorin process.
//
// The desktop app launches its own headless backend via `zorin serve` — it
...
// app hasn't updated yet, or an older `zorin` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Zorin profile to pin via `--profile`.
 */
 ...
 * `-m zorin_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `zorin_cli/subcommands/dashboard.py` source registers
