...
const READY_RE = /^ZORIN_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitZorinPath = '') {
  const explicit = psLiteral(explicitZorinPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$zorinHome=$env:ZORIN_HOME',
    'if(-not $zorinHome){$zorinHome=Join-Path $env:LOCALAPPDATA "zorin"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command zorin.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $zorinHome "zorin-agent\\venv\\Scripts\\zorin.exe")',
    '$candidates+=(Join-Path $HOME "zorin-agent\\.venv\\Scripts\\zorin.exe")',
    '$zorin=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $zorin){throw "Zorin is not installed on the remote Windows host."}',
    'if($explicit -and $zorin -ne $explicit){throw "The configured Zorin path is not an executable file."}',
    '$python=Join-Path (Split-Path $zorin) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Zorin Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;zorinHome=$zorinHome;zorinPath=$zorin;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitZorinPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitZorinPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'zorin_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.zorinPath === 'string' &&
    typeof lock.zorinHome === 'string'
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteZorinPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.zorinPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Zorin on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.zorinPath = inspection.path
  const zorinVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located zorin at ${runtime.zorinPath}`)
  ...
  if (validLock(lock, ownershipId)) {
    ...
    const reusable =
      ...
      lock.zorinPath === runtime.zorinPath &&
      lock.zorinHome === runtime.zorinHome
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, zorinPath: runtime.zorinPath })
    )
  } catch (error) {
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForZorin(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Zorin SSH"', 'powershell.exe -NoLogo')
