...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zorin-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'zorin-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Zorin Test'], { cwd: dir })
