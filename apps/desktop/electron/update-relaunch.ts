 ...
 * Background
 * ----------
 * After `zorin update` + `zorin desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ZORIN_DESKTOP_* is preserved
// plus ZORIN_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ZORIN_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ZORIN_DESKTOP_']
