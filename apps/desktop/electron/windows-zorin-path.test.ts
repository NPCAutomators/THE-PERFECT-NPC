// Unit tests for the pure Windows `zorin` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvZorinCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `zorin` shim shadows
//      the real zorin.cmd/zorin.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the zorin.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvZorinCommand() — must probe the venv python via
//      canImportZorinCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvZorinCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportZorinCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    zorinHome: '/fake/zorin-home',
...
test('resolveVenvZorinCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvZorinCommand('/root/venv/Scripts/zorin.exe', [], deps), null)
})

test('resolveVenvZorinCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvZorinCommand('/root/venv/Scripts/zorin.cmd', [], deps), null)
})

test('resolveVenvZorinCommand: returns null when the basename is not zorin/zorin.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvZorinCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvZorinCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvZorinCommand('/root/venv/bin/zorin.exe', [], deps), null)
})

test('resolveVenvZorinCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvZorinCommand('/root/venv/Scripts/zorin.exe', [], deps), null)
})

test('resolveVenvZorinCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportZorinCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvZorinCommand('/root/venv/Scripts/zorin.exe', ['serve'], deps)
...
test('resolveVenvZorinCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvZorinCommand('/root/venv/Scripts/zorin.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'zorin_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvZorinCommand: is case-insensitive on zorin.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvZorinCommand('/root/venv/Scripts/ZORIN.EXE', [], deps))
  assert.ok(resolveVenvZorinCommand('/root/venv/SCRIPTS/zorin.exe', [], deps))
})
