// Profile-delete routing logic for the `zorin:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_zorin_home() and
...
/**
 * Parse a `zorin:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `zorin:api` request away from the primary/window backend
 ...
 * ensure_zorin_home() recreates the directory the delete just removed.
 */
