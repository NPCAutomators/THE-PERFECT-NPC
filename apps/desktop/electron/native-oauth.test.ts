...
test('buildNativeAuthorizeUrl encodes params and honours a path prefix', () => {
  ...
  assert.equal(parsed.searchParams.get('provider'), 'npcautomators')
})

test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/zorin', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/zorin/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/zorin'), 'https://gw.example.com/zorin/auth/native/refresh')
})
...
test('parseTokenResponse maps a well-formed body', () => {
  ...
  assert.equal(t.provider, 'npcautomators')
