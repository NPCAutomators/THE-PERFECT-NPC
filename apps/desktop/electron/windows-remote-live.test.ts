...
//   ZORIN_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ZORIN_WIN_SSH_USER   remote user
//   ZORIN_WIN_SSH_ZORIN absolute path to the remote zorin.exe under test
const liveHost = process.env.ZORIN_WIN_SSH_HOST || ''
const liveUser = process.env.ZORIN_WIN_SSH_USER || ''
const configuredZorin = process.env.ZORIN_WIN_SSH_ZORIN || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Zorin-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredZorin)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredZorin.replace('zorin.exe', 'python.exe')}' -m zorin_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredZorin.replace('zorin.exe', 'python.exe')
        const terminate = `& '${python}' -m zorin_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.zorinPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m zorin_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m zorin_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
