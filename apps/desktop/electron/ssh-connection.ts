...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ZORIN_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Zorin-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.zorin/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'zorin-desktop-ssh')
  }

  return path.join(os.homedir(), '.zorin', 'desktop-ssh')
}
