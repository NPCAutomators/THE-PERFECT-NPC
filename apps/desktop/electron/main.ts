...
import { buildDesktopBackendEnv, normalizeZorinHomeRoot } from './backend-env'
import { canImportZorinCli, verifyZorinCli } from './backend-probes'
...
import {
  ...
  resolveVenvZorinCommand
} from './windows-zorin-path'
import { isPackagedInstallPath as isPackagedInstallPathUnderRoots } from './workspace-cwd'
import { readWslWindowsClipboardImage } from './wsl-clipboard-image'
import { resolvePickerDefaultPath } from './wsl-path-bridge'

const USER_DATA_OVERRIDE = process.env.ZORIN_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ZORIN_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ZORIN_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ZORIN_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[zorin] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[zorin] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `zorin desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Zorin sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[zorin] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[zorin] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[zorin] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[zorin] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[zorin] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('zorin:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[zorin] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[zorin] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[zorin] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[zorin] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ZORIN_HOME — the user-facing root for everything Zorin-related. Mirrors
// scripts/install.ps1's $ZorinHome and scripts/install.sh's $ZORIN_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\zorin (matches install.ps1)
//   macOS / Linux: ~/.zorin (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.zorin directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\zorin yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ZORIN_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ZORIN_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.zorin / %LOCALAPPDATA%\zorin.
function resolveZorinHome() {
  if (process.env.ZORIN_HOME) {
    return normalizeZorinHomeRoot(process.env.ZORIN_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'zorin-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ZORIN_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\zorin and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ZORIN_HOME')

    if (fromRegistry) {
      return normalizeZorinHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'zorin')
    const legacy = path.join(app.getPath('home'), '.zorin')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.zorin setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.zorin')
}

const ZORIN_HOME = resolveZorinHome()

function zorinManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_zorin_node_dirs() in
  // zorin_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(ZORIN_HOME, 'node')
...
function pathWithZorinManagedNode(...entries) {
  return [...zorinManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ZORIN_ROOT — the canonical mutable Zorin install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ZORIN_ROOT = path.join(ZORIN_HOME, 'zorin-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ZORIN_ROOT, 'venv')
...
// boots, going straight to `resolveZorinBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ZORIN_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ZORIN_ROOT, '.zorin-bootstrap-complete')
...
// active-profile.json records which Zorin profile the desktop launches its
// local backend as. When set, startZorin() passes `zorin --profile <name>
// dashboard …`, which deterministically pins ZORIN_HOME (see
// _apply_profile_override in zorin_cli/main.py) and bypasses the sticky
// ~/.zorin/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors zorin_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// zorinDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ZORIN_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by zorin_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ZORIN_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (zorin_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ZORIN_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.ZORIN_DESKTOP_BOOT_FAKE_ERROR || ''

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ZORIN_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ZORIN_DESKTOP_APP_NAME || 'Zorin'
...
// 'zorin:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.npcautomators.zorin) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.npcautomators.zorin')
}

// Seed the native About panel with the live Zorin version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `zorin update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveZorinVersion(),
  copyright: 'Copyright © 2026 NPCAUTOMATORS'
})
...
const MEDIA_PROTOCOL = 'zorin-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(zorin-media://…)`.
...
// startZorin(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ZORIN_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ZORIN_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startZorin() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning zorin serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Zorin backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (zorinLog.length > 300) {
    zorinLog.splice(0, zorinLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-zorin-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('zorin:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the zorin:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('zorin:bootstrap:event', ev)
}
...
// The Tauri updater writes ZORIN_HOME/.zorin-update-in-progress for the whole
...
// (`force_kill_other_zorin`, taskkill /IM zorin.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(ZORIN_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Zorin will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(ZORIN_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `zorin` — must not shadow `zorin.cmd`/`zorin.exe`.
...
function unwrapWindowsVenvZorinCommand(command, backendArgs) {
  return resolveVenvZorinCommand(command, backendArgs, {
...
// spawns `zorin serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `zorin` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'zorin_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ZORIN_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isZorinSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'zorin_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ZORIN_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Zorin deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Zorin' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\zorin\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'zorin', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'zorin', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal ZORIN_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\zorin\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'zorin', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'zorin', 'git', 'bin', 'git.exe'))
  }
...
function recentZorinLog() {
  return zorinLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's zorin root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ZORIN_ROOT.
// ZORIN_DESKTOP_ZORIN_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ZORIN_DESKTOP_ZORIN_ROOT && path.resolve(process.env.ZORIN_DESKTOP_ZORIN_ROOT),
    !IS_PACKAGED && isZorinSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isZorinSourceRoot(ACTIVE_ZORIN_ROOT) ? ACTIVE_ZORIN_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ZORIN_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('zorin:updates:progress', merged)
  }
}
...
// ZORIN_HOME/zorin-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_zorin_home). That binary owns
// ALL repo mutation — running `zorin update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'zorin-setup.exe' : 'zorin-setup'
  const candidate = path.join(ZORIN_HOME, name)
...
// Path to the venv shim whose lock decides whether `zorin update` can write
// fresh entry points. On Windows this is the file the running backend
// `zorin.exe` holds open; on POSIX it's never mandatory-locked.
function venvZorinShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'zorin.exe')
    : path.join(updateRoot, 'venv', 'bin', 'zorin')
}
...
// only signals the direct child, so on Windows a backend `zorin.exe` that
// spawned its own grandchildren (a `zorin` REPL, a pty terminal session, the
...
// `zorinProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `zorin.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a zorin
// REPL, a pty terminal, the gateway) keep `zorin.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const zorinProcess = backendConnectionState.getProcess()

  if (zorinProcess && Number.isInteger(zorinProcess.pid)) {
    pids.push(zorinProcess.pid)
  }
  ...
  if (zorinProcess && !zorinProcess.killed) {
    try {
      zorinProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvZorinShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentZorinProcess = backendConnectionState.getProcess()

    if (currentZorinProcess && Number.isInteger(currentZorinProcess.pid)) {
      stragglers.push(currentZorinProcess.pid)
    }
...
// `zorin update`). Instead we spawn the staged Zorin-Setup binary with
// --update and quit, so it can run `zorin update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri zorin-setup. Unlike Windows
      ...
      // whole update itself: `zorin update` (backend) + `zorin desktop
    ...
    if (!updater) {
      ...
      // `zorin desktop`, never the Tauri installer that self-copies
      // zorin-setup.exe into ZORIN_HOME). They DO have a working `zorin`
      ...
      // checkout they're on — bare `zorin update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `zorin update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `zorin update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, zorinRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Zorin — this window will close and the updater will open. Don’t reopen Zorin yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // zorin.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running zorin, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Zorin install open ' +
        '(a second Zorin window or a terminal running zorin?). Close it and retry.'
    ...
    // `zorin update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: ZORIN_HOME,
      env: {
        ...process.env,
        ZORIN_HOME,
        PATH: pathWithZorinManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(ZORIN_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvZorin = path.join(venvBin, IS_WINDOWS ? 'zorin.exe' : 'zorin')
  ...
  // not just the `zorin.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvZorin) || fileExists(path.join(updateRoot, '.zorin-bootstrap-complete'))
...
// Resolve the zorin CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `zorin` on PATH.
function resolveZorinCliBinary(updateRoot) {
  const venvZorin = path.join(updateRoot, 'venv', 'bin', 'zorin')

  if (fileExists(venvZorin)) {
    return venvZorin
  }

  return findOnPath('zorin') || null
}
...
// macOS/Linux in-app update: backend (`zorin update`) + OS-aware GUI rebuild
// (`zorin desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const zorin = resolveZorinCliBinary(updateRoot)

  if (!zorin) {
    emitUpdateProgress({ stage: 'manual', message: 'zorin update', percent: null })

    return { ok: true, manual: true, command: 'zorin update', zorinRoot: updateRoot }
  }

  // Put the Zorin-managed Node and the venv on PATH so `zorin desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\zorin\node, not node\bin.
  // PYTHONUNBUFFERED: `zorin update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    ZORIN_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithZorinManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `zorin update` reaps stale `zorin serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ZORIN_DESKTOP_CHILD_PID and excludes
  ...
  if (zorinProcess && Number.isInteger(zorinProcess.pid)) {
    desktopChildPids.push(zorinProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.ZORIN_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Zorin (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(zorin, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'zorin update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'zorin update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(zorin, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Zorin to retry.',
  ...
  // Linux in-app update terminal state (#45205). `zorin desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Zorin…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `zorin-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Zorin to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Zorin desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Zorin to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Zorin.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Zorin.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Zorin to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.zorin-update-new"; then
    rm -rf "$DST.zorin-update-old" 2>/dev/null || true
    mv "$DST" "$DST.zorin-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.zorin-update-new" "$DST"
    rm -rf "$DST.zorin-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Zorin to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_ZORIN_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isZorinSourceRoot(ACTIVE_ZORIN_ROOT) &&
    fileExists(venvPython) &&
    canImportZorinCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ZORIN_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `zorin
...
function resolveWebDist() {
  const override = process.env.ZORIN_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ZORIN_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: zorin desktop --force-build`
  )
...
function resolveZorinCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Zorin.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveZorinCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'zorin_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_ZORIN_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Zorin at ${ACTIVE_ZORIN_ROOT}`,
    command,
    args: ['-m', 'zorin_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      zorinHome: ZORIN_HOME,
      pythonPathEntries: [ACTIVE_ZORIN_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ZORIN_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveZorinBackend(backendArgs) {
  // 1. Explicit override -- ZORIN_DESKTOP_ZORIN_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ZORIN_DESKTOP_ZORIN_ROOT && path.resolve(process.env.ZORIN_DESKTOP_ZORIN_ROOT)

  if (overrideRoot && isZorinSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Zorin source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `zorin` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isZorinSourceRoot.)
  if (!IS_PACKAGED && isZorinSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Zorin source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_ZORIN_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\zorin\zorin-agent (Windows) or ~/.zorin/zorin-agent.
  ...
  //    to spawning zorin. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `zorin update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `zorin` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ZORIN_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ZORIN_DESKTOP_IGNORE_EXISTING !== '1') {
    let zorinCommand = null
    const zorinOverride = process.env.ZORIN_DESKTOP_ZORIN

    if (zorinOverride) {
      const resolvedOverride = findOnPath(zorinOverride)

      if (resolvedOverride) {
        zorinCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(zorinOverride, { isWsl: IS_WSL })) {
        zorinCommand = zorinOverride
      } else {
        rememberLog(`Ignoring Windows Zorin override under WSL: ${zorinOverride}`)
      }
    } else {
      zorinCommand = findOnPath('zorin')
    }

    if (zorinCommand) {
      if (looksLikeDesktopAppBinary(zorinCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Zorin CLI: ${zorinCommand}`)
        zorinCommand = null
      }
    }

    if (zorinCommand) {
      const unwrapped = unwrapWindowsVenvZorinCommand(zorinCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `zorin` shim
      ...
      const shellForProbe = isCommandScript(zorinCommand)

      if (verifyZorinCli(zorinCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvZorinCommand(zorinCommand, backendArgs) || {
            label: `existing Zorin CLI at ${zorinCommand}`,
            command: zorinCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Zorin CLI at ${zorinCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed zorin_cli module via system Python.
  ...
  if (python) {
    ...
    // having zorin_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\zorin\zorin-agent\venv.
    if (canImportZorinCli(python)) {
      return {
        kind: 'python',
        label: `installed zorin_cli module via ${python}`,
        command: python,
        args: ['-m', 'zorin_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: zorin_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveZorinBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'ZORIN not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveZorinBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Zorin install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Zorin recovery was handed off to Zorin Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      zorinHome: ZORIN_HOME,
      logRoot: path.join(ZORIN_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Zorin install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Zorin bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ZORIN_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startZorin() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // zorin:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveZorinBackend(backend.args))
  }
  ...
  if (!isZorinSourceRoot(ACTIVE_ZORIN_ROOT)) {
    throw new Error(
      `Zorin install at ${ACTIVE_ZORIN_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Zorin' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\zorin\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `zorin` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Zorin on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isZorinSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Zorin venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Zorin at ${ACTIVE_ZORIN_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Zorin runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----zorin-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Zorin backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Zorin backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Zorin backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Zorin-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('zorin:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveZorinCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('zorin:preview-file-changed', payload)
}
...
async function waitForZorin(baseUrl, token, signal?) {
  ...
  throw new Error(`Zorin backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('zorin:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('zorin:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Zorin...".
function sendPowerResume() {
  ...
  webContents.send('zorin:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('zorin:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('zorin:window-state-changed', state)
}
...
//
// Hosted Zorin gateways gate the dashboard behind an OAuth provider (e.g.
// NPCAUTOMATORS) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``zorin_session_at``),
...
//   * NPCAUTOMATORS Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / zorin #37247).
//     Both are set as HttpOnly cookies (``zorin_session_at`` ~15 min,
//     ``zorin_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:zorin-remote-oauth'
...
// "not signed in", which on the initial boot path (startZorin → the renderer's
// single-shot boot() with no retry) surfaced as the "Zorin couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Zorin
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Zorin Cloud agent…' : 'Sign in to Zorin gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Zorin backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in zorin_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Zorin Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the NpcAutomators portal ONCE in
...
// portal session cookie living in `persist:zorin-remote-oauth`:
...
// Canonical NpcAutomators portal base URL, overridable for staging/dev. Mirrors the CLI
// convention (zorin_cli/auth.py DEFAULT_NPCAUTOMATORS_PORTAL_URL + the same env names)
// so a single override flips every Zorin surface to the same portal.
const DEFAULT_NPCAUTOMATORS_PORTAL_URL = 'https://portal.npcautomators.com'

function resolvePortalBaseUrl() {
  const raw =
    process.env.ZORIN_PORTAL_BASE_URL || process.env.NPCAUTOMATORS_PORTAL_BASE_URL || DEFAULT_NPCAUTOMATORS_PORTAL_URL

  return String(raw).trim().replace(/\/+$/, '')
}

// Whether the OAuth partition currently holds a live NpcAutomators portal session — the
// credential that powers both discovery and the silent cascade. The portal
// authenticates via PRIVY, not the Zorin gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for zorin_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Zorin Cloud sign-in.'))
...
// Discover the hosted (Zorin Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Zorin Cloud. Open Settings → Gateway, choose Zorin Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Zorin Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Zorin Cloud session has expired. Sign in to Zorin Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Zorin Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root ZORIN_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ZORIN_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.ZORIN_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Zorin Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteZorinPath: (ssh || savedSsh)?.remoteZorinPath || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by ZORIN_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Zorin Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Zorin Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteZorinPath: input.sshRemoteZorinPath ?? existingBlock.remoteZorinPath
  })
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (zorin #37247), and the gateway middleware transparently
    ...
    if (!oauthSessionIsLive(hasNativeSession(baseUrl), await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Zorin gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Zorin gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Zorin gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.ZORIN_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    ...
    result = await lifecycle({
      ...
      pickLocalPort,
      waitForZorin: (baseUrl, token) => waitForZorin(baseUrl, token, lease.signal),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    zorinVersion: result.zorinVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.zorinVersion || 'zorin (version unknown)'} at ${result.zorinPath || '?'}`
  )
  ...
  return { ...connection, remoteZorinVersion: result.zorinVersion || '' }
}
...
//   2. env override (ZORIN_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ZORIN_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ZORIN_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ZORIN_DESKTOP_REMOTE_URL is set but ZORIN_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Zorin backend.'
      )
    }
...
// env → global) so a startZorin() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Zorin
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with NPCAUTOMATORS" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteZorinPath: input.sshRemoteZorinPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteZorinPath || '')
          let zorinPath
          let zorinVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            zorinPath = runtime.zorinPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.zorinPath])
            zorinVersion = inspection.version
            supported = inspection.supported
          } else {
            zorinPath = await remoteLifecycle.locateZorin(ssh, sshConfig.remoteZorinPath || '')
            zorinVersion = await remoteLifecycle.probeZorinVersion(ssh, zorinPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, zorinPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Zorin on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteZorinPath: zorinPath,
            remoteZorinVersion: zorinVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startZorin())
  ...
  // connect to Zorin gateway". Mirror the renderer's connect here so the test
...
function resetZorinConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteLiveness.clear()
  const zorinProcess = backendConnectionState.invalidate()
  stopBackendChild(zorinProcess)
...
// startZorin() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetZorinConnection() invalidates it.
  const zorinProcess = backendConnectionState.getProcess()
  const dying = zorinProcess && !zorinProcess.killed ? zorinProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('zorin:connection:applied')
}
...
// profile to startZorin() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startZorin() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForZorin(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: zorinLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited ZORIN_HOME env (see _apply_profile_override
  // step 3 in zorin_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveZorinBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const zorinCwd = resolveZorinCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Zorin backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: zorinCwd,
        ZORIN_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        ZORIN_DESKTOP: '1',
        ZORIN_WEB_DIST: webDist,
        ...(readyFile ? { ZORIN_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Zorin backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Zorin backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Zorin backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForZorin(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Zorin backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_zorin_home() recreates the deleted profile directory.
//
...
async function startZorin() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startZorin() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Zorin backend', 8)
  ...
  const connectionPromise = (async () => {
    ...
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Zorin backend at ${remote.baseUrl}`, 24)
      await waitForZorin(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Zorin backend is ready',
        progress: 94,
        running: true,
        error: null
      })
      ...
      return {
        ...
        remoteZorinVersion: remote.remoteZorinVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: zorinLog.slice(-80),
        ...getWindowState()
    ...
    // deterministic (it wins over the sticky ~/.zorin/active_profile file) and
    // resolves ZORIN_HOME the same way `zorin -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Zorin runtime', 28)
    const backend = await ensureRuntime(resolveZorinBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const zorinCwd = resolveZorinCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Zorin backend via ${backend.label}`, 84)
    rememberLog(`Starting Zorin backend via ${backend.label}`)

    const zorinProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: zorinCwd,
        env: {
          ...process.env,
          // Explicitly pin ZORIN_HOME for the child so Python's get_zorin_home()
          // resolves to the SAME location our resolveZorinHome() picked. Without
          // this pin, Python falls back to ~/.zorin on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\zorin, which differs from C:\Users\<u>\.zorin.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ZORIN_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, zorinProcess)

    if (!processOwner) {
      stopBackendChild(zorinProcess)
      throw new Error('Zorin backend start was superseded by a newer connection attempt.')
    }

    zorinProcess.stdout.on('data', rememberLog)
    zorinProcess.stderr.on('data', rememberLog)
    ...
    zorinProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Zorin backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Zorin backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Zorin backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Zorin backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    zorinProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Zorin backend exit (${signal || code})`)
      ...
      rememberLog(`Zorin backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Zorin backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Zorin backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentZorinLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Zorin backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPortAnnouncement(zorinProcess, { readyFile }), backendStartFailed])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Zorin backend to become ready', 90)
    await Promise.race([waitForZorin(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => zorinProcess.exitCode === null && !zorinProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Zorin backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Zorin is
...
// pushes pet state over IPC (zorin:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via zorin:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Zorin app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Zorin icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see zorin:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Zorin in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('zorin:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  startZorin().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('zorin:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Zorin…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('zorin:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const result = await revalidateRemoteConnection({
      ...
      resetConnection: resetZorinConnection,
...
ipcMain.handle('zorin:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('zorin:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('zorin:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('zorin:window:openInstance', async () => {
...
ipcMain.handle('zorin:zoom:get', event => {
...
ipcMain.on('zorin:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('zorin:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('zorin:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('zorin:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('zorin:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('zorin:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('zorin:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('zorin:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('zorin:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('zorin:pet-overlay:control', payload)
})
ipcMain.handle('zorin:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startZorin() call restarts the
...
ipcMain.handle('zorin:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startZorin() re-runs the full installer (refreshing a broken/partial
  ...
  bootstrapFailure = null
  backendStartFailure = null
  resetZorinConnection()

  return { ok: true }
})
ipcMain.handle('zorin:bootstrap:cancel', async () => {
...
ipcMain.handle('zorin:boot-progress:get', async () => bootProgressState)
ipcMain.handle('zorin:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('zorin:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('zorin:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('zorin:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('zorin:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('zorin:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('zorin:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('zorin:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Zorin Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('zorin:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('zorin:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('zorin:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('zorin:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('zorin:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ZORIN_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('zorin:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('zorin:requestMicrophoneAccess', async () => {
...
ipcMain.handle('zorin:api', async (_event, request) => {
  ...
  // backend calls ensure_zorin_home() which recreates the profile directory,
...
ipcMain.handle('zorin:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('zorin:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('zorin:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('zorin:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('zorin:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('zorin:readFileText', async (_event, filePath) => {
...
ipcMain.handle('zorin:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('zorin:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('zorin:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('zorin:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('zorin:saveClipboardImage', async () => {
...
ipcMain.handle('zorin:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('zorin:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('zorin:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('zorin:titlebar-theme', (_event, payload) => {
...
ipcMain.on('zorin:native-theme', (_event, mode) => {
...
ipcMain.on('zorin:translucency', (_event, payload) => {
...
ipcMain.on('zorin:keep-awake', (_event, on) => {
...
ipcMain.handle('zorin:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('zorin:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveZorinCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('zorin:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveZorinCwd()
}))

ipcMain.handle('zorin:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('zorin:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('zorin:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('zorin:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('zorin:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('zorin:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: zorinLog.slice(-200) }))
...
function terminalShellCommand() {
  // ZORIN_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.ZORIN_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Zorin' TUI into
  ...
  env.TERM_PROGRAM = 'Zorin'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a zorin/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from ZORIN_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.ZORIN_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `zorin:terminal:${id}:${suffix}`
}
...
ipcMain.handle('zorin:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('zorin:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('zorin:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('zorin:fs:openDir', async (_event, dirPath) => {
...
ipcMain.handle('zorin:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('zorin:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('zorin:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('zorin:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('zorin:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('zorin:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('zorin:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('zorin:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('zorin:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('zorin:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('zorin:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('zorin:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('zorin:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('zorin:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zorin:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zorin:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zorin:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('zorin:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('zorin:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('zorin:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('zorin:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('zorin:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('zorin:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('zorin:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('zorin:terminal:write', (_event, id, data) => {
...
ipcMain.handle('zorin:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('zorin:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('zorin:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('zorin:updates:check', async () =>
...
ipcMain.handle('zorin:updates:apply', async (_event, payload) =>
...
ipcMain.handle('zorin:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('zorin:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Zorin version (the one `release.py` bumps in
// zorin_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Zorin version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveZorinVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'zorin_cli', '__init__.py')
...
// Re-resolve the live Zorin version and push it into the native About panel
// just before showing it, so an in-place `zorin update` is reflected without
...
ipcMain.handle('zorin:version', async () => ({
  appVersion: resolveZorinVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  zorinRoot: resolveUpdateRoot()
}))
...
// the actual removal via `zorin uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (zorin_cli/uninstall.py + zorin_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ZORIN_ROOT
  ...
  const fallback = () => ({
    zorin_home: ZORIN_HOME,
    agent_installed: isZorinSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'zorin_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ZORIN_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Zorin agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import zorin_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ZORIN_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `zorin-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `zorin-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('zorin:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('zorin:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('zorin:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('zorin:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// zorin:// deep links (e.g. zorin://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const ZORIN_PROTOCOL = 'zorin'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${ZORIN_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // zorin://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('zorin:deep-link', payload)
...
ipcMain.handle('zorin:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ZORIN_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ZORIN_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(ZORIN_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `zorin://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching zorin:// URL is in our own argv.
