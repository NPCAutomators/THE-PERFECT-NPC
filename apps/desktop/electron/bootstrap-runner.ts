 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of ZORIN by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.zorin-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(zorinHome) {
  return path.join(zorinHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.zorin/zorin-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(zorinHome) {
  if (!zorinHome) {
    return null
  }

  const candidate = path.join(zorinHome, 'zorin-agent', 'scripts', installScriptName())
...
function cachedScriptPath(zorinHome, commit) {
  return path.join(bootstrapCacheDir(zorinHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NPCAutomators/AGENT-ZORIN/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(zorinHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(zorinHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, zorinHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ZORIN_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ZORIN_HOME: zorinHome || process.env.ZORIN_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, zorinHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, zorinHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--zorin-home', zorinHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, zorinHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, zorinHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, zorinHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(zorinHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, zorinHome, emit })
