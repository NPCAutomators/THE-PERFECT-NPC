 ...
 *     `X-Zorin-Session-Token` header; WS uses `?token=`.
...
// zorin_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - zorin_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - zorin_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / zorin #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-zorin_session_at', '__Secure-zorin_session_at', 'zorin_session_at']
const RT_COOKIE_VARIANTS = ['__Host-zorin_session_rt', '__Secure-zorin_session_rt', 'zorin_session_rt']

// The NpcAutomators portal (NAS) does NOT use Zorin gateway session cookies — it is a
...
// Zorin Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteZorinPath = String(entry.remoteZorinPath || '').trim()

  if (remoteZorinPath) {
    out.remoteZorinPath = remoteZorinPath
  }
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://zorin.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a zorin session ACCESS-token cookie
...
/**
 * True if the cookie jar holds a live NpcAutomators PORTAL (Privy) session — a non-empty
 ...
 * the Zorin gateway session cookies, so cloud sign-in / discovery liveness
