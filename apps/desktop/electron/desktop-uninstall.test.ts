...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'zorin_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'zorin_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'zorin_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Zorin.app/Contents/MacOS/Zorin', 'darwin'),
    '/Applications/Zorin.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Zorin.app/Contents/MacOS/Zorin', 'darwin'),
    '/Users/x/Applications/Zorin.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Zorin\\Zorin.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Zorin'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\zorin-desktop\\Zorin.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\zorin-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Zorin.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_ZorinXXXX/zorin', 'linux', { APPIMAGE: '/home/x/Apps/Zorin.AppImage' }),
    '/home/x/Apps/Zorin.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/zorin/linux-unpacked/zorin', 'linux', {}), '/opt/zorin/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/zorin', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Zorin.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Zorin.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.zorin/zorin-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.zorin/zorin-agent',
    uninstallArgs: ['-m', 'zorin_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/zorin/linux-unpacked',
    zorinHome: '/home/x/.zorin'
  })
  ...
  assert.match(script, /'-m' 'zorin_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/zorin\/linux-unpacked'/)
  assert.match(script, /export ZORIN_HOME='\/home\/x\/\.zorin'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.zorin/zorin-agent',
    agentRoot: '/home/x/.zorin/zorin-agent',
    uninstallArgs: ['-m', 'zorin_cli.uninstall', '--mode', 'full'],
    appPath: null,
    zorinHome: '/home/x/.zorin'
  })

  // System python + source on PYTHONPATH so import zorin_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.zorin\/zorin-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'zorin_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'zorin_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    zorinHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'zorin_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\zorin',
    uninstallArgs: ['-m', 'zorin_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Zorin',
    zorinHome: 'C:\\Users\\x\\AppData\\Local\\zorin'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import zorin_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\zorin;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "zorin_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Zorin" >nul 2>&1/)
