...
export function markerPath(zorinHome) {
  return path.join(zorinHome, '.zorin-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(zorinHome)
  let raw
 ...
 *
 * The Tauri-based zorin-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(zorinHome, pid, { now = Date.now } = {}) {
  const file = markerPath(zorinHome)
