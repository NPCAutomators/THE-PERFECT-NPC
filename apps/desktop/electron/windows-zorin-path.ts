/**
 * windows-zorin-path.ts
 *
 * Pure, dependency-injected pieces of Windows `zorin` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvZorinCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `zorin` shim shadowed the real
 *      zorin.cmd/zorin.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\zorin.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvZorinCommand() — unwrapWindowsVenvZorinCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `zorin`) must
 * not shadow `zorin.cmd`/`zorin.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv zorin shim, bootstrap-complete
 * marker) — gating on just the zorin.exe console-script shim alone is the
...
export interface ResolveVenvZorinCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportZorinCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `zorin`/`zorin.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/zorin(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m zorin_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportZorinCli(). A
 * venv whose update died mid-`pip install` still has python.exe + zorin.exe
 ...
 *
 * Returns null when `command` is not a venv zorin shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvZorinCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvZorinCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^zorin(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Zorin at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Zorin Python at ${python}`,
    command: python,
