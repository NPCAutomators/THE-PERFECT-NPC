...
test('locateZorin prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/zorin/, 'OK']])
  assert.equal(await locateZorin(ssh, '/opt/zorin'), '/opt/zorin')
})

test('locateZorin throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong zorin" bug.
  const ssh = fakeSsh([
    [/command -v zorin/, '/home/u/.local/bin/zorin\n'],
    [/\[ -x .*\.local\/bin\/zorin/, 'OK']
  ])

  await assert.rejects(
    () => locateZorin(ssh, '/bad/path/zorin'),
    (err: any) => {
      assert.equal(err.kind, 'zorin-not-found')
      assert.match(err.message, /\/bad\/path\/zorin/)
...
test('locateZorin falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v zorin/, '/home/u/.local/bin/zorin\n'],
    [/\[ -x .*\.local\/bin\/zorin/, 'OK']
  ])

  assert.equal(await locateZorin(ssh, ''), '/home/u/.local/bin/zorin')
})

test('locateZorin canonicalizes an installer wrapper to its executable target', async () => {
  const ssh = fakeSsh([
    [/command -v zorin/, '/home/u/.local/bin/zorin\n'],
    [/\[ -x .*\.local\/bin\/zorin/, 'OK'],
    [/python3 -c/, '/home/u/.zorin/zorin-agent/venv/bin/zorin\n']
  ])

  assert.equal(await locateZorin(ssh, ''), '/home/u/.zorin/zorin-agent/venv/bin/zorin')
})

test('locateZorin falls back to ~/.local/bin/zorin when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateZorin(ssh, ''), '~/.local/bin/zorin')
})

test('locateZorin tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/zorin/, 'OK']])
  assert.equal(await locateZorin(ssh, ''), '~/.zorin/zorin-agent/venv/bin/zorin')
})

test('locateZorin throws a zorin-not-found error with an install hint', async () => {
...
test('locateZorin uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v zorin/, '/x/zorin'],
    [/\[ -x/, 'OK']
  ])

  await locateZorin(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.zorin/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.zorin/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.zorin/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/zorin'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/zorin serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/zorin'), true)
  assert.equal(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/zorin', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('ZORIN_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { zorinPath: '/x/zorin', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('ZORIN_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('ZORIN_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { zorinPath: '/x/zorin', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nZORIN_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'ZORIN_DASHBOARD_READY port=51999\n']
  ])
...
test('connect() respawns when the lockfile zorinPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ zorinPath: '/old/stale/zorin', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const ssh = fakeSsh([
    ...
    [/kill -0/, 'ALIVE'],
    [/print\("OWNED"/, 'FOREIGN\n'],
    [/--version/, 'ZORIN v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/cat .*\.log/, 'ZORIN_DASHBOARD_READY port=52050\n']
  ])

  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteZorinPath: '/new/zorin', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'ZORIN_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes zorinHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/ZORIN_HOME/, '/home/alice/.zorin\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'ZORIN_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"zorinHome":"\/home\/alice\/\.zorin"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'ZORIN_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'ZORIN_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/zorin'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.zorin/zorin-agent/venv/bin/zorin'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/zorin'))
  assert.doesNotThrow(() => validateRemotePath('~/.zorin/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/zorin'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('zorin'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/zorin'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/zorin\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/zorin\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/zorin\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/zorin', '/usr/`id`/zorin', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.zorin/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/zorin')
  assert.ok(result.includes('/usr/local/bin/zorin'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/zorin')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('ZORIN_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/zorin', 'work', {
    tokenFilePath: `~/.zorin/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.zorin\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/zorin', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { zorinPath: '/x/zorin', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { zorinPath: '/x/zorin', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile rejects a log path outside the exact ownership and spawn path', async () => {
  const lock = ownedLock({ logPath: '~/.zorin/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.zorin/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/zorin serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/zorin serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/zorin'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/zorin'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/zorin'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/zorin'), false)
})
