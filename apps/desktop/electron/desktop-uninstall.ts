 ...
 * the three user-facing uninstall modes to the `zorin uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m zorin_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `zorin_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'zorin_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Zorin.app/Contents/MacOS/Zorin  → …/Zorin.app
 *   Windows: …\Zorin\Zorin.exe                 → …\Zorin  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Zorin.app/Contents/MacOS/Zorin → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Zorin.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Zorin.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Zorin$/i.test(dir) || /[\\/]zorin-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/zorin
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import zorin_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, zorinHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import zorin_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Zorin install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ZORIN_HOME=${String(zorinHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
