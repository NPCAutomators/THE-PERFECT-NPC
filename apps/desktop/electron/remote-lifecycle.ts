 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Zorin dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Zorin install on the remote (login-shell probe),
 ...
 *     `ZORIN_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForZorin in.
 *
 * The minted ZORIN_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^ZORIN_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.zorin/desktop-ssh'
...
// Resolve the remote zorin executable. An EXPLICIT path is honored strictly
...
async function locateZorin(ssh, remoteZorinPath) {
  ...
  if (remoteZorinPath) {
    if (await isExecutable(remoteZorinPath)) {
      return resolveLauncher(remoteZorinPath)
    }

    const err: any = new Error(
      `The Zorin path you set is not an executable on the remote host: "${remoteZorinPath}". ` +
        'Check the path (it must be the full path to the `zorin` binary on the remote, e.g. ' +
        '~/zorin-agent/.venv/bin/zorin), or clear it to auto-detect.'
    )

    err.kind = 'zorin-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v zorin')}`)).trim()
  ...
  candidates.push('~/.local/bin/zorin')
  candidates.push('/usr/local/bin/zorin')
  candidates.push('~/.zorin/zorin-agent/venv/bin/zorin')
  ...
  const err: any = new Error(
    'Zorin is not installed on the remote host (could not find a `zorin` executable). ' +
      'Install it on the remote with:  curl -fsSL https://raw.githubusercontent.com/NPCAutomators/AGENT-ZORIN/main/scripts/install.sh | sh  ' +
      '— or set the Zorin path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<zorin> --version`,
// e.g. "ZORIN v0.18.2 ..."), or '' on failure. Surfaces WHICH zorin a
// connection uses, so a stale/unexpected install is visible.
async function probeZorinVersion(ssh, zorinPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(zorinPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Zorin Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ZORIN_HOME the remote dashboard will use (explicit env wins, else
// ~/.zorin). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteZorinHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ZORIN_HOME:-$HOME/.zorin}"')).trim().split('\n').pop()

    return out || '~/.zorin'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Zorin home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'zorinPath', 'zorinHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, zorinPath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !zorinPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(zorinPath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.zorinPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(zorinPath, profile, opts: any = {}) {
  const zorin = expandRemotePath(zorinPath)
  ...
  const dashCmd = `env ZORIN_DESKTOP=1 ${zorin} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, zorinPath) {
  const zorin = expandRemotePath(zorinPath)

  const out = await ssh.exec(
    `help="$(${zorin} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { zorinPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, zorinPath))) {
    const err: any = new Error(
      'The remote Zorin install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Zorin on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(zorinPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForZorin, a token-gated
...
async function connect(deps) {
  ...
  const zorinPath = await locateZorin(ssh, remoteZorinPath)
  log(`located zorin at ${zorinPath}`)
  const zorinVersion = await probeZorinVersion(ssh, zorinPath)

  if (zorinVersion) {
    log(`remote zorin version: ${zorinVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const zorinHome = await probeRemoteZorinHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.zorinPath))
    ...
    const reusable =
      ...
      lock.zorinPath === zorinPath &&
      lock.zorinHome === zorinHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForZorin(baseUrl, spawnToken)
    assertNotAborted(signal)
