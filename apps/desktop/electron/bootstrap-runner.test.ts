...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'zorin-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/zorin-runner-test',
    sourceRepoRoot: null,
    zorinHome: '/tmp/zorin-runner-test',
    logRoot: '/tmp/zorin-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'zorin-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'zorin-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/zorin-agent',
      zorinHome: '/tmp/zorin'
    }),
    ['--dir', '/tmp/zorin-agent', '--zorin-home', '/tmp/zorin', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/zorin-agent', '--zorin-home', '/tmp/zorin', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/zorin',
      zorinHome: '/tmp/home'
    }),
    ['--dir', '/tmp/zorin', '--zorin-home', '/tmp/home', '--branch', 'main']
  )
})
