 ...
 * product name (`Zorin`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Zorin'
 *
 * This is a hard failure with no obvious cause for the user — `zorin desktop`
 ...
 * (the mac-unpacked Zorin.app bundle) and Windows (win-unpacked), so we
