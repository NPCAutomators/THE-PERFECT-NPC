...
// no longer bundles the ZORIN Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Zorin.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Zorin'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Zorin.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Zorin'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ZORIN_HOME for non-sandboxed runs -- matches main.ts's
// resolveZorinHome(). On Windows it's %LOCALAPPDATA%\zorin; elsewhere
// it's ~/.zorin. The fresh-install sandbox launchFresh() sets its own
// ZORIN_HOME and never touches this.
const DEFAULT_ZORIN_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'zorin')
  }
  return path.join(os.homedir(), '.zorin')
})()
const VENV_ROOT = path.join(DEFAULT_ZORIN_HOME, 'zorin-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'zorin-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ZORIN_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Zorin-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Zorin-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Zorin-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Zorin-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ZORIN_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ZORIN_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const zorinHome = path.join(sandbox, 'zorin-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(zorinHome, { recursive: true })
  ...
  env.ZORIN_DESKTOP_CWD = cwd
  env.ZORIN_DESKTOP_IGNORE_EXISTING = '1'
  env.ZORIN_DESKTOP_TEST_MODE = 'fresh-install'
  env.ZORIN_DESKTOP_USER_DATA_DIR = userDataDir
  env.ZORIN_HOME = zorinHome
  delete env.ZORIN_DESKTOP_ZORIN
  delete env.ZORIN_DESKTOP_ZORIN_ROOT
  ...
  console.log(`  ZORIN_HOME: ${zorinHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(zorinHome, 'zorin-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The ZORIN Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of zorin_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'zorin-agent', 'zorin_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Zorin
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ZORIN_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ZORIN_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
