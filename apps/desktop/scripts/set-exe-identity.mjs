#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Zorin icon + version metadata onto the
// built Zorin.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Zorin.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `zorin desktop`, the installer's
...
// path (which rebuilds via `zorin desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Zorin.exe>
//
...
// Stamp the Zorin icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Zorin',
      FileDescription: 'Zorin',
      CompanyName: 'NPCAUTOMATORS',
      LegalCopyright: 'Copyright (c) 2026 NPCAUTOMATORS'
    }
  })

  console.log('[set-exe-identity] done — Zorin icon + identity stamped')
}
