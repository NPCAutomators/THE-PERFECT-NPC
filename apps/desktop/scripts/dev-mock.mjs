...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `zorin-dev-mock-${Date.now()}`))
  const zorinHome = path.join(root, 'zorin-home')
  const userDataDir = path.join(root, 'electron-user-data')
  fs.mkdirSync(zorinHome, { recursive: true })
  fs.mkdirSync(userDataDir, { recursive: true })
  return { root, zorinHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(zorinHome, mockUrl) {
  fs.writeFileSync(
    path.join(zorinHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(zorinHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.zorinHome, mock.url)
  console.log(`  ZORIN_HOME: ${sandbox.zorinHome}`)

  const electronBin = findElectron()

  const env = {
    ...process.env,
    ZORIN_HOME: sandbox.zorinHome,
    ZORIN_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ZORIN_DESKTOP_IGNORE_EXISTING: '1',
    ZORIN_DESKTOP_ZORIN_ROOT: REPO_ROOT,
    ZORIN_DESKTOP_APP_NAME: `ZorinDevMock-${Date.now()}`,
  }
