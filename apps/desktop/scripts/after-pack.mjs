 ...
 *
 * Stamps the Zorin icon + identity onto the packed Windows Zorin.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `zorin desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Zorin')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Zorin'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Zorin.exe keeps the stock Electron icon`)
  }
}
