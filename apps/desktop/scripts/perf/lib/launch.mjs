...
//   · its own ZORIN_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · ZORIN_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated ZORIN_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate ZORIN_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = zorinHome ?? mkTemp('zorin-perf-home-')
  const userData = userDataDir ?? mkTemp('zorin-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !zorinHome) {
    seedConfigFrom(join(homedir(), '.zorin'), home)
  }
  ...
  try {
    ...
    // ZORIN_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set ZORIN_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    if (devUrl) {
      env.ZORIN_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'zorin-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'zorin-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.zorin'), home)
