/**
 * Shared E2E fixtures for the Zorin desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → zorin serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `zorin desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `zorin-e2e-${prefix}-${Math.random()}`))
 ...
 */
export function writeMockProviderConfig(zorinHome: string, mockUrl: string): void {
  const configPath = path.join(zorinHome, 'config.yaml')
 ...
 */
export function writeEnvFile(zorinHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(zorinHome, '.env')
 ...
 */
function writeEmptyConfig(zorinHome: string): void {
  const configPath = path.join(zorinHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ZORIN_HOME → sandbox zorin-home (isolated config/sessions)
 *  - ZORIN_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ZORIN_DESKTOP_IGNORE_EXISTING=1 → don't pick up `zorin` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ZORIN_DESKTOP_ZORIN_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ZORIN_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...
    ZORIN_DESKTOP_APP_NAME: `ZorinE2E-${Date.now()}`,
 ...
 *
 * @param sandbox  - isolated ZORIN_HOME + userData
 * @param env      - the process environment (already has ZORIN_HOME etc.)
...
export async function setupMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.zorinHome, mock.url)
  writeEnvFile(sandbox.zorinHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.zorinHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ZORIN_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`zorin serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.zorinHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ZORIN_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Zorin backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Zorin.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Zorin.app', 'Contents', 'MacOS', 'Zorin')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'zorin')
}
 ...
 * progress without spawning a real Zorin backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ZORIN_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ZORIN_DESKTOP_BOOT_FAKE: '1',
    ZORIN_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + zorin-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ZORIN_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ZORIN_DESKTOP_ZORIN
  delete (env as Record<string, string | undefined>).ZORIN_DESKTOP_ZORIN_ROOT
