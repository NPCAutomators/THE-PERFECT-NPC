 ...
 * packaged binary) with a real `zorin serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → zorin serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Zorin title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Zorin')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → zorin serve is spawned → WS connects → config
