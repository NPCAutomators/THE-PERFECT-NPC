...
How to build Zorin Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Zorin surface can also
...
- A **runtime home change** (switching the underlying `ZORIN_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Zorin — do not assume
