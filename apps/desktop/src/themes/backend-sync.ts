/**
 * Live skin sync from the Zorin backend.
 *
 * The backend resolves the active skin (built-in or `$ZORIN_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Zorin
 ...
 */
export function ingestBackendSkin(skin: ZorinSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (npcautomators), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → npcautomators). So we only
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as ZorinSkin)
