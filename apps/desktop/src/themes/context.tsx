...
import { BUILTIN_THEME_LIST, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, npcAutomatorsTheme } from './presets'
...
const SKIN_KEY = 'zorin-desktop-theme-v2'
const MODE_KEY = 'zorin-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'zorin-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'zorin-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'zorin-desktop-active-profile-v1'
const RETIRED_SKINS = new Set(['npcautomators-light', 'default', 'gold'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? npcAutomatorsTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...npcAutomatorsTheme.typography, ...theme.typography }
  ...
  root.dataset.zorinTheme = skinName
  root.dataset.zorinMode = rendered
  ...
  window.zorinDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('zorin-boot-background', chromeBg)
    window.localStorage.setItem('zorin-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.zorinThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.zorinDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Zorin authoring/activating a skin from a
