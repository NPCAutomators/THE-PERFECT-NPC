...
const globalOptions = { model: 'zorin-4', provider: 'npcautomators', providers: [] }
...
describe('manualPickRemoved', () => {
  const providers = [
    ...
    { name: 'NpcAutomators', slug: 'npcautomators', models: [] } // present but unconfigured / re-auth
  ]
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'npcautomators', 'zorin-4')).toBe(false)
  })
