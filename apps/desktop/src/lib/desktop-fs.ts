...
function connectionCacheKey(connection: ZorinConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
function bridge() {
  ...
  if (!desktop) {
    throw new Error('Zorin Desktop bridge is unavailable')
  }
...
export async function readDesktopDir(path: string): Promise<ZorinReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<ZorinReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<ZorinReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<ZorinReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: ZorinSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
