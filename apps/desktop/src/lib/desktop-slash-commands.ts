...
/** Why a known Zorin command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Zorin profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show ZORIN version', surface: exec() },
...
function isKnownZorinSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Zorin' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownZorinSlashCommand(normalized)
}
