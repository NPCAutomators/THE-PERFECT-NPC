...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'zorin:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.zorinDesktop?.saveImageFromUrl) {
        if (await window.zorinDesktop.saveImageFromUrl(src)) {
