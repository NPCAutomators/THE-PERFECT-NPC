import type { GatewayWsUrlResult } from '@zorin/shared'
...
declare global {
  interface Window {
    zorinDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ZorinConnection>
      ...
      // Zorin Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new ZORIN_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ZorinApiRequest) => Promise<T>
      notify: (payload: ZorinNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<ZorinReadFileTextResult>
      selectPaths: (options?: ZorinSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ZorinPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ZorinPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: ZorinTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ZorinReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ZorinGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ZorinGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<ZorinGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ZorinRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ZorinReviewScope, baseRef?: null | string) => Promise<ZorinReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ZorinReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ZorinReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ZorinTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ZorinTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ZorinWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ZorinPreviewFileChanged) => void) => () => void
...
export interface ZorinTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ZorinTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `zorin update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  zorinRoot?: string
...
export interface ZorinConnection {
  baseUrl: string
...
export interface ZorinTitleBarTheme {
  background: string
  foreground: string
}

export interface ZorinWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Zorin Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Zorin Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Zorin Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // Whether the OAuth partition holds a live NpcAutomators portal (Privy) session — the
  // portal authenticates via Privy, so this reflects the privy-token cookie, NOT
  // the zorin gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Zorin Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface ZorinApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface ZorinNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface ZorinPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ZorinReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ZorinPreviewWatch {
  id: string
  path: string
}
...
export interface ZorinGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ZorinGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface ZorinGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface ZorinRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ZorinRepoStatus {
  branch: null | string
...
// turn began.
export type ZorinReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ZorinReviewFile {
  path: string
  added: number
  removed: number
...
export interface ZorinReviewList {
  files: ZorinReviewFile[]
...
export interface ZorinReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ZorinReviewShipInfo {
  ghReady: boolean
  pr: ZorinReviewPr | null
}

export interface ZorinReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ZorinReadDirResult {
  entries: ZorinReadDirEntry[]
  error?: string
}

export interface ZorinPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ZorinSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
