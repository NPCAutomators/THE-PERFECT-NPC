...
vi.mock('@/zorin', () => ({
  getZorinConfig: vi.fn(),
  getZorinConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'zorin.desktop.workspace-cwd'
...
const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getZorinConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getZorinConfig>>)

describe('useZorinConfig refreshZorinConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshZorinConfig()
    })
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getZorinConfig>>>()
    vi.mocked(getZorinConfig).mockReturnValueOnce(profileConfig.promise)
    ...
    act(() => {
      pendingRefresh = result.current.refreshZorinConfig(true)
    })
    expect(getZorinConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getZorinConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getZorinConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getZorinConfig>>>()
    vi.mocked(getZorinConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshZorinConfig(true)
      refreshC = result.current.refreshZorinConfig(true)
    })
