...
export function useZorinConfig({ activeSessionIdRef, refreshProjectBranch }: ZorinConfigOptions) {
  ...
  const refreshZorinConfig = useCallback(
    async (force = false) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getZorinConfig(), getZorinConfigDefaults().catch(() => ({}))])
  ...
  return { refreshZorinConfig, sttEnabled, voiceMaxRecordingSeconds }
}
