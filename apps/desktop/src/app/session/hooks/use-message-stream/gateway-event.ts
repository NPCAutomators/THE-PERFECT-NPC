...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshZorinConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: ZorinSkin } | undefined)?.skin, { apply: false })

        return
      } else if (event.type === 'skin.changed') {
        // A runtime skin switch (Zorin activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as ZorinSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshZorinConfig applies is either active-session
        ...
        const errorMessage = payload?.message || 'Zorin reported an error'
