...
import { getSessionMessages, triggerCronJob } from '@/zorin'
...
import { useZorinConfig } from '../session/hooks/use-zorin-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshZorinConfig, sttEnabled, voiceMaxRecordingSeconds } = useZorinConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    ...
    window.addEventListener('zorin:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('zorin:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshZorinConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshZorinConfig])
