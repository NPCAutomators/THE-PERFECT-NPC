...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.zorinDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ZorinGateway | null) => void
  refreshZorinConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ZorinConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Zorin...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Zorin…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Zorin gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshZorinConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshZorinConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startZorin re-emits progress — ignore so the
    ...
    const gateway = new ZorinGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshZorinConfig(),
          ...
          // retry it), not brick boot behind the "Zorin couldn't start"
