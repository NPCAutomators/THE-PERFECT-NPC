...
// bug that drives the REAL useGatewayBoot hook + REAL ZorinGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshZorinConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { zorinDesktop?: unknown }).zorinDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { zorinDesktop?: unknown }).zorinDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForZorin) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startZorin()'s remote branch awaits waitForZorin() for 45s before it
    ...
    ;(window as { zorinDesktop?: unknown }).zorinDesktop = desktop
    ...
    // After ~45s waitForZorin gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Zorin backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Zorin couldn't start" overlay — the socket is open
