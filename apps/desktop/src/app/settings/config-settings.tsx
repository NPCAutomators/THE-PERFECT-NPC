...
import type { ConfigFieldSchema, ZorinConfigRecord } from '@/types/zorin'
...
export function voiceFieldVisible(key: string, config: ZorinConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ZorinConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useZorinConfigRecord()
  ...
  const {
    ...
    queryKey: ['zorin-config-schema'],
    queryFn: getZorinConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveZorinConfig(config)
  ...
  const updateConfig = (next: ZorinConfigRecord) => {
