 ...
 * server-computed `status` (keys ∧ NpcAutomators entitlement ∧ post-setup install
 ...
 * log inline — the GUI equivalent of the install step `zorin tools` runs
...
/**
 * Backend model catalog — the GUI counterpart of the model picker `zorin
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange }: ToolsetConfigPanelProps) {
  ...
  // Guard the NPCAUTOMATORS Portal sign-in poll loop against unmount/state updates.
  ...
  // panel highlighted the first keyless provider (e.g. NPCAUTOMATORS Portal) even when
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      ...
      if (result.needs_npcautomators_auth) {
        // Managed NpcAutomators row selected without Portal entitlement: the config
        ...
        // sign-in action). Reuses the existing NPCAUTOMATORS Portal device-code flow.
        notify({
          kind: 'warning',
          title: copy.npcAutomatorsAuthNeededTitle,
          message: copy.npcAutomatorsAuthNeededMessage(provider.name),
          action: { label: copy.npcAutomatorsAuthSignIn, onClick: () => void signInToNpcAutomatorsPortal() }
        })
  ...
  // Drive the existing NPCAUTOMATORS Portal OAuth device-code flow (the same session
  ...
  async function signInToNpcAutomatorsPortal() {
    try {
      const start = await startOAuthLogin('npcautomators')

      if (start.flow !== 'device_code') {
        notifyError(new Error(`unexpected flow: ${start.flow}`), copy.npcAutomatorsAuthFailed)
      ...
      if (window.zorinDesktop?.openExternal) {
        try {
          await window.zorinDesktop.openExternal(url)
        } catch {
      ...
      for (let attempt = 0; attempt < 120 && mountedRef.current; attempt += 1) {
        ...
        if (polled.status !== 'pending') {
          notifyError(new Error(polled.error_message || `Sign-in ${polled.status}`), copy.npcAutomatorsAuthFailed)
      ...
      if (mountedRef.current) {
        notifyError(err, copy.npcAutomatorsAuthFailed)
      }
    }
  }
          ...
          <div className="overflow-hidden rounded-xl bg-background/60" key={provider.name}>
            ...
            {isActive && (
              <div className="grid gap-2 bg-muted/20 p-3">
                ...
                {provider.requires_npcautomators_auth && (
                  <p className="text-[0.72rem] text-muted-foreground">{copy.npcAutomatorsIncluded}</p>
                )}
