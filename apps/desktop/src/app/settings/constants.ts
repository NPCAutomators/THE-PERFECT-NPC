...
export const EMPTY_SELECT_VALUE = '__zorin_empty__'
export const CONTROL_TEXT = 'text-xs'

export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NPCAUTOMATORS_',
    name: 'NPCAUTOMATORS Portal',
    description: 'Hosted Zorin & NpcAutomators-trained models',
    docsUrl: 'https://portal.npcautomators.com',
    priority: 0
  },
  {
    prefix: 'FIREWORKS_',
    ...
    // Slot #2 — mirrors CANONICAL_PROVIDERS (after NpcAutomators, ahead of OpenRouter).
  ...
  { prefix: 'ZORIN_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // zorin_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Zorin needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Zorin stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Zorin can read from one file request.',
  approvals: {
    mode: 'How Zorin handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Zorin updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
