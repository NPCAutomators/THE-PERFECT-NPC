...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'npcautomators', model: 'zorin-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'NpcAutomators',
        slug: 'npcautomators',
        models: ['zorin-4', 'zorin-4-mini'],
        authenticated: true,
        capabilities: { 'zorin-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'npcautomators', model: 'zorin-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ provider: 'npcautomators', model: 'zorin-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'npcautomators', model: 'zorin-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getZorinConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
...
describe('ModelSettings', () => {
  it('loads the current main model and lists configured providers only', async () => {
    ...
    // "NpcAutomators" shows in both the trigger and the open list.
    expect((await screen.findAllByText('NpcAutomators')).length).toBeGreaterThan(0)
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'npcautomators', model: 'zorin-4' })
    ...
    await waitFor(() => expect(screen.getAllByRole('combobox')[0].textContent).toContain('NpcAutomators'))
    expect(screen.queryByRole('button', { name: 'Set up provider' })).toBeNull()
  })

  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getZorinConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveZorinConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['zorin-4'],
          authenticated: true,
          capabilities: { 'zorin-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'npcautomators', model: 'zorin-4' }]
    })
    ...
    expect(screen.getByText('npcautomators')).toBeTruthy()
  })
...
describe('ModelSettings MoA preset editor', () => {
  const moaConfig = () => ({
    ...
    presets: {
      default: {
        reference_models: [
          { provider: 'npcautomators', model: 'zorin-4' },
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      fireEvent.click(slotSelects().ref1Provider)
      fireEvent.click(await screen.findByRole('option', { name: 'NpcAutomators' }))
      await vi.advanceTimersByTimeAsync(700)

      // Radix treats re-picking the current value as a no-op (no
      // onValueChange), so nothing changes: no save, model still shown.
      expect(saveMoaModels).not.toHaveBeenCalled()
      expect(screen.getByText('npcautomators · zorin-4')).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  })
