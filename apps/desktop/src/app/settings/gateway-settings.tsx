...
// Zorin Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.zorinDesktop
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.zorinDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.zorinDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    setLastTest(null)
  }, [scope, state.mode, state.sshHost, state.sshUser, state.sshPort, state.sshKeyPath, state.sshRemoteZorinPath])
  ...
  const payload = () => ({
    ...
    sshRemoteZorinPath: state.sshRemoteZorinPath.trim()
  })
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.zorinDesktop.applyConnectionConfig(payload())
        : await window.zorinDesktop.saveConnectionConfig(payload())
      ...
      const errors = {
        ...
        'zorin-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.zorinDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.zorinDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.zorinDesktop.getConnectionConfig(scope)
  ...
  const signOut = async () => {
    ...
    try {
      await window.zorinDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.zorinDesktop.getConnectionConfig(scope)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.zorinDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.zorinDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.zorinDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.zorinDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.zorinDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Zorin Cloud panel: one portal sign-in, then a discovered-agent picker
                ...
                {cloudDiscover === 'loading' ? (
                  ...
                  <div className="flex items-start gap-2 py-3 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                    ...
                    <span>
                      ...
                      <ExternalLink href="https://portal.npcautomators.com/agents" showExternalIcon={false}>
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/zorin"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteZorinPath: event.target.value }))}
                placeholder={g.sshZorinPathPlaceholder}
                value={state.sshRemoteZorinPath}
              />
            }
            description={g.sshZorinPathDesc}
            title={g.sshZorinPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.zorinDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
