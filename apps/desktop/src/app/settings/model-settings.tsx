...
import { invalidateZorinConfig, setZorinConfigCache, useZorinConfigRecord } from '../hooks/use-config-record'
...
// Zorin' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Zorin default (medium), shown as Medium.
...
// backend now surfaces the full `zorin model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in zorin_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useZorinConfigRecord()
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `zorin model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Zorin runs the flow for you.`}
          </p>
        )}
