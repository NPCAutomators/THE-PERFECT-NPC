...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `zorin auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'npcautomators' ? 'NPCAUTOMATORS Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
beforeEach(() => {
  ...
  disconnectOAuthProvider.mockResolvedValue({ ok: true, provider: 'npcautomators' })
  listOAuthProviders.mockResolvedValue({
    providers: [provider('npcautomators', true), provider('minimax-oauth', false)]
  })
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove NPCAUTOMATORS Portal' })
    await act(async () => {
      fireEvent.click(remove)
    })

    await waitFor(() => expect(disconnectOAuthProvider).toHaveBeenCalledWith('npcautomators'))
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByText('NPCAUTOMATORS Portal'))
    })

    expect(startManualProviderOAuth).toHaveBeenCalledWith('npcautomators')
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          ...
          disconnect_hint: "Use `zorin auth add qwen-oauth` or that provider's CLI to remove it.",
