...
import { disconnectOAuthProvider, listOAuthProviders } from '@/zorin'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.zorinDesktop?.terminal)
...
//      in zorin_cli/provider_catalog.py) — the SAME provider identity
//      `zorin model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Zorin can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Zorin can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Zorin never deletes creds another tool owns behind a silent API call.
