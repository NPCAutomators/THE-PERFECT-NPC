...
const getZorinConfigRecord = vi.fn()
const getZorinConfigSchema = vi.fn()
const saveZorinConfig = vi.fn()
...
vi.mock('@/zorin', () => ({
  ...
  getZorinConfigRecord: () => getZorinConfigRecord(),
  getZorinConfigSchema: () => getZorinConfigSchema(),
  saveZorinConfig: (config: unknown) => saveZorinConfig(config),
    ...
    providers: [
      {
        ...
        env_vars: [],
        post_setup: null,
        requires_npcautomators_auth: false,
        is_active: false
      },
      {
        name: 'ElevenLabs',
...
beforeEach(() => {
  ...
  getZorinConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getZorinConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
  saveZorinConfig.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveZorinConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveZorinConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a logged-out NpcAutomators Subscription row and a
  ...
  describe('managed NPCAUTOMATORS provider activation', () => {
    const npcAutomatorsBrowserConfig = () =>
      config({
        name: 'browser',
        active_provider: null,
        providers: [
    ...
    it('surfaces a sign-in notice when the PUT reports needs_npcautomators_auth', async () => {
      ...
      // reports needs_npcautomators_auth and the panel must surface a sign-in action
      // instead of the misleading "provider selected" success toast.
      const { notify } = await import('@/store/notifications')

      getToolsetConfig.mockResolvedValue(npcAutomatorsBrowserConfig())
      selectToolsetProvider.mockResolvedValue({
        ok: true,
        name: 'browser',
        provider: 'NpcAutomators Subscription (Browser Use cloud)',
        needs_npcautomators_auth: true,
        feature: 'browser'
      })
      ...
      fireEvent.click(await screen.findByRole('button', { name: /NpcAutomators Subscription/ }))

      await waitFor(() =>
        expect(selectToolsetProvider).toHaveBeenCalledWith('browser', 'NpcAutomators Subscription (Browser Use cloud)')
      )
      await waitFor(() =>
    ...
    it('drives the existing NpcAutomators OAuth device-code flow from the sign-in action and refetches', async () => {
      const { notify } = await import('@/store/notifications')
      ...
      startOAuthLogin.mockResolvedValue({
        ...
        verification_url: 'https://portal.npcautomators.com/device?user_code=NPCAUTOMATORS-1234',
        poll_interval: 5,
        expires_in: 600
      })
      ...
      try {
        ...
        await waitFor(() => expect(startOAuthLogin).toHaveBeenCalledWith('npcautomators'))
        ...
        await waitFor(() => expect(pollOAuthSession).toHaveBeenCalledWith('npcautomators', 'sess-1'), { timeout: 8000 })
