...
import type { ZorinGitBranch } from '@/global'
...
const branchActionLabel = (branch: ZorinGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<ZorinGitBranch[]>([])
  ...
  const convert = async (branch: ZorinGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
