...
async function readTextPreview(filePath: string) {
  try {
    ...
    if (!message.includes("No handler registered for 'zorin:readFileText'")) {
      throw error
    }
  }
  ...
  const dataUrl = await window.zorinDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(ZORIN_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
