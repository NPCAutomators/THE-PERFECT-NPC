        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.zorinDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Zorin to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.zorinDesktop?.watchPreviewFile ||
      !window.zorinDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.zorinDesktop.onPreviewFileChanged(payload => {
    ...
    void window.zorinDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.zorinDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.zorinDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:zorin-preview')
