...
function ignoredBy(rules: GitignoreRule[], entry: ZorinReadDirEntry) {
...
async function filterIgnored(entries: ZorinReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ZorinReadDirResult> {
  if (!window.zorinDesktop) {
