...
const readDir = vi.fn<(path: string) => Promise<ZorinReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { zorinDesktop: { readDir: typeof readDir } }).zorinDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ZorinReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { zorinDesktop: unknown }).zorinDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ZorinReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ZorinReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { zorinDesktop: unknown }).zorinDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.zorinDesktop is missing', async () => {
    delete (window as unknown as { zorinDesktop?: unknown }).zorinDesktop
