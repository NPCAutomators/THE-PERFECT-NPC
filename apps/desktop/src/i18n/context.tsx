...
export interface I18nConfigClient {
  getConfig: () => Promise<ZorinConfigRecord>
  saveConfig: (config: ZorinConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.zorinDesktop?.api) {
      return Promise.resolve({})
    }

    return getZorinConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.zorinDesktop?.api) {
...
export function getConfigDisplayLanguage(config: ZorinConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ZorinConfigRecord, locale: Locale): ZorinConfigRecord {
