...
import { formatRefValue, zorinDirectiveFormatter } from './directive-text'
...
describe('zorinDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = zorinDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = zorinDirectiveFormatter.parse('@file:src/main.tsx the entry point')
