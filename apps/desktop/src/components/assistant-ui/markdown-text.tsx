...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.zorinDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.zorinDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.zorinDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.zorinDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.zorinDesktop && text && normalizeExternalUrl(text) === target) {
