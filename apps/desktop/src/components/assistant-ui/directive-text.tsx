...
const ICON_PATHS: Record<ZorinRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ZorinRefType] ?? ICON_FALLBACK
...
const ZORIN_DIRECTIVE_RE = new RegExp(
...
export const zorinDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ZORIN_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ZorinRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ZorinRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...zorinDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Zorin directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.zorinDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.zorinDesktop?.readFileDataUrl(id)
