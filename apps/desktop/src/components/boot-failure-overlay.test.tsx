...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Zorin gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.zorinDesktop
  Object.defineProperty(window, 'zorinDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'zorinDesktop', { configurable: true, value: original })
}
