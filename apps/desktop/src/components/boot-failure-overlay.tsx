...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.zorinDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.zorinDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.zorinDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }
  ...
  const signInRemote = async () => {
    ...
    try {
      await window.zorinDesktop?.oauthLogoutConnectionConfig?.()
      const result = await window.zorinDesktop?.oauthLoginConnectionConfig(remoteReauth.url)
  ...
  const openLogs = () => void window.zorinDesktop?.revealLogs().catch(() => undefined)
  ...
  if (view === 'connect') {
    return (
      <div className="fixed inset-0 z-[1400] flex items-center justify-center bg-(--ui-chat-surface-background) p-6">
        <div className="flex max-h-[86vh] w-full max-w-[46rem] flex-col overflow-hidden rounded-xl border border-(--stroke-npcautomators) bg-(--ui-chat-bubble-background) shadow-npcautomators">
  ...
  return (
    <div className="fixed inset-0 z-[1400] flex items-center justify-center bg-(--ui-chat-surface-background) p-6">
      <div className="w-full max-w-[40rem] overflow-hidden rounded-xl border border-(--stroke-npcautomators) bg-(--ui-chat-bubble-background) shadow-npcautomators">
