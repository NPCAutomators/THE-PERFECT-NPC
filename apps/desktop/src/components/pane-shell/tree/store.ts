...
const STORAGE_KEY = 'zorin.desktop.layoutTree.v2'

writeKey('zorin.desktop.layoutTree.v1', null)
 ...
 */
export const $activePresetId = atom<string>(readKey('zorin.desktop.layoutPreset.active') ?? 'default')

export function markActivePreset(id: string) {
  $activePresetId.set(id)
  writeKey('zorin.desktop.layoutPreset.active', id)
}
 ...
 */
const DISMISSED_KEY = 'zorin.desktop.dismissedPanes.v1'
...
const USER_PLACED_KEY = 'zorin.desktop.userPlacedPanes.v1'
...
if (import.meta.env.DEV && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__ZORIN_LAYOUT_TREE__ = {
    close: closeTreePane,
