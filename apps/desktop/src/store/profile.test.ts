...
const { getProfiles } = await import('@/zorin')
...
const remoteConn = (over: Partial<ZorinConnection> = {}): ZorinConnection =>
  ({ baseUrl: 'https://zorin-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ZorinConnection

const localConn = (over: Partial<ZorinConnection> = {}): ZorinConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ZorinConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ZorinConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { zorinDesktop: { getConnection } })
