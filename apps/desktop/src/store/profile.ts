...
// /api/profiles/active `current`). "default" is the root ~/.zorin. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'zorin.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'zorin.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.zorinDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ZORIN_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.zorinDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.zorinDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'zorin.desktop.showAllProfiles'
...
// Switch to the default (root ~/.zorin) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.zorinDesktop?.touchBackend?.(target).catch(() => undefined)
}
