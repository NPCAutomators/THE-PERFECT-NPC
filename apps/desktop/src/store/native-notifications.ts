...
const STORAGE_KEY = 'zorin:native-notifications'
...
// "Backgrounded" = the user isn't on Zorin. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.zorinDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.zorinDesktop
