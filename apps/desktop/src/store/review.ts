...
//
// Scope is always "uncommitted" — Zorin' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'zorin.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'zorin.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'zorin.desktop.reviewTreeMode'
const SELECTED_KEY = 'zorin.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ZorinReviewFile[]>([])
...
export const $reviewShipInfo = atom<ZorinReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['zorinDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ZorinReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.zorinDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.zorinDesktop?.openExternal?.(url)
    }
