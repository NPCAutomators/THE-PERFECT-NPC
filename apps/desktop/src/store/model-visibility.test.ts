...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    // User explicitly hid all models for "npcautomators" — sentinel marks this choice.
    const stored = new Set([emptyProviderSentinelKey('npcautomators')])

    const visible = effectiveVisibleKeys(stored, [
      provider('npcautomators', ['zorin-3-llama-3.1-70b', 'zorin-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('npcautomators', 'zorin-3-llama-3.1-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('npcautomators', 'zorin-3-llama-3.1-8b'))).toBe(false)
    // Sentinel itself is stripped from the result.
    expect(visible.has(emptyProviderSentinelKey('npcautomators'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    // Simulates: user hid all "npcautomators" models, then toggles one back on.
    const stored = new Set([emptyProviderSentinelKey('npcautomators'), modelVisibilityKey('ollama', 'qwen3:latest')])

    // After toggle: sentinel removed, one model added.
    const afterToggle = new Set(stored)
    afterToggle.delete(emptyProviderSentinelKey('npcautomators'))
    afterToggle.add(modelVisibilityKey('npcautomators', 'zorin-3-llama-3.1-70b'))
    ...
    expect(visible.has(modelVisibilityKey('npcautomators', 'zorin-3-llama-3.1-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('npcautomators', 'zorin-3-llama-3.1-8b'))).toBe(false)
  })
  ...
  it('resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips', () => {
    const stored = new Set([emptyProviderSentinelKey('npcautomators')])
    const providers = [provider('npcautomators', ['zorin-x', 'zorin-y']), provider('ollama', ['qwen3:latest'])]

    const resolved = resolveVisibleKeys(stored, providers)
    expect(resolved.has(emptyProviderSentinelKey('npcautomators'))).toBe(true)
    expect(resolved.has(modelVisibilityKey('npcautomators', 'zorin-x'))).toBe(false)
    ...
    expect(effectiveVisibleKeys(stored, providers).has(emptyProviderSentinelKey('npcautomators'))).toBe(false)
  })
})

describe('toggleModelVisibility', () => {
  const providers = [provider('openai', ['gpt-a', 'gpt-b']), provider('npcautomators', ['zorin-x', 'zorin-y'])]
  ...
  it('keeps a hidden provider hidden when a different provider is toggled (regression for #43485)', () => {
    // Hide ALL of npcautomators — its sentinel is now stored.
    let stored: Set<string> | null = null
    stored = apply(stored, 'npcautomators', 'zorin-x')
    stored = apply(stored, 'npcautomators', 'zorin-y')
    expect(stored.has(emptyProviderSentinelKey('npcautomators'))).toBe(true)

    // Toggle a model in another provider. npcautomators must NOT snap back on.
    stored = apply(stored, 'openai', 'gpt-a')

    expect(stored.has(emptyProviderSentinelKey('npcautomators'))).toBe(true)
    const visible = effectiveVisibleKeys(stored, providers)
    expect(visible.has(modelVisibilityKey('npcautomators', 'zorin-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('npcautomators', 'zorin-y'))).toBe(false)
  })
  ...
  it('re-enabling one model of a hidden-all provider restores ONLY that model, not the curated defaults', () => {
    // openai hidden-all, npcautomators untouched.
  ...
  it('toggling from an empty (non-null) stored set adds the model without expanding defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('npcautomators', 'zorin-x'))).toBe(false)
  })
  ...
  it('toggling off one default model from null stored keeps the rest of the curated defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('npcautomators', 'zorin-x'))).toBe(true)
