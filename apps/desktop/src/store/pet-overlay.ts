 ...
 * app's bounds and stays visible while Zorin is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'zorin.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'zorin.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.zorinDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.zorinDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.zorinDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.zorinDesktop?.petOverlay?.close()
}
