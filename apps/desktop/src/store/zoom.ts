...
export function setZoomPercent(percent: number): void {
  window.zorinDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.zorinDesktop?.zoom) {
  void window.zorinDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.zorinDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
