...
// reaches the gateway); everything else routes through window.zorinDesktop.git,
...
function file(path: string, over: Partial<ZorinReviewFile> = {}): ZorinReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as ZorinReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.zorinDesktop. Any op not supplied defaults
...
function stubReview(over: ReviewStub = {}) {
  ...
  ;(window as unknown as { zorinDesktop?: unknown }).zorinDesktop = {
    git: { review },
    openExternal: vi.fn()
  }
...
afterEach(() => {
  delete (window as unknown as { zorinDesktop?: unknown }).zorinDesktop
})
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as ZorinReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.zorinDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: ZorinReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as ZorinReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { zorinDesktop?: unknown }).zorinDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as ZorinReviewShipInfo)
