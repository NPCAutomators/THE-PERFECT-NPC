...
const PROJECT_SCOPE_KEY = 'zorin.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Zorin gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active Zorin profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: ZorinGateway): Promise<void> {
...
const repoScanStates = new WeakMap<ZorinGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<ZorinGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getZorinConfig(context.profile))
...
// repo, returning where Zorin should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ZorinGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<ZorinGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.zorinDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.zorinDesktop?.writeClipboard?.(path)
  }
}
