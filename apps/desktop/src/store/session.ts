...
const COMPOSER_MODEL_KEY = 'zorin.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'zorin.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'zorin.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'zorin.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'zorin.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'zorin.desktop.lastSessionId'
...
const LAST_ROUTE_KEY = 'zorin.desktop.lastRoute'
...
function workspaceCwdKey(connection: ZorinConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.zorinDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.zorinDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ZorinConnection | null>(null)
...
export const setConnection = (next: Updater<ZorinConnection | null>) => updateAtom($connection, next)
