...
function installApiMock(api: (request: { path: string }) => Promise<unknown>) {
  Object.defineProperty(window, 'zorinDesktop', {
    configurable: true,
    value: { api }
  })
}
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('zorin-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('zorin-desktop-onboarded-v1')).toBe('1')
  })
...
describe('OAuth onboarding', () => {
  ...
  it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
    ...
    installApiMock(async ({ body, path }: { body?: unknown; path: string }) => {
      ...
      if (path === '/api/providers/oauth/npcautomators/submit') {
      ...
      if (path.startsWith('/api/model/recommended-default?')) {
        return { provider: 'npcautomators', model, free_tier: false }
      }

      if (path === '/api/model/set') {
        return { ok: true, provider: 'npcautomators', model, gateway_tools: [] }
      }
    ...
    const requestGateway: OnboardingContext['requestGateway'] = async (method, params) => {
      ...
      if (method === 'setup.runtime_check') {
        expect(params).toEqual({ provider: 'npcautomators' })
    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: provider('npcautomators', 'NPCAUTOMATORS Portal'),
          start: {
        ...
        reason:
          'No access token found for NPCAUTOMATORS Portal login. setup.status reports configured credentials, but runtime resolution still failed.',
        requested: true
      })
    )
    ...
    if (state.flow.status === 'confirming_model') {
      expect(state.flow.label).toBe('NPCAUTOMATORS Portal')
