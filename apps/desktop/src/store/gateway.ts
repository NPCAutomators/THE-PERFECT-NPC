...
const isOpen = (gateway: ZorinGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<ZorinGateway | null>(null)
...
let primaryGateway: ZorinGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: ZorinGateway | null, profile = 'default'): void {
...
export function activeGateway(): ZorinGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<ZorinGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
