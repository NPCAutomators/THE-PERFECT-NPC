...
const sampleStatus: ZorinRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<ZorinRepoStatus | null>) {
  ;(window as unknown as { zorinDesktop?: unknown }).zorinDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh', async () => {
    const resolvers: Array<(status: ZorinRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
