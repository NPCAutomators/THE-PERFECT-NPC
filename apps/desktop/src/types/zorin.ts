...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Zorin doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in zorin_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `zorin model` uses — instead of
...
export type ZorinConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `zorin model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** NpcAutomators only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** NpcAutomators only: paid models a free-tier user cannot select (shown disabled). */
...
// folders. Mirrors zorin_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Zorin, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `zorin tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ NpcAutomators entitlement ∧
 ...
 *  not Zorin); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/zorin/update/check` — the backend's own update state.
...
/** One NpcAutomators-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the NpcAutomators Tool Gateway as a result of
   *  switching the main provider to NpcAutomators. Empty unless provider === 'npcautomators'
