...
export const $zorinHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.zorin/logs/bootstrap-installer.log')
    $zorinHome.set('~/.zorin')
  ...
  try {
    const [logPath, zorinHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_zorin_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $zorinHome.set(zorinHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Zorin" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Zorin-Setup.exe --update);
...
export async function launchZorinDesktop(): Promise<void> {
  ...
  await invoke('launch_zorin_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Zorin repository' },
