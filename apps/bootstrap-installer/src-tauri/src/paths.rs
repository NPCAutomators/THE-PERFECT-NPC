...
//!
//! Mirrors `zorin_constants.get_zorin_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\zorin
//!   macOS:   ~/.zorin
//!   Linux:   ~/.zorin  (override via $ZORIN_HOME)
//!
//! NOTE (macOS): Python's get_zorin_home(), scripts/install.sh, and the
//! Electron desktop's resolveZorinHome() ALL use ~/.zorin on macOS — there
...
/// Returns the canonical Zorin home directory, respecting $ZORIN_HOME if set.
pub fn zorin_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ZORIN_HOME") {
    ...
    {
        // %LOCALAPPDATA%\zorin — matches scripts/install.ps1's $ZorinHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("zorin");
        }
    }

    // macOS + Linux + fallback: ~/.zorin (matches Python get_zorin_home(),
    // install.sh, and the Electron desktop's resolveZorinHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".zorin");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    zorin_home().join("bootstrap-cache")
}
...
/// ZORIN_HOME so it survives repo checkout deletion (unlike anything under
/// zorin-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\zorin\zorin-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_zorin`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ZORIN_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ZORIN_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    zorin_home().join(".zorin-update-in-progress")
}
...
pub fn copy_self_to_zorin_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ZORIN_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ZORIN_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".zorin-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ZORIN_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[zorin-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ZORIN_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_zorin_home() -> String {
    zorin_home().to_string_lossy().into_owned()
}
