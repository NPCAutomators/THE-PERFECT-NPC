...
//!
//! Driven when the installer is launched as `Zorin-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Zorin desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `zorin update`
//!      or repair bootstrap can race locked files),
//!   2. run `zorin update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in zorin_cli/main.py),
//!   3. run `zorin desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `zorin update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_zorin) and
...
/// `zorin update` exit code meaning "another zorin process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// zorin_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `zorin update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let zorin_home = crate::paths::zorin_home();
    let install_root = zorin_home.join("zorin-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_zorin`
    ...
    let zorin = resolve_zorin(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the zorin CLI under {}. Is Zorin installed? \
    ...
    // async on Windows. If it still holds the venv shim, `zorin update`
    ...
    // ---- stage 2: zorin update -----------------------------------------
    // Pass --branch so `zorin update` targets the branch this installer was
    ...
    // without --branch, `zorin update` switches the checkout to `main` (a
    ...
    // --force skips `zorin update`'s Windows running-exe guard (which would
    ...
    // time `zorin update` runs there is no legitimate zorin.exe to protect,
    // and the guard would only produce a false "Zorin is still running" stop.
    //
    ...
    // probe only checks the zorin.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Zorin windows" message.
    ...
    // Retry-once for the update-boundary crash. `zorin update` lazily imports
    ...
    // second `zorin update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Zorin" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Zorin is still running. Close all Zorin windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "zorin update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::zorin_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: zorin desktop --build-only ----------------------------
    // `zorin update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `zorin update` already does above, and `zorin update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `zorin desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Zorin manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_zorin_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Zorin to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend zorin.exe (or the desktop Zorin.exe
            ...
            // force-kill every Zorin.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Zorin still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_zorin();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_zorin(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Zorin.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Zorin.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `zorin.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `zorin.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\zorin.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startZorin()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// zorin.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named zorin.exe.)
fn force_kill_other_zorin() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `zorin <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv zorin shim under an install root, regardless of existence.
fn venv_zorin(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("zorin.exe")
    } else {
        install_root.join("venv").join("bin").join("zorin")
    }
}

/// Resolve the zorin CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `zorin` on PATH.
fn resolve_zorin(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_zorin(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "zorin.exe" } else { "zorin" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let zorin_home = crate::paths::zorin_home();
    let mut envs = vec![(
        "ZORIN_HOME".to_string(),
        zorin_home.as_os_str().to_os_string(),
    )];
    // `zorin update` is a Python CLI writing to a pipe here, so CPython
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_zorin_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Zorin.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.zorin-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.zorin-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::zorin_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_zorin_is_under_install_root() {
        let root = Path::new("/x/zorin-agent");
        let shim = venv_zorin(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/zorin-agent"));
        assert!(
            envs.iter()
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_zorin(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/zorin-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".zorin-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Zorin.app"]),
            Some(PathBuf::from("/Applications/Zorin.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Zorin.app");
        let tmp = base.join("Zorin.app.zorin-update-new");
        let old = base.join("Zorin.app.zorin-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Zorin.app");
        let tmp = base.join("Zorin.app.zorin-update-new"); // intentionally absent
        let old = base.join("Zorin.app.zorin-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Zorin.app");
        let tmp = base.join("Zorin.app.zorin-update-new"); // absent
        let old = base.join("Zorin.app.zorin-update-old");
        write_marker(&target, "OLD");
