...
//! ZORIN_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for ZORIN_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub zorin_home: Option<String>,
}
...
/// Spawn the locally-built Zorin desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_zorin_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_zorin_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Zorin desktop at {}. The desktop build step \
             may have been skipped or failed. Run `zorin desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Zorin desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Zorin
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_zorin_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Zorin.exe"),
            ("win-arm64-unpacked", "Zorin.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Zorin.app/Contents/MacOS", "Zorin"),
            ("mac-arm64/Zorin.app/Contents/MacOS", "Zorin"),
        ]
    } else {
        &[("linux-unpacked", "zorin")]
    };
...
pub(crate) fn resolve_zorin_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_zorin_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Zorin.app/Contents/MacOS/Zorin -> .../Zorin.app
...
/// path so a bare re-open just opens Zorin instead of re-running setup.
pub(crate) fn zorin_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".zorin-bootstrap-complete").exists()
        && resolve_zorin_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_zorin_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Zorin desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_zorin_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::zorin_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $ZorinHome which Stage-Repository clones
    // the repo INTO at $ZorinHome\zorin-agent. Mirrors zorin_constants.
    let zorin_home = args
        .zorin_home
        .clone()
        .unwrap_or_else(|| crate::paths::zorin_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&zorin_home).join("zorin-agent");

    // Copy ourselves to ZORIN_HOME/zorin-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_zorin_home() {
        tracing::warn!(?err, "failed to copy installer into ZORIN_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    zorin_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, zorin_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Zorin"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Zorin.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Zorin.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("zorin");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Zorin.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_zorin_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_zorin_desktop_app(&root)
    ...
    #[test]
    fn resolve_zorin_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_zorin_desktop_app(&root).is_none(),
