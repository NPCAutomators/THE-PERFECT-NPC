...
_ZORIN_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on ZORIN_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (ZORIN_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `zorin tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ZORIN_KANBAN_TASK env "
    ...
    # Coding posture (base Zorin — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in zorin_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Zorin toolsets (CLI + messaging platforms)
    #
    ...
    # the `zorin send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "zorin-cli": {
        ...
        "tools": _ZORIN_CORE_TOOLS,
        "includes": []
    },

    "zorin-cron": {
        # Mirrors zorin-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `zorin tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as zorin-cli; gated by `zorin tools`",
        "tools": _ZORIN_CORE_TOOLS,
        "includes": []
    },
    ...
    "zorin-email": {
        "description": "Email bot toolset - interact with Zorin via email (IMAP/SMTP)",
        "tools": _ZORIN_CORE_TOOLS,
        "includes": []
    },
    ...
    "zorin-sms": {
        "description": "SMS bot toolset - interact with Zorin via SMS (Twilio)",
        "tools": _ZORIN_CORE_TOOLS,
        "includes": []
    },
    ...
    "zorin-webhook": {
        ...
        "tools": _ZORIN_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "zorin-gateway": {
        ...
        "tools": [],
        "includes": ["zorin-telegram", "zorin-discord", "zorin-whatsapp", "zorin-slack", "zorin-signal", "zorin-bluebubbles", "zorin-homeassistant", "zorin-email", "zorin-sms", "zorin-mattermost", "zorin-matrix", "zorin-dingtalk", "zorin-feishu", "zorin-wecom", "zorin-wecom-callback", "zorin-weixin", "zorin-qqbot", "zorin-webhook", "zorin-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``zorin-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ZORIN_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``zorin-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (zorin-<name>).
        # Gives them _ZORIN_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("zorin-"):
            platform_name = name[len("zorin-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ZORIN_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
