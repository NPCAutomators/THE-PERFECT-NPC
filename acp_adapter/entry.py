"""CLI entry point for the zorin-agent ACP adapter.

Loads environment variables from ``~/.zorin/.env``, configures logging
...
try:
    ...
    # shadowing Zorin's own modules — ``zorin acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    zorin_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from ZORIN_HOME (default ``~/.zorin``)."""
    from zorin_cli.env_loader import load_zorin_dotenv

    zorin_home = get_zorin_home()
    loaded = load_zorin_dotenv(zorin_home=zorin_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", zorin_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="zorin-acp",
        description="Run ZORIN as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Zorin version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Zorin provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.zorin/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import ZorinACPAgent  # noqa: F401

    print("Zorin ACP check OK")


def _run_setup() -> None:
    from zorin_cli.main import main as zorin_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "zorin", "model"]
        zorin_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``zorin postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting zorin-agent ACP adapter")
    ...
    import acp
    from .server import ZorinACPAgent
