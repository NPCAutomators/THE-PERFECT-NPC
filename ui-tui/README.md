...
React + Ink terminal UI for Zorin. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ZORIN_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `zorin setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage NpcAutomators remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.zorin/.zorin_history` or under `ZORIN_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage NpcAutomators remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — NpcAutomators credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `zorin setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/zorin-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external zorin setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage NpcAutomators remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.zorin/tui-parent.log
    ...
    types/
      zorin-ink.d.ts            type declarations for @zorin/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ZorinCLI subprocess for slash commands
```
