import type { SkinBranding, SkinColors } from '@zorin/shared/skin'
...
// Mirrors @zorin/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @zorin/ink is bundled separately from packages/.
 ...
 *  answer when known (cached in ZORIN_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.ZORIN_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit ZORIN_TUI_THEME / COLORFGBG signals above still win,
...
// `ZORIN_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `ZORIN_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `ZORIN_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `ZORIN_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Zorin palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.ZORIN_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.ZORIN_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.ZORIN_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into ZORIN_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into ZORIN_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground.
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Zorin navy
  ...
  // the derived value already carries the identity seeds (e.g. Zorin navy).
  const activeRow =
