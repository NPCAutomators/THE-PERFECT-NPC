import type { RunExternalProcess } from '@zorin/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`zorin ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching zorin: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`zorin ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
