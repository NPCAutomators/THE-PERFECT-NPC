...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@zorin/ink'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.ZORIN_TUI_THEME : undefined

  writeBootTheme(theme, process.env.ZORIN_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
...
const applySkin = (s: GatewaySkin) => {
  ...
  // terminal default), so Zorin owns its background instead of inheriting it.
 ...
 * to ZORIN_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit ZORIN_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the ZORIN_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.ZORIN_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.ZORIN_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a ZORIN_TUI_THEME the user
    ...
    delete process.env.ZORIN_TUI_THEME
  }
 ...
 * is cached into ZORIN_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * ZORIN_TUI_LIGHT / ZORIN_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.ZORIN_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.ZORIN_TUI_THEME || process.env.ZORIN_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.ZORIN_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.ZORIN_TUI_BACKGROUND ||
      process.env.ZORIN_TUI_THEME ||
      process.env.ZORIN_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.ZORIN_TUI_BACKGROUND || process.env.ZORIN_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.ZORIN_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // forging a brand-new one.  Mirrors classic CLI's `zorin -c` /
    // `zorin --tui` muscle memory and addresses the audit's "session
