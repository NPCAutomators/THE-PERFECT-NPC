...
import { terminalBackgroundHex } from '@zorin/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ZORIN_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ZORIN_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ZORIN_TUI_BACKGROUND', process.env.ZORIN_TUI_BACKGROUND ?? '(unset)'],
            ['ZORIN_TUI_THEME', process.env.ZORIN_TUI_THEME ?? '(unset)'],
