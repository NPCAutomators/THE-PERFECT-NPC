...
const logDir = join(process.env.ZORIN_HOME?.trim() || join(homedir(), '.zorin'), 'logs')
...
// a real ~/.zorin (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('zorin-tui: parent lifecycle log unavailable\n')
    }
  }
}
