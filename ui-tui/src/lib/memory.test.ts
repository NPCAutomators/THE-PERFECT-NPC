...
const ENV_KEYS = ['ZORIN_AUTO_HEAPDUMP', 'ZORIN_HEAPDUMP_DIR', 'ZORIN_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'zorin-heapdump-test-'))
    process.env.ZORIN_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without ZORIN_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without ZORIN_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when ZORIN_AUTO_HEAPDUMP=1', async () => {
    process.env.ZORIN_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.ZORIN_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of ZORIN_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.ZORIN_HEAPDUMP_DIR
    savedMax = process.env.ZORIN_HEAPDUMP_MAX_BYTES
    delete process.env.ZORIN_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'zorin-heapdump-prune-'))
    process.env.ZORIN_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.ZORIN_HEAPDUMP_DIR
    } else {
      process.env.ZORIN_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.ZORIN_HEAPDUMP_MAX_BYTES
    } else {
      process.env.ZORIN_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.ZORIN_HEAPDUMP_MAX_BYTES = String(2 * 1024)
