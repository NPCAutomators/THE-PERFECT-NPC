...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ZORIN_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Zorin on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ZORIN_TUI_TRUECOLOR=1.
