 ...
 * `zorin-boot-background` / `zorin-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ZORIN_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.zorin matches get_zorin_home()'s default.
const bootFilePath = () => join(process.env.ZORIN_HOME ?? join(homedir(), '.zorin'), 'tui-theme-boot.json')

// Never touch the user's real ~/.zorin from test runs (the TS suite has no
// ZORIN_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ZORIN_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ZORIN_TUI_THEME || env.ZORIN_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.ZORIN_TUI_BACKGROUND
  ) {
    env.ZORIN_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ZORIN_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ZORIN_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ZORIN_TUI_THEME.
