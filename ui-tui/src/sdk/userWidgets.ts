...
import { Box, Text } from '@zorin/ink'
...
/**
 * User widget apps — Zorin authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ZORIN_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.zorin/plugins/`: files under ZORIN_HOME execute
...
const widgetsDir = () => join(process.env.ZORIN_HOME?.trim() || join(homedir(), '.zorin'), 'tui-widgets')
 ...
 *  every change, so a widget Zorin writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Zorin) ever writes must hot-load too — a 10s poll here read
