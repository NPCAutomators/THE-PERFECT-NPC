...
from zorin_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from zorin_cli.config import load_config, save_config, save_env_value
from zorin_cli.npcautomators_subscription import NpcAutomatorsFeatureState, NpcAutomatorsSubscriptionFeatures
from zorin_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside zorin setup must survive the wizard's final save."""
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("zorin_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("zorin_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'zorin setup' to configure" not in output
    ...
    monkeypatch.setattr(
        ...
        lambda config: NpcAutomatorsSubscriptionFeatures(
            ...
            features={
                ...
                "browser": NpcAutomatorsFeatureState("browser", "Browser automation", True, True, True, False, True, True, "Camofox"),
                "modal": NpcAutomatorsFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
    ...
    monkeypatch.setattr(
        ...
        lambda config: NpcAutomatorsSubscriptionFeatures(
            ...
            features={
                ...
                "video_gen": NpcAutomatorsFeatureState("video_gen", "Video generation", False, False, False, False, False, False, ""),
                "tts": NpcAutomatorsFeatureState("tts", "OpenAI TTS", True, False, False, False, False, True, ""),
                "browser": NpcAutomatorsFeatureState("browser", "Browser automation", True, False, False, False, False, True, "Browserbase"),
                "modal": NpcAutomatorsFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    ``get_npcautomators_subscription_features`` so the surface stays aligned with the
    ...
    monkeypatch.setattr("zorin_cli.npcautomators_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(
