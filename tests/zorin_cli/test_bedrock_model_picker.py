...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ZORIN_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ZORIN_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ZORIN_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from zorin_cli.providers import ZORIN_OVERLAYS
        assert "bedrock" in ZORIN_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from zorin_cli.providers import ZORIN_OVERLAYS
        assert ZORIN_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from zorin_cli.providers import ZORIN_OVERLAYS
        assert ZORIN_OVERLAYS["bedrock"].auth_type == "aws_sdk"
