"""Tests for ``zorin debug`` CLI command and debug utilities."""
...
@pytest.fixture
def zorin_home(tmp_path, monkeypatch):
    """Set up an isolated ZORIN_HOME with minimal logs."""
    home = tmp_path / ".zorin"
    home.mkdir()
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO zorin_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from zorin_cli.debug import _upload_paste_rs
        ...
        with patch("zorin_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "zorin_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from zorin_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from zorin_cli.debug import upload_to_pastebin

        with patch("zorin_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("zorin_cli.debug._upload_paste_rs",
             ...
             patch("zorin_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, zorin_home):
        from zorin_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, zorin_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (zorin_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, zorin_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = zorin_home / "logs" / "agent.log"
        from zorin_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, zorin_home):
        ...
        (zorin_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, zorin_home):
    ...
    def test_unknown_log_returns_none(self, zorin_home):
        from zorin_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, zorin_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from zorin_cli.debug import _capture_log_snapshot

        logs_dir = zorin_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, zorin_home):
    ...
    def test_falls_back_when_primary_empty(self, zorin_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of ZORIN_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def zorin_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated ZORIN_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".zorin"
        home.mkdir()
        monkeypatch.setenv("ZORIN_HOME", str(home))
        ...
        monkeypatch.delenv("ZORIN_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, zorin_home_with_secret):
        from zorin_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, zorin_home_with_secret):
        from zorin_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, zorin_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when ZORIN_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("ZORIN_REDACT_SECRETS", "false")

        from zorin_cli.debug import _capture_log_snapshot

        assert os.environ.get("ZORIN_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, zorin_home_with_secret
    ):
        from zorin_cli.debug import _capture_log_snapshot

        log_path = zorin_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, zorin_home_with_secret):
        from zorin_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from zorin_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, zorin_home):
        from zorin_cli.debug import collect_debug_report

        with patch("zorin_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- zorin dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- zorin dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, zorin_home):
        from zorin_cli.debug import collect_debug_report

        with patch("zorin_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, zorin_home):
        from zorin_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, zorin_home):
        from zorin_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, zorin_home):
        from zorin_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, zorin_home):
        from zorin_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, zorin_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from zorin_cli.debug import run_debug_share

        args = MagicMock()
        args.lines = 50
        args.expire = 7
        args.local = False
        args.npcautomators = False

        with patch("zorin_cli.dump.run_dump"), \
             patch("zorin_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("zorin_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, zorin_home, capsys):
        ...
        with patch("zorin_cli.dump.run_dump"), \
             patch(
                 "zorin_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, zorin_home, capsys):
    ...
    def test_share_uploads_five_pastes(self, zorin_home, capsys):
        ...
        with patch("zorin_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- zorin dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- zorin dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- zorin dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        gui_paste = uploaded_content[3]
        assert "--- zorin dump ---" in gui_paste
        assert "--- full gui.log ---" in gui_paste
        desktop_paste = uploaded_content[4]
        assert "--- zorin dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, zorin_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from zorin_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("zorin_cli.dump.run_dump"), \
             patch("zorin_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("zorin_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, zorin_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, zorin_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, zorin_home_with_secret, capsys
    ):
        ...
        with patch("zorin_cli.dump.run_dump"), \
             patch("zorin_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("zorin_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "zorin debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, zorin_home):
        from zorin_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from zorin_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from zorin_cli.debug import delete_paste
        ...
        with patch("zorin_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.zorin/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``zorin debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, zorin_home):
        ...
        import ast
        import inspect
        from zorin_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, zorin_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from zorin_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, zorin_home):
    ...
    def test_merges_with_existing_pending(self, zorin_home):
        """Subsequent calls merge into existing pending.json."""
        from zorin_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, zorin_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, zorin_home):
        from zorin_cli.debug import _sweep_expired_pastes

        deleted, remaining = _sweep_expired_pastes()
        assert deleted == 0
        assert remaining == 0

    def test_sweep_deletes_expired_entries(self, zorin_home):
        from zorin_cli.debug import (
        ...
        with patch("zorin_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, zorin_home):
        from zorin_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("zorin_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, zorin_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, zorin_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, zorin_home):
        from zorin_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("zorin_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, zorin_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: zorin debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from zorin_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("zorin_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("zorin_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, zorin_home, capsys):
        from zorin_cli.debug import run_debug_share
        ...
        with patch("zorin_cli.dump.run_dump"), \
             ...
             patch("zorin_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, zorin_home, capsys):
        from zorin_cli.debug import run_debug_share
        ...
        with patch("zorin_cli.dump.run_dump"), \
             patch("zorin_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("zorin_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
    ...
    def test_local_no_privacy_notice(self, zorin_home, capsys):
        from zorin_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``zorin debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, zorin_home):
        from zorin_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("zorin_cli.dump.run_dump"), patch(
            "zorin_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("zorin_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, zorin_home):
        from zorin_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (zorin_home / "logs" / "desktop.log").unlink()

        with patch("zorin_cli.dump.run_dump"), patch(
            "zorin_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, zorin_home):
        from zorin_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, zorin_home):
        from zorin_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, zorin_home):
        from zorin_cli.debug import build_debug_share
...
# ---------------------------------------------------------------------------
# Shared bundle collection + NpcAutomators-S3 path
# ---------------------------------------------------------------------------

class TestCollectShareBundle:
    def test_returns_report_and_logs(self, zorin_home):
        from zorin_cli.debug import collect_share_bundle
    ...
    def test_no_redact_omits_banner(self, zorin_home):
        from zorin_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, zorin_home):
        from zorin_cli.debug import collect_share_bundle
    ...
    def test_build_debug_share_uses_collector(self, zorin_home):
        # build_debug_share must produce the same report text the collector does
        # (i.e. the refactor preserved paste.rs behaviour).
        from zorin_cli.debug import build_debug_share, collect_share_bundle
...
class TestBuildNpcAutomatorsBundle:
    def test_envelope_shape_and_gzip(self, zorin_home):
        import gzip
        import json as _json

        from zorin_cli.debug import build_npcautomators_bundle

        files = {"report": "hello", "agent.log": "log line"}
        blob = build_npcautomators_bundle(files, redact=True)

        # It's gzip — magic bytes.
        assert blob[:2] == b"\x1f\x8b"
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["format"] == "zorin-debug-share/1"
    ...
    def test_redacted_false_recorded(self):
        ...
        blob = build_npcautomators_bundle({"report": "x"}, redact=False)
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["redacted"] is False


class TestRunDebugShareNpcAutomators:
    def _args(self, **over):
        class _A:
            lines = 50
            expire = 7
    ...
    def test_npcautomators_success_prints_view_url(self, zorin_home, capsys):
        from zorin_cli.debug import run_debug_share
        ...
        with patch("zorin_cli.dump.run_dump"), patch(
            "zorin_cli.diagnostics_upload.share_to_npcautomators", return_value=res
        ) as share:
            run_debug_share(self._args())

        out = capsys.readouterr().out
        assert "NpcAutomators-INTERNAL" in out
        assert "https://support.example.com/diagnostics/id-1" in out
        assert "2026-06-20T00:00:00Z" in out
        # The blob passed to share_to_npcautomators must be gzip bytes.
        blob = share.call_args[0][0]
        assert isinstance(blob, (bytes, bytearray)) and blob[:2] == b"\x1f\x8b"

    def test_npcautomators_failure_suggests_local(self, zorin_home, capsys):
        from zorin_cli.debug import run_debug_share

        with patch("zorin_cli.dump.run_dump"), patch(
            "zorin_cli.diagnostics_upload.share_to_npcautomators",
        ...
        assert "NpcAutomators upload failed" in err
        assert "--local" in err

    def test_npcautomators_does_not_touch_pastebin(self, zorin_home):
        from zorin_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("zorin_cli.dump.run_dump"), patch(
            "zorin_cli.diagnostics_upload.share_to_npcautomators", return_value=res
        ), patch("zorin_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
        paste.assert_not_called()


class TestDebugSlashCommand:
    """`/debug [npcautomators|local]` parsing in the CLI/TUI handler.

    The classic CLI and the TUI slash worker both dispatch through
    ``ZorinCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("zorin_cli.debug.run_debug_share", _fake_run):
            self._handler()(cmd_original)
        return captured

    def test_bare_debug_defaults_to_paste(self):
        c = self._captured("/debug")
        assert c["npcautomators"] is False and c["local"] is False
    ...
    def test_npcautomators_word_sets_npcautomators(self):
        c = self._captured("/debug npcautomators")
        assert c["npcautomators"] is True and c["local"] is False

    def test_local_word_sets_local(self):
        c = self._captured("/debug local")
        assert c["local"] is True and c["npcautomators"] is False

    def test_word_parsing_is_case_insensitive(self):
        c = self._captured("/debug NPCAUTOMATORS")
        assert c["npcautomators"] is True

    def test_local_wins_over_npcautomators(self):
        # local never touches the network, so it takes precedence.
        c = self._captured("/debug npcautomators local")
        assert c["local"] is True and c["npcautomators"] is False
...
class TestShareConsentGate:
    """`zorin debug share` requires explicit consent before uploading.
    ...
    def _args(self, **over):
        ...
        base = dict(lines=50, expire=7, local=False, npcautomators=False,
                    no_redact=False, yes=False)
        base.update(over)
        return SimpleNamespace(**base)

    def test_aborts_on_user_decline(self, zorin_home, capsys, monkeypatch):
        ...
        with patch("zorin_cli.dump.run_dump"), \
             patch("zorin_cli.debug.upload_to_pastebin") as mock_upload:
            run_debug_share(self._args())

        mock_upload.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_proceeds_on_user_accept(self, zorin_home, capsys, monkeypatch):
    ...
    def test_yes_flag_skips_prompt(self, zorin_home, capsys, monkeypatch):
    ...
    def test_non_interactive_requires_yes(self, zorin_home, capsys, monkeypatch):
    ...
    def test_non_interactive_with_yes_succeeds(self, zorin_home, capsys, monkeypatch):
    ...
    def test_npcautomators_path_also_gated(self, zorin_home, capsys, monkeypatch):
        """The --npcautomators S3 path enforces the same consent gate (sibling site)."""
        from zorin_cli.debug import run_debug_share
        ...
        with patch("zorin_cli.dump.run_dump"), \
             patch("zorin_cli.diagnostics_upload.share_to_npcautomators") as mock_npcautomators:
            run_debug_share(self._args(npcautomators=True))

        mock_npcautomators.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_local_never_prompts(self, zorin_home, capsys, monkeypatch):
