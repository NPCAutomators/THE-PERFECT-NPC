"""Portal-URL resolution for Phase 2b billing errors (npcautomators_billing).
...
def _preview(monkeypatch):
    monkeypatch.setenv("ZORIN_PORTAL_BASE_URL", "https://nas-pr-412.npcautomators.wtf")


def test_absolutize_resolves_relative(_preview):
    assert (
        _absolutize_portal_url("/billing?topup=open")
        == "https://nas-pr-412.npcautomators.wtf/billing?topup=open"
    )
