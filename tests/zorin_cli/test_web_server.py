"""Tests for zorin_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_zorin_home):
    """Drop the example-dashboard fixture into the per-test ZORIN_HOME
    ...
    ``ZORIN_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, zorin-achievements, model providers
    ...
    from zorin_cli import web_server
    ...
    # plugin in exactly as a real operator would with `zorin plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down ZORIN_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Zorin vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Zorin env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints ZORIN_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import zorin_cli.web_server as ws

        monkeypatch.setenv("ZORIN_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("ZORIN_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_zorin_home):
        """Create a TestClient and isolate the state DB under the test ZORIN_HOME."""
        try:
        ...
        import zorin_state
        from zorin_constants import get_zorin_home
        from zorin_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(zorin_state, "DEFAULT_DB_PATH", get_zorin_home() / "state.db")
    ...
    def test_get_status(self):
        ...
        assert "zorin_home" in data
        assert "active_sessions" in data
        assert data["can_update_zorin"] is True

    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        import zorin_cli.web_server as web_server
        import zorin_state
        ...
        fake_db_path.touch()
        monkeypatch.setattr(zorin_state, "DEFAULT_DB_PATH", fake_db_path)
        ...
        monkeypatch.setattr("zorin_state.SessionDB", _FakeDB)
    ...
    def test_status_active_session_count_fresh_install_returns_zero(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(zorin_state, "DEFAULT_DB_PATH", tmp_path / "absent.db")

        def _boom(*a, **k):
            raise AssertionError("SessionDB must not be constructed when db file is absent")

        monkeypatch.setattr("zorin_state.SessionDB", _boom)
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        ...
        resp = self.client.get("/api/status")
        assert resp.status_code == 200
        assert resp.json()["can_update_zorin"] is False

    def test_dashboard_update_capability_detects_generic_container(self, monkeypatch):
        import zorin_constants
        import zorin_cli.web_server as web_server

        monkeypatch.setattr(zorin_constants, "is_container", lambda: True)
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in zorin-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "zorin"
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from zorin_constants import get_zorin_home
        from zorin_cli.config import load_env
        ...
        config_path = get_zorin_home() / "hindsight" / "config.json"
    ...
    def test_post_memory_provider_setup_persists_values_without_activation(self):
        from zorin_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide ZORIN_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from zorin_constants import get_zorin_home

        path = get_zorin_home() / "honcho.json"
    ...
    def test_get_honcho_config_returns_safe_defaults(self, monkeypatch, tmp_path):
        ...
        assert data["docs_url"] == "https://docs.honcho.dev/v3/guides/integrations/zorin"
        ...
        assert fields["workspace"]["placeholder"] == "zorin"
        assert fields["aiPeer"]["placeholder"] == "zorin"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_zorin_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["zorin"]["workspace"] == "myws"
        assert cfg["hosts"]["zorin"]["peerName"] == "eri"
        assert cfg["hosts"]["zorin"]["environment"] == "local"
        assert cfg["hosts"]["zorin"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["zorin"]["apiKey"] == "hch-test-key"
    ...
    def test_put_honcho_blank_text_clears_key(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_zorin_home() / "honcho.json").read_text(encoding="utf-8"))
        assert "workspace" not in cfg.get("hosts", {}).get("zorin", {})
    ...
    def test_put_honcho_partial_save_preserves_other_keys(self, monkeypatch, tmp_path):
        ...
        host = json.loads((get_zorin_home() / "honcho.json").read_text(encoding="utf-8"))["hosts"]["zorin"]
    ...
    def test_put_honcho_number_stored_as_native_number(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_zorin_home() / "honcho.json").read_text(encoding="utf-8"))
        assert cfg["hosts"]["zorin"]["dialecticMaxChars"] == 1200
        assert isinstance(cfg["hosts"]["zorin"]["dialecticMaxChars"], int)
    ...
    def test_put_honcho_first_save_merges_into_resolved_config(self, monkeypatch, tmp_path):
        ...
        global_path.write_text(
            json.dumps({"baseUrl": "https://kept.example", "hosts": {"zorin": {"workspace": "kept"}}}),
            encoding="utf-8",
        )
        ...
        assert resp.status_code == 200
        assert not (get_zorin_home() / "honcho.json").exists()
        cfg = json.loads(global_path.read_text(encoding="utf-8"))
        assert cfg["baseUrl"] == "https://kept.example"
        assert cfg["hosts"]["zorin"] == {"workspace": "kept", "peerName": "eri"}
    ...
    def test_put_honcho_updates_legacy_dot_form_host_block(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("ZORIN_HONCHO_HOST", "zorin_work")

        path = self._seed_local_honcho({"hosts": {"zorin.work": {"workspace": "w", "peerName": "eri"}}})
        ...
        assert set(hosts) == {"zorin.work"}
        assert hosts["zorin.work"] == {"workspace": "w", "peerName": "eri", "sessionStrategy": "per-repo"}
    ...
    def test_put_honcho_api_key_never_overwrites_oauth_token(self, monkeypatch, tmp_path):
        ...
        path = self._seed_local_honcho({"hosts": {"zorin": {"apiKey": "hch-at-oauth-token"}}})
        ...
        assert cfg["hosts"]["zorin"]["apiKey"] == "hch-at-oauth-token"
    ...
    def test_put_honcho_tolerates_null_hosts(self, monkeypatch, tmp_path):
        ...
        assert resp.status_code == 200
        assert json.loads(path.read_text(encoding="utf-8"))["hosts"]["zorin"]["workspace"] == "myws"
    ...
    def test_memory_provider_config_honors_profile_param(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        # The suite pins ZORIN_HONCHO_HOST=zorin; this test exercises
        # profile-driven host resolution, so drop the override explicitly.
        monkeypatch.delenv("ZORIN_HONCHO_HOST", raising=False)
        from zorin_constants import get_zorin_home
        from zorin_cli.profiles import get_profile_dir
        ...
        worker_cfg.write_text(json.dumps({"hosts": {"zorin_worker": {"workspace": "kept"}}}), encoding="utf-8")
        ...
        own = json.loads((get_zorin_home() / "honcho.json").read_text(encoding="utf-8"))
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_zorin_home() / "images" / "nope.png"
        resp = self.client.get("/api/media", params={"path": str(missing)})
        assert resp.status_code == 404

    def test_get_media_requires_auth(self):
        from zorin_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_chat_image_upload_writes_to_default_profile_images(self):
        ...
        assert target.parent == get_zorin_home() / "images"
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from zorin_cli.web_server import app
    ...
    def test_update_zorin_returns_docker_guidance_without_spawning(self, monkeypatch):
        import zorin_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn zorin update")

        # Bypass the managed-externally gate so we reach the docker install check.
        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_zorin_action", fail_spawn)
        web_server._ACTION_PROCS.pop("zorin-update", None)
        web_server._ACTION_RESULTS.pop("zorin-update", None)

        resp = self.client.post("/api/zorin/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "zorin-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull npcautomators/agent-zorin:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/zorin-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull npcautomators/agent-zorin:latest" in line for line in status_data["lines"])

    def test_update_zorin_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import zorin_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn zorin update")
    ...
    def test_update_zorin_spawns_on_non_docker_install(self, monkeypatch):
        import zorin_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        monkeypatch.setattr(web_server, "_spawn_zorin_action", fake_spawn)
        web_server._ACTION_PROCS.pop("zorin-update", None)
        web_server._ACTION_RESULTS.pop("zorin-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "zorin-update"}
        assert calls == [(["update"], "zorin-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("zorin-update", None)
        web_server._ACTION_RESULTS.pop("zorin-update", None)
        web_server._ACTION_PROCS["zorin-update"] = proc

        resp = self.client.get("/api/actions/zorin-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "zorin-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["zorin-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_action_status_tails_large_log_without_read_text(self, tmp_path, monkeypatch):
        ...
        log_path = tmp_path / web_server._ACTION_LOG_FILES["zorin-update"]
        log_path.write_text(
            "stale-start\n"
        ...
        resp = self.client.get("/api/actions/zorin-update/status?lines=3")
    ...
    def test_config_schema_merges_custom_command_tts_provider(self):
        ...
        from zorin_cli.config import load_config, save_config
        from zorin_cli.web_server import CONFIG_SCHEMA
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from zorin_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `zorin model` but
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from zorin_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "zorin_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Zorin provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `zorin import` doesn't auto-abort at the overwrite prompt."""
        import zorin_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_zorin_action", fake_spawn)
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        assert archive.parent == get_zorin_home() / "backups"
        assert archive.name.startswith("zorin-backup-")
        assert archive.suffix == ".zip"

    def test_ops_backup_uses_hosted_zorin_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import zorin_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("ZORIN_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "zorin-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_zorin_action", fail_spawn)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from zorin_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from zorin_cli.config import save_env_value
        from zorin_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from zorin_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "ZORIN"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("ZorinDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "zorin_pair123_bot",
                "deep_link": "https://t.me/newbot/ZorinSetupBot/zorin_pair123_bot",
                "qr_payload": "https://t.me/newbot/ZorinSetupBot/zorin_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "zorin_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/ZorinSetupBot/zorin_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/ZorinSetupBot/zorin_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "zorin_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_zorin_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "zorin_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/ZorinSetupBot/zorin_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/ZorinSetupBot/zorin_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "zorin_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_zorin_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``zorin gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "zorin_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/ZorinSetupBot/zorin_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/ZorinSetupBot/zorin_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "zorin_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "zorin_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/ZorinSetupBot/zorin_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/ZorinSetupBot/zorin_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "zorin_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/ZorinSetupBot/zorin_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/ZorinSetupBot/zorin_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/zorin_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/zorin"})
    ...
    def test_headless_serve_disables_spa_even_with_a_dist(self, monkeypatch, tmp_path):
        """`zorin serve` (ZORIN_SERVE_HEADLESS) must NOT serve the SPA even
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.setenv("ZORIN_SERVE_HEADLESS", "1")
        app_ = FastAPI()
        ws.mount_spa(app_)
    ...
    def test_set_model_main_npcautomators_applies_gateway_defaults(self, monkeypatch):
        """Switching the main provider to NpcAutomators calls apply_npcautomators_managed_defaults
        ...
        monkeypatch.setattr(ns, "apply_npcautomators_managed_defaults", fake_apply)

        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "npcautomators", "model": "zorin-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["provider"] == "npcautomators"
        assert data["gateway_tools"] == ["web"]
        assert called["force_fresh"] is True

    def test_set_model_main_non_npcautomators_skips_gateway_defaults(self, monkeypatch):
        """Non-NPCAUTOMATORS providers must NOT trigger Tool Gateway auto-routing."""
        import zorin_cli.npcautomators_subscription as ns

        def boom(*args, **kwargs):  # pragma: no cover - must not be called
            raise AssertionError("apply_npcautomators_managed_defaults called for non-npcautomators provider")

        monkeypatch.setattr(ns, "apply_npcautomators_managed_defaults", boom)
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from zorin_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from zorin_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``zorin model`` custom flow. Regression for the desktop loop where a
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "npcautomators", "default": "zorin-4"}
        cfg["auxiliary"] = {
            # Pinned to npcautomators — same as the OLD main, becomes stale after switch.
            "compression": {"provider": "npcautomators", "model": "anthropic/claude-sonnet-4.6"},
        ...
        assert comp["provider"] == "npcautomators"
    ...
    def test_custom_endpoints_list_includes_direct_custom_config(self):
        ...
        """
        from zorin_cli.config import save_config
    ...
    def test_set_model_main_preserves_base_url_for_named_custom_provider(self):
        ...
        save_config({
            "model": {"provider": "npcautomators", "default": "zorin-4"},
            "providers": {
                "axet-proxy": {
                    "name": "Axet Proxy",
                    "base_url": "http://127.0.0.1:8081/v1",
    ...
    def test_recommended_default_npcautomators_honors_free_tier(self, monkeypatch):
        """For a free-tier NpcAutomators user, the recommended default must be a free
        model (mirroring `zorin model`), not the first curated paid entry."""
        import zorin_cli.models as models_mod

        monkeypatch.setattr(models_mod, "get_curated_npcautomators_model_ids", lambda: ["paid/expensive", "free/cheap"])
        monkeypatch.setattr(
            models_mod, "get_pricing_for_provider",
            lambda provider: {"paid/expensive": {"input": "1"}, "free/cheap": {"input": "0"}},
        )
        monkeypatch.setattr(models_mod, "check_npcautomators_free_tier", lambda *, force_fresh=False: True)
        monkeypatch.setattr(
        ...
        resp = self.client.get("/api/model/recommended-default?provider=npcautomators")
        assert resp.status_code == 200
        data = resp.json()
        assert data["provider"] == "npcautomators"
    ...
    def test_recommended_default_npcautomators_paid_uses_curated_default(self, monkeypatch):
        """A paid NpcAutomators user gets the first curated/paid-augmented model."""
        import zorin_cli.models as models_mod

        monkeypatch.setattr(models_mod, "get_curated_npcautomators_model_ids", lambda: ["top/model", "other/model"])
        monkeypatch.setattr(models_mod, "get_pricing_for_provider", lambda provider: {})
        monkeypatch.setattr(models_mod, "check_npcautomators_free_tier", lambda *, force_fresh=False: False)
        monkeypatch.setattr(
    ...
    def test_recommended_default_handles_failure_gracefully(self, monkeypatch):
        ...
        monkeypatch.setattr(models_mod, "get_curated_npcautomators_model_ids", boom)
...
class TestBuildSchemaFromConfig:
    ...
    def test_approvals_mode_options_match_config_values(self):
        ...
        are 'manual', 'smart', and 'off' (see zorin_cli/config.py).
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from zorin_cli.web_server import _build_schema_from_config
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from zorin_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_zorin_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("zorin://")
    ...
    def test_profiles_list_includes_default(self):
        from zorin_constants import get_zorin_home
        get_zorin_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from zorin_constants import get_zorin_home
        import zorin_cli.profiles as profiles_mod

        zorin_home = get_zorin_home()
        zorin_home.mkdir(parents=True, exist_ok=True)
        (zorin_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = zorin_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_zorin_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_zorin_for_default_profile(self):
        from zorin_constants import get_zorin_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "zorin setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/zorin/bin/zorin")
        ...
        if is_windows:
            assert lines == ["@echo off", "zorin -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/zorin/bin/zorin -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_zorin_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_zorin_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_zorin_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_zorin_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_zorin_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_zorin_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_zorin_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_zorin_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_zorin_home() / "profiles" / "builder"
        token = set_zorin_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_zorin_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import zorin_cli.skills_config as skills_config
        import zorin_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import zorin_cli.tools_config as tools_config
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out NpcAutomators Subscription rows, xAI TTS without Grok OAuth, and
        never-installed KittenTTS/Piper. The endpoint now reports the honest
        state so keyless ≠ ready.
        """
        import zorin_cli.tools_config as tools_config
        from zorin_cli.npcautomators_account import NpcAutomatorsPortalAccountInfo

        # Logged out of NPCAUTOMATORS Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "zorin_cli.npcautomators_subscription.get_npcautomators_portal_account_info",
            lambda *a, **k: NpcAutomatorsPortalAccountInfo(
        ...
        assert by_name["NpcAutomators Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_npcautomators_provider_reports_needs_npcautomators_auth(self, monkeypatch):
        """Selecting a managed NpcAutomators row while logged out flags needs_npcautomators_auth.
        ...
        (ensure_npcautomators_portal_access) — so the row never activated and nothing
        told the user to sign in. The endpoint now reports the entitlement
        gap so the client can drive the existing NpcAutomators OAuth flow.
        """
        from zorin_cli.npcautomators_account import NpcAutomatorsPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "NpcAutomators Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["needs_npcautomators_auth"] is True
    ...
    def test_select_managed_npcautomators_provider_entitled_no_auth_flag(self, monkeypatch):
        """A signed-in, entitled subscriber gets no needs_npcautomators_auth field."""
        from zorin_cli.npcautomators_account import NpcAutomatorsPortalAccountInfo
        ...
        data = resp.json()
        assert data["ok"] is True
        assert "needs_npcautomators_auth" not in data

    def test_select_unmanaged_provider_has_no_npcautomators_auth_field(self):
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "zorin"
        save_config(config)

        body = self.client.get("/api/tools/terminal/backends").json()
        ssh = next(r for r in body["backends"] if r["name"] == "ssh")
        assert ssh["status"] == "ready"
        assert "zorin@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from zorin_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from zorin_cli.web_server import _denormalize_config_from_web
        from zorin_cli.config import save_config
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("ZORIN_RESTART_DRAIN_TIMEOUT", "90")
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from zorin_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.zorin/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_zorin_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
            reset_zorin_home_override(token)
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(zorin_home, name="ocean"):
        themes_dir = zorin_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="zorin-theme-bootstrap">')
    ...
    def test_serve_index_no_bootstrap_for_builtin_theme(self, tmp_path, monkeypatch):
        ...
        resp = client.get("/chat")
        assert resp.status_code == 200
        assert "zorin-theme-bootstrap" not in resp.text
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_zorin_home):
        ...
        monkeypatch.setattr(
            zorin_state, "DEFAULT_DB_PATH", get_zorin_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``zorin_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in zorin_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_zorin_home):
        ...
        # Pin the SessionDB to the isolated ZORIN_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_zorin_home, _install_example_plugin):
        ...
        installs it into the per-test ``ZORIN_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into ZORIN_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, zorin-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (zorin-achievements is loaded by default).
        resp = self.client.get("/api/plugins/zorin-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        ZORIN_HOME override scopes a request to another profile."""
        from zorin_constants import (
            reset_zorin_home_override,
            set_zorin_home_override,
        )
        ...
        monkeypatch.setenv("ZORIN_HOME", str(launch_home))
        from zorin_cli import web_server
        token = set_zorin_home_override(str(other))
        try:
...
# or ``sh -c 'printf …'``) instead of the real ``zorin --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import zorin_cli.main as main_mod
        ...
        assert env["ZORIN_TUI_DASHBOARD"] == "1"
        assert env["ZORIN_TUI_INLINE"] == "1"
        assert env["ZORIN_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_resolve_chat_argv_sets_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.delenv("ZORIN_PYTHON_SRC_ROOT", raising=False)
        monkeypatch.delenv("ZORIN_PYTHON", raising=False)
        monkeypatch.delenv("ZORIN_CWD", raising=False)
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
        ...
        assert env["ZORIN_PYTHON_SRC_ROOT"] == str(main_mod.PROJECT_ROOT)
        assert env["ZORIN_PYTHON"] == sys.executable
        assert env["ZORIN_CWD"] == os.getcwd()
    ...
    def test_resolve_chat_argv_replaces_invalid_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.setenv("ZORIN_PYTHON_SRC_ROOT", "/definitely/missing/zorin-src")
        monkeypatch.setenv("ZORIN_PYTHON", "/definitely/missing/python")
        monkeypatch.setenv("ZORIN_CWD", "/definitely/missing/cwd")
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
    ...
    def test_resolve_chat_argv_keeps_relative_python_under_tui_cwd(
        ...
        monkeypatch.setenv("ZORIN_CWD", str(tmp_path))
        monkeypatch.setenv("ZORIN_PYTHON", str(relative_python))
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (["node", "dist/entry.js"], "/tmp/ui-tui"),
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        env = {
            "ZORIN_CWD": str(tmp_path),
            "ZORIN_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["ZORIN_PYTHON"] == command
    ...
    def test_resolve_chat_argv_falls_back_when_getcwd_is_missing(self, monkeypatch, tmp_path):
        ...
        assert env is not None
        assert env["ZORIN_CWD"] == str(tmp_path)

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_zorin_home
    ):
        import zorin_cli.main as main_mod

        config_path = Path(os.environ["ZORIN_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/zorin-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/zorin-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"zorin-ws-ok" in buf:
                    break
            assert b"zorin-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from zorin_cli.pty_bridge import PtyUnavailableError
        ...
        import zorin_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        ZORIN_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from zorin_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import zorin_cli.main as cli_main
    import zorin_cli.web_server as ws
    ...
    assert env is not None
    gateway_url = env.get("ZORIN_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_zorin_home, _install_example_plugin):
        ...
        the per-test ``ZORIN_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_zorin_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("ZORIN_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under ZORIN_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_zorin_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("ZORIN_DESKTOP", raising=False)
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("ZORIN_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
