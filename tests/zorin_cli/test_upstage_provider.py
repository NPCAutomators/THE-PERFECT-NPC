...
Regression guard for the bug where `zorin model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`zorin_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageResolver:
    ...
    def test_solar_alias_normalizes_to_upstage(self):
        from zorin_cli.providers import normalize_provider, resolve_provider_full
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in ZORIN_OVERLAYS
        overlay = ZORIN_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `zorin_cli/main.py` (cmd_model): read
