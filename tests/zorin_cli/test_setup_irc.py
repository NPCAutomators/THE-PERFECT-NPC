"""Tests for IRC gateway configuration via `zorin setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Zorin install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Zorin install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#zorin")
            monkeypatch.setenv("IRC_NICKNAME", "zorin-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `zorin setup gateway` experience with IRC present."""
