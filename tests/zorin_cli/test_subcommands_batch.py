...
def test_single_handler_builders(name, builder, kw, argv):
    parser = argparse.ArgumentParser(prog="zorin")
...
def test_config_get_unset_subcommands_parse():
    """`zorin config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="zorin")
    sub = parser.add_subparsers(dest="command")
    handler = _h("config")
...
# ── deprecated `zorin login` fails gracefully, not with argparse error ────
#
# `zorin login` is a removed command; its handler (`login_command` in
# `zorin_cli/auth.py`) prints a deprecation notice pointing at `zorin auth` /
# `zorin model` and exits 0.  Two behavior contracts guard the UX:
...
@pytest.mark.parametrize("provider", ["anthropic", "npcautomators", "openai-codex", "totally-made-up"])
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `zorin --help`.
