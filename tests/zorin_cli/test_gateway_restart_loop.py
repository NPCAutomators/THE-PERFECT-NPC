...
Covers:
- Defense 1: gateway stop/restart refuse when _ZORIN_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.zorin.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.zorin.gateway.plist",
        "launchctl stop ai.zorin.gateway",
        "systemctl restart zorin-gateway",
        "systemctl stop zorin-gateway.service",
        "systemctl start zorin-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill zorin gateway process",
        "pkill -f zorin.*gateway",
        "pkill -f gateway.*zorin",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "zorin cron list",
        "zorin update",
        "zorin config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `zorin gateway start` is benign — starting a gateway from inside a
        ...
        "zorin gateway start",
        "zorin gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway zorin
        # services must not be falsely blocked (the old `.*zorin` matched any
        # zorin token).
        "launchctl unload ai.zorin.update-checker.plist",
        "launchctl restart ai.zorin.daemon",
        "systemctl restart zorin-meta.service",
        "systemctl restart zorin-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_zorin_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade zorin then run zorin gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.zorin.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_zorin_gateway_once.sh). The script must live under
        # ZORIN_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ZORIN_HOME", str(tmp_path / ".zorin"))
        scripts_dir = tmp_path / ".zorin" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify zorin gateway stop/restart refuse when _ZORIN_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ZORIN_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ZORIN_GATEWAY=1.

    Issue #37453: systemctl --user restart zorin-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart zorin-gateway",
        "systemctl --user restart zorin-gateway",
        "systemctl stop zorin-gateway.service",
        "zorin gateway restart",
        "launchctl kickstart gui/501/ai.zorin.gateway",
        "pkill -f zorin.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart zorin-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-zorin systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _ZORIN_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart zorin-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart zorin-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run zorin gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("zorin gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfezorin gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ZORIN_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.zorin.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    zorin_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run zorin gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZORIN_HOME", str(tmp_path / ".zorin"))
        (tmp_path / ".zorin").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run zorin gateway restart nightly",
        ))
