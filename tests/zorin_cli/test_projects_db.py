"""Tests for the per-profile Projects store (zorin_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="ZORIN", folders=["/tmp/zorin"])
    ...
    assert [f.path for f in proj.folders] == ["/tmp/zorin"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ZORIN_HOME.
...
def test_db_path_under_zorin_home():
    # Resolves under ZORIN_HOME (set by the autouse isolation fixture).
