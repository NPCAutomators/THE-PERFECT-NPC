...
import zorin_cli.doctor as doctor
import zorin_cli.gateway as gateway_cli
from zorin_cli import doctor as doctor_mod
from zorin_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `zorin doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "ZORIN_HOME", zorin_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("ZORIN_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("ZORIN_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_zorin_home(self, tmp_path, provider=""):
        """Create a minimal ZORIN_HOME with config.yaml."""
        home = tmp_path / ".zorin"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_zorin_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "ZORIN_HOME", home)
        ...
        try:
            from zorin_cli import auth as _auth_mod
            monkeypatch.setattr(_auth_mod, "get_npcautomators_auth_status", lambda: {})
...
def test_run_doctor_accepts_zorin_provider_ids_that_catalog_aliases(
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp ZORIN_HOME.

        ``run_doctor`` reads the module-level ``ZORIN_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("ZORIN_HOME")`` alone
        leaves doctor probing the REAL ~/.zorin. On a dev machine with a
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_npcautomators_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from zorin_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("zorin_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "zorin_cli.doctor", raising=False)
        from zorin_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the NpcAutomators / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from zorin_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_npcautomators_auth_status", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """NpcAutomators / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".zorin"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "NPCAUTOMATORS Portal auth" in out
        assert "logged in" in out
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale ZORIN_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir(parents=True)
        (zorin_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"ZORIN_MAX_ITERATIONS={ghost}\n")
        (zorin_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "ZORIN_HOME", zorin_home)
        monkeypatch.setattr(doctor_mod, "get_zorin_home", lambda: zorin_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("ZORIN_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("ZORIN_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale ZORIN_MAX_ITERATIONS" in out
        env_after = (zorin_home / ".env").read_text(encoding="utf-8")
        assert "ZORIN_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`zorin doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Zorin install.
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_collect_deprecated_env_vars(self):
        ...
        assert "ZORIN_TOOL_PROGRESS" in names
        assert "TERMINAL_CWD" in names
        assert "QQ_HOME_CHANNEL" in names
        assert "OPENAI_API_KEY" not in names
        by_name = dict(findings)
        assert "display.tool_progress" in by_name["ZORIN_TOOL_PROGRESS"]
    ...
    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir(parents=True)
        (zorin_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (zorin_home / ".env").write_text(env_body, encoding="utf-8")
...
"""
        out, zorin_home = self._run_doctor_with_config(monkeypatch, tmp_path, config_yaml=cfg)
        ...
        on_disk = (zorin_home / "config.yaml").read_text(encoding="utf-8")
...
"""
        ...
        assert "Deprecated: ZORIN_TOOL_PROGRESS" in out
...
"""
        ...
        assert "Deprecated: ZORIN_TOOL_PROGRESS" not in out
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"ZORIN_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: ZORIN_TOOL_PROGRESS_MODE" in out
