...
``zorin_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``zorin --help``, ``zorin version``,
``zorin logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``zorin --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from zorin_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["zorin", "-w", "--tui"], None),
        (["zorin", "version"], "version"),
        (["zorin", "--tui", "chat"], "chat"),
        (["zorin", "-w", "logs"], "logs"),
        (["zorin", "chat", "hello world"], "chat"),
        (["zorin", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["zorin", "-m", "gpt5", "chat"], "chat"),
        (["zorin", "--model", "gpt5", "chat", "hi"], "chat"),
        (["zorin", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["zorin", "-z", "hello world"], None),
        (["zorin", "-z", "hello", "chat"], "chat"),
        (["zorin", "--model=gpt5", "chat"], "chat"),     # inline form
        (["zorin", "--", "chat"], "chat"),               # -- terminator
        (["zorin", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["zorin", "some-plugin-cmd"], "some-plugin-cmd"),
        (["zorin", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["zorin"],                          # bare → chat
        ["zorin", "--help"],                # top-level help
        ["zorin", "-h"],
        ["zorin", "version"],               # known built-in
        ["zorin", "logs"],
        ["zorin", "gateway", "run"],
        ["zorin", "--tui"],
        ["zorin", "-w", "--tui"],
        ["zorin", "chat", "hi"],
        ["zorin", "help"],                  # accepted built-in-ish
        ["zorin", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["zorin", "meet", "join"],          # potential google_meet plugin
        ["zorin", "honcho", "status"],      # potential memory plugin
        ["zorin", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``zorin help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"zorin_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
