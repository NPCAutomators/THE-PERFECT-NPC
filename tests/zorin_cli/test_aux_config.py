"""Tests for the auxiliary-model configuration UI in ``zorin model``.
...
from zorin_cli.config import DEFAULT_CONFIG, load_config
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"provider": "npcautomators", "model": "gemini-3-flash"}, "npcautomators · gemini-3-flash"),
        (
...
def test_save_aux_choice_persists_to_config_yaml(tmp_path, monkeypatch):
    ...
    (tmp_path / ".zorin").mkdir(exist_ok=True)
...
def test_save_aux_choice_preserves_timeout(tmp_path, monkeypatch):
    ...
    _save_aux_choice("vision", provider="npcautomators", model="gemini-3-flash")
...
def test_save_aux_choice_creates_missing_task_entry(tmp_path, monkeypatch):
    ...
    _save_aux_choice("vision", provider="npcautomators", model="gemini-3-flash")
    cfg = load_config()
    assert cfg["auxiliary"]["vision"]["provider"] == "npcautomators"
...
def test_reset_aux_to_auto_clears_routing_preserves_timeouts(tmp_path, monkeypatch):
    ...
    _save_aux_choice("compression", provider="npcautomators", model="gemini-3-flash")
    from zorin_cli.config import save_config
