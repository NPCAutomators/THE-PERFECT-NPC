...
from zorin_cli import runtime_provider as rp
...
def test_resolve_runtime_provider_npcautomators_pool_uses_env_base_url_override(monkeypatch):
    ...
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "npcautomators")
    monkeypatch.setattr(rp, "_agent_key_is_usable", lambda *a, **k: True)
    monkeypatch.setattr(rp, "load_pool", lambda provider: _Pool())

    resolved = rp.resolve_runtime_provider(requested="npcautomators")

    assert resolved["provider"] == "npcautomators"
    assert resolved["api_key"] == "pool-token"
...
def test_named_custom_provider_does_not_shadow_builtin_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            ...
            "api_key": "npcautomators-runtime-key",
            "source": "portal",
            "expires_at": None,
        },
    )
    ...
    assert resolved["provider"] == "npcautomators"
    assert resolved["base_url"] == "https://inference-api.npcautomators.com/v1"
    assert resolved["api_key"] == "npcautomators-runtime-key"
    assert resolved["requested_provider"] == "npcautomators"
...
def test_npcautomators_pool_entry_refreshes_expired_agent_key(monkeypatch):
    ...
    class _Entry:
        def __init__(self, token):
            ...
            self.source = "manual:npcautomators"
    ...
    monkeypatch.setattr(rp, "_get_model_config", lambda: {"provider": "npcautomators"})
...
def test_named_custom_provider_skipped_for_canonical_built_in(monkeypatch):
    """Companion to the test above: ``npcautomators`` is a canonical provider name
    (``resolve_provider('npcautomators') == 'npcautomators'``), so a custom entry with that name
    ...
    entry = rp._get_named_custom_provider("npcautomators")
...
def test_resolve_requested_provider_precedence(monkeypatch):
    ...
    monkeypatch.setattr(rp, "_get_model_config", lambda: {})
    assert rp.resolve_requested_provider() == "npcautomators"
...
def test_resolve_provider_custom_returns_custom():
    ...
    from zorin_cli.auth import resolve_provider
...
def test_auto_detected_npcautomators_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks NpcAutomators but credentials are revoked, fall through to OpenRouter."""
    from zorin_cli.auth import AuthError
    ...
    # resolve_provider returns "npcautomators" (stale active_provider in auth.json)
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "npcautomators")
    ...
    # NpcAutomators credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_npcautomators_runtime_credentials",
        lambda **kw: (_ for _ in ()).throw(
            AuthError("Refresh session has been revoked",
                      provider="npcautomators", code="invalid_grant", relogin_required=True)
        ),
    )
...
def test_explicit_npcautomators_auth_failure_still_raises(monkeypatch):
    """When user explicitly requests NpcAutomators and auth fails, the error should propagate."""
    from zorin_cli.auth import AuthError
    import pytest
    ...
    # With explicit "npcautomators", should raise — don't silently switch providers
    with pytest.raises(AuthError, match="Refresh session has been revoked"):
        rp.resolve_runtime_provider(requested="npcautomators")
...
class TestAzureFoundryResolution:
    ...
    def test_azure_foundry_missing_api_key_raises(self, monkeypatch):
        ...
        # `get_env_value` reads from ~/.zorin/.env — mock it to return None
        # so the resolver can't find a key there either.
        import zorin_cli.config as cfg_mod
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from zorin_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from zorin_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from zorin_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
...
def test_minimax_oauth_runtime_uses_inference_base_url(monkeypatch):
    ...
    from zorin_cli.auth import MINIMAX_OAUTH_CN_INFERENCE
