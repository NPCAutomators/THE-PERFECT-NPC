...
def test_ungrouped_providers_pass_through_in_order():
    rows = group_providers(["npcautomators", "openrouter", "deepseek"])
    assert all(r["kind"] == "single" for r in rows)
    assert [r["slug"] for r in rows] == ["npcautomators", "openrouter", "deepseek"]
...
def test_group_appears_at_first_member_position():
    ...
    rows = group_providers(["npcautomators", "minimax", "deepseek", "minimax-cn"])
    kinds = [(r["kind"], r.get("group_id") or r.get("slug")) for r in rows]
    assert kinds == [
        ("single", "npcautomators"),
        ("group", "minimax"),
        ("single", "deepseek"),
    ]
...
def test_duplicate_slugs_ignored():
    rows = group_providers(["npcautomators", "npcautomators", "minimax", "minimax"])
    assert [r.get("slug") or r["group_id"] for r in rows] == ["npcautomators", "minimax"]
