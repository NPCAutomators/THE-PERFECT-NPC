...
When ``ZORIN_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.zorin/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.zorin            (has its own auth.json fixture)
    * Profile     -> tmp_path/.zorin/profiles/coder   (active, ZORIN_HOME points here)
...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "npcautomators": {"access_token": "npcautomators-global", "refresh_token": "rt-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))

    state = get_provider_auth_state("npcautomators")
    assert state is not None
    assert state["access_token"] == "npcautomators-global"
...
def test_provider_auth_state_profile_wins_when_present(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "npcautomators": {"access_token": "npcautomators-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={
        "npcautomators": {"access_token": "npcautomators-profile"},
    }))

    state = get_provider_auth_state("npcautomators")
    assert state is not None
    assert state["access_token"] == "npcautomators-profile"
...
def test_provider_auth_state_returns_none_when_neither_has_it(profile_env):
    ...
    assert get_provider_auth_state("npcautomators") is None


# ---------------------------------------------------------------------------
# _load_provider_state — internal global fallback (issue #18594 follow-up)
#
# Several runtime helpers (notably ``resolve_npcautomators_runtime_credentials`` and
# ``resolve_npcautomators_access_token``) call ``_load_provider_state`` directly with
# a profile-loaded auth store rather than going through
# ``get_provider_auth_state``. Without the fallback wired into
# ``_load_provider_state`` itself, those helpers raise ``"Zorin is not
# logged into NPCAUTOMATORS Portal"`` even though the user has a valid global NpcAutomators
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from zorin_cli.auth import _load_auth_store, _load_provider_state

    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "npcautomators": {"access_token": "global-npcautomators-token", "refresh_token": "rt"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))
    ...
    assert state is not None
    assert state["access_token"] == "global-npcautomators-token"
...
def test_load_provider_state_profile_wins_over_global(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "npcautomators": {"access_token": "global-token"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={
        "npcautomators": {"access_token": "profile-token"},
    }))
...
def test_load_provider_state_returns_none_when_neither_has_it(profile_env):
    ...
    auth_store = _load_auth_store()
    assert _load_provider_state(auth_store, "npcautomators") is None
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    zorin_home = tmp_path / "classic"
    zorin_home.mkdir()
    monkeypatch.setenv("ZORIN_HOME", str(zorin_home))

    _write(zorin_home / "auth.json", _make_auth_store(providers={
        "npcautomators": {"access_token": "classic-token"},
    }))
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ZORIN_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.zorin as the "real home" — which is NOT equal
    # to the ZORIN_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    zorin_home = tmp_path / "classic"
    ...
    _write(zorin_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from zorin_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ZORIN_HOME is set to a custom path that is NOT under
    # ~/.zorin/profiles/ — get_default_zorin_root() returns ZORIN_HOME
...
def test_provider_state_transaction_locks_global_fallback_before_use(
    ...
    _write(
        ...
        _make_auth_store(providers={"npcautomators": {"access_token": "global-token"}}),
    )
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))
    ...
    with auth._provider_state_transaction("npcautomators") as (_store, state, source):
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_zorin_home_override(profile_b)
        try:
