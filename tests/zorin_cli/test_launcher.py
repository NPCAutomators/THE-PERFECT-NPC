"""Tests for the top-level `./zorin` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./zorin` should use `zorin_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "zorin"
    called = []

    fake_main_module = types.ModuleType("zorin_cli.main")

    def fake_main():
        called.append("zorin_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "zorin_cli.main", fake_main_module)
