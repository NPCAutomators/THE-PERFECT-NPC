"""Tests for the `zorin memory reset` CLI command.
...
- Profile-scoped reset (uses ZORIN_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ZORIN_HOME with memory files."""
    zorin_home = tmp_path / ".zorin"
    memories = zorin_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ZORIN_HOME", str(zorin_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nZorin repo is at ~/.zorin/zorin-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `zorin memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        zorin_home = tmp_path / ".zorin"
        (zorin_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ZORIN_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_zorin_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
