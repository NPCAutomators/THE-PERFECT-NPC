"""ensure_zorin_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".zorin"
    second = tmp_path / "b" / ".zorin"

    monkeypatch.setenv("ZORIN_HOME", str(first))
    cfg.ensure_zorin_home()

    # Profile switch: ZORIN_HOME moves → the new path is ensured too.
    monkeypatch.setenv("ZORIN_HOME", str(second))
    cfg.ensure_zorin_home()
