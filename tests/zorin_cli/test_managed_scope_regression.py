...
def zorin_home(tmp_path, monkeypatch):
    ...
    # /etc/zorin on the dev/CI box can't influence the test.
    monkeypatch.setenv("ZORIN_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(zorin_home, monkeypatch):
    from zorin_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(zorin_home, monkeypatch):
    from zorin_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(zorin_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_zorin_dotenv(zorin_home=str(home))
