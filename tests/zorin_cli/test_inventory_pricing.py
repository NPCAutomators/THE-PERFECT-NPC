...
columns + Free/Pro badges and gate paid models on free NPCAUTOMATORS accounts, the
same way the `zorin model` CLI picker does.
"""

import zorin_cli.inventory as inv
...
def _patch_pricing(monkeypatch, *, free_tier, pricing, unavailable=None):
    ...
    monkeypatch.setattr(models_mod, "check_npcautomators_free_tier", lambda *, force_fresh=False: free_tier)
    monkeypatch.setattr(
...
def test_apply_pricing_npcautomators_free_tier_gates_paid_models(monkeypatch):
    """A free-tier NPCAUTOMATORS account marks paid models unavailable and sets the flag."""
    _patch_pricing(
        monkeypatch,
        free_tier=True,
        pricing={
    ...
    rows = [{"slug": "npcautomators", "models": ["free/model", "paid/model"]}]
    inv._apply_pricing(rows)
...
def test_apply_pricing_npcautomators_paid_tier_no_gating(monkeypatch):
    """A paid NPCAUTOMATORS account gates nothing."""
    _patch_pricing(
        monkeypatch,
        free_tier=False,
        pricing={"npcautomators": {"x/model": {"prompt": "0.000001", "completion": "0.000002"}}},
    )
    rows = [{"slug": "npcautomators", "models": ["x/model"]}]
    inv._apply_pricing(rows)
