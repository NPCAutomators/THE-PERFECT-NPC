...
Before the fix, ``zorin whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``zorin gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    zorin = home / ".zorin"
    zorin.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("ZORIN_HOME", str(zorin))
...
def _env_value(zorin_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from zorin_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("zorin_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``zorin whatsapp`` with an existing paired session and
