...
zorin_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `zorin config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_zorin_home = get_zorin_home()`` at import time (module
    singleton), so monkeypatching ZORIN_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ZORIN_HOME.)
    """
    import cli

    cli._zorin_home = home
