"""Unit tests for zorin_cli.toolset_validation (see #38798).

Pure logic — the validity predicate is injected, so these tests need neither the
tool registry nor a running Zorin.
"""
...
# A representative set of real toolset names. `zorin` is deliberately absent —
# that is the corruption #38798 reported (`zorin-cli` rewritten to `zorin`).
_KNOWN = {
    "zorin-cli",
    "zorin-telegram",
    "zorin-discord",
...
def test_valid_config_produces_no_warnings():
    cfg = {"cli": ["zorin-cli"], "telegram": ["zorin-telegram"]}
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'zorin' instead of 'zorin-cli'.
    warnings = validate_platform_toolsets({"cli": ["zorin"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'zorin'" in w]
    assert len(unknown) == 1
    # Actionable: points at the valid name the entry should have been.
    assert "did you mean 'zorin-cli'?" in unknown[0]
    # And the zero-valid-toolsets safety net fires.
    assert any("zero valid toolsets" in w for w in warnings)


def test_mixed_valid_and_invalid_flags_only_the_invalid():
    cfg = {"cli": ["zorin-cli"], "discord": ["bogus"]}
...
def test_unknown_without_valid_platform_default_omits_suggestion():
    # zorin-mystery is not a known toolset, so no "did you mean" hint.
...
@pytest.mark.parametrize("value", [None, {}, [], "zorin-cli", 42])
...
def test_scalar_toolset_value_is_accepted():
    ...
    assert validate_platform_toolsets({"cli": "zorin-cli"}, _is_valid) == []


def test_non_string_entries_are_skipped_not_counted_invalid():
    cfg = {"cli": [None, 123, "zorin-cli"]}
    # The junk entries are ignored; the valid one keeps it from being "zero".
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_all_invalid_reports_each_and_the_zero_state():
    cfg = {"cli": ["zorin"], "discord": ["zorin"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert sum("unknown toolset" in w for w in warnings) == 2
    assert any("zero valid toolsets" in w for w in warnings)


def test_real_validate_toolset_treats_zorin_cli_valid_and_zorin_invalid():
    # Ties the helper to reality: the canonical registry check agrees that
    # `zorin-cli` is the real toolset and `zorin` is not (the #38798 crux).
    from toolsets import validate_toolset

    assert validate_toolset("zorin-cli") is True
    assert validate_toolset("zorin") is False
    warnings = validate_platform_toolsets({"cli": ["zorin"]}, validate_toolset)
    assert any("did you mean 'zorin-cli'?" in w for w in warnings)
