"""Tests for Bug #12905 fix — stale OAuth token detection in zorin model flow.

Bug 3: `zorin model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.zorin/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("zorin_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from zorin_cli.main import _model_flow_anthropic
        cfg = {}
