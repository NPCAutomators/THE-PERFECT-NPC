...
On hosts with many profile-backed ``zorin-gateway*.service`` units,
``zorin update`` used to wrap the entire per-scope unit loop in a single
...
class TestFleetRestartTimeoutIsolation:
    def test_timeout_on_middle_unit_continues_remaining_units(self):
        units = [
            "zorin-gateway-xiaomo1",
            "zorin-gateway-xiaomo2",
            "zorin-gateway-xiaomo3",
            "zorin-gateway-xiaomo4",
            "zorin-gateway-xiaomo5",
            "zorin-gateway-xiaomo6",
            "zorin-gateway-xiaomo7",
            "zorin-gateway",
        ]
        restarted: list[str] = []
        failed: list[str] = []
        ...
        def process_unit(svc_name: str) -> None:
            if svc_name == "zorin-gateway-xiaomo5":
        ...
        assert failed == ["zorin-gateway-xiaomo5"]
        assert restarted == [
            "zorin-gateway-xiaomo1",
            "zorin-gateway-xiaomo2",
            "zorin-gateway-xiaomo3",
        ...
        assert timeout_cmds == [
            ["systemctl", "--user", "--no-ask-password", "restart", "zorin-gateway-xiaomo5"]
        ]
    ...
    def test_non_gateway_units_in_list_output_are_ignored(self):
        ...
        _for_each_systemd_gateway_unit(
            "\n".join(
                [
                    ...
                    "zorin-gateway-coder.service loaded active running",
        ...
        assert seen == ["zorin-gateway-coder"]
    ...
    def test_process_errors_other_than_timeout_still_propagate(self):
        ...
        with pytest.raises(RuntimeError, match="not a timeout"):
            _for_each_systemd_gateway_unit(
                _list_units_stdout(["zorin-gateway"]),
                process_unit=process_unit,
...
class TestIncompleteFleetRestartWarning:
    def test_warns_with_exact_unrestarted_units(self, capsys):
        _warn_incomplete_gateway_fleet_restart(
            ["zorin-gateway-xiaomo5", "zorin-gateway-xiaomo6", "zorin-gateway-xiaomo5"]
        )
        out = capsys.readouterr().out
        assert "Update incomplete" in out
        assert out.count("zorin-gateway-xiaomo5") == 1
        assert "zorin-gateway-xiaomo6" in out
