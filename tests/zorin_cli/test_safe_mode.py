"""Tests for `zorin chat --safe-mode` isolation."""
...
_VARS = ("ZORIN_SAFE_MODE", "ZORIN_IGNORE_USER_CONFIG", "ZORIN_IGNORE_RULES")
...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["ZORIN_SAFE_MODE"] == "1"
        assert os.environ["ZORIN_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["ZORIN_IGNORE_RULES"] == "1"
        return True
...
def test_prepare_agent_startup_applies_safe_mode_before_plugin_discovery(monkeypatch):
    ...
    plugins = types.ModuleType("zorin_cli.plugins")
    ...
    monkeypatch.setitem(sys.modules, "zorin_cli.plugins", plugins)
...
def test_plugin_discovery_skipped(monkeypatch):
    monkeypatch.setenv("ZORIN_SAFE_MODE", "1")
