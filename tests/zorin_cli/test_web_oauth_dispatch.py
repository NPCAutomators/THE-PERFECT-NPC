"""Regression tests for the OAuth dispatcher in zorin_cli.web_server.
...
from zorin_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Zorin-Session-Token": _SESSION_TOKEN}
...
def _fake_npcautomators_device_data():
    return {
        "device_code": "device-code",
        "user_code": "NPCAUTOMATORS-1234",
        "verification_uri": "https://portal.npcautomators.com/device",
        "verification_uri_complete": (
            "https://portal.npcautomators.com/device?user_code=NPCAUTOMATORS-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.npcautomators.com/oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "zorin_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "zorin_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "zorin_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_npcautomators_dashboard_device_flow_ignores_legacy_scope_override(monkeypatch):
    from zorin_cli import auth as auth_mod
    from zorin_cli import web_server as ws

    requested_scopes = []

    def fake_request_device_code(**kwargs):
        requested_scopes.append(kwargs["scope"])
        return _fake_npcautomators_device_data()
    ...
    monkeypatch.setattr(ws, "_npcautomators_poller", lambda sid: None)

    result = asyncio.run(ws._start_device_code_flow("npcautomators"))
    try:
        assert requested_scopes == [auth_mod.DEFAULT_NPCAUTOMATORS_SCOPE]
        assert result["flow"] == "device_code"
        assert result["user_code"] == "NPCAUTOMATORS-1234"
        assert (
            ws._oauth_sessions[result["session_id"]]["scope"]
            == auth_mod.DEFAULT_NPCAUTOMATORS_SCOPE
        )
    finally:
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    ...
    def fake_status():
        observed_homes.append(get_zorin_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "zorin auth add fake-oauth",
...
def test_npcautomators_dashboard_device_flow_does_not_retry_legacy_scope_on_invoke_refusal(monkeypatch):
    from zorin_cli import auth as auth_mod
    from zorin_cli import web_server as ws
    ...
    monkeypatch.delenv("ZORIN_AGENT_USE_LEGACY_SESSION_KEYS", raising=False)
    monkeypatch.setattr(auth_mod, "_request_device_code", fake_request_device_code)
    monkeypatch.setattr(ws, "_npcautomators_poller", lambda sid: None)

    with pytest.raises(httpx.HTTPStatusError):
        asyncio.run(ws._start_device_code_flow("npcautomators"))
    assert requested_scopes == [auth_mod.DEFAULT_NPCAUTOMATORS_SCOPE]
...
def test_codex_dashboard_worker_persists_inside_session_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_mod,
        "_save_codex_tokens",
        lambda tokens: saved_homes.append(get_zorin_home()),
    )
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "zorin auth" not in detail
    finally:
        for sid in set(ws._oauth_sessions) - before_sessions:
            ws._oauth_sessions.pop(sid, None)


def test_npcautomators_dashboard_poller_preserves_effective_scope_when_token_omits_scope(monkeypatch):
    from zorin_cli import auth as auth_mod
    from zorin_cli import web_server as ws

    session_id = "npcautomators-effective-scope-test"
    ...
    def fake_refresh_npcautomators_oauth_from_state(state, **kwargs):
    ...
    monkeypatch.setattr(
        ...
        fake_refresh_npcautomators_oauth_from_state,
    )
    monkeypatch.setattr(auth_mod, "persist_npcautomators_credentials", lambda state: None)

    try:
        ws._npcautomators_poller(session_id)
        assert captured_state["scope"] == auth_mod.DEFAULT_NPCAUTOMATORS_SCOPE
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "zorin_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "zorin_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "zorin_cli.web_server._start_anthropic_pkce",
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `zorin model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `zorin model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Zorin."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``zorin auth remove
    xai-oauth``.
    """
    from zorin_cli import auth as auth_mod
    from zorin_cli import web_server as ws
    ...
    # Prior `zorin auth remove xai-oauth` left the source suppressed.
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "zorin auth add hypothetical-pkce-provider",
        "docs_url": "https://example.com",
        "status_fn": None,
    }
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to zorin_cli.auth.get_auth_status (the
    ...
    with patch("zorin_cli.auth.get_auth_status", return_value=fake_status):
...
def test_status_hardcoded_branch_wins_over_generic_fallback():
    """An existing hardcoded branch (npcautomators) is unaffected by the fallthrough."""
    import zorin_cli.web_server as ws

    with patch(
        "zorin_cli.auth.get_npcautomators_auth_status",
        return_value={"logged_in": True, "portal_base_url": "https://portal.test"},
    ):
        out = ws._resolve_provider_status("npcautomators", None)
    assert out["source"] == "npcautomators_portal"
...
def test_status_unknown_provider_degrades_to_logged_out():
    ...
    with patch("zorin_cli.auth.get_auth_status", return_value={"logged_in": False}):
