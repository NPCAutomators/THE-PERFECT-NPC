...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ZORIN_HOME with .container-mode file."""
    zorin_home = tmp_path / ".zorin"
    zorin_home.mkdir()
    monkeypatch.setenv("ZORIN_HOME", str(zorin_home))
    monkeypatch.delenv("ZORIN_DEV", raising=False)

    container_mode = zorin_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=zorin-agent\n"
        "exec_user=zorin\n"
        "zorin_bin=/data/current-package/bin/zorin\n"
    )
    return zorin_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("zorin_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "zorin-agent"
    assert info["exec_user"] == "zorin"
    assert info["zorin_bin"] == "/data/current-package/bin/zorin"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("zorin_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_zorin_dev(container_env, monkeypatch):
    """Returns None when ZORIN_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ZORIN_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_zorin_dev_zero(container_env, monkeypatch):
    """ZORIN_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ZORIN_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        zorin_home = Path(tmpdir) / ".zorin"
        zorin_home.mkdir()
        (zorin_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("zorin_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_zorin_home": lambda: zorin_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZORIN_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=zorin-custom\n"
        "exec_user=myuser\n"
        "zorin_bin=/opt/zorin/bin/zorin\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "zorin-custom"
    assert info["exec_user"] == "myuser"
    assert info["zorin_bin"] == "/opt/zorin/bin/zorin"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from zorin_cli.main import _exec_in_container
