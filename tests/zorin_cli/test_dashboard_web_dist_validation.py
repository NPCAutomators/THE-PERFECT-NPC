"""Regression tests: `zorin dashboard` validates ZORIN_WEB_DIST before serving.

A custom ZORIN_WEB_DIST without --skip-build previously skipped BOTH the
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """ZORIN_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("ZORIN_WEB_DIST", str(empty_dist))
    ...
    assert "ZORIN_WEB_DIST" in out and str(empty_dist) in out


def test_env_dist_with_index_starts_server(main_mod, monkeypatch, tmp_path):
    """A valid ZORIN_WEB_DIST (has index.html) proceeds to start_server
    without building."""
    _wire_common(main_mod, monkeypatch)
    dist = tmp_path / "dist"
    dist.mkdir()
    (dist / "index.html").write_text("<html></html>", encoding="utf-8")
    monkeypatch.setenv("ZORIN_WEB_DIST", str(dist))
...
def test_env_dist_tilde_expanded_for_web_server(main_mod, monkeypatch, tmp_path):
    """A '~/...' ZORIN_WEB_DIST must be written back expanded so
    ...
    monkeypatch.setenv("ZORIN_WEB_DIST", "~/mydist")
    ...
    import os
    assert os.environ["ZORIN_WEB_DIST"] == str(dist)
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    dist = project_root / "zorin_cli" / "web_dist"
    dist.mkdir(parents=True)
...
def test_skip_build_recovery_build_failure_preserves_fatal_exit(
    ...
    monkeypatch.delenv("ZORIN_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    (project_root / "zorin_cli" / "web_dist").mkdir(parents=True)
    monkeypatch.setattr(main_mod, "PROJECT_ROOT", project_root)
...
def test_skip_build_custom_env_dist_missing_does_not_attempt_recovery(
    ...
    """A custom ZORIN_WEB_DIST is caller-managed: the recovery build writes
