"""Tests for the ``zorin prompt-size`` diagnostic (issue #34667)."""
...
from zorin_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(zorin_home, memory_text="", user_text=""):
    mem_dir = zorin_home / "memories"
...
def _seed_skill(zorin_home, name, description):
    skill_dir = zorin_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "NPCAUTOMATORS_API_KEY",
                "ANTHROPIC_API_KEY"):
