...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    ZORIN_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_zorin_argv", lambda: ["zorin"])
    ...
    assert pid == 4242
    assert captured["env"]["ZORIN_HOME"] == str(profile)
    assert captured["env"]["ZORIN_KANBAN_TASK"] == "t_spawn_tools"
...
def test_default_spawn_never_boots_the_tui(monkeypatch, tmp_path):
    """Workers are headless: an inherited ZORIN_TUI=1 (or a TUI-default
    ...
    flag) and strips ZORIN_TUI from the child env."""
    root = tmp_path / ".zorin"
    ...
    assert "ZORIN_TUI" not in captured["env"]
