...
the SAME provider universe as ``zorin model`` (the CLI/TUI picker).
...
def test_every_zorin_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `zorin model` universe."""
    ...
    assert not missing, (
        "providers shown in `zorin model` but not configurable in the desktop "
