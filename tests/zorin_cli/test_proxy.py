"""Tests for the `zorin proxy` subcommand and its upstream adapters."""
...
from zorin_cli.proxy.adapters import ADAPTERS, get_adapter
from zorin_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from zorin_cli.proxy.adapters.npcautomators_portal import NpcAutomatorsPortalAdapter
from zorin_cli.proxy.adapters.xai import XAIGrokAdapter
...
def test_registry_lists_npcautomators():
    assert "npcautomators" in ADAPTERS
...
def test_get_adapter_returns_instance():
    adapter = get_adapter("npcautomators")
    assert isinstance(adapter, NpcAutomatorsPortalAdapter)
...
def test_get_adapter_case_insensitive():
    assert isinstance(get_adapter("NPCAUTOMATORS"), NpcAutomatorsPortalAdapter)
    assert isinstance(get_adapter("  NpcAutomators  "), NpcAutomatorsPortalAdapter)
...
# ---------------------------------------------------------------------------
# NpcAutomatorsPortalAdapter
# ---------------------------------------------------------------------------


def _write_auth_store(zorin_home: Path, npcautomators_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given npcautomators state into a hermetic ZORIN_HOME."""
    auth_path = zorin_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"npcautomators": npcautomators_state},
    }))
    return auth_path


def test_npcautomators_adapter_metadata():
    adapter = NpcAutomatorsPortalAdapter()
    assert adapter.name == "npcautomators"
    assert adapter.display_name == "NPCAUTOMATORS Portal"
...
def test_npcautomators_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # ZORIN_HOME is already set by conftest, but make doubly sure
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    adapter = NpcAutomatorsPortalAdapter()
    assert not adapter.is_authenticated()


def test_npcautomators_adapter_not_authenticated_when_provider_missing(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    (tmp_path / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    }))
    assert not NpcAutomatorsPortalAdapter().is_authenticated()


def test_npcautomators_adapter_authenticated_with_agent_key(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    ...
    assert NpcAutomatorsPortalAdapter().is_authenticated()


def test_npcautomators_adapter_authenticated_with_refresh_token_only(tmp_path, monkeypatch):
...
def test_npcautomators_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "access-tok",
        "refresh_token": "refresh-tok",
    ...
    with patch(
        "zorin_cli.proxy.adapters.npcautomators_portal.resolve_npcautomators_runtime_credentials",
    ...
    assert cred.base_url == "https://inference-api.npcautomators.com/v1"
    assert cred.expires_at == "2099-01-01T00:00:00Z"
    assert cred.token_type == "Bearer"


def test_npcautomators_adapter_retry_credential_force_refreshes_on_jwt_401(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
...
def test_npcautomators_adapter_retry_credential_skips_non_401(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "jwt-access",
        "refresh_token": "refresh-tok",
...
def test_npcautomators_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    adapter = NpcAutomatorsPortalAdapter()
    with pytest.raises(RuntimeError, match="zorin auth add npcautomators"):
        adapter.get_credential()


def test_npcautomators_adapter_get_credential_raises_on_refresh_failure(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "access-tok",
        "refresh_token": "refresh-tok",
...
def test_npcautomators_adapter_quarantines_terminal_refresh_failure(tmp_path, monkeypatch):
    ...
    npcautomators_state = stored["providers"]["npcautomators"]
    ...
    assert stored.get("credential_pool", {}).get("npcautomators") == []


def test_npcautomators_adapter_get_credential_raises_when_no_jwt_returned(tmp_path, monkeypatch):
...
def test_npcautomators_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic ZORIN_HOME."""
    auth_path = zorin_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from zorin_cli.proxy.server import create_app  # noqa: E402
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "Zorin-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    from zorin_cli.proxy.cli import cmd_proxy_status

    args = MagicMock()
    rc = cmd_proxy_status(args)
    assert rc == 0
    out = capsys.readouterr().out
    assert "npcautomators" in out
    assert "NPCAUTOMATORS Portal" in out
    assert "not logged in" in out


def test_cmd_proxy_providers_runs(capsys):
    from zorin_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from zorin_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    args = MagicMock()
    args.provider = "npcautomators"
    args.host = None
    args.port = None
    rc = cmd_proxy_start(args)
    assert rc == 2
    err = capsys.readouterr().err
    assert "zorin auth add npcautomators" in err
