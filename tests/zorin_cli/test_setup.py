...
from zorin_cli import setup as setup_mod
from zorin_cli.setup import setup_model_provider
...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("zorin_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.main.select_provider_and_model", fake_select)
...
def test_setup_syncs_npcautomators_from_disk(tmp_path, monkeypatch):
    """NpcAutomators OAuth writes config to disk; wizard config dict must pick it up."""
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    _stub_tts(monkeypatch)
    ...
    def fake_select():
        _write_model_config(tmp_path, "npcautomators", "https://inference.example.com/v1", "gemini-3-flash")
    ...
    reloaded = load_config()
    assert isinstance(reloaded["model"], dict)
    assert reloaded["model"]["provider"] == "npcautomators"
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import zorin_cli.gateway as _gw_mod
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("zorin_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("zorin_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "zorin_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "zorin_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_npcautomators_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("zorin_cli.setup.managed_npcautomators_tools_enabled", lambda: True)
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("zorin_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("zorin_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("zorin_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "zorin_cli.setup.get_npcautomators_subscription_features",
        lambda config: type("Features", (), {"npcautomators_auth_present": True})(),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    from zorin_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("zorin_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("zorin_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "zorin_cli.setup.get_npcautomators_subscription_features",
        lambda config: type("Features", (), {"npcautomators_auth_present": True})(),
...
def test_prompt_yes_no_returns_default_when_noninteractive_env_set(monkeypatch):
    """ZORIN_NONINTERACTIVE=1 (set by dashboard/desktop spawns) must make
    prompt_yes_no fall back to its default instead of reading stdin."""
    monkeypatch.setenv("ZORIN_NONINTERACTIVE", "1")
...
def test_prompt_yes_no_eof_returns_default_instead_of_exiting(monkeypatch):
    ...
    monkeypatch.delenv("ZORIN_NONINTERACTIVE", raising=False)
