"""Tests for normalized NPCAUTOMATORS Portal account entitlement helpers."""
...
from zorin_cli.npcautomators_account import (
    ...
    npcautomators_portal_topup_url,
    reset_npcautomators_portal_account_info_cache,
)
...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "zorin-cli",
            "product_id": "npcautomators-zorin-agent",
            "npcautomators_client": "zorin-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("zorin_cli.auth.get_provider_auth_state", lambda provider: _state(token))

    info = get_npcautomators_portal_account_info()
    ...
    assert info.product_id == "npcautomators-zorin-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("zorin_cli.auth.resolve_npcautomators_access_token", lambda: "fresh-token")
    monkeypatch.setattr("zorin_cli.npcautomators_account._fetch_npcautomators_account_info", lambda *a, **kw: payload)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    assert isinstance(info, NpcAutomatorsPortalAccountInfo)
    assert info.source == "account_api"
    assert info.fresh is True
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("zorin_cli.auth.get_provider_auth_state", lambda provider: {})

    class _Entry:
        label = "manual-npcautomators"
        access_token = ""
    ...
    assert info.credential_source == "pool:manual-npcautomators"
    assert info.paid_service_access is None
...
def test_entitlement_message_returns_none_for_paid_access():
    info = NpcAutomatorsPortalAccountInfo(
        logged_in=True,
        source="account_api",
        fresh=True,
    ...
    assert format_npcautomators_portal_entitlement_message(info, capability="paid models") is None
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "NpcAutomators inference credentials are configured" in message
    assert "cannot verify your NPCAUTOMATORS Portal paid access" in message
    assert "Log in with `zorin model`" in message
...
def test_entitlement_message_for_no_subscription_or_credits():
    ...
    message = format_npcautomators_portal_entitlement_message(info, capability="paid models")
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    message = format_npcautomators_portal_entitlement_message(info, capability="Tool Gateway")

    assert message is not None
    assert "could not verify" in message
    assert "account_api_timeout" in message
    assert "Run `zorin model`" in message
...
def test_entitlement_message_for_account_missing():
    ...
    assert message is not None
    assert "could not find a NPCAUTOMATORS Portal account or organisation" in message
...
def test_topup_url_is_org_pinned_when_slug_present():
    ...
    assert (
        npcautomators_portal_topup_url(info)
...
def test_topup_url_falls_back_to_legacy_when_slug_null():
    ...
    url = npcautomators_portal_topup_url(info)
...
def test_topup_url_defaults_to_production_portal_for_none():
    url = npcautomators_portal_topup_url(None)
    assert url == "https://portal.npcautomators.com/billing?topup=open"
