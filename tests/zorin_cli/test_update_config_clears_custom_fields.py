"""Tests for zorin_cli.auth._update_config_for_provider clearing stale fields.
...
from zorin_cli.config import clear_model_endpoint_credentials, get_config_path
...
class TestUpdateConfigForProviderClearsStaleCustomFields:
    ...
    def test_switching_to_npcautomators_clears_stale_api_mode(self):
        _seed_custom_provider_config()
        _update_config_for_provider("npcautomators", "https://inference-api.npcautomators.com/v1")
        model_cfg = _read_model_cfg()
        assert model_cfg.get("provider") == "npcautomators"
        assert "api_mode" not in model_cfg
        assert "api_key" not in model_cfg
