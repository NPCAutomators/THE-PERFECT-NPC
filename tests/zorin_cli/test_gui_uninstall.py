"""Tests for zorin_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import zorin_cli.gui_uninstall as gu


def _make_agent(zorin_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = zorin_home / "zorin-agent"
    (agent_root / "zorin_cli").mkdir(parents=True)
    (agent_root / "zorin_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(zorin_home: Path) -> None:
    """Create the source-built GUI artifacts a `zorin desktop` run produces."""
    desktop = zorin_home / "zorin-agent" / "apps" / "desktop"
    ...
    (zorin_home / "zorin-agent" / "node_modules").mkdir(parents=True)
    (zorin_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(zorin_home: Path) -> None:
    (zorin_home / "config.yaml").write_text("x: 1\n")
    (zorin_home / ".env").write_text("KEY=secret\n")
    (zorin_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    zorin_home = tmp_path / ".zorin"
    zorin_home.mkdir()
    assert gu.agent_is_installed(zorin_home) is False
    _make_agent(zorin_home)
    assert gu.agent_is_installed(zorin_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    zorin_home = tmp_path / ".zorin"
    (zorin_home / "zorin-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(zorin_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    zorin_home = tmp_path / ".zorin"
    _make_gui_build(zorin_home)
    artifacts = gu.source_built_gui_artifacts(zorin_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(zorin_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(zorin_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    zorin_home = tmp_path / ".zorin"
    agent_root = _make_agent(zorin_home)
    _make_gui_build(zorin_home)
    _make_user_data(zorin_home)
    ...
    removed = gu.uninstall_gui(zorin_home)
    ...
    assert not (zorin_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "zorin_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (zorin_home / "config.yaml").exists()
    assert (zorin_home / ".env").exists()
    assert (zorin_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    zorin_home = tmp_path / ".zorin"
    _make_agent(zorin_home)
    userdata = tmp_path / "Zorin-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(zorin_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(zorin_home)
    ...
    assert summary["zorin_home"] == str(zorin_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Zorin"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Zorin"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Zorin"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Zorin"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    ZORIN_HOME with all the destructive externals stubbed out.
    """
    import zorin_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_zorin_home", lambda: zorin_home)
    ...
    from zorin_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (zorin_home / "config.yaml").exists()
    assert zorin_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole ZORIN_HOME non-interactively."""
    import zorin_cli.uninstall as uninstall

    zorin_home = tmp_path / ".zorin"
    (zorin_home / "zorin-agent" / "zorin_cli").mkdir(parents=True)
    (zorin_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not zorin_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m zorin_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_zorin_home", lambda: zorin_home)
    ...
    assert not (desktop / "release").exists()
    assert not (zorin_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "zorin_cli").exists()
    assert (zorin_home / "config.yaml").exists()
