"""Regression tests for #34107 — Docker UID/GID handling in ensure_zorin_home.

When Zorin runs in Docker with ``ZORIN_UID=1000`` / ``ZORIN_GID=911``,
the entrypoint chowns the top-level ``ZORIN_HOME`` once at startup. But
...
The fix is a ``_chown_to_zorin_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_zorin_uid_gid
# ---------------------------------------------------------------------------


class TestResolveZorinUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ZORIN_UID", "1000")
        monkeypatch.setenv("ZORIN_GID", "911")
        from zorin_cli.config import _resolve_zorin_uid_gid
        uid, gid = _resolve_zorin_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ZORIN_UID", raising=False)
        monkeypatch.delenv("ZORIN_GID", raising=False)
        from zorin_cli.config import _resolve_zorin_uid_gid
        uid, gid = _resolve_zorin_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("ZORIN_UID", "not-a-number")
        monkeypatch.setenv("ZORIN_GID", "911")
        from zorin_cli.config import _resolve_zorin_uid_gid
        uid, gid = _resolve_zorin_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("ZORIN_UID", "")
        monkeypatch.setenv("ZORIN_GID", "")
        from zorin_cli.config import _resolve_zorin_uid_gid
        uid, gid = _resolve_zorin_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToZorinUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_zorin_uid(d)  # must not raise
