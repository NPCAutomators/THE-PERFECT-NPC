"""Tests for zorin_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Zorin entry point
(zorin, zorin-agent, zorin-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Zorin entry point imports zorin_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported zorin_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("zorin_bootstrap", None)
    import zorin_bootstrap  # noqa: WPS433
    return zorin_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Zorin CLI
...
class TestEntryPointsImportBootstrap:
    """Every Zorin entry point must import zorin_bootstrap as its
    ...
    # Entry points that invoke Zorin as a process.  Each one must
    # import zorin_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "zorin_cli/main.py",   # zorin CLI (console_script)
        "run_agent.py",          # zorin-agent (console_script)
        "acp_adapter/entry.py",  # zorin-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import zorin_bootstrap' and that
        ...
        half-finished ``zorin update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``zorin_bootstrap`` as a top-level module) leaves zorin
        ...
        """
        # Resolve relative to the zorin-agent repo root.  Tests live
        # at tests/test_zorin_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # zorin start even when zorin_bootstrap hasn't been
        ...
        assert first_import_name == "zorin_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'zorin_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import zorin_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Zorin's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ZORIN_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ZORIN_PYTHON_SRC_ROOT"] = env
            elif "ZORIN_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ZORIN_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/zorin")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ZORIN_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ZORIN_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/zorin", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Zorin root.  The guard must relocate Zorin to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/zorin"])
        assert result[0] == "/opt/zorin"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Zorin root.
        assert result.index("/opt/zorin") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/zorin", "/opt/zorin", ""])
        assert result.count("/opt/zorin") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ZORIN_PYTHON_SRC_ROOT"] = "/env/zorin"
            hb.harden_import_path()
            assert sys.path[0] == "/env/zorin"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ZORIN_PYTHON_SRC_ROOT", None)
