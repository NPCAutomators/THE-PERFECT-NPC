...
from zorin_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(zorin_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <zorin_home>/plugins/<name> and opt it in."""
    plugin_dir = zorin_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path / "zorin_empty"))
