"""Shared fixtures for the zorin-agent test suite.
...
2. **Isolated ZORIN_HOME.** ZORIN_HOME points to a per-test tempdir so
   code reading ``~/.zorin/*`` via ``get_zorin_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".zorin"`` instead of the canonical
   ``get_zorin_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ZORIN_SESSION_* inheritance** — the agent's current gateway
...
# ZORIN_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ZORIN_BEHAVIORAL_VARS = frozenset({
    "ZORIN_YOLO_MODE",
    "ZORIN_INTERACTIVE",
    "ZORIN_QUIET",
    "ZORIN_TOOL_PROGRESS",
    "ZORIN_TOOL_PROGRESS_MODE",
    "ZORIN_MAX_ITERATIONS",
    "ZORIN_SESSION_PLATFORM",
    "ZORIN_SESSION_CHAT_ID",
    "ZORIN_SESSION_CHAT_NAME",
    "ZORIN_SESSION_THREAD_ID",
    "ZORIN_SESSION_SOURCE",
    "ZORIN_SESSION_KEY",
    "ZORIN_GATEWAY_SESSION",
    "ZORIN_CRON_SESSION",
    "_ZORIN_GATEWAY",
    "ZORIN_PLATFORM",
    "ZORIN_MODEL",
    "ZORIN_INFERENCE_MODEL",
    "ZORIN_INFERENCE_PROVIDER",
    "ZORIN_TUI_PROVIDER",
    "ZORIN_MANAGED",
    "ZORIN_MANAGED_DIR",
    "ZORIN_DEV",
    ...
    "ZORIN_TIMEZONE",
    "ZORIN_REDACT_SECRETS",
    "ZORIN_BACKGROUND_NOTIFICATIONS",
    "ZORIN_EXEC_ASK",
    "ZORIN_HOME_MODE",
    "ZORIN_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.zorin/kanban.db instead of the per-test ZORIN_HOME.
    "ZORIN_KANBAN_DB",
    "ZORIN_KANBAN_BOARD",
    "ZORIN_KANBAN_HOME",
    "ZORIN_KANBAN_WORKSPACES_ROOT",
    "ZORIN_KANBAN_LOGS_ROOT",
    "ZORIN_KANBAN_TASK",
    "ZORIN_KANBAN_WORKSPACE",
    "ZORIN_KANBAN_RUN_ID",
    "ZORIN_KANBAN_CLAIM_LOCK",
    "ZORIN_KANBAN_DISPATCH_IN_GATEWAY",
    "ZORIN_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "zorin" host.
    "ZORIN_HONCHO_HOST",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `npcautomators` plugin auto-registers itself on plugin discovery,
    ...
    "ZORIN_DASHBOARD_OAUTH_CLIENT_ID",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ZORIN_HOME to per-test tempdirs so code that
    reads ``~/.zorin/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ZORIN_* vars that could change test semantics.
    for name in _ZORIN_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ZORIN_HONCHO_HOST", "zorin")

    # 3. Redirect ZORIN_HOME to a per-test tempdir. Code that reads
    #    ``~/.zorin/*`` via ``get_zorin_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.zorin/*`` via
    #    ``Path.home() / ".zorin"`` instead of ``get_zorin_home()``
    #    is a bug to fix at the callsite.
    fake_zorin_home = tmp_path / "zorin_test"
    fake_zorin_home.mkdir()
    (fake_zorin_home / "sessions").mkdir()
    (fake_zorin_home / "cron").mkdir()
    (fake_zorin_home / "memories").mkdir()
    (fake_zorin_home / "skills").mkdir()
    monkeypatch.setenv("ZORIN_HOME", str(fake_zorin_home))
    ...
    #    ~/.zorin/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import zorin_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_zorin_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal zorin config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``zorin-gateway`` process
...
# correlated with a ``tests/zorin_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> zorin-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart zorin-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting zorin/python patterns are also blocked.
    """
    ...
    def _matches_zorin_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ZORIN_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_zorin_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch zorin-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m zorin_cli.main".
                if (
                    "zorin" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live zorin-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting zorin/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `zorin update` (or the
        # equivalent `python -m zorin_cli.main update`).  These commands
        ...
        if "update" in low and (
            # zorin update / zorin update --gateway / setsid bash -c ... zorin update
            ("zorin" in low and "update" in low.split())
            or
            # python -m zorin_cli.main update --gateway
            ("zorin_cli" in low and "update" in low.split())
            or
            # venv/bin/zorin update  (absolute path variant used in tests)
            (".venv/bin/zorin" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`zorin update` against the real checkout, fetching "
