...
class TestGetDefaultModelForProvider:
    """Unit tests for zorin_cli.models.get_default_model_for_provider."""
    ...
    def test_npcautomators_silent_default_is_not_the_expensive_flagship(self):
        """NPCAUTOMATORS Portal is a metered aggregator whose curated list is ordered
        ...
        result = get_default_model_for_provider("npcautomators")
        assert result, "npcautomators must resolve to a usable default model"
        ...
        assert result != _PROVIDER_MODELS["npcautomators"][0], (
        ...
        assert result == get_preferred_silent_default_model("npcautomators")
        assert result in _PROVIDER_MODELS["npcautomators"]
    ...
    def test_catalog_label_overrides_constant(self):
        ...
        with patch(
            "zorin_cli.model_catalog.get_default_model_from_cache",
            return_value="qwen/qwen3.7-plus",
        ):
            assert (
                models_mod.get_preferred_silent_default_model("npcautomators")
                == "qwen/qwen3.7-plus"
            )
            # npcautomators catalog carries qwen3.7-plus, so the full resolver follows.
            assert (
                models_mod.get_default_model_for_provider("npcautomators")
                == "qwen/qwen3.7-plus"
            )
    ...
    def test_stale_label_not_in_catalog_falls_back(self):
        ...
        with patch(
            ...
            result = models_mod.get_default_model_for_provider("npcautomators")
            assert result == models_mod._PROVIDER_MODELS["npcautomators"][0]
...
class TestDetectStaticProviderCostSafeDefault:
    ...
    model (e.g. ``/model npcautomators``)."""

    def test_bare_npcautomators_does_not_escalate_to_flagship(self):
        from zorin_cli.models import (
            _PROVIDER_MODELS,
            get_default_model_for_provider,
        ...
        result = detect_static_provider_for_model("npcautomators", "openrouter")
        assert result is not None
        provider, model = result
        assert provider == "npcautomators"
        # Must match the cost-safe silent default, NOT the priciest catalog
        # entry [0]. Regression: this path returned _PROVIDER_MODELS["npcautomators"][0]
        # directly, re-introducing the billing footgun on the interactive
        # ``/model npcautomators`` path.
        assert model == get_default_model_for_provider("npcautomators")
        assert "opus" not in model.lower()
        assert model != _PROVIDER_MODELS["npcautomators"][0]
