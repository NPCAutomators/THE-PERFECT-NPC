"""Tests for the secret-source tracking in ``zorin_cli.env_loader``.

These cover the small public surface that lets `zorin model` / `zorin setup`
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_zorin_dotenv()`` is called at module-import time from several
    hot modules (cli.py, zorin_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per ZORIN_HOME per process.
    """
    ...
    # cli.py, zorin_cli/main.py, run_agent.py, trajectory_compressor.py,
