...
class TestKreaImageGenProvider:
    ...
    def test_is_available_via_managed_gateway_without_key(self, monkeypatch):
        ...
        # No direct key but the managed NpcAutomators gateway is ready → available.
...
class TestGenerate:
    ...
    def test_aspect_ratio_mapping(self):
        """Zorin 'square' must map to Krea '1:1' in the wire payload."""
        from plugins.image_gen.krea import KreaImageGenProvider
...
# ---------------------------------------------------------------------------
# Managed NpcAutomators gateway path
# ---------------------------------------------------------------------------


def _managed_cfg(
    origin: str = "https://krea-gateway.example.com",
    token: str = "npcautomators-tok-abc",
):
    ...
    return SimpleNamespace(
        ...
        npcautomators_user_token=token,
        managed_mode=True,
    )


class TestManagedGateway:
    def test_managed_submit_uses_gateway_origin_and_npcautomators_token(self, monkeypatch):
        """Managed mode submits to the gateway origin with the NpcAutomators token."""
        import plugins.image_gen.krea as krea_mod
        from plugins.image_gen.krea import KreaImageGenProvider
    ...
    def test_managed_4xx_returns_actionable_remediation(self, monkeypatch):
        ...
        assert "NpcAutomators Subscription Krea gateway" in result["error"]
        assert "KREA_API_KEY" in result["error"]
