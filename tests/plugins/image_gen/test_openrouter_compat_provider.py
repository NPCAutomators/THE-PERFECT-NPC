#!/usr/bin/env python3
"""Tests for the OpenRouter-compatible image gen provider (OpenRouter + NpcAutomators)."""
...
_RUNTIME = "zorin_cli.runtime_provider.resolve_runtime_provider"
...
class TestProviderClass:
    def test_names(self):
        ...
        assert names == {"openrouter", "npcautomators"}
    ...
    def test_display_names(self):
        ...
        assert by_name["npcautomators"].display_name == "NPCAUTOMATORS Portal"
    ...
    def test_npcautomators_honors_top_level_model(self):
        from plugins.image_gen.openrouter import _build_providers

        cfg = {"model": "openai/gpt-image-2"}
        npcautomators = {p.name: p for p in _build_providers()}["npcautomators"]
        with patch("plugins.image_gen.openrouter._load_image_gen_config", return_value=cfg):
            assert npcautomators._resolve_model_chain() == ["openai/gpt-image-2"]
...
class TestGenerate:
    ...
    def test_posts_to_resolved_base_url(self):
        """NpcAutomators routes to its own base URL — proves the same code serves both."""
        npcautomators_runtime = _runtime_ok(
            provider="npcautomators", base_url="https://inference.npcautomators.com/v1", api_key="npcautomators-tok"
        )
        with patch(_RUNTIME, return_value=npcautomators_runtime), \
             patch("requests.post", return_value=_mock_chat_response([_PNG_DATA_URI])) as mock_post, \
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            from plugins.image_gen.openrouter import _build_providers

            npcautomators = {p.name: p for p in _build_providers()}["npcautomators"]
            result = npcautomators.generate(prompt="a pet")

        assert result["success"] is True
        assert result["provider"] == "npcautomators"
        url = mock_post.call_args[0][0]
        assert url == "https://inference.npcautomators.com/v1/chat/completions"
...
class TestRegistration:
    def test_register_both(self):
        ...
        assert set(registered) == {"openrouter", "npcautomators"}
    ...
    def test_both_are_reference_capable_for_pets(self):
        ...
        assert "npcautomators" in _REF_CAPABLE
