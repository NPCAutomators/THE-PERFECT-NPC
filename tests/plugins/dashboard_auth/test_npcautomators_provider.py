"""Tests for the bundled NpcAutomators dashboard-auth plugin.

Covers four shapes from Phase 4 of ``.zorin/plans/2026-05-21-dashboard-oauth-auth.md``:
...
import plugins.dashboard_auth.npcautomators as npcautomators_plugin
...
def _patched_jwks(provider: npcautomators_plugin.NpcAutomatorsDashboardAuthProvider, rsa_keypair):
...
class TestConstruction:
    def test_protocol_compliance(self):
        assert_protocol_compliance(npcautomators_plugin.NpcAutomatorsDashboardAuthProvider)

    def test_name_and_display(self):
        p = npcautomators_plugin.NpcAutomatorsDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
        assert p.name == "npcautomators"
        assert p.display_name == "NPCAUTOMATORS"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            npcautomators_plugin.NpcAutomatorsDashboardAuthProvider(
                client_id="zorin-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        monkeypatch.delenv("ZORIN_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
        monkeypatch.delenv("ZORIN_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        npcautomators_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason is surfaced for the gate's fail-closed message.
        assert "ZORIN_DASHBOARD_OAUTH_CLIENT_ID" in npcautomators_plugin.LAST_SKIP_REASON

    def test_registers_with_default_portal_url_when_only_client_id_set(
        self, monkeypatch
    ):
        """Phase 7 follow-up: ZORIN_DASHBOARD_PORTAL_URL is optional —
        defaults to the production NPCAUTOMATORS Portal. The user shouldn't have
        to set it for the common production deployment path."""
        monkeypatch.setenv("ZORIN_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.delenv("ZORIN_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        npcautomators_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert isinstance(registered, npcautomators_plugin.NpcAutomatorsDashboardAuthProvider)
        assert registered._portal_url == "https://portal.npcautomators.com"
        # Skip reason cleared on successful registration.
        assert npcautomators_plugin.LAST_SKIP_REASON == ""

    def test_skips_when_client_id_malformed(self, monkeypatch):
        monkeypatch.setenv("ZORIN_DASHBOARD_OAUTH_CLIENT_ID", "zorin-dashboard")
        monkeypatch.setenv("ZORIN_DASHBOARD_PORTAL_URL", "https://p.example")
        ctx = MagicMock()
        npcautomators_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason names the offending value + contract shape.
        assert "agent:" in npcautomators_plugin.LAST_SKIP_REASON
        assert "zorin-dashboard" in npcautomators_plugin.LAST_SKIP_REASON
    ...
    def test_strips_whitespace_from_env_vars(self, monkeypatch):
        monkeypatch.setenv("ZORIN_DASHBOARD_OAUTH_CLIENT_ID", "  agent:x  ")
        monkeypatch.setenv("ZORIN_DASHBOARD_PORTAL_URL", "  https://p.example  ")
        ctx = MagicMock()
        npcautomators_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        monkeypatch.setenv("ZORIN_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.setenv("ZORIN_DASHBOARD_PORTAL_URL", "")
        ctx = MagicMock()
        npcautomators_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._portal_url == "https://portal.npcautomators.com"
...
class TestConfigYamlSource:
    ...
    canonical surface for these settings. ``ZORIN_DASHBOARD_OAUTH_CLIENT_ID``
    and ``ZORIN_DASHBOARD_PORTAL_URL`` are operator overrides that win when
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``zorin_cli.config.load_config``
        ...
        def _set(oauth_block: Dict[str, Any] | None) -> None:
            ...
            monkeypatch.setattr(
                "zorin_cli.config.load_config", lambda: cfg
            )
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        ZORIN_DASHBOARD_OAUTH_CLIENT_ID at deploy time and we MUST
        honour it even if a stale config.yaml ships in the image."""
        monkeypatch.setenv("ZORIN_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        npcautomators_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_env_overrides_config_portal_url(self, patch_config, monkeypatch):
        monkeypatch.setenv("ZORIN_DASHBOARD_OAUTH_CLIENT_ID", "agent:x")
        monkeypatch.setenv(
            "ZORIN_DASHBOARD_PORTAL_URL", "https://env.portal.example",
        )
        patch_config({
            "client_id": "agent:x",
    ...
    def test_empty_env_string_does_not_shadow_config(
        ...
        """``ZORIN_DASHBOARD_OAUTH_CLIENT_ID=`` (set but empty) is
        ...
        monkeypatch.setenv("ZORIN_DASHBOARD_OAUTH_CLIENT_ID", "")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        npcautomators_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
    ...
    def test_neither_source_skips_with_helpful_reason(
        ...
        assert "dashboard.oauth.client_id" in npcautomators_plugin.LAST_SKIP_REASON, (
            f"skip reason omits the config.yaml surface — operators "
            f"won't know it exists. got: {npcautomators_plugin.LAST_SKIP_REASON!r}"
        )
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        monkeypatch.setattr(
            "zorin_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        npcautomators_plugin.register(ctx)
...
class TestStartLogin:
    @pytest.fixture
    def provider(self):
        return npcautomators_plugin.NpcAutomatorsDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )

    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://zorin.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://zorin.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "zorin_session_pkce" in result.cookie_payload
        pkce = result.cookie_payload["zorin_session_pkce"]
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["zorin_session_pkce"] != b.cookie_payload[
            "zorin_session_pkce"
        ]
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://zorin.fly.dev/auth/callback")
...
class TestCompleteLogin:
    ...
    def test_happy_path_returns_session(self, provider, rsa_keypair):
        ...
        with patch("plugins.dashboard_auth.npcautomators.httpx.post", return_value=mock_resp):
        ...
        assert session.provider == "npcautomators"
    ...
    def test_network_error_raises_provider_error(self, provider):
        with patch(
            "plugins.dashboard_auth.npcautomators.httpx.post",
...
class TestVerifySession:
    ...
    def test_verification_failure_message_surfaces_token_claims(
        ...
        config drift between ZORIN_DASHBOARD_PORTAL_URL/CLIENT_ID and Portal."""
        token = _mint_token(rsa_keypair, iss="https://evil.example")
    ...
    def test_contract_version_missing_warns_but_succeeds(
        ...
        with caplog.at_level(logging.WARNING, logger="plugins.dashboard_auth.npcautomators"):
            session = provider.verify_session(access_token=token)
        assert session is not None
        assert any(
...
class TestRefreshAndRevoke:
    ...
    def test_refresh_happy_path_returns_rotated_session(self, provider, rsa_keypair):
        ...
        with patch(
            "plugins.dashboard_auth.npcautomators.httpx.post", return_value=mock_resp
        ) as mock_post:
        ...
        assert kwargs["headers"]["x-npcautomators-refresh-token"] == "rt_old_value"
    ...
    def test_refresh_empty_token_raises_refresh_expired_without_network(self, provider):
        ...
        with patch("plugins.dashboard_auth.npcautomators.httpx.post") as mock_post:
