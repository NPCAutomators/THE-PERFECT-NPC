...
Run from the PR worktree:
    ...
    cd ~/.zorin/zorin-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.zorin/zorin-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.zorin/zorin-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a zorin-agent checkout"
)
...
# Isolated ZORIN_HOME for the config write.
