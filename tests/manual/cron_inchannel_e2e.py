...
Uses a throwaway ZORIN_HOME so it never touches ~/.zorin. No real names.
"""
...
def _fresh_home():
    """Point ZORIN_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_zorin_home() at import time)."""
...
# Import AFTER ZORIN_HOME is set.
...
# a different get_zorin_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ZORIN_HOME (throwaway): {HOME}")
