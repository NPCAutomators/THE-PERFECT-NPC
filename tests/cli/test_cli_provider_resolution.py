...
from zorin_cli import main as zorin_main
...
def test_zorin_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in ZorinCLI.__init__")

    monkeypatch.setattr("zorin_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("zorin_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.ZorinCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("zorin_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.ZorinCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "zorin_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.ZorinCLI(compact=True, max_turns=1)
...
def test_model_flow_npcautomators_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setattr(
        "zorin_cli.npcautomators_subscription.managed_npcautomators_tools_enabled",
        lambda *args, **kwargs: True,
    )
    config = {
        "model": {"provider": "npcautomators", "default": "claude-opus-4-6"},
        "tts": {"provider": "elevenlabs"},
        "browser": {"cloud_provider": "browser-use"},
    }

    monkeypatch.setattr(
        "zorin_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "npcautomators-token"},
    )
    monkeypatch.setattr(
        "zorin_cli.auth.resolve_npcautomators_runtime_credentials",
        lambda *args, **kwargs: {
            "base_url": "https://inference.example.com/v1",
            "api_key": "npcautomators-key",
        },
    )
    monkeypatch.setattr(
        "zorin_cli.auth.fetch_npcautomators_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("zorin_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("zorin_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("zorin_cli.auth._update_config_for_provider", lambda provider, url: None)

    zorin_main._model_flow_npcautomators(config, current_model="claude-opus-4-6")
...
def test_model_flow_npcautomators_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    import yaml

    config_home = tmp_path / "zorin"
    config_home.mkdir()
    monkeypatch.setenv("ZORIN_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "zorin_cli.auth.get_provider_auth_state",
        lambda provider: {
            "access_token": "npcautomators-token",
    ...
    monkeypatch.setattr(
        "zorin_cli.models.get_curated_npcautomators_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("zorin_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("zorin_cli.models.check_npcautomators_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "zorin_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "zorin_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "zorin_cli.npcautomators_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    zorin_main._model_flow_npcautomators(stale_config, current_model="glm-5.2")

    config = yaml.safe_load(config_path.read_text()) or {}
    model = config.get("model")
    assert model["provider"] == "npcautomators"
    assert model["default"] == selected_model
    assert model["base_url"] == "https://inference-api.npcautomators.com/v1"
...
def test_model_flow_openrouter_clears_stale_custom_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zorin_cli.main._prompt_api_key",
        lambda *args, **kwargs: ("sk-openrouter", False),
    )
    monkeypatch.setattr(
        "zorin_cli.models.model_ids",
        lambda **kwargs: ["anthropic/claude-sonnet-4.6"],
    )
    monkeypatch.setattr("zorin_cli.models.get_pricing_for_provider", lambda *a, **k: {})
    monkeypatch.setattr(
        "zorin_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: "anthropic/claude-sonnet-4.6",
    )
    monkeypatch.setattr("zorin_cli.auth.deactivate_provider", lambda: None)

    zorin_main._model_flow_openrouter({}, current_model="glm-5.2")
...
def test_model_flow_anthropic_clears_stale_custom_key_and_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.auth.get_anthropic_key", lambda: "sk-ant-api03-test")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "zorin_cli.model_setup_flows._prompt_auth_credentials_choice",
        lambda title: "use",
    )
    monkeypatch.setattr(
        "zorin_cli.auth._prompt_model_selection",
    ...
    zorin_main._model_flow_anthropic({}, current_model="glm-5.2")
...
def test_model_flow_npcautomators_offers_tool_gateway_prompt_when_unconfigured(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: NpcAutomatorsPortalAccountInfo(
            logged_in=True,
            source="account_api",
            fresh=True,
    ...
    monkeypatch.setattr("zorin_cli.setup.prompt_checklist", _fake_checklist, raising=False)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.ZorinCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.ZorinCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "zorin_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("zorin_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("zorin_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("zorin_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("zorin_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(zorin_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    zorin_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "zorin_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("zorin_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("zorin_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("zorin_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("zorin_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "zorin_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("zorin_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    zorin_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("zorin_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "zorin_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("zorin_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    zorin_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_npcautomators_login_tls_options(monkeypatch):
    monkeypatch.setattr(zorin_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "zorin_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "npcautomators"}},
    )
    monkeypatch.setattr("zorin_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("zorin_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("zorin_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("zorin_cli.auth.resolve_provider", lambda requested, **kwargs: "npcautomators")
    monkeypatch.setattr("zorin_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(zorin_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("zorin_cli.auth._login_npcautomators", _fake_login)

    zorin_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.npcautomators.com",
            inference_url="https://inference.npcautomators.com/v1",
            client_id="zorin-local",
...
def test_auto_provider_name_localhost():
    from zorin_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from zorin_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "zorin_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("zorin_cli.config.save_config", _save)
