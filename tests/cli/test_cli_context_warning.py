...
def _isolate(tmp_path, monkeypatch):
    """Isolate ZORIN_HOME so tests don't touch real config."""
    home = tmp_path / ".zorin"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ZorinCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ZorinCLI.__new__(ZorinCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["zorin-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Zorin' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Zorin' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Zorin' minimum context length."""
