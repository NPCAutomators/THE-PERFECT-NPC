...
def test_installed_wheel_renders_i18n_strings(tmp_path):
    ...
    #    (no PYTHONPATH leaking the repo, no ZORIN_BUNDLED_LOCALES).
    probe = (
    ...
    env = {k: v for k, v in os.environ.items() if k not in ("PYTHONPATH", "ZORIN_BUNDLED_LOCALES")}
...
def test_built_sdist_ships_locale_catalogs(tmp_path):
    ...
    with tarfile.open(tarballs[0]) as tf:
        ...
        # zorin_agent-0.15.1/locales/en.yaml — match on the suffix.
...
def test_built_sdist_ships_web_dist(tmp_path):
    """The sdist must carry zorin_cli/web_dist/ too.

    MANIFEST.in `graft zorin_cli/web_dist` is what puts the frontend assets
    in the source distribution tarball. This test builds the sdist and asserts
    that index.html exists inside it.
    """
    # Create a dummy index.html in zorin_cli/web_dist if it doesn't exist
    # so that the sdist build actually has files to bundle.
    web_dist_dir = REPO_ROOT / "zorin_cli" / "web_dist"
    ...
    try:
        ...
        with tarfile.open(tarballs[0]) as tf:
            names = tf.getnames()
            web_assets = [m for m in names if "zorin_cli/web_dist/" in m]

        assert any(m.endswith("/zorin_cli/web_dist/index.html") for m in web_assets), (
            f"sdist missing zorin_cli/web_dist/index.html; shipped files in web_dist: {web_assets}"
        )
    finally:
