...
class TestWriteDenyExactPaths:
    ...
    def test_zorin_env(self):
        # ``.env`` under the active ZORIN_HOME (profile-aware, not just
        # ``~/.zorin``) must be write-denied. The hermetic test conftest
        # points ZORIN_HOME at a tempdir — resolve via get_zorin_home()
        # to match the denylist.
        from zorin_constants import get_zorin_home
        path = str(get_zorin_home() / ".env")
        assert _is_write_denied(path) is True

    def test_encrypted_bitwarden_cache(self):
        from zorin_constants import get_zorin_home

        path = get_zorin_home() / "cache" / "bws_cache.enc.json"
        assert _is_write_denied(str(path)) is True

    def test_zorin_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.zorin/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "zorin_root"
        ...
        # Sanity check: ZORIN_HOME does point to the profile dir, not the root.
        from zorin_constants import get_zorin_home, get_default_zorin_root
        assert get_zorin_home() == profile_home
        assert get_default_zorin_root() == root
...
class TestWriteAllowed:
    ...
    def test_zorin_control_files_requested_writable(self):
        from zorin_constants import get_zorin_home
