...
class TestGptQualityPinnedToMedium:
    ...
    and cannot be overridden via config. Pinning keeps NPCAUTOMATORS Portal billing
...
class TestManagedGatewayErrorTranslation:
    """4xx from the NpcAutomators managed gateway should be translated to a user-actionable message."""

    def test_4xx_translates_to_value_error_with_remediation(self, image_tool, monkeypatch):
        """403 from managed gateway → ValueError mentioning FAL_KEY + zorin tools."""
        ...
        managed_gateway.npcautomators_user_token = "test-token"
...
class TestManagedKreaRouting:
    ...
    def test_routes_native_krea_model_to_krea_plugin_in_managed_mode(
        ...
        monkeypatch.setattr(
            "zorin_cli.plugins._ensure_plugins_discovered", lambda *a, **k: None
        )
