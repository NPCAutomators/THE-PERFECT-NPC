...
class TestIsWriteDenied:
    ...
    def test_zorin_control_files_requested_writable(self, path):
        from zorin_constants import get_zorin_home

        assert _is_write_denied(str(get_zorin_home() / path)) is False
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.zorin/pairing/{platform}-approved.json as the
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__ZORIN_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__ZORIN_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert result.error is None
        assert "ZORIN_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__ZORIN_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__ZORIN_FENCE_a9f7b3__\n"
        )
