...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ZORIN_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ZORIN_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_zorin_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ZORIN_HOME": "/h", "ZORIN_PROFILE": "p",
        "ZORIN_CONFIG": "/c.yaml", "ZORIN_ENV": "/e",
        # other ZORIN_* → dropped (broad prefix removed)
        "ZORIN_BASE_URL": "https://x", "ZORIN_INTERACTIVE": "1",
        "ZORIN_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ZORIN_HOME", "ZORIN_PROFILE", "ZORIN_CONFIG", "ZORIN_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ZORIN_BASE_URL", "ZORIN_INTERACTIVE", "ZORIN_KANBAN_DB",
...
def test_env_scrub_logs_dropped_zorin_vars(caplog):
    """Dropping a non-allowlisted, non-secret ZORIN_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ZORIN_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ZORIN_HOME": "/h",          # allowlisted → kept, not logged
        "ZORIN_BASE_URL": "https://x",   # dropped → logged
        "ZORIN_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ZORIN_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ZORIN_HOME" in out and "PATH" in out
    assert "ZORIN_BASE_URL" not in out and "ZORIN_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ZORIN_BASE_URL" in msgs and "ZORIN_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ZORIN_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ZORIN_* vars."""
    import logging
