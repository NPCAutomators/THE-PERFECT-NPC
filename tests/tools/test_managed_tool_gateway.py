...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        ...
        clear=False,
    ), patch.object(managed_tool_gateway, "managed_npcautomators_tools_enabled", return_value=True):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "npcautomators-token",
        )

    assert result is not None
    assert result.gateway_origin == "https://firecrawl-gateway.npcautomators.com"
    assert result.npcautomators_user_token == "npcautomators-token"
...
def test_resolve_managed_tool_gateway_is_inactive_without_npcautomators_token():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "npcautomators.com",
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "npcautomators.com"}, clear=False), \
         patch.object(managed_tool_gateway, "managed_npcautomators_tools_enabled", return_value=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "npcautomators-token",
        )
...
def test_read_npcautomators_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    assert managed_tool_gateway.read_npcautomators_access_token() == "fresh-token"
