...
Older images made /opt/zorin/ui-tui and /opt/zorin/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ZORIN_TUI_DIR=/opt/zorin/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R zorin:zorin /opt/zorin/ui-tui" not in text
    assert "chown -R zorin:zorin /opt/zorin/node_modules" not in text
