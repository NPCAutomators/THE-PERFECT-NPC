...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("zorin_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("zorin_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("ZORIN_SESSION_KEY", session_key)
        monkeypatch.setenv("ZORIN_EXEC_ASK", "1")
        monkeypatch.delenv("ZORIN_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("zorin-verify-", "zorin-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "zorin-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/zorin-verify-example.py",
            "rm -f /tmp/zorin-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/zorin-verify-example.py",
            "rm -f /tmp/nested/../zorin-verify-example.py",
            "rm -f /tmp/./zorin-verify-example.py",
            "rm -f /tmp//zorin-verify-example.py",
            "rm -f /tmp/a/../../tmp/zorin-verify-example.py",
            "rm -f /var/tmp/zorin-verify-example.py",
            "rm -f /tmp/unrelated.py",
            "rm -f /tmp/zorin-verify-*",
            "rm -f /tmp/zorin-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/zorin-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/zorin-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\zorin-victim\file.txt"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\zorin-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\zorin-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\zorin-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
    ...
    def test_powershell_bare_remove_item_requires_approval(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            r"powershell Remove-Item -Recurse -Force C:\tmp\zorin-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows PowerShell destructive delete"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ZORIN_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_zorin_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.zorin/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_zorin_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ZORIN_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_zorin_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$ZORIN_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestZorinConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.zorin/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.zorin/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.zorin/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.zorin/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.zorin/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.zorin/config.yaml")
        assert dangerous is True
        assert "zorin config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.zorin/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_zorin_home_config(self):
    ...
    def test_sed_in_place_absolute_zorin_home_env(self):
        env_path = get_zorin_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_zorin_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ZORIN_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.zorin/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_zorin_home_config(self):
        config_path = get_zorin_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.zorin/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_zorin_home_env(self):
        env_path = get_zorin_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.zorin/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.zorin/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.zorin/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.zorin/config.yaml"
        )
        assert dangerous is True
        assert key != "in-place edit of Zorin config/env (perl/ruby)"

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.zorin/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Zorin config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_zorin_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $ZORIN_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.zorin/config.yaml/.env must require approval — the
    ...
    def test_cp_to_zorin_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.zorin/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Zorin-home prefixes must fold to ~/ and
    ~/.zorin/ in dangerous-command detection.
    ...
    SSH, and Zorin config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_zorin_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/ZORIN_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_zorin_home_config_folds(self, monkeypatch):
        # Zorin home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("ZORIN_HOME", r"C:\Users\tester\.zorin")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.zorin\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(
        ...
        monkeypatch.setenv("ZORIN_LANGUAGE", lang)
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.zorin/zorin-agent && source venv/bin/activate && python -m zorin_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m zorin_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m zorin_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "zorin_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart zorin-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_zorin_gateway_stop_detected(self):
        cmd = "zorin gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_zorin_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `zorin` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "zorin -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_zorin_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "zorin --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_zorin_gateway_multiple_flags_detected(self):
        cmd = "zorin -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_zorin_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "zorin -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_zorin_gateway_start_not_flagged(self):
        cmd = "zorin gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_zorin_detected(self):
        """pkill targeting zorin/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_zorin_detected(self):
        cmd = "killall zorin"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Zorin security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.zorin/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep zorin) bypasses the pkill/killall name-matching
    ...
    def test_pkill_zorin_still_detected(self):
    ...
    def test_kill_dollar_pidof_detected(self):
        """`kill $(pidof zorin)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        cmd = "kill -TERM $(pidof zorin_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()

    def test_kill_backtick_pidof_detected(self):
        cmd = "kill -9 `pidof zorin`"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True


class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Zorin service
    label achieves the same effect as `zorin gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_stop_zorin_detected(self):
        cmd = "launchctl stop ai.zorin.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "zorin" in desc.lower()

    def test_launchctl_kickstart_zorin_detected(self):
        cmd = "launchctl kickstart -k system/ai.zorin.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_zorin_detected(self):
        cmd = "launchctl bootout system/ai.zorin.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_zorin_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.zorin.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Zorin label is out of scope for the
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ZORIN_GATEWAY_SESSION", "ZORIN_CRON_SESSION",
                      "ZORIN_YOLO_MODE",
                      "ZORIN_SESSION_KEY", "ZORIN_INTERACTIVE")
        }
        os.environ.pop("ZORIN_YOLO_MODE", None)
        os.environ.pop("ZORIN_INTERACTIVE", None)
        # ZORIN_CRON_SESSION takes priority over ZORIN_GATEWAY_SESSION in
        ...
        os.environ.pop("ZORIN_CRON_SESSION", None)
        os.environ["ZORIN_GATEWAY_SESSION"] = "1"
        os.environ["ZORIN_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("zorin_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"ZORIN_INTERACTIVE": "1"}, clear=False):
