...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        zorin_home.mkdir()
        (zorin_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"ZORIN_HOME": str(zorin_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(zorin_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.zorin/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        (zorin_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        (zorin_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        (zorin_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.zorin/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        zorin_home = tmp_path / ".zorin"
        (zorin_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ZORIN_HOME": str(zorin_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.zorin")

        assert mounts[0]["container_path"] == "/home/user/.zorin/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.zorin/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.zorin/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ZORIN_HOME."""
        monkeypatch.setenv("ZORIN_HOME", str(tmp_path / ".zorin"))
        (tmp_path / ".zorin").mkdir()

        # Create a sensitive file one level above zorin_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside zorin_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ZORIN_HOME must still be registered."""
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))
        (zorin_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_zorin_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ZORIN_HOME must be allowed."""
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        subdir = zorin_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ZORIN_HOME pointing outside must be rejected."""
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))

        # Create a sensitive file outside zorin_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside zorin_home pointing outside
        symlink = zorin_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ZORIN_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, zorin_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ZORIN_HOME."""
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))
        ...
        sensitive.write_text("{}")
        self._write_config(zorin_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(zorin_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ZORIN_HOME via config must still mount."""
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))

        (zorin_home / "oauth.json").write_text("{}")
        self._write_config(zorin_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        (zorin_home / "cache" / "documents").mkdir(parents=True)
        (zorin_home / "cache" / "audio").mkdir(parents=True)
        (zorin_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.zorin/cache/documents" in paths
        assert "/root/.zorin/cache/audio" in paths
        assert "/root/.zorin/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.zorin/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_zorin_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        # Use legacy dir name with content — get_zorin_dir prefers
        # populated old over new.
        legacy_doc = zorin_home / "document_cache"
        legacy_img = zorin_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(zorin_home / "document_cache") in host_paths
        assert str(zorin_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.zorin/cache/documents" in container_paths
        assert "/root/.zorin/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        zorin_home = tmp_path / ".zorin"
        img_dir = zorin_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.zorin/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.zorin")
            == "/home/agent/.zorin/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        zorin_home = tmp_path / ".zorin"
        (zorin_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(zorin_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        zorin_home = tmp_path / ".zorin"
        doc_dir = zorin_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        zorin_home = tmp_path / ".zorin"
        ss_dir = zorin_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.zorin/cache/screenshots/session_abc/screen1.png"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — ZORIN_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping ZORIN_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"ZORIN_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.zorin/google_token.json" in paths
        assert "/root/.zorin/.env" not in paths
