...
class TestStripBlockedTools(unittest.TestCase):
    ...
    def test_mixed_composite_is_subtracted_at_child_assembly(self):
        ...
        ``zorin-cli`` contains both allowed tools and every sensitive
        ...
        parent.enabled_toolsets = ["zorin-cli"]
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    def test_cross_provider_delegation(self, mock_creds, mock_cfg):
        """Parent on NpcAutomators, subagent on OpenRouter — full credential switch."""
        mock_cfg.return_value = {
            "max_iterations": 45,
            "model": "google/gemini-3-flash-preview",
            "provider": "openrouter",
        ...
        parent = _make_mock_parent(depth=0)
        parent.provider = "npcautomators"
        parent.base_url = "https://inference-api.npcautomators.com/v1"
        parent.api_key = "npcautomators-key-abc"
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            _, kwargs = MockAgent.call_args
            # Child should use OpenRouter, NOT NpcAutomators
            self.assertEqual(kwargs["provider"], "openrouter")
            self.assertEqual(kwargs["base_url"], "https://openrouter.ai/api/v1")
...
class TestConcurrencyDefaults(unittest.TestCase):
    ...
    def test_load_config_prefers_active_persistent_config_over_cli_defaults(self):
        ...
        with patch.dict("sys.modules", {"cli": stale_cli}):
            with patch(
                "zorin_cli.config.load_config_readonly", return_value=active_config
            ):
    ...
    def test_load_config_falls_back_to_cli_config_when_persistent_load_fails(self):
        ...
        with patch.dict("sys.modules", {"cli": fallback_cli}):
            with patch(
                "zorin_cli.config.load_config_readonly",
    ...
    def test_load_config_prefers_cli_config_when_user_config_ignored(self):
        # `zorin chat --ignore-user-config` sets ZORIN_IGNORE_USER_CONFIG=1,
        ...
        with patch.dict("sys.modules", {"cli": ignoring_cli}):
            with patch.dict(os.environ, {"ZORIN_IGNORE_USER_CONFIG": "1"}):
                with patch(
                    "zorin_cli.config.load_config_readonly",
