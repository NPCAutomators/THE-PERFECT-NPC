...
def _restore_tool_modules():
    original_zorin_home = os.environ.get("ZORIN_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_zorin_home is None:
            os.environ.pop("ZORIN_HOME", None)
        else:
            os.environ["ZORIN_HOME"] = original_zorin_home
        _reset_modules(("tools", "zorin_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    zorin_cli = types.ModuleType("zorin_cli")
    zorin_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["zorin_cli"] = zorin_cli
    zorin_home = tmp_path / "zorin-home"
    os.environ["ZORIN_HOME"] = str(zorin_home)
    sys.modules["zorin_cli.config"] = types.SimpleNamespace(
        get_zorin_home=lambda: zorin_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="zorin-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": zorin_home / "modal_snapshots.json",
