...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `zorin-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.zorin/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.zorin/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because zorin uses an internal scheduler, not system crontab.
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("ZORIN_INTERACTIVE", false_like_value)
        monkeypatch.delenv("ZORIN_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("ZORIN_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["ZORIN_INTERACTIVE", "ZORIN_GATEWAY_SESSION", "ZORIN_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("ZORIN_INTERACTIVE", "ZORIN_GATEWAY_SESSION", "ZORIN_EXEC_ASK"):
...
class TestUnifiedCronjobTool:
    ...
    def _patch_named_legit(monkeypatch):
        import zorin_cli.runtime_provider as rp
...
class TestResolveModelOverride:
    ...
    def test_keeps_bare_custom_when_a_named_entry_exists(self, monkeypatch):
        import zorin_cli.runtime_provider as rp_mod
