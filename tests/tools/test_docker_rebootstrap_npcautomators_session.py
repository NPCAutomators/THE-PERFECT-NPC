"""Unit tests for scripts/docker_rebootstrap_npcautomators_session.py.

The boot-time re-seed is the load-bearing "does not clobber a healthy session"
guard: it may overwrite the on-disk NPCAUTOMATORS provider entry when that entry is
...
_SCRIPT = Path(__file__).resolve().parents[2] / "scripts" / "docker_rebootstrap_npcautomators_session.py"
_spec = importlib.util.spec_from_file_location("docker_rebootstrap_npcautomators_session", _SCRIPT)
mod = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(mod)


def _terminal_npcautomators_state():
...
def _healthy_npcautomators_state():
    return {
        "portal_base_url": "https://portal.example.com",
        "client_id": "zorin-cli-vps",
...
def test_reseeds_terminal_entry(tmp_path):
    """Terminal on-disk entry + valid seed → providers.npcautomators replaced."""
    auth = _write_auth(tmp_path, {"npcautomators": _terminal_npcautomators_state()})
    result = mod.reseed_if_terminal(auth, _FRESH_SEED)
    assert result == "reseeded"
    store = json.loads(Path(auth).read_text())
    assert store["providers"]["npcautomators"]["refresh_token"] == "FRESH-rt"
    assert "last_auth_error" not in store["providers"]["npcautomators"]


def test_does_not_clobber_healthy_entry(tmp_path):
    """LOAD-BEARING: a healthy (live-token) entry must never be overwritten."""
    auth = _write_auth(tmp_path, {"npcautomators": _healthy_npcautomators_state()})
    result = mod.reseed_if_terminal(auth, _FRESH_SEED)
    assert result == "not_terminal"
    store = json.loads(Path(auth).read_text())
    # Untouched — still the live tokens, not the seed.
    assert store["providers"]["npcautomators"]["refresh_token"] == "live-rt"


def test_marker_but_live_token_is_not_terminal(tmp_path):
    """Stale marker + a live token present → NOT terminal (don't clobber)."""
    state = _terminal_npcautomators_state()
    state["refresh_token"] = "somehow-live"
    auth = _write_auth(tmp_path, {"npcautomators": state})
...
def test_reseeds_newer_orchestrator_session_over_healthy_stale_entry(tmp_path):
    ...
    """
    auth = _write_auth(tmp_path, {"npcautomators": {
        **_healthy_npcautomators_state(),
...
def test_preserves_other_providers(tmp_path):
    """Re-seed swaps ONLY providers.npcautomators; other providers survive intact."""
    auth = _write_auth(tmp_path, {
        "npcautomators": _terminal_npcautomators_state(),
...
def test_bad_seed_is_noop(tmp_path):
    ...
    store = json.loads(Path(auth).read_text())
    assert store["providers"]["npcautomators"]["last_auth_error"]["relogin_required"] is True


def test_seed_without_npcautomators_entry_is_noop(tmp_path):
    auth = _write_auth(tmp_path, {"npcautomators": _terminal_npcautomators_state()})
...
def test_terminal_entry_missing_marker_is_not_terminal(tmp_path):
    ...
    auth = _write_auth(tmp_path, {"npcautomators": {"client_id": "zorin-cli-vps"}})
    assert mod.reseed_if_terminal(auth, _FRESH_SEED) == "not_terminal"
