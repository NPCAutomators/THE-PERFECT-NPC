...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zorin_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(zorin_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(zorin_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_zorin_bin(self):
        """After _INSTALL_FAILED, manual install in ZORIN_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        zorin_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(zorin_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(zorin_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zorin_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == zorin_bin
            assert _tirith_mod._resolved_path == zorin_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ZORIN_HOME isolation
# ---------------------------------------------------------------------------

class TestZorinHomeIsolation:
    def test_zorin_bin_dir_respects_zorin_home(self):
        """_zorin_bin_dir must use ZORIN_HOME, not hardcoded ~/.zorin."""
        from tools.tirith_security import _zorin_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ZORIN_HOME": tmpdir}):
            result = _zorin_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_zorin_home(self):
        """_failure_marker_path must use ZORIN_HOME, not hardcoded ~/.zorin."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ZORIN_HOME": "/custom/zorin"}):
            result = _failure_marker_path()
        assert result == "/custom/zorin/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ZORIN_HOME; verify it's active."""
        zorin_home = os.getenv("ZORIN_HOME")
        assert zorin_home is not None, "ZORIN_HOME should be set by conftest"
        assert "zorin_test" in zorin_home, "Should point to test temp dir"

    def test_get_zorin_home_fallback(self):
        """Without ZORIN_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_zorin_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ZORIN_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".zorin")
            result = _get_zorin_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._zorin_bin_dir",
