...
def test_manager_isolates_same_named_servers_by_profile_home(tmp_path, monkeypatch):
    ...
    for home, access_token in ((profile_a, "TOKEN_A"), (profile_b, "TOKEN_B")):
        ...
        try:
            storage = ZorinTokenStorage("shared")
...
def test_manager_explicit_home_removes_only_that_profiles_tokens(tmp_path):
    ...
    token = set_zorin_home_override(profile_a)
    try:
        MCPOAuthManager().remove("shared", zorin_home=profile_b)
    finally:
        reset_zorin_home_override(token)
...
def test_zorin_provider_subclass_exists():
    """ZorinMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ZORIN_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ZORIN_PROVIDER_CLS is not None
    assert issubclass(_ZORIN_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_zorin_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ZorinMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ZORIN_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _ZORIN_PROVIDER_CLS is not None
    assert isinstance(provider, _ZORIN_PROVIDER_CLS)
    assert provider._zorin_server_name == "srv"
