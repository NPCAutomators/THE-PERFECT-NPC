...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.web_tools.managed_npcautomators_tools_enabled", return_value=True),
            patch("tools.managed_tool_gateway.managed_npcautomators_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("tools.web_tools.Firecrawl"):
            with patch("tools.web_tools._read_npcautomators_access_token", return_value=None):
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "npcautomators.com"}):
            with patch("tools.web_tools._read_npcautomators_access_token", return_value="npcautomators-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    from tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="npcautomators-token",
                        api_url="https://firecrawl-gateway.npcautomators.com",
                    )
    ...
    def test_tool_gateway_scheme_can_switch_derived_gateway_origin_to_http(self):
        ...
        with patch.dict(os.environ, {
            "TOOL_GATEWAY_DOMAIN": "npcautomators.com",
            ...
            with patch("tools.web_tools._read_npcautomators_access_token", return_value="npcautomators-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="npcautomators-token",
                        api_url="http://firecrawl-gateway.npcautomators.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_default_gateway_domain_targets_npcautomators_production_origin(self):
    ...
    def test_npcautomators_auth_token_respects_zorin_home_override(self, tmp_path):
        """Auth lookup should read from ZORIN_HOME/auth.json, not ~/.zorin/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".zorin").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "ZORIN_HOME": str(zorin_home),
        }, clear=False):
            import tools.web_tools
            importlib.reload(tools.web_tools)
            assert tools.web_tools._read_npcautomators_access_token() == "npcautomators-token"
...
class TestBackendSelection:
    ...
    ``zorin tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def test_managed_gateway_does_not_preempt_explicit_tavily(self):
        """Regression: a NpcAutomators OAuth token (managed gateway "ready") must NOT
        beat an explicitly configured TAVILY_API_KEY in the fallback path.
        Free NpcAutomators tiers don't include web search, so the user's deliberate
    ...
    def test_managed_gateway_only_falls_through_to_firecrawl(self):
        """When no explicit-credential backend is configured, a NpcAutomators-managed
...
class TestCheckWebApiKey:
    ...
    def test_null_web_section_does_not_crash(self):
        ...
        with patch("zorin_cli.config.load_config", return_value={"web": None}):
    ...
    def test_tool_gateway_returns_true(self):
        with patch("tools.web_tools._peek_npcautomators_access_token", return_value="npcautomators-token"):
            from tools.web_tools import check_web_api_key
            assert check_web_api_key() is True
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "zorin_cli.auth.resolve_npcautomators_access_token",
            _record_refresh,
        )
...
class TestNonBuiltinProviderAvailability:
    ...
    def test_check_web_api_key_returns_true_for_custom_provider(self):
        ...
        with patch("tools.web_tools._ddgs_package_importable", return_value=False), \
             patch("tools.web_tools._peek_npcautomators_access_token", return_value=None):
            from tools.web_tools import check_web_api_key
            assert check_web_api_key() is True
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from zorin_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.zorin/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.zorin/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_get_provider_env_falls_back_to_os_environ(self, monkeypatch):
        ...
        with patch("zorin_cli.config.get_env_value", return_value=None):
