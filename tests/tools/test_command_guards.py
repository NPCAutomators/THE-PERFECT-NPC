...
def _clean_state():
    ...
    saved = {}
    for k in ("ZORIN_INTERACTIVE", "ZORIN_GATEWAY_SESSION", "ZORIN_EXEC_ASK", "ZORIN_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ZORIN_INTERACTIVE"] = "1"
        session_key = os.getenv("ZORIN_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ZORIN_INTERACTIVE or ZORIN_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
