...
def _is_git_spawn(cmd) -> bool:
    ...
    ``bounded_git_probe`` lives in ``zorin_cli._subprocess_compat`` and both
...
def test_copilot_gh_cli_probe_hides_gh_windows(monkeypatch):
    from zorin_cli import copilot_auth
...
def test_gateway_pid_scan_hides_wmic_and_powershell_windows(monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        ...
        return _Completed(stdout="CommandLine=zorin gateway\nProcessId=123\n")
...
def test_stale_dashboard_windows_scan_hides_wmic(monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        captured.append((cmd, kwargs))
        return _Completed(stdout="CommandLine=zorin dashboard\nProcessId=123\n")
