"""Regression guard: a genuine NpcAutomators 429 must re-enter the retry loop so the
top-of-loop NpcAutomators rate-limit guard can activate the fallback chain.
...
``record_npcautomators_rate_limit()`` moments earlier and either activates a fallback
...
class TestGenuineNpcAutomators429ReentersLoop:
...
class TestSourceUsesReentrantAssignment:
    ...
    form in the genuine-NpcAutomators-429 branch.  Protects against an accidental
    ...
    def test_genuine_branch_does_not_skip_to_max_retries(self):
        ...
        match = re.search(
            r"if _genuine_npcautomators_rate_limit:\n(?:.*\n)*?\s*continue\n",
            src,
        )
        # There are two `if _genuine_npcautomators_rate_limit` sites (record + branch);
        # the regex above finds the first block ending in `continue`, which is
        # the retry-count branch.
        assert match is not None, (
            "genuine-NpcAutomators-429 branch not found in conversation_loop — "
            "update this test if the branch was refactored"
        )
        block = match.group(0)
        assert "retry_count = max(0, max_retries - 1)" in block, (
            "genuine-NpcAutomators-429 branch must re-enter the retry loop "
            "(retry_count = max(0, max_retries - 1)); "
