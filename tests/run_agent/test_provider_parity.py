...
class TestDeveloperRoleSwap:
    ...
    def test_developer_role_via_npcautomators_portal(self, monkeypatch):
        agent = _make_agent(
            monkeypatch,
            "npcautomators",
...
class TestBuildApiKwargsNpcAutomatorsPortal:
    def test_includes_npcautomators_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == npcautomators_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/NpcAutomators) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_npcautomators_when_no_openrouter(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_npcautomators_auth", return_value=npcautomators_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("zorin_cli.models.get_npcautomators_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == "google/gemini-3-flash-preview"

    def test_custom_endpoint_when_no_npcautomators(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/NpcAutomators keys are available.
