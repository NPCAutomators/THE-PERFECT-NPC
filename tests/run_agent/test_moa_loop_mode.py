...
def test_moa_slots_routed_through_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "zorin_cli.runtime_provider.resolve_runtime_provider", fake_resolve
    )
...
def test_moa_provider_backed_slot_survives_aux_resolution(monkeypatch, provider):
    ...
    bedrock, npcautomators, openai-codex, xai-oauth) are intentionally NOT forwarded —
...
def test_moa_slot_runtime_falls_back_on_resolution_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "zorin_cli.runtime_provider.resolve_runtime_provider", boom
    )
...
def test_reference_messages_drops_system_but_renders_tools_as_text():
    ...
    messages = [
        {"role": "system", "content": "huge zorin system prompt"},
    ...
    assert all("huge zorin system prompt" not in m["content"] for m in view)
...
def test_reference_messages_flattens_cache_decorated_content():
    ...
    plain = [
        {"role": "system", "content": "zorin system prompt"},
        {"role": "user", "content": "Can we get codex usage resets into zorin?"},
    ]
