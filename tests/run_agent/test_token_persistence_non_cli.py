...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    zorin_state = ModuleType("zorin_state")
    zorin_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "zorin_state", zorin_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Zorin"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Zorin"
