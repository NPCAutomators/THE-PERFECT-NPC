...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._zorin_home / "logs" / "errors.log").resolve()
...
class TestSessionJsonSnapshotOptIn:
    ...
    def test_traversal_session_id_cannot_escape_logs_dir(self, agent, tmp_path):
        ...
        # originate from the untrusted X-Zorin-Session-Id API header) must not
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host ZORIN_REDACT_SECRETS state.
...
class TestInit:
    ...
    def test_prompt_caching_cache_ttl_defaults_without_config(self):
        ...
        with (
            ...
            patch("run_agent.check_toolset_requirements", return_value={}),
            patch("run_agent.OpenAI"),
            patch("zorin_cli.config.load_config", return_value={}),
        ):
            a = AIAgent(
                api_key="test-k...7890",
                model="anthropic/claude-sonnet-4-20250514",
...
class TestBuildSystemPrompt:
    ...
    def test_includes_npcautomators_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_npcautomators_subscription_prompt", lambda tool_names: "NPCAUTOMATORS SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "NPCAUTOMATORS SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_npcautomators_route(self, agent):
        agent.provider = "npcautomators"
        agent.base_url = "https://inference-api.npcautomators.com/v1"
        agent.model = "minimax/minimax-m2.5"
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        assert kwargs["extra_body"]["reasoning"]["effort"] == "medium"
    ...
    def test_reasoning_xhigh_preserved_for_copilot_when_supported(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "zorin_cli.models.github_model_reasoning_efforts",
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("zorin_cli.plugins.invoke_hook", _capture_hook)
        monkeypatch.setattr("zorin_cli.plugins.has_hook", lambda name: True)
...
class TestConcurrentToolExecution:
    ...
    def test_concurrent_timeout_returns_finished_tools_without_hanging(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("ZORIN_CONCURRENT_TOOL_TIMEOUT_S", "0.1")
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(self, agent, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr(
            "zorin_cli.plugins.resolve_pre_tool_block",
            lambda *args, **kwargs: None,
        )
        monkeypatch.setattr(
            "zorin_cli.plugins.invoke_hook",
    ...
    def test_sequential_agent_level_tool_execution_middleware_wraps_inline_dispatch(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("zorin_cli.plugins.get_plugin_manager", lambda: manager)
        monkeypatch.setattr(
            "zorin_cli.plugins.invoke_middleware",
...
class TestHandleMaxIterations:
    ...
    def test_summary_keeps_provider_preferences_for_npcautomators(self, agent):
        ...
        assert kwargs["extra_body"]["tags"] == npcautomators_portal_tags(
            session_id=agent.session_id
        )
    ...
    def test_summary_keeps_npcautomators_profile_body_without_routing_preferences(self, agent):
        agent.base_url = "https://proxy.example.com/v1"
        agent._base_url_lower = agent.base_url.lower()
        agent.provider = "npcautomators"
        agent.client.chat.completions.create.return_value = _mock_response(content="Summary")
        ...
        expected = {"tags": npcautomators_portal_tags(session_id=agent.session_id)}
        if agent.session_id:
    ...
    def test_api_sanitizer_repairs_tool_call_with_empty_function_name(self, agent):
        ...
        call, because zorin' dispatch loop keeps empty-name calls paired with
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Zorin tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "zorin_cli.plugins.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("zorin_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("zorin_cli.plugins.has_hook", lambda name: False)
        monkeypatch.setattr("zorin_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("zorin_cli.plugins.has_hook", _has_hook)
        ...
        with (
            patch("zorin_cli.plugins.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_npcautomators_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "npcautomators"
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_npcautomators_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("ZORIN_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("zorin_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("zorin_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when ZORIN_KANBAN_TASK
...
class TestRetryExhaustion:
    ...
    def test_content_filter_refusal_surfaced_not_retried(self, agent):
        ...
        portal (NPCAUTOMATORS Portal fronting Anthropic) returns ``message.refusal``
...
class TestNpcAutomatorsCredentialRefresh:
    """Verify NpcAutomators credential refresh rebuilds the runtime client."""

    def test_try_refresh_npcautomators_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "zorin_cli.auth.resolve_npcautomators_runtime_credentials", _fake_resolve
        )

        agent.client = _ExistingClient()
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_npcautomators_client_credentials(force=True)

        assert ok is True
        assert closed["value"] is True
        assert captured["force_refresh"] is True
        assert rebuilt["kwargs"]["api_key"] == "new-npcautomators-key"
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.npcautomators.com/v1"
        )
...
class TestGpt5ApiModeRouting:
    ...
    def test_npcautomators_gpt5_stays_on_chat_completions(self, agent):
        """NpcAutomators serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "npcautomators"
        agent.base_url = "https://inference-api.npcautomators.com/v1"
        agent.api_mode = "chat_completions"
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "ZORIN" in agent._cached_system_prompt
