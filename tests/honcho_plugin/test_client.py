...
from zorin_cli.profiles import _get_default_zorin_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "zorin"
        assert config.workspace_id == "zorin"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.zorin/honcho.json
        monkeypatch.setenv("ZORIN_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"zorin": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"zorin": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"zorin": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="zorin-agent"
        ):
            result = config.resolve_session_name("/home/user/zorin-agent/subdir")
        assert result == "zorin-agent"
...
class TestResolveConfigPath:
    def test_prefers_zorin_home_when_exists(self, tmp_path):
        zorin_home = tmp_path / "zorin"
        zorin_home.mkdir()
        local_cfg = zorin_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ZORIN_HOME points at ~/.zorin/profiles/<name>, so
        # _get_default_zorin_home() must resolve back to ~/.zorin — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".zorin"
        ...
        assert _get_default_zorin_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_zorin_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("ZORIN_HOME", None)
            result = resolve_config_path()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".zorin".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".zorin"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "zorin_coder"
        assert profile_host_key("default") == "zorin"

    def test_default_returns_zorin(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("ZORIN_HONCHO_HOST", None)
            os.environ.pop("ZORIN_HOME", None)
            with patch(
                "plugins.memory.honcho.client.resolve_config_path",
                return_value=Path("/nonexistent/honcho.json"),
            ):
                assert resolve_active_host() == "zorin"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ZORIN_HONCHO_HOST": "zorin.coder"}):
            assert resolve_active_host() == "zorin.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZORIN_HONCHO_HOST", None)
            with patch("zorin_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "zorin_coder"
    ...
    def test_default_profile_returns_zorin(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZORIN_HONCHO_HOST", None)
            with patch("zorin_cli.profiles.get_active_profile_name", return_value="default"), \
                 patch(
    ...
    def test_custom_profile_returns_zorin(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZORIN_HONCHO_HOST", None)
            with patch("zorin_cli.profiles.get_active_profile_name", return_value="custom"), \
                 patch(
                     "plugins.memory.honcho.client.resolve_config_path",
                     return_value=Path("/nonexistent/honcho.json"),
                 ):
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("ZORIN_HONCHO_HOST", None)
            # Temporarily remove zorin_cli.profiles to simulate import failure
            saved = sys.modules.get("zorin_cli.profiles")
            sys.modules["zorin_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "zorin"
            finally:
                if saved is not None:
                    sys.modules["zorin_cli.profiles"] = saved
                else:
                    sys.modules.pop("zorin_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="zorin_coder")
        assert config.host == "zorin_coder"
        assert config.workspace_id == "zorin"  # shared workspace
        assert config.ai_peer == "zorin_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="zorin")
        assert config.host == "zorin"
        assert config.workspace_id == "zorin"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "zorin": {"aiPeer": "zorin", "peerName": "alice"},
                "zorin_coder": {
                    "aiPeer": "zorin_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="zorin_coder", config_path=config_file,
        )
        assert config.host == "zorin_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "zorin_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="zorin_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "zorin_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "zorin.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="zorin_dreamer",
        ...
        assert config.host == "zorin_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "zorin_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zorin": {"enabled": True, "aiPeer": "zorin"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zorin": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_zorin_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="zorin",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("zorin_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_zorin_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="zorin",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("zorin_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_zorin_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZORIN_MANAGED_DIR", str(managed_dir))
    ...
    def test_honcho_json_timeout_does_not_thrash_singleton(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "cloud-key",
            "hosts": {"zorin": {"workspace": "ws", "requestTimeout": 120}},
        }))
        ...
        with patch("plugins.memory.honcho.client.resolve_config_path", return_value=config_file), \
             patch("zorin_cli.profiles.get_active_profile_name", return_value="default"):
            with patch("honcho.Honcho", return_value=fake_honcho_1) as mock_h1:
            ...
            config_file.write_text(json.dumps({
                "apiKey": "cloud-key",
                "hosts": {"zorin": {"workspace": "ws", "requestTimeout": 240}},
            }))
            st = config_file.stat()
