...
def server():
    with patch.dict("sys.modules", {
        "zorin_constants": MagicMock(get_zorin_home=MagicMock(return_value="/tmp/zorin_test")),
        "zorin_cli.env_loader": MagicMock(),
        "zorin_cli.banner": MagicMock(),
        "zorin_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `ZORIN_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("ZORIN_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("ZORIN_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_sync_session_key_after_compress_reanchors_active_session_lease(
    ...
    from zorin_cli.active_sessions import (
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/zorin-agent-dev": {"name": "zorin-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "zorin-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "zorin_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "zorin-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "zorin-agent-dev"
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `zorin config set` does)
    ...
    import zorin_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_zorin_home", lambda: tmp_path)
    ...
    # Activate midnight, as `zorin config set display.skin midnight` would.
