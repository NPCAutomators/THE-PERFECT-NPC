...
def server():
    with patch.dict(
        "sys.modules",
        {
            "zorin_constants": MagicMock(
                get_zorin_home=MagicMock(return_value="/tmp/zorin_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'zorin-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'zorin-release' patched"
    }
