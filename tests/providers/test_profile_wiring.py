...
class TestNpcAutomatorsProfileParity:
    def test_tags(self, transport):
        legacy = transport.build_kwargs(
            model="zorin-3", messages=_msgs(), tools=None, provider_profile=get_provider_profile("npcautomators"),
        )
        profile = transport.build_kwargs(
            model="zorin-3", messages=_msgs(), tools=None,
            provider_profile=get_provider_profile("npcautomators"),
        )
    ...
    def test_reasoning_omitted_when_disabled(self, transport):
        ...
        legacy = transport.build_kwargs(
            model="zorin-3", messages=_msgs(), tools=None,
            provider_profile=get_provider_profile("npcautomators"), supports_reasoning=True, reasoning_config=rc,
        )
        profile = transport.build_kwargs(
            model="zorin-3", messages=_msgs(), tools=None,
            provider_profile=get_provider_profile("npcautomators"),
...
class TestRequestOverridesParity:
    ...
    def test_extra_body_override_merges_with_provider_body(self, transport):
        ...
        assert kw["extra_body"]["tags"] == npcautomators_portal_tags()  # from profile
