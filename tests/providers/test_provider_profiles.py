...
class TestRegistry:
    ...
    def test_alias_lookup(self):
        ...
        assert get_provider_profile("npcautomators-portal").name == "npcautomators"
...
class TestOpenRouterProfile:
    ...
    def test_reasoning_disabled_still_passes(self):
        """OpenRouter passes disabled reasoning through (unlike NpcAutomators)."""
        p = get_provider_profile("openrouter")
        eb, _ = p.build_api_kwargs_extras(
    ...
    def test_mandatory_anthropic_effort_routes_to_verbosity(self):
        ...
        ``zorin_constants.parse_reasoning_effort`` —
...
class TestNpcAutomatorsProfile:
    def test_tags(self):
        from agent.portal_tags import npcautomators_portal_tags
        p = get_provider_profile("npcautomators")
        body = p.build_extra_body()
