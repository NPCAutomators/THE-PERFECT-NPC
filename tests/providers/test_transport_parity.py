...
class TestNpcAutomatorsParity:
    """NpcAutomators: product tags, reasoning, omit when disabled."""

    def test_tags(self, transport):
        from agent.portal_tags import npcautomators_portal_tags
        kw = transport.build_kwargs(
            model="zorin-3-llama-3.1-405b",
            messages=_simple_messages(),
            tools=None,
    ...
    def test_reasoning_omitted_when_disabled(self, transport):
        """NpcAutomators special case: reasoning omitted entirely when disabled."""
        kw = transport.build_kwargs(
            model="zorin-3-llama-3.1-405b",
            messages=_simple_messages(),
            tools=None,
