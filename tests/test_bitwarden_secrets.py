...
binary download are exercised manually by `zorin secrets bitwarden
...
def zorin_home(tmp_path, monkeypatch):
    """Point Zorin at an isolated home directory."""
    home = tmp_path / ".zorin"
    home.mkdir()
    monkeypatch.setenv("ZORIN_HOME", str(home))
    # Some modules cache get_zorin_home; clear if needed.
    import zorin_constants
    if hasattr(zorin_constants, "_ZORIN_HOME_CACHE"):
        zorin_constants._ZORIN_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(zorin_home, monkeypatch):
...
def test_install_bws_happy_path(zorin_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(zorin_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(zorin_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from zorin_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under zorin_home/cache/."""
    home = tmp_path / ".zorin"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
