"""
Tests for mcp_serve — Zorin MCP server.
...
def _isolate_zorin_home(tmp_path, monkeypatch):
    """Redirect ZORIN_HOME to a temp directory."""
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path))
    try:
        import zorin_constants
        monkeypatch.setattr(zorin_constants, "get_zorin_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking zorin_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <ZORIN_HOME>/state.db for its mtime gate; the autouse
        # fixture points ZORIN_HOME at tmp_path.
        db_path = tmp_path / "state.db"
