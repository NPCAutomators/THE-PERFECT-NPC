"""Unit tests for the generic-OIDC / NpcAutomators-Portal caller-identity token resolver.

Covers gateway.relay._resolve_relay_identity_token() — the canonical resolver
shared by the runtime self-provision path and the `zorin gateway enroll` CLI.
...
Two modes:
  ...
  2. NPCAUTOMATORS Portal (resolve_npcautomators_access_token) otherwise — the default.

The HTTP POST and the NpcAutomators resolver are monkeypatched; these prove the mode
...
def test_defaults_to_npcautomators_portal_when_no_idp_configured(monkeypatch):
    called = {}

    def fake_resolve():
        called["yes"] = True
        return "npcautomators-portal-token"

    monkeypatch.setattr(
        "zorin_cli.auth.resolve_npcautomators_access_token", fake_resolve, raising=False
    )
    assert relay._resolve_relay_identity_token() == "npcautomators-portal-token"
