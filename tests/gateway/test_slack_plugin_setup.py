...
``zorin_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``zorin_cli.config`` (get_env_value / save_env_value) and
``zorin_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch those
source modules.
"""
import zorin_cli.config as config_mod
import zorin_cli.cli_output as cli_output_mod
...
def _patch_setup_io(monkeypatch, prompts, saved):
    ...
    # Manifest writing reaches out to zorin_cli.slack_cli + filesystem; stub it.
    import zorin_cli.slack_cli as slack_cli_mod
