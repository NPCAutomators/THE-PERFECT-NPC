...
class TestResolutionOrder:
    ...
    def test_source_profile_wins_over_routing(self, mock_runner, discord_source):
        ...
        with patch("zorin_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("zorin_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("zorin_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/zorin/profiles/from-source")
                    result = mock_runner._resolve_profile_home_for_source(discord_source)
                    
                    assert result == Path("/zorin/profiles/from-source")
    ...
    def test_routing_wins_over_active_profile(self, mock_runner, discord_source):
        ...
        with patch("zorin_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("zorin_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("zorin_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/zorin/profiles/routed")
                    ...
                    assert result == Path("/zorin/profiles/routed")
    ...
    def test_active_profile_fallback(self, mock_runner, discord_source):
        ...
        with patch("zorin_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("zorin_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/zorin/profiles/active")
                ...
                assert result == Path("/zorin/profiles/active")
    ...
    def test_default_fallback_when_no_active(self, mock_runner, discord_source):
        ...
        with patch("zorin_cli.profiles.get_active_profile_name", return_value=None):
            with patch("zorin_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/zorin")
                
                mock_runner._profile_name_for_source = MagicMock(return_value=None)
                
                result = mock_runner._resolve_profile_home_for_source(discord_source)
                
                assert result == Path("/zorin")
...
class TestMissingProfileWarning:
    ...
    def test_nonexistent_profile_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("zorin_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("zorin_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/zorin/profiles/nonexistent")
                with patch("zorin_cli.profiles.profile_exists", return_value=False):
                    with patch("zorin_constants.get_zorin_home", return_value=Path("/zorin")):
                        with caplog.at_level(logging.WARNING):
                            result = mock_runner._resolve_profile_home_for_source(discord_source)
                            
                            # Should fall back to global ZORIN_HOME
                            assert result == Path("/zorin")
    ...
    def test_existing_profile_no_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("zorin_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("zorin_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/zorin/profiles/existing")
                with patch("zorin_cli.profiles.profile_exists", return_value=True):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
                        
                        assert result == Path("/zorin/profiles/existing")
...
class TestExceptionHandling:
    ...
    def test_get_profile_dir_exception_logs_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("zorin_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("zorin_cli.profiles.get_profile_dir", side_effect=ValueError("Invalid profile name")):
                with patch("zorin_constants.get_zorin_home", return_value=Path("/zorin")):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
    ...
    def test_exception_with_no_profile_name(self, mock_runner, discord_source, caplog):
        ...
        with patch("zorin_cli.profiles.get_active_profile_name", return_value=None):
            with patch("zorin_cli.profiles.get_profile_dir", side_effect=RuntimeError("Filesystem error")):
                with patch("zorin_constants.get_zorin_home", return_value=Path("/zorin")):
                    mock_runner._profile_name_for_source = MagicMock(return_value=None)
