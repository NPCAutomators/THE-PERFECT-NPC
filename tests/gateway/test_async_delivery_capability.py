...
class TestAsyncDeliverySupported:
    ...
    def test_set_false_is_unsupported(self):
        ...
        try:
            ...
            assert get_session_env("ZORIN_SESSION_PLATFORM") == "api_server"
        finally:
            clear_session_vars(tokens)
...
class TestDeclareStatelessChannel:
    """``zorin -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``ZORIN_SESSION_*`` routing keys, so an async delegation's
