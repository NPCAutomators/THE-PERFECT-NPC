...
closure the PR changed, against a real temp ``ZORIN_HOME``.
"""
...
def _fake_switch_result():
    ...
    from zorin_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "zorin_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "zorin_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_zorin_home() / config.yaml`` — point it here.
    monkeypatch.setattr("zorin_constants.get_zorin_home", lambda: zorin_home)
    monkeypatch.setattr("zorin_cli.config.get_zorin_home", lambda: zorin_home)
    return cfg_path
...
async def test_multiplex_picker_keeps_profile_adapter_and_callback_scope(
    ...
    monkeypatch.setattr("zorin_cli.model_switch.switch_model", _profile_switch)
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    named_adapter = _FakePickerAdapter()
    runner = _make_named_runner(monkeypatch, default_adapter, named_adapter, named_home)
    monkeypatch.setattr(gateway_run, "_zorin_home", default_home)
    _stub_picker_dependencies(monkeypatch)
