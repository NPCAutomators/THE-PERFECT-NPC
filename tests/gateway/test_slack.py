...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.zorin."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /zorin, /btw, /stop,
        ...
        for expected in ("/zorin", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``zorin gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "zorin_feedback" in created_apps[0].registered_actions
        assert "zorin_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("zorin_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("zorin_clarify_choice")
            for pattern in clarify_choice_patterns
        )
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@zorin`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@zorin"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@zorin")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /zorin subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /zorin btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_zorin_prefix_still_works(self, adapter):
        """Backward compat: /zorin btw foo must still route to /btw foo.

        Old workspace manifests only declared /zorin as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_zorin_freeform_question(self, adapter):
        """/zorin <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/zorin",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_zorin_slash_stashes_context(self, adapter):
        """Legacy /zorin <subcommand> also stashes context."""
        command = {
            "command": "/zorin",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_zorin_question_does_not_stash_context(self, adapter):
        """Free-form /zorin <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/zorin",
...
class TestMissingCredentials:
    ...
    async def test_missing_bot_token_sets_fatal_error(self):
        ...
        assert "zorin gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
