...
SESSION_REVOKED / REFRESH_DEAD), the ZORIN_HOME resolution fallback, the
...
def _run_main(mod, tokens_dir, argv, responses):
    ...
    with patch.object(mod.os, "environ", dict(mod.os.environ, ZORIN_HOME=str(tokens_dir.parent))), \
         patch.object(mod.urllib.request, "urlopen", side_effect=fake_urlopen), \
         patch.object(sys, "argv", ["diagnose-oauth-mcp.py", *argv]):
        # Force the env-var fallback path (ignore any importable zorin_constants).
        with patch.object(mod, "_zorin_home", lambda: str(tokens_dir.parent)):
            buf = io.StringIO()
...
def test_zorin_home_env_fallback(tmp_path, monkeypatch):
    mod = load_module()
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path / "custom-home"))
    # Block the zorin_constants import so the env fallback is exercised
    with patch.dict(sys.modules, {"zorin_constants": None}):
        home = mod._zorin_home()
...
def test_skill_md_frontmatter_invariants():
    ...
    assert fm["author"].split(",")[0].strip() != "ZORIN"  # human credited first
