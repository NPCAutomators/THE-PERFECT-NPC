...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.zorin/zorin-agent/utils.py)
...
2. ``zorin update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``zorin_cli/main.py`` (the ``zorin update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``zorin_constants`` after the
