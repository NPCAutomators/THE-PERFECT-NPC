...
class TestToolsetConsistency:
    ...
    def test_zorin_platforms_share_core_tools(self):
        """All zorin-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on zorin-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["zorin-cli", "zorin-telegram", "zorin-discord", "zorin-whatsapp", "zorin-slack", "zorin-signal", "zorin-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_zorin_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("zorin-whatsapp")

    def test_zorin_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("zorin-api-server")
