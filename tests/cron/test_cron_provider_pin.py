...
npcautomators/claude-fable-5), the job would silently inherit it on its next tick and
...
resolve_runtime_provider against a temp ZORIN_HOME) and the create_job
...
class TestProviderDriftGuard:
    ...
    def test_b_unpinned_snapshot_differs_fails_closed(self, tmp_path):
        ...
        job = _base_job(provider_snapshot="openrouter")
        success, output, final_response, error, agent_constructed = \
            _run_with_current_provider(job, "npcautomators", tmp_path)
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("zorin_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
                prompt="do a thing", schedule="every 1 hour", provider="npcautomators"
            )

        # Explicit provider → pinned → no snapshot needed, and resolution skipped.
        assert job["provider"] == "npcautomators"
    ...
    def test_unpinned_model_captures_model_snapshot(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "cron.jobs.get_zorin_home", lambda: tmp_path, raising=True
        )
        with patch(
            "zorin_cli.runtime_provider.resolve_runtime_provider",
            return_value={"provider": "openrouter"},
...
def _run_with_current_provider_and_model(job, current_provider, current_model, tmp_path):
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._zorin_home", tmp_path), \
         patch("cron.scheduler._get_zorin_home", return_value=tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("zorin_cli.env_loader.load_zorin_dotenv"), \
         patch("zorin_cli.env_loader.reset_secret_source_cache"), \
         patch("zorin_state.SessionDB", return_value=fake_db), \
