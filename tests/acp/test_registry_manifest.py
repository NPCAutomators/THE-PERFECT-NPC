"""Tests for ACP Registry metadata shipped with Zorin."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "zorin-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "ZORIN"
    assert data["description"]
    assert data["repository"] == "https://github.com/NPCAutomators/AGENT-ZORIN"
    assert data["website"].startswith("https://github.com/NPCAutomators/AGENT-ZORIN/")
    assert data["authors"] == ["NPCAUTOMATORS"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"zorin-agent[acp]=={data['version']}"
    assert uvx["args"] == ["zorin-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"zorin-agent[acp]=={_pyproject_version()}"
    )
