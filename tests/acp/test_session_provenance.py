...
def test_root_session_no_compression(db):
    ...
    assert prov["currentZorinSessionId"] == "root1"
    assert prov["rootZorinSessionId"] == "root1"
    assert prov["parentZorinSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_zorin_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentZorinSessionId"] == "old"
    assert prov["rootZorinSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousZorinSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootZorinSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootZorinSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_zorin_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousZorinSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"zorin"}
    assert "sessionProvenance" in meta["zorin"]
    assert meta["zorin"]["sessionProvenance"]["currentZorinSessionId"] == "root1"
