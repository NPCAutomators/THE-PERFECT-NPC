...
The ``zorin update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m zorin_cli.main gateway run`` — image name
    ``pythonw``, not ``zorin.exe`` — so the ``taskkill /IM zorin.exe`` guard
    ...
    # The zorin.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM zorin.exe' in text
