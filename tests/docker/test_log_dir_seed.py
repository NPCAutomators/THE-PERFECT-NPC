"""Runtime smoke test for Docker $ZORIN_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the zorin user after container boot.
...
registered later runs its log service as the dropped zorin user and
...
def test_logs_gateways_seeded_and_zorin_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by zorin after boot."""
    ...
    assert "logs=zorin" in r.stdout, (
        f"logs/ not owned by zorin: {r.stdout}"
    )
    assert "gateways=zorin" in r.stdout, (
        f"logs/gateways/ not owned by zorin: {r.stdout}"
    )
