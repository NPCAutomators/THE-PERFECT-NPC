...
Build the real image and verify at runtime:
  ...
  1. /opt/zorin is not writable by the zorin user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and ZORIN_DISABLE_LAZY_INSTALLS are set
  3. /opt/zorin/.install_method contains "docker" (code-scoped stamp)
  4. $ZORIN_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $ZORIN_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_zorin(
    built_image: str, container_name: str,
) -> None:
    """The zorin user must not be able to modify /opt/zorin.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/zorin as the zorin user
        "touch /opt/zorin/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"zorin user can write to /opt/zorin (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/zorin/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"zorin user can write to /opt/zorin/.venv: {r.stdout}"
    )


def test_zorin_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    ZORIN_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$ZORIN_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"ZORIN_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/zorin/.install_method (code-scoped), NOT in $ZORIN_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/zorin/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/zorin/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $ZORIN_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        "$ZORIN_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $ZORIN_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $ZORIN_HOME was not healed on boot: "
        f"{r.stdout}"
    )
