...
Override the image with ``ZORIN_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ZORIN_TEST_IMAGE", "zorin-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ZORIN_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ZORIN_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged zorin user
# ---------------------------------------------------------------------------
#
# Background: every Zorin runtime path inside the container drops to UID
# 10000 (the ``zorin`` user) via ``s6-setuidgid zorin``. ``docker exec``
...
# fails (PermissionError swallowed) for zorin, so a test that ran as root
...
# helpers below run every probe under ``-u zorin`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: zorin).
