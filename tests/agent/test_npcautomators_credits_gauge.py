"""Tests for the NpcAutomators-credits subscription % gauge in build_npcautomators_credits_snapshot.
...
from zorin_cli.npcautomators_account import (
    ...
    NpcAutomatorsPaidServiceAccessInfo,
    NpcAutomatorsPortalSubscriptionInfo,
    _subscription_from_payload,
)
from agent.account_usage import build_npcautomators_credits_snapshot, render_account_usage_lines
...
def _acct(**kwargs):
    ...
    kwargs.setdefault("portal_base_url", "https://portal.npcautomators.com")
    return NpcAutomatorsPortalAccountInfo(**kwargs)
...
def test_gauge_present_with_monthly_credits():
    snap = build_npcautomators_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=NpcAutomatorsPortalSubscriptionInfo(
...
def test_gauge_90pct():
    snap = build_npcautomators_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=NpcAutomatorsPortalSubscriptionInfo(monthly_credits=220, credits_remaining=22.0),
    ))
...
def test_gauge_debt_clamps_to_100():
    snap = build_npcautomators_credits_snapshot(_acct(
        ...
        subscription=NpcAutomatorsPortalSubscriptionInfo(monthly_credits=220, credits_remaining=-5.0),
        paid_service_access_info=NpcAutomatorsPaidServiceAccessInfo(subscription_credits_remaining=-5.0),
    ))
...
def test_gauge_at_cap_is_zero_used():
    snap = build_npcautomators_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=NpcAutomatorsPortalSubscriptionInfo(monthly_credits=220, credits_remaining=220.0),
    ))
...
def test_no_monthly_credits_falls_back_to_magnitudes():
    snap = build_npcautomators_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=NpcAutomatorsPortalSubscriptionInfo(plan="Ultra", credits_remaining=-0.79),
        paid_service_access_info=NpcAutomatorsPaidServiceAccessInfo(purchased_credits_remaining=991.96),
    ))
...
def test_nan_remaining_no_window_no_nan_string():
    ...
    snap = build_npcautomators_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=NpcAutomatorsPortalSubscriptionInfo(monthly_credits=220, credits_remaining=float("nan")),
        paid_service_access_info=NpcAutomatorsPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
    assert _window(snap) is None
...
def test_inf_cap_no_window():
    snap = build_npcautomators_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=NpcAutomatorsPortalSubscriptionInfo(monthly_credits=float("inf"), credits_remaining=10.0),
        paid_service_access_info=NpcAutomatorsPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
    assert _window(snap) is None
...
def test_rollover_balance_exceeds_cap_no_window():
    ...
    snap = build_npcautomators_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=NpcAutomatorsPortalSubscriptionInfo(monthly_credits=220, credits_remaining=300, rollover_credits=80),
        paid_service_access_info=NpcAutomatorsPaidServiceAccessInfo(subscription_credits_remaining=300.0),
    ))
    assert _window(snap) is None
...
def test_bool_monthly_credits_no_window():
    snap = build_npcautomators_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=NpcAutomatorsPortalSubscriptionInfo(monthly_credits=True, credits_remaining=1.0),
        paid_service_access_info=NpcAutomatorsPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
    assert _window(snap) is None
...
def test_zero_monthly_credits_no_divzero():
    snap = build_npcautomators_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=NpcAutomatorsPortalSubscriptionInfo(monthly_credits=0, credits_remaining=0.0),
        paid_service_access_info=NpcAutomatorsPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
    assert _window(snap) is None
...
def test_failopen_none_and_logged_out():
    assert build_npcautomators_credits_snapshot(None) is None
    assert build_npcautomators_credits_snapshot(_acct(logged_in=False)) is None
