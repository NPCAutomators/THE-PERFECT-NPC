...
pytestmark = pytest.mark.skipif(
    os.environ.get("ZORIN_RUN_SLOW_PET_TESTS") != "1",
    reason=(
        "pet generation image-processing suite is opt-in; run with "
        "ZORIN_RUN_SLOW_PET_TESTS=1 scripts/run_tests.sh tests/agent/test_pet_generate.py"
    ),
)
...
def test_list_sprite_providers_marks_default(monkeypatch):
    ...
    registry = {"openai": _FakeImgProvider("openai"), "npcautomators": _FakeImgProvider("npcautomators")}
    monkeypatch.setattr(imagegen, "_discover", lambda: None)
    monkeypatch.setattr("agent.image_gen_registry.get_active_provider", lambda: registry["openai"])
    monkeypatch.setattr("agent.image_gen_registry.get_provider", lambda name: registry.get(name))
    ...
    assert names == {"openai", "npcautomators"}
    ...
    # Listed in preference order: NPCAUTOMATORS Portal before OpenAI.
    assert [p["name"] for p in listed] == ["npcautomators", "openai"]
