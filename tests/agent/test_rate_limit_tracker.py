...
# ── Sample headers from NpcAutomators inference API ──────────────────────────────

NPCAUTOMATORS_HEADERS = {
...
class TestParseHeaders:
    def test_basic_parsing(self):
        state = parse_rate_limit_headers(NPCAUTOMATORS_HEADERS, provider="npcautomators")
        assert state is not None
        assert state.provider == "npcautomators"
...
class TestFormatting:
    ...
    def test_format_display_with_data(self):
        state = parse_rate_limit_headers(NPCAUTOMATORS_HEADERS, provider="npcautomators")
        result = format_rate_limit_display(state)
        assert "NpcAutomators" in result
...
class TestAgentIntegration:
    ...
    def test_capture_rate_limits_from_headers(self):
        ...
        class MockResponse:
            headers = NPCAUTOMATORS_HEADERS

        # Import AIAgent minimally

        # Test the parsing directly
        state = parse_rate_limit_headers(MockResponse.headers, provider="npcautomators")
        assert state is not None
        assert state.requests_min.limit == 800
        assert state.tokens_hour.limit == 336000000
