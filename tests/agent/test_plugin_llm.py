...
""",
        ...
        from zorin_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
        (zorin_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))
        from zorin_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from zorin_cli.plugins import PluginContext, PluginManifest, PluginManager
