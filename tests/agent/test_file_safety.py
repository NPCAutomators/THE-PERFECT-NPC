...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_zorin_env(self):
        """Zorin' own .env inside ZORIN_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: zorin internal .env is in ~/.zorin/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.zorin/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.zorin/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Zorin cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        zorin_home = tmp_path / ".zorin"
        cache = zorin_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._zorin_home_path", return_value=zorin_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Zorin cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        zorin_home = tmp_path / ".zorin"
        hub = zorin_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_zorin_home(self, tmp_path):
        """The env basename guard does not depend on ZORIN_HOME resolution."""
        zorin_home = tmp_path / ".zorin"
        zorin_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        zorin_home = tmp_path / ".zorin"
        cache = zorin_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
