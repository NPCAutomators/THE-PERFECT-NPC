...
ZORIN_DEV_CREDITS_FIXTURE) so depletion AND the 90% grant warning fire immediately
...
def test_dev_fixtures_drive_cold_start():
    """Every ZORIN_DEV_CREDITS_FIXTURE state produces a valid seed CreditsState."""
    import os
    ...
    for name, want in expected.items():
        os.environ["ZORIN_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
        os.environ["ZORIN_DEV_CREDITS_FIXTURE"] = name
        try:
            fx = dev_fixture_credits_state()
            assert fx is not None, name
            assert _cold_start_notices(fx) == want, (name, _cold_start_notices(fx))
        finally:
            os.environ.pop("ZORIN_DEV_CREDITS_FIXTURE", None)
            os.environ.pop("ZORIN_DEV_CREDITS", None)
...
class _FakeAgent:
    ...
    def __init__(self, provider="npcautomators", model=""):
...
def _seed(agent, fixture):
    ...
    os.environ["ZORIN_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
    os.environ["ZORIN_DEV_CREDITS_FIXTURE"] = fixture
    try:
...
def test_seed_depleted_suppressed_on_free_model():
    """A session that opens depleted but on a NpcAutomators ``:free`` model must NOT show
...
def test_seed_skips_non_npcautomators():
    from agent.credits_tracker import seed_credits_at_session_start
