"""Tests for build_npcautomators_credits_snapshot (L6-A, magnitudes-only)."""

from __future__ import annotations

from agent.account_usage import build_npcautomators_credits_snapshot
...
def _account(**kwargs) -> NpcAutomatorsPortalAccountInfo:
...
def test_healthy():
    ...
    snap = build_npcautomators_credits_snapshot(info)
    assert snap is not None
    assert snap.available is True
    assert snap.plan == "Pro"
    assert snap.provider == "npcautomators"
    assert snap.title == "NpcAutomators credits"
...
def test_money_rule_no_percent():
    info = _account(
        ...
        subscription=NpcAutomatorsPortalSubscriptionInfo(plan="Pro"),
    )
    snap = build_npcautomators_credits_snapshot(info)
    assert snap is not None
...
def test_logged_out():
    ...
    assert build_npcautomators_credits_snapshot(info) is None
