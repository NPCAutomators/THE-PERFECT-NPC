...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ZORIN_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="zorin_e2e_47967_")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("zorin_"):
            del sys.modules[mod]
