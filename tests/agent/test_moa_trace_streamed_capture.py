...
path via ``zorin chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ZORIN_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ZORIN_HOME at a temp dir and turn moa.save_traces on."""
    zorin_home = tmp_path / ".zorin"
    zorin_home.mkdir()
    monkeypatch.setenv("ZORIN_HOME", str(zorin_home))

    # save_moa_turn reads config via zorin_cli.config.load_config; stub it to
