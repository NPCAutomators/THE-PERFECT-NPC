...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "zorin" / "config.yaml"
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zorin_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "zorin" / "auth.json").read_text())
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "zorin" / "auth.json").read_text()
...
def test_load_pool_collapses_duplicate_env_rows_to_active_key(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "zorin" / "auth.json").read_text())
...
def test_load_pool_persists_bitwarden_origin_metadata_without_secret(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", sentinel)
    monkeypatch.setattr(
        "zorin_cli.env_loader.get_secret_source",
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    from zorin_cli.auth import write_credential_pool
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://zorin/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
    ...
    assert borrowed["source"] == "systemd://zorin/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    write_credential_pool("npcautomators", [
        {
            "id": "npcautomators-device",
            "label": "device-code",
            "auth_type": "oauth",
            "priority": 0,
            "source": "device_code",
    ...
    persisted = json.loads((tmp_path / "zorin" / "auth.json").read_text())["credential_pool"]["npcautomators"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.zorin/.env when
    ...
    # User edited ~/.zorin/.env with the fresh key
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.zorin/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (zorin_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_load_pool_migrates_npcautomators_provider_state(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path / "zorin"))
    _write_auth_store(
        tmp_path,
        {
...
def test_load_pool_mirrors_npcautomators_invoke_jwt_agent_key_runtime_api_key(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path / "zorin"))
    ...
    auth_payload = json.loads((tmp_path / "zorin" / "auth.json").read_text())
    pool_entry = auth_payload["credential_pool"]["npcautomators"][0]
    assert pool_entry["agent_key"] == token
    assert pool_entry["agent_key_expires_at"] == expires_at


def test_npcautomators_runtime_api_key_rejects_opaque_agent_key():
    from agent.credential_pool import PooledCredential

    entry = PooledCredential(
        provider="npcautomators",
        id="npcautomators-opaque",
        label="opaque",
        auth_type="oauth",
        priority=0,
        source="device_code",
...
def test_npcautomators_pool_terminal_refresh_removes_device_code_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path / "zorin"))
    monkeypatch.setenv("ZORIN_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    pool.add_entry(PooledCredential.from_dict("npcautomators", {
        "id": "legacy-seeded",
        "source": "manual:device_code",
        "auth_type": "oauth",
    ...
    pool.add_entry(PooledCredential.from_dict("npcautomators", {
        ...
        "source": "manual",
        "auth_type": "api_key",
        "access_token": "manual-npcautomators-key",
    }))

    monkeypatch.setattr(auth_mod, "resolve_npcautomators_runtime_credentials", _terminal_refresh_failure)
    ...
    auth_payload = json.loads((tmp_path / "zorin" / "auth.json").read_text())
    npcautomators_state = auth_payload["providers"]["npcautomators"]
    assert not npcautomators_state.get("refresh_token")
    assert not npcautomators_state.get("access_token")
    assert not npcautomators_state.get("agent_key")
    assert npcautomators_state["last_auth_error"]["code"] == "invalid_grant"
    assert [entry["id"] for entry in auth_payload["credential_pool"]["npcautomators"]] == ["manual-key"]

    assert pool.try_refresh_current() is None
    assert refresh_calls["count"] == 1


def test_load_pool_removes_npcautomators_device_code_when_singleton_quarantined(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path / "zorin"))
    _write_auth_store(
        tmp_path,
        {
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_zorin_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_load_pool_migrates_npcautomators_provider_state_preserves_tls(tmp_path, monkeypatch):
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path / "zorin"))
    _write_auth_store(
        tmp_path,
        {
            ...
            "providers": {
                "npcautomators": {
                    ...
                    "tls": {
                        "insecure": True,
                        "ca_bundle": "/tmp/npcautomators-ca.pem",
                    },
                }
            },
        },
    ...
    auth_payload = json.loads((tmp_path / "zorin" / "auth.json").read_text())
    assert auth_payload["credential_pool"]["npcautomators"][0]["tls"] == {
        "insecure": True,
        "ca_bundle": "/tmp/npcautomators-ca.pem",
    }
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("zorin_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    assert {entry.source for entry in entries} == {"manual:zorin_pkce", "zorin_pkce"}
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `zorin setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Zorin PKCE
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_zorin_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `zorin_pkce` into auth.json) and later switches to the API key at
    `zorin setup` would still have those OAuth entries dormant on disk.
    ...
    monkeypatch.setattr("zorin_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("agent.anthropic_adapter.read_zorin_oauth_credentials", lambda: None)
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `zorin setup`.  When ANTHROPIC_TOKEN is set (and
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path / "zorin"))
    (tmp_path / "zorin").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "zorin" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
...
def test_npcautomators_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
    """Regression test for #15099 secondary issue.

    When ``_seed_from_singletons`` materialises a device_code pool entry from
    the ``providers.npcautomators`` singleton, it must carry the mint/refresh
        ...
        tmp_path,
        {
            ...
            "providers": {
                "npcautomators": {
                    ...
                    "inference_base_url": "https://inference.npcautomators.com/v1",
                    "token_type": "Bearer",
                    "scope": "openid profile",
                    "obtained_at": "2026-04-24T10:00:00+00:00",
                    "expires_at": "2026-04-24T11:00:00+00:00",
                    "expires_in": 3600,
                    "agent_key": "sk-npcautomators-AAAA",
                    "agent_key_id": "ak_123",
    ...
    assert e.agent_key == "sk-npcautomators-AAAA"
...
# ── PR #10160 salvage: NpcAutomators OAuth cross-process sync tests ─────────────────

def test_sync_npcautomators_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    synced = pool._sync_npcautomators_entry_from_auth_store(entry)
...
def test_sync_npcautomators_entry_noop_when_tokens_match(tmp_path, monkeypatch):
...
def test_npcautomators_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    """An exhausted NpcAutomators entry should recover when auth.json has newer tokens."""
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path / "zorin"))
    from agent.credential_pool import load_pool, STATUS_EXHAUSTED
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `zorin auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `zorin model` to reauth, but the pool
    ...
    # Simulate `zorin model` / `zorin auth` refreshing the tokens.
...
def test_is_terminal_xai_oauth_refresh_error():
    from zorin_cli.auth import AuthError, _is_terminal_xai_oauth_refresh_error
    ...
    # NpcAutomators error does not trigger xAI check
    assert not _is_terminal_xai_oauth_refresh_error(
        AuthError("Revoked", provider="npcautomators", code="invalid_grant", relogin_required=True)
    )
    # Generic exception
...
def test_is_terminal_codex_oauth_refresh_error():
    from zorin_cli.auth import AuthError, _is_terminal_codex_oauth_refresh_error
...
def test_persist_preserves_concurrent_disk_only_entry(tmp_path, monkeypatch):
    ...
    final = json.loads((tmp_path / "zorin" / "auth.json").read_text())
...
def _make_anthropic_claude_code_pool(tmp_path, monkeypatch, *, access_token, refresh_token, expires_at_ms=9_999_999_999_000):
    ...
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {}})
    monkeypatch.setattr("zorin_cli.auth.is_provider_explicitly_configured", lambda pid: pid == "anthropic")
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_zorin_oauth_credentials",
        lambda: None,
    )
