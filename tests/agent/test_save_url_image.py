...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ZORIN_HOME under tmp_path."""
    monkeypatch.setenv("ZORIN_HOME", str(tmp_path / ".zorin"))
    (tmp_path / ".zorin").mkdir()

    # Force the constants/image cache helpers to re-read ZORIN_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("zorin_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_zorin_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ZORIN_HOME — gateway cleanup
