...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("zorin_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated ZORIN_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="zorin_api_content_")
    os.makedirs(os.path.join(test_home, ".zorin"))
    prev_home = os.environ.get("ZORIN_HOME")
    os.environ["ZORIN_HOME"] = os.path.join(test_home, ".zorin")
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["ZORIN_HOME"] = prev_home
