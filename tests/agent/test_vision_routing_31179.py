...
def isolated_home(monkeypatch):
    """Temp ZORIN_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="zorin_test_31179_")
    zorin_home = os.path.join(test_home, ".zorin")
    os.makedirs(zorin_home)
...
def _fresh_modules():
    """Drop cached zorin modules so each test reloads against current env."""
