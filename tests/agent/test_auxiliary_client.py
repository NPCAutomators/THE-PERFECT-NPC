...
from agent.auxiliary_client import (
    ...
    _refresh_npcautomators_recommended_model,
...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "NPCAUTOMATORS_INFERENCE_BASE_URL",
...
class TestAuxiliaryMaxTokensParam:
    def test_uses_max_completion_tokens_for_github_copilot_custom_base(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime", return_value=("https://api.githubcopilot.com", "key", None)), \
             patch("agent.auxiliary_client._read_npcautomators_auth", return_value=None):
...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_differing_from_config_ignores_config_endpoint(self):
        ...
        assert resolved_provider == "npcautomators"
        assert base_url is None
        assert api_key is None
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("npcautomators", "zorin-4", "https://inference-api.npcautomators.com/v1"),
...
class TestNpcAutomatorsTagsScoping:
    def test_tags_injected_when_provider_is_npcautomators(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_npcautomators", False)
        ...
        assert kwargs["extra_body"]["tags"] == aux._npcautomators_portal_tags()

    def test_tags_not_injected_for_gemini_when_main_is_npcautomators(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_npcautomators", True)
    ...
    def test_tags_not_injected_for_openrouter_when_main_is_npcautomators(self, monkeypatch):
        import agent.auxiliary_client as aux
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("zorin_cli.auth._read_codex_tokens", return_value={
    ...
    def test_missing_returns_none(self, tmp_path, monkeypatch):
        zorin_home = tmp_path / "zorin"
        zorin_home.mkdir(parents=True, exist_ok=True)
        (zorin_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``zorin auth status`` already reports these as logged in; compression
        ...
        from zorin_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
        ...
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))
        monkeypatch.delenv("ZORIN_XAI_BASE_URL", raising=False)
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZORIN_HOME", str(zorin_home))
        monkeypatch.setenv("ZORIN_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``zorin model``, no per-task override is
    ...
    fallback chain (OpenRouter / NpcAutomators / etc.), so aux tasks billed
    ...
    def test_empty_model_for_catalog_provider_uses_catalog_default(self):
        """anthropic / npcautomators / openrouter / etc.: catalog default wins
        ...
        with (
            ...
            patch(
                "agent.auxiliary_client._read_npcautomators_auth", return_value=None
            ),
        ):
...
class TestExpiredCodexFallback:
    ...
    def test_zorin_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-zorin-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("zorin_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        with patch("agent.auxiliary_client._read_npcautomators_auth", return_value=None), \
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime",
             ...
             patch("agent.auxiliary_client._read_npcautomators_auth", return_value=None), \
             patch("agent.auxiliary_client._resolve_npcautomators_runtime_api", return_value=None), \
...
class TestAuxiliaryPoolAwareness:
    def test_try_npcautomators_uses_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=_Pool()),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("zorin_cli.models.get_npcautomators_recommended_aux_model", return_value=None),
        ):
            from agent.auxiliary_client import _try_npcautomators

            client, model = _try_npcautomators()
    ...
    def test_try_npcautomators_refreshes_stale_pool_entry(self):
    ...
    def test_resolve_npcautomators_runtime_api_rejects_stale_pool_entry_when_refresh_fails(self):
        stale_token = _jwt_with_claims({
            "scope": "inference:invoke",
            "exp": int(time.time() - 60),
        })
        ...
        with (
            ...
            from agent.auxiliary_client import _resolve_npcautomators_runtime_api

            runtime = _resolve_npcautomators_runtime_api()

        assert runtime is None

    def test_try_npcautomators_uses_portal_recommendation_for_text(self):
        """When the Portal recommends a compaction model, _try_npcautomators honors it."""
        fresh_base = "https://inference-api.npcautomators.com/v1"
        with (
            patch("agent.auxiliary_client._read_npcautomators_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_npcautomators_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("zorin_cli.models.get_npcautomators_recommended_aux_model", return_value="minimax/minimax-m2.7") as mock_rec,
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            from agent.auxiliary_client import _try_npcautomators

            mock_openai.return_value = MagicMock()
            client, model = _try_npcautomators(vision=False)

        assert client is not None
        assert model == "minimax/minimax-m2.7"
        assert mock_rec.call_args.kwargs["vision"] is False

    def test_try_npcautomators_uses_portal_recommendation_for_vision(self):
        """Vision tasks should ask for the vision-specific recommendation."""
        fresh_base = "https://inference-api.npcautomators.com/v1"
        with (
            patch("agent.auxiliary_client._read_npcautomators_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_npcautomators_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("zorin_cli.models.get_npcautomators_recommended_aux_model", return_value="google/gemini-3-flash-preview") as mock_rec,
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_npcautomators
            client, model = _try_npcautomators(vision=True)

        assert client is not None
        assert model == "google/gemini-3-flash-preview"
        assert mock_rec.call_args.kwargs["vision"] is True

    def test_try_npcautomators_falls_back_when_recommendation_lookup_raises(self):
        """If the Portal lookup throws, we must still return a usable model."""
        fresh_base = "https://inference-api.npcautomators.com/v1"
        with (
            patch("agent.auxiliary_client._read_npcautomators_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_npcautomators_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("zorin_cli.models.get_npcautomators_recommended_aux_model", side_effect=RuntimeError("portal down")),
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_npcautomators
            client, model = _try_npcautomators()
    ...
    def test_call_llm_retries_npcautomators_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https://inference-api.npcautomators.com/v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale npcautomators key")

        fresh_client = MagicMock()
        fresh_client.base_url = "https://inference-api.npcautomators.com/v1"
        fresh_client.chat.completions.create.return_value = {"ok": True}

        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("npcautomators", "npcautomators-model", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", return_value=(stale_client, "npcautomators-model")),
            ...
            patch("agent.auxiliary_client._resolve_npcautomators_runtime_api", return_value=("fresh-agent-key", "https://inference-api.npcautomators.com/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
    ...
    def test_call_llm_refreshes_npcautomators_after_free_tier_block_when_account_paid(self):
        ...
        with (
            ...
            patch("agent.auxiliary_client._resolve_npcautomators_runtime_api", return_value=("fresh-agent-key", "https://inference-api.npcautomators.com/v1")),
            patch(
                "zorin_cli.npcautomators_account.get_npcautomators_portal_account_info",
                return_value=NpcAutomatorsPortalAccountInfo(
                    logged_in=True,
                    source="account_api",
                    fresh=True,
    ...
    @pytest.mark.asyncio
    async def test_async_call_llm_retries_npcautomators_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https://inference-api.npcautomators.com/v1"
        stale_client.chat.completions.create = AsyncMock(side_effect=_Auth401("stale npcautomators key"))

        fresh_async_client = MagicMock()
        fresh_async_client.base_url = "https://inference-api.npcautomators.com/v1"
        fresh_async_client.chat.completions.create = AsyncMock(return_value={"ok": True})

        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("npcautomators", "npcautomators-model", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", return_value=(stale_client, "npcautomators-model")),
            patch("agent.auxiliary_client._to_async_client", return_value=(fresh_async_client, "npcautomators-model")),
            patch("agent.auxiliary_client._validate_llm_response", side_effect=lambda resp, _task, **_kw: resp),
            patch("agent.auxiliary_client._resolve_npcautomators_runtime_api", return_value=("fresh-agent-key", "https://inference-api.npcautomators.com/v1")),
        ):
    ...
    @pytest.mark.asyncio
    async def test_async_call_llm_refreshes_npcautomators_after_free_tier_block_when_account_paid(self):
        from zorin_cli.npcautomators_account import NpcAutomatorsPortalAccountInfo
...
class TestIsPaymentError:
    ...
    def test_404_free_tier_model_block_is_payment(self):
        exc = Exception(
            ...
            "Upgrade at https://portal.npcautomators.com or pick a free model."
        )
        exc.status_code = 404
        assert _is_payment_error(exc) is True
...
class TestIsModelNotFoundError:
    ...
    def test_npcautomators_openrouter_catalog_404(self):
        """The exact incident error: a Portal-recommended model dropped from
        the NpcAutomators → OpenRouter catalog."""
        exc = Exception(
...
class TestRefreshNpcAutomatorsRecommendedModel:
    """_refresh_npcautomators_recommended_model picks a fresh model after a stale 404."""

    def test_returns_fresh_portal_recommendation(self, monkeypatch):
        monkeypatch.setattr(
            "zorin_cli.models.get_npcautomators_recommended_aux_model",
            lambda **kw: "stepfun/step-3.7-flash:free",
        )
        out = _refresh_npcautomators_recommended_model(
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "zorin_cli.models.get_npcautomators_recommended_aux_model", _boom)
        out = _refresh_npcautomators_recommended_model(
...
class TestIsRateLimitError:
    ...
    def test_429_with_resets_in_message(self):
        """NpcAutomators-style 429: 'resets in 3508s'."""
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        assert labels == ["openrouter", "npcautomators", "local/custom", "api-key"]
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_npcautomators", return_value=(mock_client, "npcautomators-model")), \
             patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
        assert client is mock_client
        assert model == "npcautomators-model"
        assert label == "npcautomators"

    def test_returns_none_when_no_fallback(self):
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_npcautomators", return_value=(None, None)), \
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/NpcAutomators/custom/api-key all fail, payment-fallback returns None —
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Zorin auth / credential pool."""
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from zorin_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("zorin_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Zorin owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("zorin_cli.config.load_config", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_empty_reasoning_effort_is_noop(self):
        ...
        with patch("zorin_cli.config.load_config", return_value=config):
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Zorin reasoning.

    MoA slot reasoning is carried through call_llm as a Zorin
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/NPCAutomators/AGENT-ZORIN/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/NPCAutomators/AGENT-ZORIN/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "ZorinAgent"
...
def test_pool_runtime_base_url_uses_npcautomators_env_override(monkeypatch):
    entry = SimpleNamespace(
        provider="npcautomators",
        runtime_base_url="https://inference-api.npcautomators.com/v1",
        inference_base_url="https://inference-api.npcautomators.com/v1",
        base_url="https://inference-api.npcautomators.com/v1",
    )
    monkeypatch.setenv("NPCAUTOMATORS_INFERENCE_BASE_URL", "https://ai.wildebeest-newton.ts.net/v1")
...
class TestAuxUnhealthyCache:
    ...
    def test_resolve_auto_skips_unhealthy_step2(self):
        ...
        npcautomators_client = MagicMock()
        # Mark OpenRouter unhealthy → chain should skip it and pick npcautomators.
        _mark_provider_unhealthy("openrouter")
        with patch("agent.auxiliary_client._read_main_provider", return_value=""), \
             ...
             patch("agent.auxiliary_client._try_npcautomators", return_value=(npcautomators_client, "npcautomators-model")), \
             patch("agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
        assert client is npcautomators_client
        assert model == "npcautomators-model"
    ...
    def test_resolve_auto_skips_unhealthy_main_in_step1(self):
        ...
        with patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"), \
             ...
             patch("agent.auxiliary_client._try_openrouter") as or_try, \
             patch("agent.auxiliary_client._try_npcautomators", return_value=(npcautomators_client, "n-model")), \
             patch("agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
        # Step-1 was bypassed — resolve_provider_client never invoked
        step1.assert_not_called()
        # Step-2 also skipped openrouter and landed on npcautomators
        or_try.assert_not_called()
        assert client is npcautomators_client
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        # unhealthy. The chain should still find npcautomators.
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        npcautomators_client = MagicMock()
        npcautomators_resp = MagicMock()
        npcautomators_resp.choices = [MagicMock(message=MagicMock(content="ok"))]
        npcautomators_client.chat.completions.create.return_value = npcautomators_resp
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._resolve_task_provider_model",
                    return_value=("auto", "google/gemini-3-flash-preview", None, None, None)), \
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(npcautomators_client, "n-model", "npcautomators")), \
...
class TestCompressionFallbackContextFilter:
    ...
    def test_main_chain_skips_too_small_candidate_for_compression(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "zorin_cli.fallback_config.get_fallback_chain",
            lambda cfg: chain,
        )
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("zorin_cli.config.load_config", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("zorin_cli.config.load_config", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
