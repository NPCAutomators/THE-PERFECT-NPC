...
Prior to this change, aggregator users (OpenRouter / NPCAUTOMATORS Portal) had aux
...
class TestResolveAutoMainFirst:
    ...
    def test_npcautomators_main_uses_main_model_for_aux(self, monkeypatch):
        """NPCAUTOMATORS Portal main user → aux uses their picked NpcAutomators model, not free-tier MiMo."""
        # No OPENROUTER_API_KEY → ensures if main failed we'd fall to chain
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="npcautomators",
        ), patch(
            "agent.auxiliary_client._read_main_model",
        ...
        assert mock_resolve.call_args.args[0] == "npcautomators"
...
class TestResolveVisionMainFirst:
    ...
    def test_npcautomators_main_vision_uses_paid_npcautomators_vision_backend(self):
        """Paid NpcAutomators main → aux vision uses the dedicated NpcAutomators vision backend."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="npcautomators",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_npcautomators_main_vision_uses_free_tier_npcautomators_vision_backend(self):
        """Free-tier NpcAutomators main → aux vision uses MiMo omni, not the text main model."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="npcautomators",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            ...
            "zorin_cli.copilot_auth.copilot_request_headers",
    ...
    def test_main_unavailable_vision_falls_through_to_aggregators(self):
        """Main provider fails → fall back to OpenRouter/NpcAutomators strict backends."""
        ...
        assert provider in {"openrouter", "npcautomators"}
    ...
    def test_explicit_provider_override_still_wins(self):
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("npcautomators", None, None, None, None),  # explicit override
        ), patch(
            "agent.auxiliary_client._resolve_strict_vision_backend"
        ) as mock_strict:
            mock_strict.return_value = (MagicMock(), "npcautomators-default-model")

            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        # Explicit "npcautomators" override → uses strict backend, NOT main model path
        assert provider == "npcautomators"
        mock_strict.assert_called_once_with("npcautomators", None)
...
class TestResolveVisionCustomProvider:
    ...
    it returns ``(None, None)`` and vision falls through to OpenRouter / NpcAutomators,
