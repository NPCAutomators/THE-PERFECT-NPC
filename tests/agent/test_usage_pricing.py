...
def test_npcautomators_portal_pricing_preserves_vendor_prefixed_model_ids(monkeypatch):
    seen = {}
    ...
    entry = get_pricing_entry("openai/gpt-5.5-pro", provider="npcautomators")

    assert seen["base_url"] == "https://inference-api.npcautomators.com/v1"
...
def test_deepseek_v4_pro_pricing_entry_exists():
    ...
    in zorin insights because the _OFFICIAL_DOCS_PRICING table had no
...
def test_fireworks_rows_all_carry_cache_read_pricing():
    ...
    model, and Zorin prompt caching is active on Fireworks sessions — every
