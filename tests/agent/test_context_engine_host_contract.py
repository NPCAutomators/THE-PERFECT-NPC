...
engine plugins (e.g. zorin-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``zorin plugins`` picker."""
    from zorin_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "zorin_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "zorin_cli.plugins.get_plugin_context_engine",
