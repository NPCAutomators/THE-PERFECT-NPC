...
from zorin_cli.npcautomators_account import NpcAutomatorsPortalAccountInfo, NpcAutomatorsPaidServiceAccessInfo
...
def test_view_fetch_failure_is_logged_out(monkeypatch):
    ...
    monkeypatch.setattr("zorin_cli.npcautomators_account.get_npcautomators_portal_account_info", _boom)
...
def test_gateway_topup_renders_block_and_url(monkeypatch):
    view = CreditsView(
        logged_in=True,
        balance_lines=("📈 NpcAutomators credits", "Total usable: $52.50"),
    ...
    assert "📈 NpcAutomators credits" not in out
...
def test_credits_command_fully_removed():
    ...
    from zorin_cli.commands import resolve_command, COMMAND_REGISTRY
