...
manipulation. Zorin keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point ZORIN_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_zorin_home /
    get_default_zorin_root) instead of monkeypatching private helpers —
    ...
    ZORIN_HOME=<root>/profiles/<name> makes get_default_zorin_root()
