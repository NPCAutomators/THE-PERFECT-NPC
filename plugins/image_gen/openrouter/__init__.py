"""OpenRouter-compatible image generation backend (OpenRouter + NPCAUTOMATORS Portal).

Both OpenRouter and the NPCAUTOMATORS Portal inference endpoint speak the same
...
NPCAUTOMATORS Portal proxies OpenRouter, so one implementation services both — we only
swap the resolved ``(base_url, api_key)``. Credentials are resolved through the
agent's existing :func:`~zorin_cli.runtime_provider.resolve_runtime_provider`,
which already understands OpenRouter's key pool and the NpcAutomators OAuth device-code
...
# image_gen.model from ``zorin tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, NPCAUTOMATORS Portal). The two differ only
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``zorin tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `zorin tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against NPCAUTOMATORS Portal).
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            provider_name="npcautomators",
            display_name="NPCAUTOMATORS Portal",
            runtime_name="npcautomators",
            config_key="npcautomators",
            model_env_var="NPCAUTOMATORS_IMAGE_MODEL",
            setup_schema={
                "name": "NPCAUTOMATORS Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via NPCAUTOMATORS Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + NPCAUTOMATORS Portal image gen providers."""
