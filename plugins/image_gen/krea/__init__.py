...
# Zorin uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
...
def _resolve_managed_krea_gateway():
    ...
    the NpcAutomators-hosted Krea gateway wins when it is resolvable AND either no direct
...
class KreaImageGenProvider(ImageGenProvider):
    ...
    def is_available(self) -> bool:
        # Available with a direct Krea key OR via the managed NpcAutomators gateway
        # (NpcAutomators Subscription), so portal users with no Krea key can still
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "Krea",
            "badge": "paid",
            "tag": "Krea 2 foundation model — Medium ($0.03), Large ($0.06), Medium Turbo ($0.015). Style transfer, moodboards, reference-guided generation. Direct key or managed NpcAutomators Subscription gateway.",
            "env_vars": [
                {
                    "key": "KREA_API_KEY",
                    "prompt": "Krea API key",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # Route through the managed NpcAutomators gateway (NpcAutomators Subscription) when the
        ...
        # meters/bills per generation, so the caller token is the NpcAutomators access
        # token, not a Krea key.
        managed = _resolve_managed_krea_gateway()
        if managed is not None:
            base_url = managed.gateway_origin.rstrip("/")
            auth_token = managed.npcautomators_user_token
        else:
            base_url = BASE_URL
            ...
            if not auth_token:
                return error_response(
                    error=(
                        "KREA_API_KEY not set. Run `zorin tools` → Image "
                        "Generation → Krea to configure, get a key at "
                        "https://www.krea.ai/settings/api-tokens, or sign in to "
                        "a NPCAUTOMATORS account with the managed Krea gateway enabled "
                        "(`zorin setup`)."
                    ),
        ...
        if managed is not None:
            if isinstance(kwargs.get("styles"), list) and kwargs.get("styles"):
                return error_response(
                    error=(
                        "Managed Krea (NpcAutomators Subscription) does not support "
        ...
        headers = {
            ...
            "User-Agent": "Zorin-Agent/1.0 (krea-image-gen)",
        }
        if managed is not None:
        ...
        try:
            ...
            # the NPCAUTOMATORS Portal, or the gateway's shared Krea key hit its
            # concurrency cap (429).
            if managed is not None and 400 <= status < 500:
                hint = (
                    ...
                    if status == 429
                        ...
                        "NPCAUTOMATORS Portal's Krea gateway. Set KREA_API_KEY to use "
                        "Krea directly, or pick a different model via "
                        "`zorin tools` → Image Generation."
                    )
                )
                return error_response(
                    error=(
                        f"NpcAutomators Subscription Krea gateway rejected '{model_id}' "
        ...
        # ``/jobs/{id}`` with the NpcAutomators token (404 on cross-user/unknown jobs).
