# Zorin Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: zorin-achievements plugin
...
### `GET /api/plugins/zorin-achievements/achievements`
...
### `POST /api/plugins/zorin-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/zorin-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/zorin-achievements/overview`
...
- Persist plugin data under:
  - `~/.zorin/plugins/zorin-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.zorin/plugins/zorin-achievements/scan_checkpoint.json`
...
- `~/.zorin/plugins/zorin-achievements/`
