...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `zorin slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `zorin gateway setup` "
                "or in the active profile's ~/.zorin/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `zorin gateway setup` "
                "or add it to your active profile's ~/.zorin/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `zorin gateway setup` "
                "or in the active profile's ~/.zorin/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `zorin gateway setup` "
                "or add it to your active profile's ~/.zorin/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_zorin_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Zorin does not currently need to route them into the agent loop.
            ...
            # /model work directly without /zorin prefix). A single regex
            ...
            # manifest (see `zorin slack manifest`). In Socket Mode, Slack
            ...
            from zorin_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/zorin$")

            @self._app.command(_slash_pattern)
            async def handle_zorin_command(ack, command):
            ...
            for _action_id in (
                "zorin_approve_once",
                "zorin_approve_session",
                "zorin_approve_always",
                "zorin_deny",
            ):
            ...
            for _action_id in (
                "zorin_confirm_once",
                "zorin_confirm_always",
                "zorin_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("zorin_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^zorin_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("zorin_clarify_other")(self._handle_clarify_action)
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Zorin handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Zorin session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Zorin owns. Do not write it into _channel_team: channel IDs can be
        ...
        try:
            ...
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "zorin_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "zorin_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zorin_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "zorin_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "zorin_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``zorin_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``zorin_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"zorin_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "zorin_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zorin_confirm_once": "once",
            "zorin_confirm_always": "always",
            "zorin_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zorin_approve_once": "once",
            "zorin_approve_session": "session",
            "zorin_approve_always": "always",
            "zorin_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "zorin_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/zorin <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/zorin
        ...
        if slash_name in {"zorin", ""}:
            # Legacy /zorin <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from zorin_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/zorin   ``).
        ...
        # "Running /cmd…" ack shown by handle_zorin_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/zorin <question>" must produce public replies so
...
# ``zorin_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``zorin_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ZORIN_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from zorin_cli.slack_cli import _build_full_manifest
            from zorin_constants import get_zorin_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Zorin",
                bot_description="Your Zorin agent on Slack",
            )
            target = Path(get_zorin_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `zorin slack manifest --write` anytime to refresh after "
                "Zorin adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `zorin update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``zorin_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'zorin-agent[slack]'",
        # Interactive setup wizard — replaces zorin_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in zorin_cli/gateway.py.
