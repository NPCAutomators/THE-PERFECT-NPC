"""SimpleX Chat platform adapter (Zorin plugin).
...
This adapter ships as a Zorin platform plugin under
``plugins/platforms/simplex/``. The Zorin plugin loader scans the
...
Optional environment variables:
    ...
    ZORIN_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``zorin setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Zorin — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ZORIN_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zorin setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.zorin/.env`` via
    ``zorin_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "zorin_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.zorin/.env"
        )
        return
