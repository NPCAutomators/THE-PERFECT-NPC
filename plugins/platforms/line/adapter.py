"""
LINE Messaging API platform adapter for ZORIN.
...
to ZORIN. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ZORIN_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            zorin_home = Path(get_zorin_home()).resolve()
        except Exception:
            zorin_home = Path.home().joinpath(".zorin").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``zorin status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``zorin status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zorin setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.zorin/.env`` via ``zorin_cli.config``.
    """
    print()
    ...
    try:
        from zorin_cli.config import get_env_var, set_env_var
    except ImportError:
        print("zorin_cli.config not available; set LINE_* vars manually in ~/.zorin/.env")
        return
