"""
Email platform adapter for the Zorin gateway.

Allows users to interact with Zorin by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "zorin-agent" "version" "{_zorin_version}" '
            '"vendor" "NPCAutomators" '
            '"support-email" "noreply@npcautomators.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `zorin config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `zorin gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        msg_id = f"<zorin-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# zorin_cli/gateway.py, and the _send_email dispatch in
