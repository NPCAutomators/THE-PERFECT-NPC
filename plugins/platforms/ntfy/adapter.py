"""ntfy platform adapter (Zorin plugin).
...
``plugins/platforms/ntfy/``. The Zorin plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "zorin-in"              # subscribe topic (incoming)
          publish_topic: "zorin-out"     # optional — defaults to topic
...
_ECHO_TAG = "zorin-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``zorin cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Zorin dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `zorin gateway status` without
