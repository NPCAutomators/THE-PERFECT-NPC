...
//
// Inbound  (gRPC -> Zorin): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Zorin -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Zorin-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `zorin photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Zorin can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Zorin'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Zorin allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Zorin, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-zorin-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Zorin supplied them so a known-good
