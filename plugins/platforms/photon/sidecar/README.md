...
Small Node helper that bridges ZORIN to Photon's Spectrum SDK
(`spectrum-ts`).  Zorin is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Zorin-Sidecar-Token`)
...
The Zorin plugin's `zorin photon setup` command runs `npm install`
here automatically.
