...
Credential storage mirrors every other Zorin channel:

    * runtime SDK creds  -> ``~/.zorin/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.zorin/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a zorin dependency
...
# registers Zorin as its own client_id.
...
# Default name of the project Zorin provisions for the operator.
DEFAULT_PROJECT_NAME = "ZORIN"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of zorin-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.zorin/auth.json`` honouring the active Zorin profile."""
    try:
        from zorin_constants import get_zorin_home
        return Path(get_zorin_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.zorin")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.zorin/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.zorin/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.zorin/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: zorin_cli.config unavailable — skipping .env write")
        return
    try:
...
def find_project_by_name(token: str, name: str) -> Optional[Dict[str, Any]]:
    ...
    targets = {target, f"{target} agent"}
    for proj in list_projects(token):
        if (proj.get("name") or "").strip().lower() in targets:
            return proj
    return None
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `zorin photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `zorin photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `zorin photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `zorin photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `zorin photon setup`)"
