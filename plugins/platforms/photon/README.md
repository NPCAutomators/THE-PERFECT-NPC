...
This plugin connects ZORIN to iMessage (and other Spectrum
...
TypeScript-only, Zorin runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Zorin-Sidecar-Token`.
...
There is no separate `login` command; like every other Zorin channel,
...
Run `zorin photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.zorin/.env` (the same place every other
...
Management metadata lives in `~/.zorin/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `zorin photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `zorin photon status`, a DM and a group roundtrip,
