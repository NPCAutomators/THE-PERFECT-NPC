...
def _save_mem0_json(zorin_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(zorin_home) / "mem0.json"
    existing = {}
...
def _setup_platform(zorin_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "zorin-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "zorin"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.zorin/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.zorin/.env (or unset it) or Zorin will keep "
    ...
    provider.save_config(provider_config, zorin_home)

    if env_writes:
        _write_env(Path(zorin_home) / ".env", env_writes)
...
def _setup_selfhosted(zorin_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "zorin-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "zorin")
...
def _setup_oss(zorin_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(zorin_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "zorin-user")
    ...
    if env_writes:
        _write_env(Path(zorin_home) / ".env", env_writes)
    _save_mem0_json(zorin_home, {"mode": "oss", "user_id": user_id, "agent_id": "zorin", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, zorin_home: str) -> str:
...
_PGVECTOR_CONTAINER = "zorin-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "zorin"
...
def _setup_oss_interactive(zorin_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], zorin_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], zorin_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'zorin-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "zorin-user")

    agent_id = input("  Agent ID [zorin]: ").strip()
    agent_id = agent_id or "zorin"
    ...
    if env_writes:
        _write_env(Path(zorin_home) / ".env", env_writes)
    _save_mem0_json(zorin_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(zorin_home: str, config: dict) -> None:
    """Entry point called by zorin memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(zorin_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(zorin_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(zorin_home, config, flags)
        return
