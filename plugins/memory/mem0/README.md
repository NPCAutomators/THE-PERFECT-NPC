...
```bash
zorin memory setup    # select "mem0"
```
...
Behavioral settings live in `$ZORIN_HOME/mem0.json` (set them via `zorin memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.zorin/.env`.
...
| `user_id` | `zorin-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Zorin session and call `mem0_search` — it connects to your server.
...
```bash
zorin memory setup mem0 --mode oss \
...
```bash
zorin memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ZORIN_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.zorin/mem0_qdrant"}}
  }
}
```
...
```bash
zorin memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ZORIN_HOME/mem0.json`).
