...
- Hosted: API key from [app.supermemory.ai/integrations?connect=zorin](http://app.supermemory.ai/integrations?connect=zorin)
...
Before running `zorin memory setup`, add the local endpoint to
...
Then run `zorin memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ZORIN_HOME/supermemory.json`
...
| `container_tag` | `zorin` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `zorin-{identity}` → `zorin-coder`). |
...
`https://api.supermemory.ai`. Zorin resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: zorin`, and document writes stamp
`metadata.sm_source: zorin`. This is a **functional routing key, not telemetry**:
it groups Zorin-written memories into a dedicated "Zorin" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Zorin profile:

```json
{
  "container_tag": "zorin-{identity}"
}
```

For a profile named `coder`, this resolves to `zorin-coder`. The default profile resolves to `zorin-default`. Without `{identity}`, all profiles share the same container.
