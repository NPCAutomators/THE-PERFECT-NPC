...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=zorin"
...
def _load_supermemory_config(zorin_home: str) -> dict:
    config = _default_config()
    config_path = Path(zorin_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, zorin_home: str) -> None:
    config_path = Path(zorin_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "zorin"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Zorin writes into the "Zorin" Space in the Supermemory
        ...
        merged = {"sm_source": "zorin", **(metadata or {})}
...
def _resolve_container_tag_for_setup(zorin_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(zorin_home)
...
def _probe_supermemory_connection(api_key: str, zorin_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(zorin_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(zorin_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `zorin memory setup`.
        # All other options are documented for $ZORIN_HOME/supermemory.json
    ...
    def save_config(self, values, zorin_home):
        ...
        _save_supermemory_config(sanitized, zorin_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, zorin_home)
    ...
    def post_setup(self, zorin_home: str, config: dict) -> None:
        ...
        from zorin_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(zorin_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from zorin_constants import get_zorin_home
        self._zorin_home = kwargs.get("zorin_home") or str(get_zorin_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._zorin_home)
