...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/zorin>
...
```bash
zorin memory setup honcho   # configure Honcho directly (works on a fresh install)
zorin memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
zorin config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.zorin/.env
```

> `zorin honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `zorin memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `zorin model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ZORIN_HOME/honcho.json` | Profile-local (isolated Zorin instances) |
| 2 | `~/.zorin/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Zorin profile: `zorin` (default) or `zorin_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `zorin honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Zorin session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`zorin-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`zorin`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-zorin-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening zorin in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Zorin session ID (timestamp + hex). Every `zorin` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Zorin profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`zorin`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Zorin profile: `zorin` (default) or `zorin_<profile>` (e.g. `zorin -p coder` -> host key `zorin_coder`). Older `zorin.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ZORIN_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `zorin-agent`) |
...
|---------|-------------|
| `zorin memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `zorin honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `zorin memory setup`) |
| `zorin honcho status` | Show resolved config for active profile |
| `zorin honcho enable` / `disable` | Toggle Honcho for active profile |
| `zorin honcho mode <mode>` | Change recall or observation mode |
| `zorin honcho peer --user <name>` | Update user peer name |
| `zorin honcho peer --ai <name>` | Update AI peer name |
| `zorin honcho tokens --context <N>` | Set context token budget |
| `zorin honcho tokens --dialectic <N>` | Set dialectic max chars |
| `zorin honcho map <name>` | Map current directory to a session name |
| `zorin honcho sync` | Create host blocks for all Zorin profiles |
