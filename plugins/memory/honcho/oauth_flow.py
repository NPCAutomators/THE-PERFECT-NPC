...
``zorin://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Zorin OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "zorin-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``zorin-desktop`` / ``zorin-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Zorin.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local ZORIN_HOME override can't reach.
