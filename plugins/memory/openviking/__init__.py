...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Zorin peer ID in OpenViking (default: zorin)
...
_DEFAULT_AGENT = "zorin"
_AGENT_PROMPT_LABEL = "Zorin peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "ZORIN_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Zorin slash-skill scaffolding before sending content to OpenViking.
...
def _load_zorin_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("ZORIN_HOME", "")).expanduser() if os.environ.get("ZORIN_HOME") else Path.home() / ".zorin"
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Zorin session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Zorin only", "write values only to Zorin .env"),
    ...
    _save_zorin_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Zorin .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See zorin-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_zorin_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Zorin peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, zorin_home: str, config: dict) -> None:
        ...
        from zorin_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        zorin_home_path = Path(zorin_home)
        env_path = zorin_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_zorin_openviking_config())
        ...
        self._turn_count = 0
        zorin_home = str(kwargs.get("zorin_home") or "").strip()
        if not zorin_home:
            try:
                from zorin_constants import get_zorin_home
                zorin_home = str(get_zorin_home())
            except Exception:
                zorin_home = str(Path.home() / ".zorin")
        self._zorin_home = zorin_home
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Zorin run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._zorin_home:
            return None
        return Path(self._zorin_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._zorin_home:
            return None
        return Path(self._zorin_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Zorin' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Zorin canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See zorin-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/zorin/.overview.md).
