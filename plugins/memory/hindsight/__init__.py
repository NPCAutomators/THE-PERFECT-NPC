...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: zorin)
...
Or via $ZORIN_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Zorin can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ZORIN_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_zorin_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "zorin": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "zorin"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Zorin config."""
    profile = config.get("profile", "zorin")
    return str(profile or "zorin")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Zorin profile name (from agent_identity)
      {workspace} — Zorin workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``zorin-{user}`` with no user becomes ``zorin``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between ZORIN and the User"
    ...
    def save_config(self, values, zorin_home):
        """Write config to $ZORIN_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(zorin_home) / "hindsight"
    ...
    def post_setup(self, zorin_home: str, config: dict) -> None:
        ...
        from zorin_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "zorin")
        ...
        self.save_config(provider_config, zorin_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(zorin_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(zorin_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "zorin"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: zorin-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between ZORIN and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "zorin"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "zorin"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "zorin", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "zorin")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between ZORIN and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Zorin as a non-root user, or switch "
                    "to cloud / local_external mode via 'zorin memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Zorin get sluggish. (issue #13125)
                try:
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Zorin created/used that client on the shared async loop,
