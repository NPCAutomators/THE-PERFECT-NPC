"""zorin-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $ZORIN_HOME/config.yaml (profile-scoped):
  plugins:
    zorin-memory-store:
      db_path: $ZORIN_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    ...
    try:
        ...
        return cfg_get(all_config, "plugins", "zorin-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, zorin_home):
        """Write config to config.yaml under plugins.zorin-memory-store."""
        from pathlib import Path
        config_path = Path(zorin_home) / "config.yaml"
        try:
            import yaml
            existing = {}
            ...
            existing["plugins"]["zorin-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_zorin_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from zorin_constants import get_zorin_home
        _zorin_home = str(get_zorin_home())
        _default_db = _zorin_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $ZORIN_HOME in user-supplied paths so config values like
        # "$ZORIN_HOME/memory_store.db" or "~/.zorin/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$ZORIN_HOME", _zorin_home)
            db_path = db_path.replace("${ZORIN_HOME}", _zorin_home)
