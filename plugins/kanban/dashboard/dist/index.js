/**
 * Zorin Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.zorin/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ZORIN_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ZORIN_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "zorin-kanban-dot-triage",
    todo: "zorin-kanban-dot-todo",
    ready: "zorin-kanban-dot-ready",
    running: "zorin-kanban-dot-running",
    blocked: "zorin-kanban-dot-blocked",
    done: "zorin-kanban-dot-done",
    archived: "zorin-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-zorin-task";
  ...
  const DOCS_URL = "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "zorin.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="zorin-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])zorin-kanban-md-code\1/i.test(attrs)) {
      return ' class="zorin-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "zorin-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "zorin-kanban-md",
  ...
  // standard `drop` event and our `zorin-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("zorin-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("zorin-kanban-column--drop");
          if (target) target.classList.add("zorin-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("zorin-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("zorin-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("zorin-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ZORIN_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "zorin-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "zorin-kanban-attention",
        "zorin-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "zorin-kanban-attention-bar" },
        h("span", { className: "zorin-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "zorin-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "zorin-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "zorin-kanban-attention-row",
                  "zorin-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "zorin-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "zorin-kanban-attention-row-id" }, task.id),
                h("span", { className: "zorin-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "zorin-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "zorin-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "zorin-kanban-diag-action-btn",
      action.suggested ? "zorin-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " zorin-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".zorin-kanban-drawer-comment-row input, .zorin-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "zorin-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("zorin-kanban-diag", sevClass) },
      h("div", { className: "zorin-kanban-diag-header" },
        h("span", { className: "zorin-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "zorin-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "zorin-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "zorin-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "zorin-kanban-diag-data-row" },
                  h("span", { className: "zorin-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "zorin-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "zorin-kanban-diag-data-row" },
                h("span", { className: "zorin-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "zorin-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "zorin-kanban-diag-reassign-row" },
            h("span", { className: "zorin-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "zorin-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "zorin-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "zorin-kanban-diag-msg",
              msg.ok ? "zorin-kanban-diag-msg--ok" : "zorin-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "zorin-kanban-section" },
      h("div", { className: "zorin-kanban-section-head-row" },
        h("span", { className: "zorin-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "zorin-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "zorin-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "zorin-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "zorin-kanban-docs-link",
      title: "Open Zorin Kanban docs in a new tab",
      "aria-label": "Zorin Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `zorin kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "zorin-kanban-msg-ok" : "zorin-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "zorin-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "zorin-kanban-boardswitcher" },
      h("div", { className: "zorin-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "zorin-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "zorin-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "zorin-kanban-dialog-title" },
        ...
        h("div", { className: "zorin-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Zorin profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "zorin-kanban-bulk" },
      h("span", { className: "zorin-kanban-bulk-count" },
      ...
      h("div", { className: "zorin-kanban-bulk-priority",
      ...
      h("div", { className: "zorin-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Zorin profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "zorin-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zorin-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("zorin-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "zorin-kanban-trash",
        dragOver ? "zorin-kanban-trash--drop" : "",
        props.draggingTaskId ? "zorin-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "zorin-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "zorin-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".zorin-kanban-card")) return true;
      if (target.closest && target.closest(".zorin-kanban-column-add")) return true;
      if (target.closest && target.closest(".zorin-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("zorin-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("zorin-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".zorin-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "zorin-kanban-columns",
        isScrollable ? "zorin-kanban-columns--scrollable" : "",
        isPanning ? "zorin-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zorin-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("zorin-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "zorin-kanban-column",
        dragOver ? "zorin-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "zorin-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "zorin-kanban-col-check",
        ...
        h("span", { className: cn("zorin-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "zorin-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "zorin-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "zorin-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "zorin-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "zorin-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "zorin-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "zorin-kanban-lane" },
                  h("div", { className: "zorin-kanban-lane-head" },
                    h("span", { className: "zorin-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "zorin-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "zorin-kanban-card--stale-red";
    if (age >= tier.amber) return "zorin-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".zorin-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "zorin-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "zorin-kanban-card",
        props.selected ? "zorin-kanban-card--selected" : "",
        props.failed ? "zorin-kanban-card--failed" : "",
        props.draggingSource ? "zorin-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "zorin-kanban-card-content" },
          h("div", { className: "zorin-kanban-card-row" },
            h("label", {
              className: "zorin-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "zorin-kanban-card-check",
            ...
            h("span", { className: "zorin-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or zorin kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "zorin-kanban-warning-badge",
                    "zorin-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "zorin-kanban-needs-assignee",
          ...
          h("div", { className: "zorin-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "zorin-kanban-card-row zorin-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "zorin-kanban-assignee",
                            title: `Assigned to Zorin profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "zorin-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "zorin-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "zorin-kanban-ago",
  ...
  // centered modal reusing the zorin-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "zorin-kanban-dialog zorin-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Zorin profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Zorin profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "zorin-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "zorin-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "zorin-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "zorin-kanban-drawer-close",
        ...
        data ? h("div", { className: "zorin-kanban-drawer-comment-foot" },
          h("div", {
            className: "zorin-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "zorin-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "zorin-kanban-section" },
      h("div", { className: "zorin-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "zorin-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "zorin-kanban-drawer-body" },
      h("div", { className: "zorin-kanban-drawer-title" },
        h("span", { className: cn("zorin-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "zorin-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "zorin-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "zorin-kanban-section" },
            h("div", { className: "zorin-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "zorin-kanban-section" },
            h("div", { className: "zorin-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "zorin-kanban-done-no-result zorin-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "zorin-kanban-section" },
            h("div", { className: "zorin-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "zorin-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "zorin-kanban-section" },
        h("div", { className: "zorin-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "zorin-kanban-comment" },
            h("div", { className: "zorin-kanban-comment-head" },
              h("span", { className: "zorin-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "zorin-kanban-diag-action-btn",
      ...
      h("div", { className: "zorin-kanban-section" },
        h("div", { className: "zorin-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "zorin-kanban-comment" },
                h("div", { className: "zorin-kanban-comment-head" },
                  h("span", { className: "zorin-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "zorin-kanban-comment-ago" },
      ...
      h("div", { className: "zorin-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "zorin-kanban-event",
              isDiag ? "zorin-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "zorin-kanban-event-header" },
                  h("span", { className: "zorin-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "zorin-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "zorin-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "zorin-kanban-event-header-plain" },
                  h("span", { className: "zorin-kanban-event-kind" }, e.kind),
                  h("span", { className: "zorin-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "zorin-kanban-event-phantom-row" },
                  h("span", { className: "zorin-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "zorin-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "zorin-kanban-event-payload" },
    ...
    return h("div", { className: "zorin-kanban-section" },
      h("div", { className: "zorin-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "zorin-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `zorin-kanban-run--${r.outcome || r.status || "ended"}`
          : "zorin-kanban-run--active";
        return h("div", { key: r.id, className: cn("zorin-kanban-run", outcomeClass) },
          h("div", { className: "zorin-kanban-run-head" },
            h("span", { className: "zorin-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "zorin-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "zorin-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "zorin-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "zorin-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "zorin-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "zorin-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "zorin-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "zorin-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "zorin-kanban-pre zorin-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "zorin-kanban-meta-row" },
      h("span", { className: "zorin-kanban-meta-label" }, props.label),
      h("span", { className: "zorin-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "zorin-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zorin-kanban-meta-row" },
        h("span", { className: "zorin-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "zorin-kanban-meta-value zorin-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zorin-kanban-meta-row" },
        h("span", { className: "zorin-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "zorin-kanban-meta-value zorin-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "zorin-kanban-section" },
      h("div", { className: "zorin-kanban-section-head-row" },
        h("span", { className: "zorin-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "zorin-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "zorin-kanban-section" },
      h("div", { className: "zorin-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "zorin-kanban-deps-row" },
        h("span", { className: "zorin-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "zorin-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "zorin-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "zorin-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "zorin-kanban-dep-chip-x",
      ...
      h("div", { className: "zorin-kanban-deps-row" },
        h("span", { className: "zorin-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "zorin-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "zorin-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "zorin-kanban-section" },
      ...
      h("div", { className: "zorin-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "zorin-kanban-home-sub zorin-kanban-home-sub--on"
              : "zorin-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ZORIN_PLUGINS__ && typeof window.__ZORIN_PLUGINS__.register === "function") {
    window.__ZORIN_PLUGINS__.register("kanban", KanbanPage);
  }
})();
