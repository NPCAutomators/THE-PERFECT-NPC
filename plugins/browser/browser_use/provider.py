...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed NpcAutomators tool
gateway (which Zorin uses to bill Browser Use sessions to a NpcAutomators
...
Auth env vars (one of)::
    ...
    # OR a managed NpcAutomators gateway entry (configured via 'zorin setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed NpcAutomators
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed NpcAutomators tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed NpcAutomators gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the NpcAutomators auth stack which can be
        # heavy and is not needed for direct-API-key users.
        from tools.managed_tool_gateway import (
            peek_npcautomators_access_token,
            resolve_managed_tool_gateway,
        )
        from tools.tool_backend_helpers import prefers_gateway

        # Direct API key wins unless the user has explicitly opted into the
        # managed NpcAutomators gateway via ``tool_gateway.browser: gateway``.
        ...
        managed = resolve_managed_tool_gateway(
            "browser-use",
            token_reader=None if refresh_token else peek_npcautomators_access_token,
        )
        if managed is None:
            return None

        return {
            "api_key": managed.npcautomators_user_token,
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Zorin only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"zorin_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
