"""disk_cleanup — ephemeral file cleanup for ZORIN.
...
Rules:
  ...
  - empty dirs    → always delete (under ZORIN_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ZORIN_HOME and /tmp/zorin-*
Never touches: ~/.zorin/logs/ or any system directory.
"""
...
try:
    ...
    def get_zorin_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ZORIN_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".zorin").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ZORIN_HOME/logs/``."""
    return get_zorin_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ZORIN_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ZORIN_HOME or ``/tmp/zorin-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    zorin_home = get_zorin_home()
    try:
        path.resolve().relative_to(zorin_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/zorin-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("zorin-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "zorin-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $ZORIN_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so ZORIN_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        zorin_home = get_zorin_home()
        for parent in ("cron", "cronjobs"):
            base = zorin_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ZORIN_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ZORIN_HOME, but never recurse into known
    # durable state trees.  Some installs place the Zorin checkout, venv,
    # and desktop build under ZORIN_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    zorin_home = get_zorin_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in zorin_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    zorin_home = get_zorin_home()
    try:
        rel = path.resolve().relative_to(zorin_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "zorin-agent",
        }:
            return None
        ...
        # Path isn't under ZORIN_HOME (e.g. /tmp/zorin-*) — fall through.
        pass
