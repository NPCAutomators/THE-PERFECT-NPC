...
Auto-tracks and cleans up ephemeral files created during ZORIN
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ZORIN_HOME` and `/tmp/zorin-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZORIN_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ZORIN_HOME` or `/tmp/zorin-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ZORIN_HOME/disk-cleanup/` is itself excluded
- `$ZORIN_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
