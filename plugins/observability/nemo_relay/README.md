...
Optional Zorin observability plugin that maps Zorin observer hooks to
...
not replace ZORIN's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Zorin emit NeMo Relay lifecycle events for
the work Zorin already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, ZORIN can:

- Preserve Zorin execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
zorin plugins enable observability/nemo_relay
```

The `ZORIN_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ZORIN_HOME` that the
agent run will use:

```bash
env ZORIN_HOME=/tmp/zorin-nemo-relay-test \
  zorin plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ZORIN_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ZORIN_HOME` is the Zorin profile/config home used by both
`zorin plugins enable ...` and the later `zorin chat ...` run. If unset,
Zorin uses the user's default home, usually `~/.zorin`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ZORIN_HOME=/tmp/zorin-nemo-relay-test
zorin plugins enable observability/nemo_relay
zorin chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `zorin` command you run is built from the
...
uv run zorin plugins enable observability/nemo_relay
uv run zorin chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/zorin_agent-*.whl
...
The plugin can configure exporters directly from `ZORIN_NEMO_RELAY_*`
...
```bash
export ZORIN_NEMO_RELAY_ATOF_ENABLED=1
export ZORIN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ZORIN_NEMO_RELAY_ATIF_ENABLED=1
export ZORIN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ZORIN_NEMO_RELAY_ATOF_FILENAME`
- `ZORIN_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `ZORIN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `ZORIN_NEMO_RELAY_ATIF_AGENT_NAME`
- `ZORIN_NEMO_RELAY_ATIF_AGENT_VERSION`
- `ZORIN_NEMO_RELAY_ATIF_MODEL_NAME`
- `ZORIN_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Zorin at it:

```bash
export ZORIN_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `ZORIN_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ZORIN_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`ZORIN_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Zorin does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Zorin keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Zorin routes LLM and tool
...
Zorin feature-detects the dynamic-plugin activation API available in NeMo Relay
0.6 and later. Configure native or worker plugins with Zorin-owned
...
with this Zorin-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Zorin rejects
...
Zorin activates these plugins before registering its managed LLM and tool
...
activation API, Zorin logs an actionable warning and continues with the
...
For the full generic Zorin middleware contract, see
...
export ZORIN_HOME=/tmp/zorin-nemo-relay-docs/zorin-home
mkdir -p "$ZORIN_HOME"

cat > "$ZORIN_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Zorin session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ZORIN_NEMO_RELAY_ATOF_ENABLED=1
export ZORIN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/zorin-nemo-relay-docs/subagent/atof
export ZORIN_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ZORIN_NEMO_RELAY_ATOF_MODE=overwrite
export ZORIN_NEMO_RELAY_ATIF_ENABLED=1
export ZORIN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/zorin-nemo-relay-docs/subagent/atif
export ZORIN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ZORIN_NEMO_RELAY_ATIF_AGENT_NAME='ZORIN E2E'
export ZORIN_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ZORIN_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"zorin.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "ZORIN E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Zorin dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/zorin-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/zorin-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/zorin-nemo-relay-docs/workdir/beta.txt
cd /tmp/zorin-nemo-relay-docs/workdir

export ZORIN_NEMO_RELAY_ATOF_ENABLED=1
export ZORIN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/zorin-nemo-relay-docs/parallel/atof
export ZORIN_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ZORIN_NEMO_RELAY_ATOF_MODE=overwrite
export ZORIN_NEMO_RELAY_ATIF_ENABLED=1
export ZORIN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/zorin-nemo-relay-docs/parallel/atif
export ZORIN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ZORIN_NEMO_RELAY_ATIF_AGENT_NAME='ZORIN E2E'
export ZORIN_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Zorin sessions map to `agent` scopes.
- Zorin API request hooks map to `llm` scope start/end events.
- Zorin tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Zorin execution middleware to hand
...
```bash
export ZORIN_NEMO_RELAY_PLUGINS_TOML=/tmp/zorin-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Zorin routes execution
...
Zorin provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Zorin provider adapter next_call(...)

Zorin tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Zorin tool dispatcher next_call(...)
```
...
middleware for a local Zorin run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by the supported
0.x release range beginning with 0.5.

```bash
export ZORIN_HOME=/tmp/zorin-middleware-test/zorin-home
mkdir -p "$ZORIN_HOME" /tmp/zorin-middleware-test/nemo-relay
...
cat > /tmp/zorin-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/zorin-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/zorin-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Zorin Middleware E2E"
agent_version = "local"
...
export ZORIN_NEMO_RELAY_PLUGINS_TOML=/tmp/zorin-middleware-test/nemo-relay/plugins.toml
