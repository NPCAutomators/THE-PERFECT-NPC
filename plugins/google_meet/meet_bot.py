...
``$ZORIN_HOME/workspace/meetings/<meeting-id>/``. The main zorin process
...
Run standalone for debugging::
    ...
    ZORIN_MEET_URL=https://meet.google.com/abc-defg-hij \\
    ZORIN_MEET_OUT_DIR=/tmp/meet-debug \\
    ZORIN_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``ZORIN_MEET_OUT_DIR``.
...
# and exposes ``window.__zorinMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__zorinMeetInstalled) return;
  window.__zorinMeetInstalled = true;
  window.__zorinMeetQueue = [];
  ...
  window.__zorinMeetDrain = () => {
    const out = window.__zorinMeetQueue.slice();
    window.__zorinMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "zorin_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("ZORIN_MEET_URL", "").strip()
    out_dir_env = os.environ.get("ZORIN_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("ZORIN_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("ZORIN_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("ZORIN_MEET_GUEST_NAME", "ZORIN")
    duration_s = _parse_duration(os.environ.get("ZORIN_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when ZORIN_MEET_MODE=realtime.
    mode = os.environ.get("ZORIN_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("ZORIN_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("ZORIN_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("ZORIN_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("ZORIN_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — ZORIN_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: ZORIN_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in ZORIN_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("ZORIN_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__zorinMeetDrain && window.__zorinMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__zorinMeetInstalled) {
...
def _looks_like_human_speaker(speaker: str, bot_guest_name: str) -> bool:
    ...
    bot_name = bot_guest_name.strip().lower()
    bot_aliases = {bot_name, f"{bot_name} agent"}
    if spk in {"unknown", "you", *bot_aliases}:
        return False
    return True
