"""`zorin meet node ...` subcommand tree.

Wired into the existing ``zorin meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``zorin meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="zorin-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             zorin meet node approve <name> ws://<host>:{args.port} {token}")
        try:
