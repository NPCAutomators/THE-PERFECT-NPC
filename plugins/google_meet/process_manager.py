...
session-scoped state file under ``$ZORIN_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ZORIN_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_zorin_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this zorin install, leave it first —
    ...
    env = os.environ.copy()
    env["ZORIN_MEET_URL"] = url
    env["ZORIN_MEET_OUT_DIR"] = str(out)
    env["ZORIN_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ZORIN_MEET_HEADED"] = "1"
    if auth_state:
        env["ZORIN_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ZORIN_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ZORIN_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ZORIN_MEET_MODE"] = mode
    if realtime_model:
        env["ZORIN_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ZORIN_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ZORIN_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ZORIN_MEET_REALTIME_KEY"] = realtime_api_key
