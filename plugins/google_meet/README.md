...
Let the zorin agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where zorin runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `zorin meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `zorin meet` CLI |
...
| `cli.py` | `zorin meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ZORIN_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `zorin meet node {run,list,approve,remove,status,ping}` |
...
zorin meet install                                      # pip + Chromium
zorin meet setup                                        # preflight
zorin meet auth                                         # optional
zorin meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
zorin meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.zorin/.env
zorin meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
zorin meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
zorin meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.zorin/.env
zorin meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, zorin will **not** switch your system audio input automatically — the
...
zorin plugins enable google_meet
zorin meet node run --display-name my-mac --host 0.0.0.0 --port 18789
