...
A standards-compliant OpenID Connect Relying Party for the ``zorin dashboard``
OAuth gate. Unlike the bundled ``npcautomators`` provider (which encodes NPCAUTOMATORS Portal's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-npcautomators-refresh-token`` header, an ``oauth_contract_version`` claim),
...
:class:`~zorin_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``zorin_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~zorin_cli.dashboard_auth.Session`.
...
only choice that is universally correct across self-hosted IDPs. (The ``npcautomators``
provider verifies its *access* token because NPCAUTOMATORS Portal mints a custom JWT
...
same precedence convention as the ``npcautomators`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.zorin/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    ZORIN_DASHBOARD_OIDC_ISSUER
    ZORIN_DASHBOARD_OIDC_CLIENT_ID
    ZORIN_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    ZORIN_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the npcautomators
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the npcautomators plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Zorin does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        redirect_uris are permitted. Mirrors the npcautomators provider.
        """
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``ZORIN_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "ZORIN_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "ZORIN_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("ZORIN_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.zorin/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "ZORIN_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(ZORIN_DASHBOARD_OIDC_ISSUER + ZORIN_DASHBOARD_OIDC_CLIENT_ID) "
