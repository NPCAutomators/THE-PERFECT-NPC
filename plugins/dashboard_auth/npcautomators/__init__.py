"""NpcAutomatorsDashboardAuthProvider — NPCAUTOMATORS Portal OAuth (authorization-code + PKCE).

Implements ``npcautomators-account-service/docs/agent-dashboard-oauth-contract.md``
...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      ZORIN_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      ZORIN_DASHBOARD_PORTAL_URL       — defaults to
                                          ``https://portal.npcautomators.com``
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``zorin-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``zorin_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set ZORIN_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via ZORIN_DASHBOARD_PORTAL_URL for
# staging (portal.rewbs.uk) or a custom deployment. Contract docs name
# this as the production issuer.
_DEFAULT_PORTAL_URL = "https://portal.npcautomators.com"
...
class NpcAutomatorsDashboardAuthProvider(DashboardAuthProvider):
    """NPCAUTOMATORS Portal OAuth via authorization-code + PKCE (S256)."""
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"zorin_session_pkce\"]``
        ...
        cookie_payload = {
            "zorin_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. NPCAUTOMATORS Portal
    ...
    def refresh_session(self, *, refresh_token: str) -> Session:
        ...
        try:
            response = httpx.post(
                ...
                # ``x-npcautomators-refresh-token`` header. Portal's token endpoint
                ...
                # Portal's ``REFRESH_TOKEN_HEADER`` exactly (``x-npcautomators-refresh-
                ...
                headers={
                    ...
                    "x-npcautomators-refresh-token": refresh_token,
                },
                timeout=_TOKEN_ENDPOINT_TIMEOUT_SEC,
            )
        except httpx.RequestError as exc:
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between ZORIN_DASHBOARD_PORTAL_URL / ZORIN_DASHBOARD_OAUTH_CLIENT_ID
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "NPCAUTOMATORS Portal token missing oauth_contract_version claim "
...
def _load_config_oauth_section() -> dict:
    ...
    try:
        ...
        logger.debug(
            "dashboard-auth-npcautomators: load_config() raised %s; "
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``ZORIN_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("ZORIN_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``ZORIN_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("ZORIN_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    Registers ``NpcAutomatorsDashboardAuthProvider`` only when a client_id is
    configured (either via ``ZORIN_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set ZORIN_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``zorin_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "ZORIN_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "NPCAUTOMATORS Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a ZORIN "
        ...
        logger.debug("dashboard-auth-npcautomators: %s", LAST_SKIP_REASON)
        return

    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"ZORIN_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The NPCAUTOMATORS Portal "
            f"provisions this value at deploy time; check your Fly app's "
            f"secrets or override with the value from the Portal admin UI."
        )
        logger.warning("dashboard-auth-npcautomators: %s", LAST_SKIP_REASON)
        return

    try:
        provider = NpcAutomatorsDashboardAuthProvider(
            client_id=client_id, portal_url=portal_url
        )
    except ValueError as exc:
        LAST_SKIP_REASON = f"NpcAutomatorsDashboardAuthProvider construction failed: {exc}"
        logger.warning("dashboard-auth-npcautomators: %s", LAST_SKIP_REASON)
        return

    ctx.register_dashboard_auth_provider(provider)
    logger.info(
        "dashboard-auth-npcautomators: registered provider (client_id=%s, portal=%s)",
        client_id,
        portal_url,
    )
