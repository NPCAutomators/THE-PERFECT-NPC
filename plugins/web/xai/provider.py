...
``{title, url, description, position}`` rows every other Zorin web
provider produces.
...
prefers Zorin-managed xAI Grok OAuth (via ``zorin auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.zorin/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``zorin tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `zorin auth` to sign in with "
