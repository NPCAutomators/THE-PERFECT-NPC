...
# Must match zorin_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``zorin-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``zorin-cli`` because those also contain
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on NPCAUTOMATORS Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. zorin-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (zorin-cli,
    # zorin-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # Extract parent's API key so subagents inherit auth (e.g. NPCAUTOMATORS Portal).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.zorin/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_zorin_dir("cache/delegation", "delegation_cache")
...
def delegate_task(
    ...
    if background:
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `zorin -z` or a cron job, or a "
        ...
        try:
            ...
            _source = get_session_env("ZORIN_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("ZORIN_UI_SESSION_ID", "")
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and ZORIN_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, npcautomators, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'zorin auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Zorin config.

    Prefer the shared persistent loader because it follows the active
    ZORIN_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``ZORIN_IGNORE_USER_CONFIG=1`` (``zorin chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("ZORIN_IGNORE_USER_CONFIG") == "1"
