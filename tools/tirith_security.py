...
it is automatically downloaded from GitHub releases to $ZORIN_HOME/bin/tirith.
...
def _get_zorin_home() -> str:
    """Return the Zorin home directory, respecting ZORIN_HOME env var."""
    return str(get_zorin_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_zorin_home(), ".tirith-install-failed")
...
def _zorin_bin_dir() -> str:
    """Return $ZORIN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_zorin_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ZORIN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_zorin_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ZORIN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ZORIN_HOME/bin/tirith
    ...
    zorin_bin = os.path.join(_zorin_bin_dir(), "tirith")
    if os.path.isfile(zorin_bin) and os.access(zorin_bin, os.X_OK):
        _resolved_path = zorin_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return zorin_bin
