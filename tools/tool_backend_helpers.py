...
def managed_npcautomators_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the NpcAutomators Tool Gateway.

    Entitlement is paid NPCAUTOMATORS Portal service access OR a live free tool pool
...
def npcautomators_tool_gateway_unavailable_message(
    capability: str = "the NpcAutomators Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable NpcAutomators Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `zorin model` to refresh your "
        "NPCAUTOMATORS Portal login and billing status."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_npcautomators_tools_enabled()
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.zorin/.env`` (via
    ``zorin_cli.config.get_env_value`` when available) so tool-side
