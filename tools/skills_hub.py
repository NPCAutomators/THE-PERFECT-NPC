#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Zorin Skills Hub.
...
Used by zorin_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _zorin_home() -> Path:
    return get_zorin_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _zorin_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            zorin_meta = metadata.get("zorin", {})
            if isinstance(zorin_meta, dict):
                tags = zorin_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by NPCAUTOMATORS) but not activated
    ...
    OFFICIAL_REPO = "NPCAutomators/AGENT-ZORIN"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                zorin_meta = meta_block.get("zorin", {})
                if isinstance(zorin_meta, dict):
                    tags = zorin_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Zorin centralized index source
# ---------------------------------------------------------------------------

ZORIN_INDEX_URL = "https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/api/skills-index.json"
ZORIN_INDEX_TTL = 6 * 3600  # 6 hours


def _zorin_index_cache_file() -> Path:
    return _index_cache_dir() / "zorin-index.json"


def _load_zorin_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ZORIN_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    zorin_index_cache_file = _zorin_index_cache_file()
    if zorin_index_cache_file.exists():
        try:
            age = time.time() - zorin_index_cache_file.stat().st_mtime
            if age < ZORIN_INDEX_TTL:
                return json.loads(zorin_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Zorin index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Zorin index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Zorin index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        zorin_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        zorin_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class ZorinIndexSource(SkillSource):
    """Skill source backed by the centralized Zorin Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "zorin-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "zorin-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ZorinIndexSource(auth=auth), # Centralized index (search + resolved install paths)
