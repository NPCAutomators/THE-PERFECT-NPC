...
class ManagedModalEnvironment(BaseModalExecutionEnvironment):
    """Gateway-owned Modal sandbox with Zorin-compatible execute/cleanup."""
    ...
    def __init__(
        ...
        if gateway is None:
            raise ValueError("Managed Modal requires a configured tool gateway and NpcAutomators user token")

        self._gateway_origin = gateway.gateway_origin.rstrip("/")
        self._npcautomators_user_token = gateway.npcautomators_user_token
    ...
    def _request(self, method: str, path: str, *,
        ...
        headers = {
            "Authorization": f"Bearer {self._npcautomators_user_token}",
