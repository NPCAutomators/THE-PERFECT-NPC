...
Backend is selected during ``zorin tools`` setup (web.backend in config.yaml).
When available, Zorin can route Firecrawl calls through a NpcAutomators-hosted tool-gateway
for NpcAutomators Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for NpcAutomators Subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    peek_npcautomators_access_token as _peek_npcautomators_access_token,
    read_npcautomators_access_token as _read_npcautomators_access_token,
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Zorin config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Zorin' config/.env layer (``zorin config set``, ``zorin tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.zorin/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``zorin tools``).
    ...
    # pre-empted by a NpcAutomators OAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `zorin tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_npcautomators_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the NpcAutomators portal on every CLI startup (invoked at
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `zorin plugins enable {disabled_key}` "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`zorin plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from zorin_cli.config import get_env_value as _gev
