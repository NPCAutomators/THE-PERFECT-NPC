...
One ``CDPSupervisor`` runs per Zorin ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Zorin logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "zorin-dialog-bridge.invalid"
...
(() => {
  if (window.__zorinDialogBridgeInstalled) return;
  window.__zorinDialogBridgeInstalled = true;
  const ENDPOINT = "http://zorin-dialog-bridge.invalid/";
