...
provider** — turn on a backend by enabling a plugin (``zorin plugins
enable video_gen/<name>``) and selecting it in ``zorin tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/zorin-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `zorin tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `zorin tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `zorin tools` → "
...
# `zorin tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`zorin tools` → Video Generation; the agent does not pick them. "
