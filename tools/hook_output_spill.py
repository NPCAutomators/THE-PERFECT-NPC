...
* Never raises. Any I/O error (disk full, permission denied, missing
  ZORIN_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_zorin_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: ZORIN_HOME env var, then ~/.zorin
            home = os.environ.get("ZORIN_HOME") or os.path.expanduser("~/.zorin")
