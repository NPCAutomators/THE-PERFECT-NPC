...
`zorin skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.zorin.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.zorin.blueprint`` is a mapping containing
    ...
    zorin = meta.get("zorin") if isinstance(meta, dict) else None
    blueprint = zorin.get("blueprint") if isinstance(zorin, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.zorin.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.zorin.blueprint`` block) they can hand
    to ``zorin skills publish`` to share. ``body`` is the plain-language
