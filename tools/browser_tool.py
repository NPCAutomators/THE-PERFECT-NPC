...
supports multiple backends — **Browser Use** (cloud, default for NpcAutomators
...
from zorin_constants import agent_browser_runnable, get_zorin_home
from utils import env_int, is_truthy_value
from zorin_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Zorin secret
...
def _build_browser_env() -> dict:
    ...
    Strips Zorin-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``zorin_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = zorin_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    zorin_node_bin = str(zorin_home / "node" / "bin")
    zorin_node_root = str(zorin_home / "node")
    zorin_nm_bin = str(zorin_home / "node_modules" / ".bin")
    return [zorin_node_bin, zorin_node_root, zorin_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.zorin/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed NpcAutomators gateway or
    ...
    browser plugins (``~/.zorin/plugins/browser/<vendor>/``) participate
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (managed NpcAutomators gateway or
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Zorin silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Zorin CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-zorin_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed zorin processes — this way every clean zorin exit sweeps
    ...
    # Sweep orphans from other crashed zorin processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live zorin processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current zorin PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live zorin process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning zorin process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced zorin PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent zorin instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-zorin_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live zorin process.
            continue
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Zorin-managed
    ...
    # node_modules binary, which disappears on the next ``zorin update`` and
    ...
    # Build an extended search PATH including Zorin-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_zorin_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_zorin_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_zorin_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this zorin PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_zorin_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = zorin_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Zorin
    ...
    from zorin_constants import get_zorin_dir
    screenshots_dir = get_zorin_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/npcautomators/agent-zorin:latest")
                else:
