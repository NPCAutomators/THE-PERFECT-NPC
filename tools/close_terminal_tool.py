#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Zorin desktop GUI.
...
``read_terminal`` it is gated on ``ZORIN_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — ZORIN_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("ZORIN_DESKTOP")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Zorin desktop GUI (the tabs mirroring terminal(background=true) runs). "
