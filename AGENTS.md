# ZORIN - Development Guide

Instructions for AI coding assistants and developers working on the zorin-agent codebase.

**Never give up on the right solution.**

## What Zorin Is

Zorin is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Zorin ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`zorin tools`, `zorin setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `ZORIN_HOME`. Mocks hide integration bugs.
...
- **New `ZORIN_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `zorin tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.zorin/plugins/` (or via a
  pip entry point), and promote them in the NPCAUTOMATORS Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `zorin <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `zorin webhook`, `zorin cron`, `zorin tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.zorin/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.zorin/zorin-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _ZORIN_CORE_TOOLS list
├── cli.py                # ZorinCLI class — interactive CLI orchestrator (~11k LOC)
├── zorin_state.py       # SessionDB — SQLite session store (FTS5 search)
├── zorin_constants.py   # get_zorin_home(), display_zorin_home() — profile-aware paths
├── zorin_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── zorin_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── zorin-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `zorin --tui`
...
**User config:** `~/.zorin/config.yaml` (settings), `~/.zorin/.env` (API keys only).
**Logs:** `~/.zorin/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_zorin_home()`.
Browse with `zorin logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Zorin: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`zorin_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `ZorinCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.zorin/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`zorin_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/zorin` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `zorin_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `ZorinCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `zorin --tui` or `ZORIN_TUI=1`.
...
npm run dev       # watch mode (rebuilds zorin-ink + tsx --watch)
npm start         # production
npm run build     # full build (zorin-ink + tsc)
...
### Dashboard React Chat (`zorin dashboard` → `/chat`)

The dashboard chat is a web-native React surface, not an embedded terminal. `web/src/pages/ChatPage.tsx` owns the browser transcript, composer, session rail, model controls, attachments, prompts, and responsive layout. It talks to the same Python `tui_gateway` dispatcher as the CLI TUI, but over the authenticated `@app.websocket("/api/ws")` endpoint in `zorin_cli/web_server.py` instead of stdio.

```
zorin dashboard
  ├─ React (`web/`) ──WebSocket JSON-RPC (`/api/ws`)──┐
  └─ Python dashboard server ──────────────────────────┴─ tui_gateway
                                                        └─ AIAgent + tools + sessions
```

- `web/src/lib/gatewayClient.ts` extends the framework-agnostic `JsonRpcGatewayClient` from `apps/shared`. Browser WebSocket auth uses a short-lived ticket returned by the dashboard auth API because browsers cannot set an `Authorization` header during the upgrade.
- Session lifecycle uses `session.create`, `session.list`, `session.resume`, `session.activate`, `session.history`, `session.close`, and `session.interrupt`. Hydrate a resumed transcript from the `session.resume` response (or `session.history` when refreshing it), then accept only events for the active `session_id`; events from a prior session must never leak into the current transcript.
- A normal turn uses `prompt.submit`. Render `message.start`, `message.delta`, `message.interim`, and `message.complete`; render reasoning/activity from `thinking.delta`, `reasoning.delta`, `reasoning.available`, and `status.update`; render tools from `tool.start`, `tool.progress`, `tool.generating`, and `tool.complete`. `session.info` updates model/session metadata. Treat `error` as recoverable UI state rather than destroying the transcript.
- Interactive requests arrive as `clarify.request`, `approval.request`, `sudo.request`, and `secret.request`; answer with the matching `clarify.respond`, `approval.respond`, `sudo.respond`, or `secret.respond` method. Do not silently reduce these to ordinary chat text.
- Browser attachments use `image.attach_bytes` (or `image.attach` for a server-visible path), `file.attach`, and `image.detach`. Keep attachment state scoped to the session and make detach/retry failures visible.
- Slash discovery uses `commands.catalog` and `complete.slash`. Execution follows `web/src/lib/slashExec.ts`: try `slash.exec`, then `command.dispatch`; submit `send` and skill directives as normal prompts. Reuse `web/src/components/SlashPopover.tsx` rather than maintaining a second command list.
- Reuse `web/src/components/Markdown.tsx` for assistant content and the transfer/file-validation helpers in `web/src/lib/chatImagePaste.ts` for pasted/dropped images; send the bytes through `image.attach_bytes` rather than synthesizing a terminal command. The complete JSON-RPC method/event catalog remains authoritative in `tui_gateway/server.py`.

**The CLI TUI and dashboard React chat are distinct renderers.** `zorin --tui` remains the Ink application described above and continues to use newline-delimited JSON-RPC over stdio. A UI-only change in Ink does not appear automatically in the dashboard, and a React layout change does not affect the CLI. Put shared behavior in the gateway contract or framework-agnostic helpers, then implement renderer-specific presentation on each surface.

**Do not mount xterm.js or connect `/chat` to `/api/pty`.** The PTY bridge may remain for backward compatibility and terminal-specific consumers, but it is not the dashboard chat transport. The React surface must remain usable on platforms without a POSIX PTY, preserve gateway state across responsive layout changes, expose connection/retry/interrupt state non-destructively, and keep secondary dashboard navigation behind the chat surface's designated controls.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's React chat. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@zorin/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `zorin serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `ZORIN_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `zorin` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `zorin --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Zorin built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Zorin core.
Use the plugin route instead: create `~/.zorin/plugins/<name>/plugin.yaml`
and `~/.zorin/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Zorin tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_ZORIN_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_ZORIN_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_zorin_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `ZORIN_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_zorin_home()` for the base directory — never `Path.home() / ".zorin"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `zorin_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `zorin_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `zorin tools`, `zorin setup`, most CLI subcommands | `zorin_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`zorin_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
zorin_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.zorin/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Add to `_BUILTIN_SKINS` dict in `zorin_cli/skin_engine.py`:
...
Users create `~/.zorin/skins/<name>.yaml`:
...
Zorin has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.zorin/plugins/` and pip-installed entry points.

### General plugins (`zorin_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.zorin/plugins/`, `./.zorin/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `zorin` at startup so
  `zorin <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(zorin_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `zorin <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `zorin --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `zorin_cli/main.py`, etc.).
...
into `~/.zorin/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `zorin memory setup` / `post_setup()` without
...
`~/.zorin/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the NPCAUTOMATORS Discord
...
2. User: `$ZORIN_HOME/plugins/model-providers/<name>/`
...
[`zorin-example-plugins`](https://github.com/NPCAutomators/zorin-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `zorin skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.zorin.tags`, `metadata.zorin.category`,
`metadata.zorin.related_skills`, `metadata.zorin.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.zorin.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Zorin tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "ZORIN" is the secondary collaborator. If the
   contributor's commit shows "ZORIN" as author (because they
   used Zorin to draft the skill), replace it with their actual name
...
lives in the `zorin-agent-dev` skill at
...
`"messaging"`); `_ZORIN_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `zorin tools` (the curses UI) or the
...
Zorin returns a delegation id immediately and the result re-enters the
...
go to `~/.zorin/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `zorin_cli/curator.py` wires `zorin curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.zorin/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `zorin cron <verb>`
...
- File lock at `~/.zorin/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `zorin kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `zorin_cli/kanban.py` wires `zorin kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`zorin-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `ZORIN_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Zorin-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `ZORIN_BACKGROUND_NOTIFICATIONS` env var):
...
Zorin supports **profiles** — multiple fully isolated instances, each with its own
`ZORIN_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `zorin_cli/main.py` sets
`ZORIN_HOME` before any module imports. All `get_zorin_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_zorin_home()` for all ZORIN_HOME paths.** Import from `zorin_constants`.
   NEVER hardcode `~/.zorin` or `Path.home() / ".zorin"` in code that reads/writes state.
   ```python
   # GOOD
   from zorin_constants import get_zorin_home
   config_path = get_zorin_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".zorin" / "config.yaml"
   ```

2. **Use `display_zorin_home()` for user-facing messages.** Import from `zorin_constants`.
   This returns `~/.zorin` for default or `~/.zorin/profiles/<name>` for profiles.
   ```python
   # GOOD
   from zorin_constants import display_zorin_home
   print(f"Config saved to {display_zorin_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.zorin/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_zorin_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_zorin_home()`,
   not `Path.home() / ".zorin"`.

4. **Tests that mock `Path.home()` must also set `ZORIN_HOME`** — since code now uses
   `get_zorin_home()` (reads env var), not `Path.home() / ".zorin"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"ZORIN_HOME": str(tmp_path / ".zorin")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not ZORIN_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".zorin" / "profiles"`, NOT `get_zorin_home() / "profiles"`.
   This is intentional — it lets `zorin -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.zorin` paths
Use `get_zorin_home()` from `zorin_constants` for code paths. Use `display_zorin_home()`
for user-facing print/log messages. Hardcoding `~/.zorin` breaks profiles — each profile
has its own `ZORIN_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `zorin_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `zorin_cli/curses_ui.py` — see
`zorin_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `ZORIN_HOME`.

### Tests must not write to `~/.zorin/`
The `_isolate_zorin_home` autouse fixture in `tests/conftest.py` redirects `ZORIN_HOME` to a temp dir. Never hardcode `~/.zorin/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_zorin_home()` resolve within the temp dir.
Use the pattern from `tests/zorin_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZORIN_HOME", str(home))
    return home
```
...
subprocess (`--file-retries`, default 1; `ZORIN_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.zorin/` | Your real config+auth.json                  | Temp dir per test                         |
