# ZORIN Has Had "Routines" Since March
...
Zorin equivalent — works today:
```bash
zorin webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Zorin implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | ZORIN |
|---|---|---|
...
## Things Zorin Does That Routines Can't
...
Zorin has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
ZORIN is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
zorin gateway setup    # enable webhooks
zorin webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [zorin-agent.npcautomators.com/docs/reference/automation-blueprints-catalog](https://github.com/NPCAutomators/AGENT-ZORIN/tree/main/docs/reference/automation-blueprints-catalog)

Documentation: [zorin-agent.npcautomators.com](https://github.com/NPCAutomators/AGENT-ZORIN)

GitHub: [github.com/NPCAutomators/AGENT-ZORIN](https://github.com/NPCAutomators/AGENT-ZORIN)

---

*ZORIN is built by [NPCAUTOMATORS](https://npcautomators.com). Open source, model-agnostic, runs on your infrastructure.*
