#!/usr/bin/env python3
"""Live test harness for ZORIN's Tool Search feature.
...
# Force-isolate the test environment BEFORE any zorin imports.
ORIGINAL_HOME = os.environ.get("ZORIN_HOME")
ORIGINAL_AUTH = Path.home() / ".zorin" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.zorin/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.zorin/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="zorin_ts_live_"))
    zorin_home = home_dir / ".zorin"
    zorin_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, zorin_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".zorin" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, zorin_home / ".env")
        ...
        load_zorin_dotenv(zorin_home=str(Path.home() / ".zorin"))
    ...
    (zorin_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return zorin_home
...
def reset_module_state():
    """Drop cached modules so the new ZORIN_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "zorin_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["ZORIN_HOME"] = str(home)
...
def main():
    ...
    # Restore original ZORIN_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ZORIN_HOME"] = ORIGINAL_HOME
    else:
