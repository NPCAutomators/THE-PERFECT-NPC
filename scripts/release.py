...
VERSION_FILE = REPO_ROOT / "zorin_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@zorin.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "75556242+webtecnica@users.noreply.github.com": "webtecnica",  # PR #63360 salvage (npcautomators: restore inference-api base_url)
    ...
    "kavi@local.zorin": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sonxi@npcautomators.local": "17324393074",  # PR #53196 salvage (tools_config: known_plugin_toolsets null guard; commit under unlinked local identity)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level ZORIN_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (zorin journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "zorin.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.zorin/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (NPCAUTOMATORS Portal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip NpcAutomators fallback when no local token, sync gateway session model after fallback)
    ...
    "moonsong@npcautomators.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows zorin-shim resolution + prefer --update on recovery; #52378)
    ...
    "annguyenNpcAutomators@users.noreply.github.com": "annguyenNpcAutomators",
    "32201324+simpolism@users.noreply.github.com": "simpolism",
    "simpolism@gmail.com": "simpolism",
    "jake@npcautomators.com": "simpolism",
    ...
    "annguyenNpcAutomators@users.noreply.github.com": "annguyenNpcAutomators",
    "285874597+annguyenNpcAutomators@users.noreply.github.com": "annguyenNpcAutomators",
    "perkintahmaz50@gmail.com": "devatnull",
    ...
    "teknium@npcautomators.com": "teknium1",
    ...
    "274096618+zorin-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "mind-dragon@npcautomators.research": "Mind-Dragon",
    ...
    "shannon@npcautomators.com": "shannonsands",
    ...
    "zorin@npcautomators.com": "NPCAutomators",
    ...
    "ben@npcautomators.com": "benbarclay",
    ...
    "emozilla@npcautomators.com": "emozilla",
    ...
    "jonny@npcautomators.com": "yoniebans",
    "jake@npcautomators.com": "simpolism",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on zorin mcp add)
    ...
    "zorin@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    # v0.16.0 additions
    "teknium@npcautomators.dev": "teknium1",
    ...
    "github@nadyazorin.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ZORIN_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ZORIN_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: zorin-eval community link)
    ...
    "zorinagent26@gmail.com": "zorinagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+zorinagent26@users.noreply.github.com": "zorinagent26",
    ...
    "clement@npcautomators.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@zorin": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "ethie@npcautomators": "ethernet8023",  # PR #29342 (TUI clipboard copy on linux/wayland)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Zorin runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ZORIN_HOME state files)
    ...
    "annguyen@npcautomators.com": "annguyenNpcAutomators",  # PR #52844 co-author
    ...
    "zhchl@zorin-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Zorin
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"zorin-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "zorin@npcautomators.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NPCAutomators/AGENT-ZORIN",
    ...
    lines.append(f"# ZORIN v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for ZORIN. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="ZORIN Release Tool")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"ZORIN v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"ZORIN v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'ZORIN v{new_version} ({calver_date})' "
