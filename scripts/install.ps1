...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$ZorinHome = $(if ($env:ZORIN_HOME) { $env:ZORIN_HOME } else { "$env:LOCALAPPDATA\zorin" }),
    [string]$InstallDir = $(if ($env:ZORIN_HOME) { "$env:ZORIN_HOME\zorin-agent" } else { "$env:LOCALAPPDATA\zorin\zorin-agent" }),
    ...
    # builds apps/desktop into a launchable Zorin.exe.
    #
    # Why opt-in:
    #   * Zorin-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Zorin.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Zorin.exe
    ...
    #     `zorin desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:NPCAutomators/THE-PERFECT-NPC.git"
$RepoUrlHttps = "https://github.com/NPCAutomators/THE-PERFECT-NPC.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * ZORIN Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by NPCAUTOMATORS.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ZorinHome)) {
        New-Item -ItemType Directory -Force -Path $ZorinHome | Out-Null
    }
    $envFile = Join-Path $ZorinHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ZorinHome "node"
...
function Install-Uv {
    # Zorin owns its own uv at $ZorinHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (zorin_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `zorin update` stay in sync.
    $managedUv = Join-Path $ZorinHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ZorinHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ZorinHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ZorinHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ZorinHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and ZORIN_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Zorin-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Zorin setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Zorin uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\zorin\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Zorin needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Zorin copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\zorin\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ZORIN_GIT_BASH_PATH`` (User scope) so Zorin can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Zorin-managed PortableGit install instead..."
    }

    # Download PortableGit into $ZorinHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ZorinHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Zorin features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Zorin needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ZORIN_GIT_BASH_PATH`` (User env scope) so Zorin can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ZorinHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ZorinHome\git\usr\bin\bash.exe
    $candidates += "$ZorinHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ZorinHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ZORIN_GIT_BASH_PATH", $candidate, "User")
            $env:ZORIN_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ZORIN_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Zorin may not find Git Bash."
    Write-Info "If needed, set ZORIN_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$ZorinHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ZorinHome\node;$env:Path"
        Write-Success "Node.js $version found (Zorin-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Zorin-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ZorinHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ZorinHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\zorin-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ZorinHome\node") { Remove-Item -Recurse -Force "$ZorinHome\node" }
                Move-Item $extractedDir.FullName "$ZorinHome\node"
                ...
                $version = & "$ZorinHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ZorinHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\zorin-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `zorin
                    # update` path (#4735).
                    ...
                    $stashName = "zorin-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed -- mirror ``zorin update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Zorin behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running zorin processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NPCAutomators/THE-PERFECT-NPC/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NPCAutomators/THE-PERFECT-NPC/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NPCAutomators/THE-PERFECT-NPC/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\zorin-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\zorin-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `zorin update` (see the notes at the shared clone-path
    ...
    # next `zorin update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Zorin-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running zorin process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running zorin processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Zorin_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (zorin.exe) plus its child tree.
            & taskkill /F /T /IM zorin.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM zorin.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m zorin_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not zorin.exe.
            ...
            # user (see zorin_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `zorin update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `zorin.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install zorin-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and zorin.exe broken with
    ...
    # here, not 30 seconds later when the user runs `zorin`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register zorin.exe in dist-info/RECORD but fail to
        ...
        # `zorin` command while zorin-agent.exe / zorin-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m zorin_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `zorin dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\zorin_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `zorin dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `zorin dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: zorin_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up zorin command..."
    
    if ($NoVenv) {
        $zorinBin = "$InstallDir"
    } else {
        $zorinBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so zorin is globally available
    # On Windows, the zorin.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$zorinBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$zorinBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $zorinBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ZORIN_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\zorin instead
    # of the Unix default ~/.zorin
    $currentZorinHome = [Environment]::GetEnvironmentVariable("ZORIN_HOME", "User")
    if (-not $currentZorinHome -or $currentZorinHome -ne $ZorinHome) {
        [Environment]::SetEnvironmentVariable("ZORIN_HOME", $ZorinHome, "User")
        Write-Success "Set ZORIN_HOME=$ZorinHome"
    }
    $env:ZORIN_HOME = $ZorinHome
    
    # Update current session
    $env:Path = "$zorinBin;$env:Path"
    
    Write-Success "zorin command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.zorin-bootstrap-complete which tells the Zorin
    ...
    $markerPath = Join-Path $InstallDir ".zorin-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ZORIN_HOME directory structure ($ZorinHome, default %LOCALAPPDATA%\zorin)
    New-Item -ItemType Directory -Force -Path "$ZorinHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZorinHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZorinHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZorinHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZorinHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZorinHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZorinHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZorinHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZorinHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ZorinHome\.env"
    ...
    # Create config.yaml
    $configPath = "$ZorinHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Zorin's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$ZorinHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in zorin_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $ZorinHome"
    
    # Seed bundled skills into $ZorinHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $ZorinHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $ZorinHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ZorinHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $ZorinHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Zorin-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'zorin setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\zorin-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\zorin-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\zorin-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Zorin` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Zorin.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_zorin_desktop command
    # resolves apps/desktop/release/win-unpacked/Zorin.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Zorin-managed
    ...
    # The --dir mode produces an unpacked Zorin.exe in
    ...
    # are NOT trying to work around). The Zorin icon + product name are
    # stamped onto Zorin.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\zorin-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Zorin.exe",
        "$desktopDir\release\win-arm64-unpacked\Zorin.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Zorin.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Zorin icon + identity are stamped onto Zorin.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, zorin-agent#38216).
    ...
    #    Zorin.exe. We deliberately do NOT point them at `zorin desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Zorin.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Zorin.lnk')
        )
        ...
        # with the Zorin icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.zorin/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'zorin setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m zorin_cli.main setup
    } else {
        python -m zorin_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $zorinCmd = "$InstallDir\venv\Scripts\zorin.exe"
    if (-not (Test-Path $zorinCmd)) {
        $zorinCmd = "zorin"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ZorinHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'zorin whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: zorin gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ZorinHome\logs\gateway.log"
            Start-Process -FilePath $zorinCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ZorinHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: zorin gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: zorin gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ZorinHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZorinHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZorinHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZorinHome\zorin-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   zorin              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   zorin setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   zorin config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   zorin config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   zorin gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   zorin update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Zorin repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Zorin-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Zorin to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NPCAutomators/THE-PERFECT-NPC/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
