#!/usr/bin/env bash
# Canonical test runner for zorin-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.zorin/...)
#
# Usage:
...
# (ZORIN_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.zorin/zorin-agent/venv"; do
...
elif [ -n "${ZORIN_PYTHON:-}" ] && [ -x "$ZORIN_PYTHON" ] \
    && "$ZORIN_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: ZORIN_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `zorin` binary rather
  # than the devShell hook.
  PYTHON="$ZORIN_PYTHON"
  echo "▶ no local venv — using Nix dev venv via ZORIN_PYTHON: $PYTHON"
else
  echo "error: no virtualenv found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and ZORIN_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  exit 1
fi
...
if [ -f "$HOME/.zorin/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.zorin"
...
exec env -i \
  ...
  LC_ALL=C.UTF-8 \
  PYTHONHASHSEED=0 \
  ${ZORIN_RUN_SLOW_PET_TESTS:+ZORIN_RUN_SLOW_PET_TESTS="$ZORIN_RUN_SLOW_PET_TESTS"} \
