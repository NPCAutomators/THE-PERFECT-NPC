#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead NpcAutomators bootstrap session.

Background
----------
A NpcAutomators bootstrap session (client_id ``zorin-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.npcautomators.last_auth_error.relogin_required = true``. From then on every
...
``stage2-hook.sh`` seeds ``auth.json`` from ``ZORIN_AUTH_JSON_BOOTSTRAP`` only
...
``ZORIN_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the NpcAutomators
...
Design constraints
------------------
- Pure stdlib, no zorin_cli imports: runs early in the boot hook, before the
  app venv/modules are guaranteed importable, as its own subprocess.
- Surgical: replaces ONLY ``providers.npcautomators`` in the existing auth.json, leaving
...
# from ZORIN_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead NpcAutomators entry on an existing volume.
REBOOTSTRAP_ENV = "ZORIN_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "zorin-cli-vps"


def _npcautomators_entry_is_terminal(npcautomators_state: Any) -> bool:
    """True iff the on-disk NPCAUTOMATORS provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``zorin_cli.auth.get_npcautomators_session_validity``:
    ...
    """
    if not isinstance(npcautomators_state, dict):
        return False
    last_err = npcautomators_state.get("last_auth_error")
    ...
    if npcautomators_state.get("access_token") or npcautomators_state.get("refresh_token"):
        return False
    return True


def _extract_npcautomators_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.npcautomators`` block out of a ZORIN_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    ZORIN_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
    ...
    if not isinstance(npcautomators, dict) or not npcautomators:
        return None
    if npcautomators.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(npcautomators.get("access_token"), str)
        and npcautomators["access_token"].strip()
        and isinstance(npcautomators.get("refresh_token"), str)
        and npcautomators["refresh_token"].strip()
    ):
        return None
...
def _seed_is_newer(local_npcautomators: Any, seed_npcautomators: dict) -> bool:
    ...
    """
    if not isinstance(local_npcautomators, dict):
        return False
    local_obtained = _parse_timestamp(local_npcautomators.get("obtained_at"))
    seed_obtained = _parse_timestamp(seed_npcautomators.get("obtained_at"))
    return bool(
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no npcautomators entry
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             ZORIN_AUTH_JSON_BOOTSTRAP path handle it)
    ...
    seed_npcautomators = _extract_npcautomators_from_seed(seed_raw)
    if seed_npcautomators is None:
        return "bad_seed"
    ...
    local_npcautomators = providers.get("npcautomators")
    terminal = _npcautomators_entry_is_terminal(local_npcautomators)
    newer_seed = _seed_is_newer(local_npcautomators, seed_npcautomators)
    ...
    # Surgical replacement: swap ONLY providers.npcautomators, preserve everything else.
    providers["npcautomators"] = seed_npcautomators
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] NpcAutomators bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued NpcAutomators bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
