...
When Zorin runs in TUI mode, the gateway child process communicates with
...
# ``get_zorin_home() / "plugins"`` (user) and ``./.zorin/plugins/`` (project,
# gated behind ``ZORIN_ENABLE_PROJECT_PLUGINS``) — see
# ``zorin_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``zorin_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import zorin_constants (this script
    ...
    # plugins in get_zorin_home()/plugins/ and project plugins in
    # ./.zorin/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_zorin_home() / "plugins"]
    if os.environ.get("ZORIN_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".zorin" / "plugins")
