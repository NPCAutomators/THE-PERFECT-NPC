...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``zorin -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("ZORIN_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("ZORIN_SESSION_SOURCE", "")
...
from zorin_cli.timeouts import (
...
_loaded_env_paths = load_zorin_dotenv(zorin_home=_zorin_home, project_env=_project_env)
...
from agent.prompt_builder import (  # noqa: F401  # re-exported via _ra() / mock.patch("run_agent.<name>") / from run_agent import <name>
    ...
    build_npcautomators_subscription_prompt,
    load_soul_md,
)
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"ZorinAgent/{_ZORIN_VERSION}",
    }
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Zorin-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.zorin/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[zorin-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        ...
        try:
            ...
            from zorin_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``zorin chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ZORIN_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ZORIN_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("ZORIN_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ZORIN_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("ZORIN_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        zorin-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See zorin-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # NpcAutomators serves GPT-5.x models via its OpenAI-compatible chat
        # completions endpoint; its /v1/responses endpoint returns 404.
        if normalized_provider == "npcautomators":
            return False
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Zorin against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("ZORIN_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("zorin_cli.plugins.has_hook", ...)`` still
        ...
        try:
            from zorin_cli import plugins as _plugins
    ...
    def _redact_message_content(content):
        ...
        Respects ``ZORIN_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.zorin/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Zorin-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects ZORIN_REDACT_SECRETS via
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.zorin/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``ZORIN_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("ZORIN_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (ZORIN_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        """Parse x-npcautomators-credits-* headers, cache CreditsState, fire threshold notices.
        ...
        """
        # Dev test fixture (ZORIN_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset ZORIN_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("ZORIN_DEV_CREDITS"))
        ...
        if state is None:
            if _dev:
                logger.info(
                    "credits ▸ response had no valid x-npcautomators-credits-* headers "
                    "(miss — producer off / non-NpcAutomators path / >TTL stale)"
                )
            return
        ...
        if _dev:
            # ZORIN_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `zorin logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``ZORIN_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``zorin auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_npcautomators_client_credentials(
        self,
        *,
        force: bool = True,
    ) -> bool:
        if self.api_mode != "chat_completions" or self.provider != "npcautomators":
            return False
        ...
        try:
            ...
            logger.debug("NpcAutomators credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # NpcAutomators requests should not inherit OpenRouter-only attribution headers.
        self._client_kwargs.pop("default_headers", None)

        if not self._replace_primary_openai_client(reason="npcautomators_credential_refresh"):
            return False
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from zorin_cli.copilot_auth import resolve_copilot_token
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct NPCAUTOMATORS Portal.
        """
        if base_url_host_matches(self._base_url_lower, "npcautomators.com"):
            return True
        if (
            ...
            try:
                from zorin_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from zorin_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from zorin_cli.models import ollama_model_supports_thinking
    ...
    def run_conversation(
        ...
        # Publish the conversation id for ambient NPCAUTOMATORS Portal tagging. Every
